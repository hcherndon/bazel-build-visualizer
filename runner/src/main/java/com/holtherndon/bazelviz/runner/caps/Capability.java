...
/**
 * A thing this application may want a Bazel binary to do, together with the flag names that would
 * do it (plan 7.1 {@code BazelCapabilityDetector}).
 *
 * <h2>Observed, never inferred</h2>
 *
 * <p>Every constant here names candidate flags rather than a version threshold, because {@code
 * docs/bazel-compatibility.md} makes observation the rule: forks, release candidates and vendored
 * builds all carry version strings that do not predict their flags. The detector asks the binary
 * what it accepts and records the answer; nothing in this package compares version numbers.
 *
 * <h2>Aliases are ordered</h2>
 *
 * <p>{@link #flagNames()} is the compatibility table from plan 7.1 rule 5, most preferred first. A
 * capability is supported when <em>any</em> of its names is accepted, and {@link
 * BazelCapabilities#preferredFlag} returns the first one that is — so a Bazel 6 that only has
 * {@code --execution_log_binary_file} and a Bazel 7 that also has {@code
 * --execution_log_compact_file} both report the execution log as available, and each is asked for
 * it by the name it knows.
 *
 * <p>Observed on real binaries at Phase 2 (6.5.0, 7.6.1, 8.4.1, 9.2.0); see {@code
 * docs/bazel-compatibility.md} for the measured matrix.
 */
public enum Capability {
  ...
  /**
   * Publish the four lifecycle events over {@code PublishLifecycleEvent}. Negatable, and the
   * launcher may turn it off: the lifecycle RPCs carry no BEP payload, so a capture that does not
   * want them loses nothing but the build/invocation envelope timestamps.
   */
  ...
  /**
   * Emit an event for every action rather than only for failed ones. This is the flag that turns
   * the action tables from "what broke" into "what ran", and it is the single largest driver of
   * event volume.
   */
   ...
   *
   * <p>Exists only on 6.5.0 and defaults to false. Without it that version's log carries no metrics
   * submessage at all; with it there are durations, and still no spawn start on any setting
   * (finding S2 in docs/exec-log-and-profile.md).
   */
   ...
   *
   * <p>The load-bearing one: {@code out} is the same key the BEP uses for action identity, and it
   * is the only thing tying a span to an action. {@code args.target} was measured empty on 7.6.1
   * for the workspace-status action, so the label flag is not a substitute (finding P4). Defaults
   * to false on all four supported versions.
   */
  PROFILE_PRIMARY_OUTPUT("experimental_profile_include_primary_output"),

  /** Write a pprof CPU profile of Starlark execution. */
  STARLARK_CPU_PROFILE("starlark_cpu_profile"),
  ...
  /**
   * The Bazel command a capability is probed against. Flags are per-command in {@code bazel help
   * flags-as-proto}, and asking whether {@code build} accepts {@code --output} would report the
   * {@code aquery} capability as missing.
   */
  ...
  public static Optional<Capability> forFlag(String flagName, String command) {
