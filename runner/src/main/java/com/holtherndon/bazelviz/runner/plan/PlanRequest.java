...
/**
 * Everything {@link InstrumentationPlanner} needs to produce a plan (plan 7.1 input list).
 *
 * <p>Planning is pure: the same request always yields the same plan, and building one starts
 * nothing. That is what lets the UI plan, show, let the user veto a flag or resolve a conflict, and
 * plan again — the second plan is a new value, not a mutation of the first, so there is no state to
 * get out of step with the dialog.
 *
 ...
 * @param sessionRawDirectory where session-local output files go; every generated path is under
 *     this and absolute (plan 8.4)
 * @param besEndpoint the embedded server's address, absent when it could not be started — in which
 *     case the plan says so rather than injecting a backend nothing is listening on
 ...
 * @param destinationsAreLocal whether output paths may be inspected with the desktop filesystem.
 *     SSH plans keep Linux paths as lexical planner values and set this false.
 * @param effectiveOptions the options Bazel will really apply, rc files and {@code --config}
 *     expansion included, as {@code bazel canonicalize-flags} reports them. Absent when Bazel could
 *     not be asked — which is not the same as "no extra options", and the planner says so rather
 *     than assuming
 */
public record PlanRequest(
  ...
  public PlanRequest {
    ...
    effectiveOptions =
        Objects.requireNonNull(effectiveOptions, "effectiveOptions").map(List::copyOf);
  }

  /** Compatibility constructor for local callers. */
  public PlanRequest(
      BazelCommand original,
      BazelCapabilities capabilities,
      CapturePreset preset,
  ...
  public PlanRequest resolving(PlanConflict.Kind kind, String resolutionId) {
    Map<PlanConflict.Kind, String> merged = new EnumMap<>(PlanConflict.Kind.class);
   ...
   *
   * <p>Planning happens twice: once before a session exists, to show the user what will run, and
   * once at launch against the directory that now does. The second must be the first with one field
   * changed — anything rebuilt from parts loses whatever the user decided in between.
   */
  ...
  public PlanRequest withEffectiveOptions(Optional<List<String>> options) {
    return new PlanRequest(
        original,
        capabilities,
        preset,
  ...
  /** Prevents a remote Linux output path from being probed on the desktop. */
  public PlanRequest withRemoteDestinations() {
    return new PlanRequest(
        original,
        capabilities,
        preset,
