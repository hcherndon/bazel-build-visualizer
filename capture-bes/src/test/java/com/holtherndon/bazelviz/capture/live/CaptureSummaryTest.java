...
class CaptureSummaryTest {
    ...
    void emptyCaptureIsNotComplete() {
        CaptureSummary empty = new CaptureSummary(
                0, 0, 0, 0, 0, 0, List.of(), false, Optional.empty());
    ...
    void fullCaptureIsComplete() {
        CaptureSummary full = new CaptureSummary(
                10, 10, 8, 2, 0, 4096, List.of(finished(10)), false, Optional.empty());
    ...
    void unjournaledEventIsADiscrepancy() {
        CaptureSummary lost = new CaptureSummary(
                10, 9, 7, 2, 0, 4096, List.of(finished(10)), false, Optional.empty());
    ...
    void unaccountedFrameIsADiscrepancy() {
        CaptureSummary lost = new CaptureSummary(
                10, 10, 7, 2, 0, 4096, List.of(finished(10)), false, Optional.empty());

        assertThat(lost.isComplete()).isFalse();
        assertThat(lost.discrepancies())
                .anyMatch(problem -> problem.contains("never normalized"));
    }
    ...
    void abortedStreamIsNotComplete() {
        BesStreamState aborted = new BesStreamState(
                KEY, 10, 10, 10, 10, 0, 0,
                OptionalLong.of(1), OptionalLong.of(2),
                BesStreamState.Completion.ABORTED, Optional.of("the socket died"));
        CaptureSummary summary = new CaptureSummary(
                10, 10, 8, 2, 0, 4096, List.of(aborted), false, Optional.empty());
    ...
    private static BesStreamState finished(long events) {
        return new BesStreamState(
                KEY, events, events, events, events, 0, 0,
                OptionalLong.of(1), OptionalLong.of(2),
                BesStreamState.Completion.FINISHED, Optional.empty());
    }
}
