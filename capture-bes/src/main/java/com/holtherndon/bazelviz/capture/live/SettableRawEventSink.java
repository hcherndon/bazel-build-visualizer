 ...
 *
 * <p>The instrumentation plan contains {@code --bes_backend=grpc://127.0.0.1:N}, and the user has
 * to see that plan — the real one, with the real port — before deciding to launch (ADR-007). But
 * the port is only known once the listener is up, and the pipeline behind it needs a session that
 * should not be created for a build the user may still cancel.
 *
 * <p>So the server starts first with this in front of it, the plan is built and shown, and the
 * pipeline is attached at launch. Nothing can arrive in between: Bazel has not been started, and
 * this endpoint is on loopback with a port nobody else has been told about. If something does
 * arrive anyway, it is refused rather than dropped, which is the same promise every other path
 * makes.
 */
public final class SettableRawEventSink implements RawEventSink {
   ...
   *
   * @throws IllegalStateException if one is already attached; two pipelines behind one server would
   *     each see half the stream
   */
