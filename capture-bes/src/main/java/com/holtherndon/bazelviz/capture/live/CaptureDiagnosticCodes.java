...
/**
 * Diagnostic codes a live capture can record, beyond the shared ones in {@code storage-sqlite}'s
 * {@code DiagnosticCodes}.
 *
 * <p>Codes are stable strings stored in {@code import_diagnostics.code}. They are what a user
 * filters on and what a support conversation quotes, so they are constants rather than message
 * text: the message can be reworded, the code cannot.
 */
