...
/**
 * Writes the build's console output to {@code raw/stdout.log} and
 * {@code raw/stderr.log}, verbatim (plan 10.2, ADR-004).
 *
 * <h2>Bytes, unmodified</h2>
 *
 * <p>Nothing is decoded, re-encoded, line-split or stripped of ANSI escapes.
 * Bazel's progress display is carriage returns and cursor movement, and a
 * "helpful" cleanup here would destroy the only record of what the user
 * actually saw. Rendering is the console view's problem, from these bytes.
 *
 * <h2>Two files, two threads</h2>
 *
 * <p>One writer per stream, each written from its own pump thread, so neither
 * blocks the other. That is also why the interleaving between the two files is
 * not recoverable: they are separate pipes drained by separate threads, and
 * plan 4.1 says as much — ordering between the streams is approximate, and this
 * class does not pretend otherwise by merging them.
 */
public final class ConsoleCapture implements ConsoleSink, AutoCloseable {
     ...
     *
     * @param downstream also receives every chunk, for a live console view;
     *     {@link ConsoleSink#discarding()} for a headless capture
     */
