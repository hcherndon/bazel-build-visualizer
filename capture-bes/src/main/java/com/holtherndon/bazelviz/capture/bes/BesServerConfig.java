 ...
 *
 * @param port the port to bind, or 0 to let the operating system choose. Zero is
 *     the default: a fixed port collides with the user's other tools and, worse,
 *     with a second copy of this application capturing a different build
 *     (plan 9.1)
 * @param maxMessageBytes the largest BES message accepted. Defaults to the
 *     journal's payload ceiling, because a message this server accepts but the
 *     journal refuses would be an event acknowledged and then lost
 * @param shutdownGrace how long a graceful shutdown waits for in-flight streams
 *     before it stops waiting; the capture is finalized either way
 * @param permitKeepAliveEvery the shortest client keepalive interval tolerated
 *     before the server treats the pings as abuse. Generous on purpose: this
 *     server has exactly one client, on loopback, and a GOAWAY sent to Bazel
 *     mid-build would look to the user like the capture broke
 */
