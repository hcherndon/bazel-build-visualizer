...
/**
 * The two facts the receive path needs out of a BES request, read without materializing the event
 * inside it.
 *
 * <h2>Why this is not just {@code parseFrom}</h2>
 *
 * <p>The gRPC callback must return quickly and must not do expensive protobuf traversal (plan 9.3).
 * A full {@code parseFrom} of the request would allocate a {@code ByteString} copy of every build
 * event that passes through — one extra copy of the entire event stream, on the hottest path in the
 * application, to obtain two scalar fields the journal frame needs.
 *
 * <p>So this walks the outer messages with {@link CodedInputStream}, reads the stream identity and
 * the sequence number, and <em>skips</em> the event field entirely. The bytes of the event are
 * already in the caller's array on their way to the journal verbatim; nothing here needs to look
 * inside them, and {@code EventNormalizer} does that later, off this thread.
 *
 * <p>The wire walk uses the protobuf library's own reader rather than a hand-rolled varint decoder.
 * Field numbers can be got wrong; wire-format framing should not be re-implemented to find that
 * out.
 *
...
record BesRequestHeader(
   ...
   *
   * @throws IOException when the bytes are not readable as one; the caller fails the RPC rather
   *     than journaling a frame it cannot address
   */
  ...
  static BesRequestHeader ofLifecycleRequest(byte[] payload, int offset, int length)
      throws IOException {
    return read(payload, offset, length, LIFECYCLE_REQUEST_BUILD_EVENT);
  }

  private static BesRequestHeader read(
      byte[] payload, int offset, int length, int orderedEventField) throws IOException {
  ...
  private record StreamIdFields(
      Optional<String> buildId, Optional<String> invocationId, int component) {}

  /**
   * The stream key this request belongs to.
   *
   * <p>A missing build or invocation id becomes an explicit placeholder rather than an empty
   * string, so that two streams that both lack an id do not collapse into one key and start
   * overwriting each other's sequences. It is still one key per stream, and it still says which
   * part was absent.
   */
  ...
  /**
   * The component's enum name, or its number when this build does not know the name. A newer Bazel
   * adding a component must not make the key unreadable.
   */
