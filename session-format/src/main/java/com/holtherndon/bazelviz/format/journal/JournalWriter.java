 ...
 *
 * <p>The journal is the recovery source of truth, so this class does exactly
 * one thing to the bytes it is given: it frames them. Payloads are written
 * verbatim, never re-serialized, never trimmed, and a payload larger than the
 * configured maximum is rejected at the call site rather than shortened.
 *
 * <h2>Durability</h2>
 * The balanced default from plan 9.3: frames are staged in one buffer and
 * handed to the OS when it fills, or when {@link #flush()} is called; there is
 * no {@code fsync} per event. {@link #force()} is the durability barrier and is
 * called on rotation and on {@link #close()}. A crash therefore loses at most
 * the frames still in the buffer, and {@link JournalRecovery} finds the last
 * intact frame afterwards. Anything acknowledged to a BES peer must be
 * acknowledged only after {@link #append} returns, not before.
 *
 * <h2>Threading</h2>
 * Not thread-safe, deliberately. One journal has one writer thread — the
 * pipeline stage between the bounded receive queue and the decoder (plan 9.3) —
 * and a lock here would serialize nothing that is not already serialized while
 * hiding a design error if a second thread ever appeared. Concurrent use is a
 * programming error, not a supported mode.
 *
 * <h2>Failure</h2>
 * A write failure (disk full, plan 21.2) marks the writer permanently failed:
 * every later {@link #append} throws instead of pretending the frame landed, so
 * the capture is visibly incomplete rather than quietly short.
 *
 * <p>Do not interrupt the writer thread to cancel a capture. {@link FileChannel}
 * closes itself on interruption, so an interrupt lands as a write failure and
 * ends the journal early; cancellation (plan 8.7) must stop feeding the writer
 * and then close it, which is what preserves the frames already captured.
 */
public final class JournalWriter implements AutoCloseable {
     ...
     *
     * @throws IOException if the directory already holds journal segments;
     *     appending to an existing journal is {@link #resume}, and doing it by
     *     accident would mix two captures into one file
     */
    public static JournalWriter create(Path directory, UUID sessionUuid, JournalWriterConfig config)
        ...
        if (!existing.isEmpty()) {
            throw new IOException("journal directory already has " + existing.size()
                    + " segment(s); use resume() to append to it: " + directory);
        }
    ...
    /**
     * Reopens an existing journal and appends after its last segment's current
     * end. Run {@link JournalRecovery} first after an unclean shutdown: this
     * method trusts the file length it finds, and appending after a torn tail
     * would bury the damage behind valid frames.
     */
     ...
     * @throws IllegalArgumentException if the payload is larger than the
     *     configured maximum, or the stream ordinal does not fit the format's
     *     16 bits. Neither is truncated to fit.
     * @throws IOException if the write fails; the writer is then permanently
     *     failed and the capture must be marked incomplete
     */
    public JournalLocation append(
        ...
        if (payloadLength > config.maxPayloadBytes()) {
            throw new IllegalArgumentException("payload of " + payloadLength
                    + " bytes exceeds the configured maximum of " + config.maxPayloadBytes()
    ...
    /**
     * Where the next frame will be written. This is the position to checkpoint:
     * every frame before it has been handed to {@link #append}, though not
     * necessarily forced to disk — recovery reconciles the difference.
     */
    ...
    /**
     * The sequence number of the most recently appended frame, or empty when
     * nothing has been appended by this writer. Empty means "not known", not
     * zero — zero is a legal sequence number (plan 11.4).
     */
    ...
    /**
     * Hands staged bytes to the operating system. Not a durability barrier: the
     * data is in the page cache, so it survives this process dying but not the
     * machine dying. Cheap enough to call on a timer during live capture.
     */
    ...
    /**
     * Flushes and {@code fsync}s. This is the durability barrier from plan 9.3:
     * called on rotation and on graceful close, and available to a stricter
     * durability setting that is willing to pay for it per batch.
     */
    ...
    /**
     * Flushes, forces, and closes. Idempotent. After a failed write the buffer
     * is abandoned rather than retried: the frames in it were never reported as
     * durable, and retrying a write onto a full disk only produces a second
    ...
    /**
     * Writes a frame larger than the staging buffer straight to the channel.
     * The CRC is accumulated over the header and the caller's payload without
     * copying the payload anywhere, so a 64 MiB event costs no extra 64 MiB.
     */
    ...
    private void writeFully(ByteBuffer source) throws IOException {
        while (source.hasRemaining()) {
            ...
            if (written <= 0 && source.hasRemaining()) {
                throw new IOException("journal channel accepted no bytes; " + source.remaining()
    ...
    private void rotate() throws IOException {
        ...
        log.debug("rotating journal: segment {} reached {} bytes, opening segment {}",
                segmentIndex, segmentSize, next);
        createSegment(next);
    }
