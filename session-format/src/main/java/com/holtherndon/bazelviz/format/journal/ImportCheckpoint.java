...
/**
 * A resumable position in an import or capture: how far the raw journal has
 * been written, and how far normalization has consumed it (plan 5.2 step 10,
 * plan 21.1).
 *
 * <p>The gap between {@link #framesWritten()} and {@link #eventsNormalized()}
 * is the backlog that recovery replays <em>from the journal</em>, never by
 * re-reading the original source file — the journal is the source of truth and
 * the source file may have moved, grown, or gone.
 *
 * <p>The checkpoint is a hint, not an authority. It can legitimately be ahead
 * of the bytes that reached the disk, because the journal does not fsync per
 * event; recovery treats a position past the end of a segment as "rescan from
 * a position we can prove", not as an error.
 *
 ...
 * @param lastSequence sequence of the last journaled frame; empty means none
 *     has been journaled, which is not the same as sequence zero (plan 11.4)
 * @param framesWritten frames handed to the journal writer so far
 * @param eventsNormalized frames normalized into the database so far, always
 *     {@code <= framesWritten}
...
public record ImportCheckpoint(
    ...
    public ImportCheckpoint {
        ...
        if (segmentOffset < JournalFormat.SEGMENT_HEADER_BYTES) {
            throw new IllegalArgumentException("segmentOffset must be at least the segment header ("
                    + JournalFormat.SEGMENT_HEADER_BYTES + "), got " + segmentOffset);
        }
        if (framesWritten < 0) {
        ...
        if (eventsNormalized > framesWritten) {
            throw new IllegalArgumentException("eventsNormalized (" + eventsNormalized
                    + ") cannot exceed framesWritten (" + framesWritten + ")");
        }
    ...
    /**
     * Serializes to the JSON in the Phase 1 contract. An absent
     * {@code lastSequence} is written as {@code null} rather than {@code 0},
     * because zero is a legal sequence number and "nothing journaled yet" is
     * not zero (plan 11.4).
     */
    public String toJson() {
        ...
        json.append("  \"").append(KEY_LAST_SEQUENCE).append("\": ")
    ...
    /**
     * Parses the contract JSON. Strict on purpose: a missing key, a value that
     * is not a number or {@code null}, trailing garbage, or a violated
     * invariant all fail rather than being repaired, because a repaired
     * checkpoint would silently resume from the wrong place. Unknown keys are
    ...
    public static ImportCheckpoint fromJson(String text) throws CheckpointFormatException {
        ...
        try {
            ...
            throw new CheckpointFormatException("checkpoint is internally inconsistent: "
    ...
    /**
     * A deliberately tiny JSON reader for one flat object of integer or null
     * values. The checkpoint is the only JSON this module writes and reads, and
     * it is written by this same class; pulling in a JSON dependency to parse
     * seven integers would be a larger risk than these forty lines. Anything
     * richer than {@code {"key": 123, "key": null}} is rejected, which is
     * exactly the strictness a corruption detector needs.
     */
