 ...
 *
 * <p>Carries the {@link SessionLock.LockState} that caused the refusal, because the right response
 * depends entirely on which one it is: a {@link SessionLock.Status#HELD_LIVE} lock means "close the
 * other window", a {@link SessionLock.Status#HELD_ELSEWHERE} one means "another machine may have it
 * open, and you decide". Collapsing them into one message would leave the user with no way to act.
 */
