 ...
 *
 * <p>Pretty output is the default for the files this module owns. The manifest
 * is meant to be readable with {@code cat} while debugging a broken session, and
 * a readable diff between two revisions of a session directory is worth the
 * handful of extra bytes.
 *
 * <p>Number literals are emitted verbatim, so a value parsed from a document
 * written by a newer build survives the round trip unchanged.
 */
