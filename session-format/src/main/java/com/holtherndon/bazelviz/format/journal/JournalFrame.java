...
/**
 * One verified frame read back from a journal segment. A frame only exists as an instance of this
 * type after its CRC has been checked, so holding one is proof that the bytes on disk were intact.
 *
 ...
 * @param payload the payload bytes exactly as they were journaled, or {@code null} when the reader
 *     was configured not to read payloads ({@link JournalReaderConfig#readPayloads()}). Null here
 *     means "not read", never "empty": a zero-length payload is a legal frame and comes back as a
 *     zero-length array.
 */
public record JournalFrame(int segmentIndex, long frameOffset, FrameHeader header, byte[] payload) {
  ...
  public JournalFrame {
    ...
    if (payload != null && payload.length != header.payloadLength()) {
      throw new IllegalArgumentException(
          "payload of "
              + payload.length
              + " bytes does not match the header's "
  ...
  /**
   * The payload, or a failure if this frame came from a verify-only reader. Callers that need bytes
   * use this rather than {@link #payload()} so a configuration mistake surfaces as an error instead
   * of a {@code NullPointerException} three frames later.
   */
