 ...
 *
 * <p>A session directory is a single-writer artifact: two processes appending to the same journal
 * or writing the same manifest will corrupt it. This lock exists to prevent that. It is
 * <em>not</em> an OS file lock — those behave badly on the network and container filesystems a
 * session directory can easily live on — but a small JSON record naming the process that holds it.
 *
 * <p>Which means the hard problem is the one this class is really about: a lock left behind by a
 * process that was killed must not brick the session forever.
 *
 * <h2>Staleness rule</h2>
 *
 * Given a lock record, {@link #inspect} decides as follows, and never on elapsed time alone — a
 * wall-clock timeout would either break a live eight-hour capture or leave a crashed one locked for
 * hours:
 *
 * <ol>
 *   <li>The lock file is absent → {@link Status#FREE}.
 *   <li>It is unparseable or missing a required member → {@link Status#STALE}. A record that cannot
 *       be read cannot be honoured; the alternative is an unopenable session.
 *   <li>Its host differs from this host → {@link Status#HELD_ELSEWHERE}. This machine has no way to
 *       ask about a process on another one, and a shared-filesystem session really may be open
 *       there. Not stale; breaking it requires an explicit human decision via {@link #breakLock}.
 *   <li>Same host, and the pid and process start time match this JVM → {@link
 *       Status#HELD_BY_THIS_PROCESS}.
 *   <li>Same host, and {@link ProcessHandle#of} finds no live process for the pid → {@link
 *       Status#STALE}.
 *   <li>Same host, the pid is alive, and both the record and the live process report a start time
 *       that differ by more than {@value #START_TIME_TOLERANCE_MICROS} microseconds → {@link
 *       Status#STALE}. The pid was recycled onto an unrelated process; without this check a session
 *       stays locked until the machine reboots.
 *   <li>Otherwise → {@link Status#HELD_LIVE}.
 * </ol>
 *
 * <p>The start-time tolerance is a second, not zero, because the recorded and observed values can
 * come from different reads of a clock with coarse granularity. It is far shorter than any
 * realistic pid-recycling interval, so it does not weaken the reuse check in practice.
 *
 * <p>When either side reports no start time at all — some platforms decline — the lock is treated
 * as {@link Status#HELD_LIVE}. Refusing to open a session that might genuinely be in use is the
 * safe direction to be wrong in; the missing value is recorded as absent, never as zero.
 */
public final class SessionLock implements AutoCloseable {
  ...
  public record LockRecord(
    ...
    public LockRecord {
      ...
      processStartMicros = Objects.requireNonNull(processStartMicros, "processStartMicros");
  ...
  private static LockState evaluate(LockRecord record, String thisHost) {
    ...
    if (record.processStartMicros().isPresent()
      ...
      return new LockState(
          ...
          "pid "
              + record.pid()
              + " is in use by a process started at a different time; "
   ...
   * @param breakStale when true a {@link Status#STALE} lock is removed and replaced; when false any
   *     existing lock fails the call
  ...
  static SessionLock acquire(
    ...
    switch (state.status()) {
      ...
      case HELD_LIVE, HELD_BY_THIS_PROCESS, HELD_ELSEWHERE ->
          throw new SessionLockedException(lockFile, state);
    }
  ...
  /**
   * Removes a lock unconditionally. Only for a user who has been shown {@link LockState#detail()}
   * and decided the owner is really gone; routine cleanup goes through {@code breakStale} on {@link
   * #acquire}.
   */
  ...
  /**
   * Releases the lock, but only if the file still holds <em>this</em> record. Someone else's lock
   * is left alone: having already lost ownership once (to a stale-lock break, say), deleting the
   * new owner's file would be worse than leaking ours.
   */
  @Override
  ...
  private static String stringMember(JsonObject object, String key) {
    JsonValue value =
        object.member(key).orElseThrow(() -> new JsonException("missing '" + key + "'"));
   ...
   *
   * <p>The OS hostname comes first and the environment only as a fallback, because {@code HOSTNAME}
   * is a per-process value, not the machine's identity: shells do not reliably export it, and a
   * cron job, a CI runner and a desktop launch of the same application can each see a different
   * string on one machine. When that happens a lock left by a dead process reads as held on another
   * host, which {@code isBreakable} deliberately refuses to break — permanently locking a session
   * that nothing is using. Getting this backwards trades a rare cross-host mistake for a common
   * same-host lockout.
   */
