 ...
 *
 * @param maxPayloadBytes largest payload length the reader will honour. A declared length above
 *     this is {@link JournalScanStatus#LENGTH_OUT_OF_BOUNDS} and stops the scan, so a damaged
 *     length field can never make the reader allocate an arbitrary array (plan 21.3).
 * @param bufferBytes size of the single scratch buffer the reader streams through. It is
 *     independent of file size and of frame size — payload bytes are checksummed in {@code
 *     bufferBytes}-sized chunks — so scanning a 40 GiB journal costs the same working set as
 *     scanning a 40 KiB one.
 * @param readPayloads when false the reader verifies frames without ever materialising a payload
 *     array, which is what recovery needs: it must prove the bytes are intact, not read them.
 *     Frames then carry no payload and say so via {@link JournalFrame#hasPayload()}.
 */
