...
/**
 * Where one frame's bytes live. This is what the normalizer stores in {@code
 * bep_events.raw_segment} / {@code raw_offset} / {@code raw_length}, so a row in the database can
 * always be traced back to the exact raw bytes it was derived from (ADR-004, plan 11.5).
 *
  ...
  public JournalLocation {
    ...
    if (frameOffset < JournalFormat.SEGMENT_HEADER_BYTES) {
      throw new IllegalArgumentException(
          "frameOffset must be past the segment header, got " + frameOffset);
    }
    if (payloadLength < 0) {
