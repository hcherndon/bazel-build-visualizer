 ...
 *
 * <p>{@link JournalFormat} owns the byte layout; this type owns reading and
 * writing it against a channel, so the writer, the reader and recovery all
 * agree on what a valid segment looks like before a single frame is touched.
 *
...
public record SegmentHeader(int formatVersion, int segmentIndex, UUID sessionUuid) {
    ...
    public ByteBuffer encode() {
        if (formatVersion != JournalFormat.FORMAT_VERSION) {
            throw new IllegalStateException(
                    "refusing to write format version " + formatVersion
                            + "; this build writes " + JournalFormat.FORMAT_VERSION);
        }
     ...
     *
     * @throws IOException if the segment is shorter than a header, does not
     *     start with the journal magic, or declares a format version this build
     *     cannot read. All three mean the file is not a journal segment this
     *     build may append to or interpret, which is a hard stop rather than a
    ...
    public static SegmentHeader read(FileChannel channel) throws IOException {
        ...
        while (buffer.hasRemaining()) {
            ...
            if (read < 0) {
                throw new IOException("segment is shorter than its " + JournalFormat.SEGMENT_HEADER_BYTES
                        + "-byte header: only " + buffer.position() + " bytes present");
            }
        }
        buffer.flip();
        ...
        buffer.get(magic);
        if (!java.util.Arrays.equals(magic, JournalFormat.MAGIC)) {
            throw new IOException("not a journal segment: magic mismatch");
        }
        int version = buffer.getInt();
        if (version != JournalFormat.FORMAT_VERSION) {
            throw new IOException("journal format version " + version
                    + " is not readable by this build (expected " + JournalFormat.FORMAT_VERSION + ")");
        }
    ...
    /**
     * Fails when this header does not describe the segment it was expected to.
     * A mismatched session UUID means two sessions are sharing a directory,
     * which would interleave one build's raw bytes into another's journal.
     */
    public void requireMatches(int expectedSegmentIndex, UUID expectedSessionUuid) throws IOException {
        if (segmentIndex != expectedSegmentIndex) {
            throw new IOException("segment file names index " + expectedSegmentIndex
                    + " but its header says " + segmentIndex);
        }
        if (!sessionUuid.equals(expectedSessionUuid)) {
            throw new IOException("segment belongs to session " + sessionUuid
                    + ", not " + expectedSessionUuid);
        }
    }
}
