...
/**
 * A point in the segmented journal: a segment index plus a byte offset inside
 * that segment. Positions always name a frame boundary — either the start of a
 * frame or the end of the last complete frame — never a byte inside a frame.
 *
 * <p>Ordering is lexicographic over (segment, offset), which is also the order
 * bytes were appended, because segments are only ever created in increasing
 * index order and are never rewritten.
 */
public record JournalPosition(int segmentIndex, long byteOffset)
    ...
    public JournalPosition {
        ...
        if (byteOffset < JournalFormat.SEGMENT_HEADER_BYTES) {
            throw new IllegalArgumentException("byteOffset must be at least the segment header ("
                    + JournalFormat.SEGMENT_HEADER_BYTES + "), got " + byteOffset);
        }
    }
