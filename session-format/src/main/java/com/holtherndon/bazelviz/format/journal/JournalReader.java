 ...
 *
 * <p>The reader stops at the first frame it cannot trust and says exactly why
 * and exactly where ({@link JournalScanStatus}, {@link #stopOffset()}). It
 * never discards frames it already verified because a later one is damaged:
 * everything returned before the stop is real data, which is the whole point of
 * per-frame checksums (plan 21.3).
 *
 * <h2>Bounded memory</h2>
 * The reader holds one scratch buffer of {@link JournalReaderConfig#bufferBytes()}
 * regardless of how large the segment is, and never memory-maps or slurps the
 * file. When payloads are being read, one array per frame is allocated, bounded
 * by {@link JournalReaderConfig#maxPayloadBytes()} — a per-frame cost, not a
 * per-file one. With {@link JournalReaderConfig#readPayloads()} false, used by
 * recovery, payloads stream through the scratch buffer into the checksum and
 * nothing frame-sized is allocated at all.
 *
 * <h2>Threading</h2>
 * Not thread-safe: it holds a scan cursor. Concurrent readers open their own
 * instances, which is safe because segments are append-only and already-written
...
public final class JournalReader implements AutoCloseable {
    ...
    public static JournalReader open(Path segmentFile, JournalReaderConfig config) throws IOException {
        return open(segmentFile, JournalFormat.SEGMENT_HEADER_BYTES, config);
    }

    /**
     * Opens a segment and scans from {@code startOffset}, which must be a frame
     * boundary — normally one taken from a checkpoint or from an earlier scan.
     *
     * @throws IOException if the file is not a journal segment this build can
     *     read, or if {@code startOffset} lies past the end of the file, which
     *     means the checkpoint is ahead of the data and the caller must decide
     *     how far back to rescan rather than have the reader guess
     */
    public static JournalReader open(Path segmentFile, long startOffset, JournalReaderConfig config)
        ...
        try {
            ...
            if (fromName.isPresent() && fromName.getAsInt() != header.segmentIndex()) {
                throw new IOException("segment file " + segmentFile.getFileName()
                        + " carries header index " + header.segmentIndex());
            }
            long size = channel.size();
            if (startOffset > size) {
                throw new IOException("startOffset " + startOffset + " is past the end of "
                        + segmentFile.getFileName() + " (" + size + " bytes)");
            }
    ...
    /**
     * The next verified frame, or {@code null} when the scan has stopped — call
     * {@link #status()} to find out whether that was a clean end or a defect.
     */
    public JournalFrame next() throws IOException {
        ...
        if (available == 0) {
            return stop(JournalScanStatus.OK, frameOffset,
                    "segment ends on a frame boundary after " + framesRead + " frame(s)");
        }
        if (available < JournalFormat.FRAME_HEADER_BYTES) {
            return stop(JournalScanStatus.TRUNCATED_TAIL, frameOffset,
                    "only " + available + " of " + JournalFormat.FRAME_HEADER_BYTES
        ...
        for (int i = 0; i < JournalFormat.FRAME_MAGIC.length; i++) {
            if (headerBuffer.get(i) != JournalFormat.FRAME_MAGIC[i]) {
                return stop(JournalScanStatus.BAD_MAGIC, frameOffset,
                        "expected frame magic " + JournalFormat.FRAME_MAGIC_STRING
                                + " at offset " + frameOffset);
            }
        }
        ...
        if (payloadLength < 0 || payloadLength > config.maxPayloadBytes()) {
            return stop(JournalScanStatus.LENGTH_OUT_OF_BOUNDS, frameOffset,
                    "frame declares a payload of " + payloadLength + " bytes, outside [0, "
                            + config.maxPayloadBytes() + "]");
        }

        long totalBytes = (long) JournalFormat.FRAME_HEADER_BYTES
                + payloadLength + JournalFormat.FRAME_CRC_BYTES;
        if (available < totalBytes) {
            return stop(JournalScanStatus.TRUNCATED_TAIL, frameOffset,
                    "frame needs " + totalBytes + " bytes but only " + available
        ...
        if (storedCrc != (int) crc.getValue()) {
            return stop(JournalScanStatus.CRC_MISMATCH, frameOffset,
                    "frame checksum mismatch: stored " + Integer.toHexString(storedCrc)
        ...
        try {
            ...
            return stop(JournalScanStatus.UNSUPPORTED_SOURCE_KIND, frameOffset,
    ...
    /**
     * Drains the scan and reports the result. Frames are handed to
     * {@code consumer} as they are verified, so a caller that stops early still
     * keeps every frame it was given.
     */
     ...
     *
     * @throws IllegalStateException while the scan is still running — there is
     *     no honest status to report yet, and defaulting to OK would let a
     *     half-read segment look complete
     */
    public SegmentScan result() {
        if (status == null) {
            throw new IllegalStateException(
                    "scan has not finished: " + framesRead + " frame(s) read, next offset "
                            + nextOffset + "; drain next() before asking for a result");
        }
    ...
    private void readFully(ByteBuffer destination, long position) throws IOException {
        ...
        while (destination.hasRemaining()) {
            ...
            if (read < 0) {
                ...
                throw new IOException("journal segment " + file.getFileName()
                        + " ended at " + at + " while reading; was it truncated concurrently?");
            }
            at += read;
        }
    }
