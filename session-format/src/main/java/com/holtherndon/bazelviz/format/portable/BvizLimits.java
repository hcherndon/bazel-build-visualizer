 ...
 *
 * <p>Plan 22.4: "limit expanded archive size, limit entry count". A Zip file declares how large
 * each entry expands to and can lie about it, and a hand-built archive of a few kilobytes can
 * expand to terabytes — the zip bomb is not a theoretical attack, it is the reason every archive
 * reader has these numbers. So the reader counts bytes as it decompresses and stops at the limit,
 * rather than trusting the declared size.
 *
 * <p>The defaults are generous by the standards of the attack and tight by the standards of a real
 * session: a Tier 3 capture is single-digit gigabytes, and a session with more than fifty thousand
 * files is not a session.
 *
 ...
 * @param maxCompressionRatio the largest expansion any one entry may show; a legitimate journal or
 *     database compresses perhaps 10:1, and 1000:1 is a file made of zeroes
 */
