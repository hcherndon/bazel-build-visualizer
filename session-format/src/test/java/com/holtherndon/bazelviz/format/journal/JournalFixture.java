...
final class JournalFixture {
  ...
  static List<JournalFrame> readAll(Path segmentFile, JournalReaderConfig config)
      throws IOException {
  ...
  /**
   * Builds a frame by hand so tests can produce bytes the writer never would, such as a source-kind
   * ordinal from a future release, with a correct CRC.
   */
  static byte[] handBuiltFrame(
      int sourceKindOrdinal, int streamOrdinal, long sequence, long receiveMicros, byte[] payload) {
    ByteBuffer buffer =
        JournalFormat.allocate(
