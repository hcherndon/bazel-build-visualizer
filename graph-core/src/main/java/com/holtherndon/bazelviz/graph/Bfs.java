...
/**
 * Budget-limited breadth-first traversal over a {@link CsrGraph}. Direction
 * is whatever the supplied graph encodes: hand it the forward graph for
 * downstream reachability, the {@link CsrBuilder#reverse reversed} graph for
 * upstream.
 *
 * <p>State is two {@code int[]} frontier buffers plus a {@code long[]} visited
 * bitset, reused across runs so repeated traversals (e.g. per-selection
 * impact queries) allocate nothing after warmup. Instances are not
 * thread-safe.
 */
public final class Bfs {
    ...
    /**
     * Runs BFS from {@code source}, visiting at most {@code maxNodes} nodes
     * (the source counts) and expanding at most {@code maxDepth} levels
     * ({@code maxDepth == 0} visits only the source). Returns the number of
     * nodes visited. {@code visitOrder}, when non-null, receives each node in
     * discovery order.
     */
    public long run(int source, long maxNodes, int maxDepth, IntConsumer visitOrder) {
        if (source < 0 || source >= nodeCount) {
            throw new IllegalArgumentException("source " + source + " out of range [0, " + nodeCount + ")");
        }
        if (maxDepth < 0) {
