...
/**
 * Two-pass CSR construction. Pass 1 counts out-degrees, pass 2 places targets; peak transient
 * memory beyond the final graph is one {@code int[nodeCount]} fill cursor. No boxing, no per-edge
 * objects.
 */
public final class CsrBuilder {
  ...
  public static CsrGraph build(int nodeCount, EdgeStream edges) {
    ...
    if (placed != edgeCount) {
      throw new IllegalStateException(
          "edge stream is not replayable: pass 1 counted "
              + edgeCount
              + ", pass 2 delivered "
              + placed);
    }
  ...
  /**
   * Transposed graph via the same two-pass technique, reading the source CSR arrays directly.
   * Neighbor lists of the result are sorted ascending (a consequence of scanning source nodes in
