...
/**
 * Immutable compressed-sparse-row adjacency. Storage is exactly two primitive
 * arrays: {@code offsets} (length nodeCount + 1) and {@code targets} (length
 * edgeCount); the neighbors of node {@code u} are
 * {@code targets[offsets[u] .. offsets[u+1])}. No per-node or per-edge objects
 * exist anywhere in this class.
 *
 * <p>Phase 0 constraint: {@code nodeCount} and {@code edgeCount} must each fit
 * in an {@code int} because Java arrays are int-indexed. The largest planned
 * fixture (TIER3: 5M nodes, 100M edges) fits comfortably. Offsets are kept as
 * {@code long[]} and the slice API speaks {@code long} edge indices so callers
 * survive a future widening of {@code targets} to a segmented structure
...
public final class CsrGraph {
    ...
    /**
     * Trusted-array constructor; {@link CsrBuilder} is the intended entry
     * point. Validates structural invariants (one linear pass over each
     * array) and takes ownership of both arrays without copying.
     */
    CsrGraph(long[] offsets, int[] targets) {
        ...
        if (offsets[offsets.length - 1] != targets.length) {
            throw new IllegalArgumentException(
                    "offsets[nodeCount] = " + offsets[offsets.length - 1]
                            + " does not match targets.length = " + targets.length);
        }
