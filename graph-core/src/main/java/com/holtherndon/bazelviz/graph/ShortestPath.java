 ...
 *
 * <p>A single-source breadth-first search from the start explores every node
 * within the answer's distance. Searching from both ends and meeting in the
 * middle explores every node within half of it, which on a graph whose
 * branching factor is anything above one is the difference between a fast
 * answer and walking the build.
 *
 * <p>It needs the reverse index, which is why this takes both — and the two are
 * required to be the same graph in opposite directions, which
 * {@code CsrBuilder.reverse} guarantees by construction.
 *
 * <h2>Budgeted, because plan 13.3 says so</h2>
 *
 * <p>"Never compute or store a complete transitive closure." A path query on a
 * five-million-node graph with no answer would otherwise visit every node
 * twice. The budget bounds the nodes visited, and running out returns
 * {@link Outcome#BUDGET_EXHAUSTED} rather than {@link Outcome#NO_PATH} —
 * because "I did not find one" and "there is not one" are different answers and
 * only one of them is safe to show as a fact.
 */
public final class ShortestPath {
        ...
        if (forward.nodeCount() != reverse.nodeCount()) {
            throw new IllegalArgumentException(
                    "forward and reverse indexes describe different graphs: "
                            + forward.nodeCount() + " nodes against " + reverse.nodeCount());
        }
    ...
    /**
     * The shortest path from {@code from} to {@code to}, following edge
     * direction.
     *
     ...
     *
     * <p>The forward half comes out reversed and is flipped; the backward half
     * is already in order because its parents point towards the target.
     */
    ...
    public enum Outcome {
         ...
         *
         * <p>Says nothing about whether a path exists, and must never be shown
         * as though it did.
         */
        BUDGET_EXHAUSTED
    }
    ...
    /**
     * @param path node indices from source to target inclusive; empty unless
     *     {@link Outcome#FOUND}
    ...
    public record Result(Outcome outcome, int[] path, long nodesVisited) {
        ...
        public String describe() {
            return switch (outcome) {
                case FOUND -> "A path of " + length() + " step" + (length() == 1 ? "" : "s")
                        + " connects them.";
                case NO_PATH -> "Nothing connects them: the first does not contribute to the"
                        + " second, directly or indirectly.";
                case BUDGET_EXHAUSTED -> "The search stopped after " + nodesVisited
