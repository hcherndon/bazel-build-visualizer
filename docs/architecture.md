...
|---|---|
| `core-model` | Pure domain types shared by everything: identifiers (`SessionId`), the session lifecycle state machine (`SessionState`), and — as later phases land — the normalized event/action/target value types. No I/O, no framework dependencies beyond slf4j. |
...
| `capture-file` | Ingestion of already-written BEP files (binary or JSON) into the same raw journal format, so file import and live capture converge immediately. |
| `session-format` | The managed session directory layout (docs/session-format.md): what files a session contains, journal file format, manifest read/write, integrity checks. The only module that knows paths inside a session. |
| `storage-sqlite` | Explicit-SQL persistence (ADR-006): per-session database and app catalog database (ADR-005), schema DDL and migrations, paged query APIs that return primitive arrays. Depends on `analysis-core`, following plan 6.1's split: the aggregate and metric types are analysis-core's, the streaming that fills them is this module's. |
