...
- Short probe subprocesses retain bounded stdout and stderr while continuing to drain them. A
  ...
  stdout uses the same lifecycle. On unverified hosts without the required POSIX permissions,
  `/bin/sh`, or Linux `setsid`, the fixed `ProcessHandle` tracker is a best-effort fallback; it
  cannot recover a child already reparented before Java observed it.
...
- Protobuf timestamps and durations pass through one range-, sign-, and overflow-checked boundary.
  ...
  become unavailable with a bounded `INVALID_TIME_VALUE` import diagnostic. Malformed execution-log
