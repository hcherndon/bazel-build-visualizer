 ...
 *
 * <p>A redaction that replaced every secret with the same {@code ****} would
 * destroy the one thing a reader most needs from a shared session: whether the
 * token in these ten thousand actions is <em>the same</em> token. So each
 * distinct value gets a stable pseudonym — {@code [redacted:9f2c1a7b04]} — and
 * the same value redacts to the same pseudonym everywhere in one export.
 *
 * <p>The pseudonym is a truncated digest of the value under a random key
 * generated per {@link Redactor} and never written anywhere. That matters: a
 * bare digest of a low-entropy secret is a password hash, and a password hash
 * in a file being sent to a colleague is the secret with an extra step.
 * Keying it makes the pseudonym meaningless outside this one export, which is
 * exactly the lifetime it needs.
 *
 * <h2>Paths keep their shape</h2>
 *
 * <p>An absolute path has an identifying prefix and an informative remainder.
 * The prefix is mapped — to {@code [workspace]}, {@code [output-base]},
 * {@code [home]} — and the remainder survives, so an export still shows which
 * directory a file came from without showing whose machine it was on. A path
 * under no known prefix has its user component masked, because
 * {@code /Users/<name>} is the account name of whoever ran the build.
 *
 * <h2>Not thread-safe</h2>
 *
 * <p>One export, one redactor, one thread. The pseudonym table has to be shared
 * across the whole pass or the same secret would get different names in
 * different tables, so there is nothing to gain from making it concurrent.
 */
public final class Redactor {
    ...
    /**
     * @param key the pseudonym key; a parameter so a test can assert that the
     *     same value redacts to the same name, which is otherwise unobservable
     *     by design
     */
    public Redactor(RedactionPolicy policy, byte[] key) {
        ...
        this.prefixes = policy.pathPrefixes().entrySet().stream()
                .sorted((left, right) ->
     ...
     *
     * <p>Handles {@code --flag=value} by checking the flag against the name
     * rules and redacting only the value, so an export still shows that
     * {@code --remote_header} was passed while not showing what it carried. A
     * bare argument is checked against the value rules alone.
     */
    ...
    /**
     * Redacts a whole argv, carrying the "secret follows this flag" rule
     * across the boundary between one argument and the next.
     */
    public List<String> argv(List<String> argv, String field) {
        ...
        for (String argument : argv) {
            ...
            pending = argument != null && argument.indexOf('=') < 0
                    ? matchingNameRule(argument)
                    : null;
        }
     ...
     *
     * <p>The name is never redacted: knowing that {@code GITHUB_TOKEN} was set
     * is diagnostic and knowing its value is not.
     */
    public String environmentValue(String name, String value, String field) {
        ...
        if (policy.omitEnvironmentValues()) {
            report.record("omit-environment-values", field, "[omitted]",
                    "every environment value, by policy");
            return "[omitted]";
        }
     ...
     *
     * <p>A relative path is returned unchanged: it carries no account name and
     * is the same on every machine, which is the whole reason Bazel uses them.
     */
    public String path(String path, String field) {
        ...
        for (Map.Entry<String, String> prefix : prefixes) {
            if (path.startsWith(prefix.getKey())) {
                report.record("path-prefix", field, prefix.getValue(),
                        "a known absolute prefix");
                return prefix.getValue() + path.substring(prefix.getKey().length());
            }
        }
        String masked = maskUserComponent(path);
        if (!masked.equals(path)) {
            report.record("home-directory", field, "[user]",
                    "the account name in a home-directory path");
        }
        return masked;
    }
     ...
     *
     * <p>Bazel's own failure text routinely embeds a whole command line and a
     * sandbox path, so this runs the value rules and the path mapping over the
     * text rather than treating it as opaque.
     */
    public String text(String text, String field) {
        ...
        return policy.redactAbsolutePaths() ? mapPrefixesWithin(withoutSecrets, field)
                : withoutSecrets;
    }
     ...
     *
     * <p>Off by default even for export. A label is how every other number in
     * the session is identified, so redacting it costs most of the export's
     * usefulness — a trade a person should make deliberately.
     */
    ...
    private String redactValue(String value, String field) {
        ...
        for (SecretPattern pattern : policy.patterns()) {
            ...
            for (int[] region : regions) {
                ...
                rebuilt.append(pseudonymFor(
                        current.substring(region[0], region[1]), pattern, field));
                cursor = region[1];
            }
    ...
    private String mapPrefixesWithin(String text, String field) {
        ...
        for (Map.Entry<String, String> prefix : prefixes) {
            if (current.contains(prefix.getKey())) {
                report.record("path-prefix", field, prefix.getValue(),
                        "a known absolute prefix inside a message");
     ...
     *
     * <p>Only the two shapes that carry one: macOS {@code /Users/<name>} and
     * Linux {@code /home/<name>}. Guessing more widely would mangle ordinary
     * paths, and a path this does not recognise is still covered by whichever
     * prefix the caller mapped.
     */
