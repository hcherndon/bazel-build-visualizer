 ...
 *
 * <p>Plan 11.3 asks byte totals to retain known bytes, the unknown-artifact
 * count, the unique-artifact count and the duplicate-reference count, and each
 * exists because a single number would be a claim the data does not support:
 *
 * <ul>
 *   <li>Bazel does not report a size for every file. A total that silently
 *       omitted them would read as the whole build's output when it is not,
 *       and would shrink as more artifacts arrived without sizes.
 *   <li>The same artifact is referenced by many depsets. Adding it once per
 *       reference inflates the total by whatever the sharing factor happens to
 *       be — which is exactly what a build with heavy depset reuse maximises.
 * </ul>
 *
 * <p>So the total states what it counted, over how many distinct artifacts, how
 * many it had to skip, and how many references it collapsed. A UI can then say
 * "4.2 GiB across 812 files, 137 of unknown size" rather than "4.2 GiB", which
 ...
 * @param duplicateReferences references collapsed because the artifact had
...
public record ByteTotal(
    ...
    public ByteTotal {
        if (knownBytes < 0 || uniqueArtifacts < 0 || unknownSizeArtifacts < 0
                || duplicateReferences < 0) {
            throw new IllegalArgumentException("byte totals cannot be negative: " + knownBytes + ", "
                    + uniqueArtifacts + ", " + unknownSizeArtifacts + ", " + duplicateReferences);
        }
    }
     ...
     *
     * <p>Empty as soon as one did not — because at that point the sum is a
     * lower bound and presenting it as the total is the "never convert
     * unavailable data to zero" rule broken by omission rather than by
     * substitution.
     */
     ...
     *
     * <p>Formatting of the byte figure itself belongs to the view; this decides
     * only what has to be said.
     */
    public String describe(java.util.function.LongFunction<String> formatBytes) {
        String bytes = formatBytes.apply(knownBytes);
        if (uniqueArtifacts == 0) {
            return "no artifacts";
        }
        String base = bytes + " across " + uniqueArtifacts + " artifact"
                + (uniqueArtifacts == 1 ? "" : "s");
        return isPartial()
                ? base + ", " + unknownSizeArtifacts + " of unknown size"
                : base;
    }
}
