 ...
 *
 * <ul>
 *   <li><b>6.5.0 and 7.6.1</b> emit no action timestamps at all. The fields do not exist in the
 *       stream.
 *   <li><b>8.4.1</b> emits both, and {@code endTime} equals {@code startTime} for <em>every</em>
 *       action — a deliberate five-second sleep included. The timestamps are present and the
 *       duration they describe is fiction.
 *   <li><b>9.2.0</b> emits usable timestamps, but only for actions that ran a spawn: roughly a
 *       third of action events carry none.
 * </ul>
 *
...
public record ActionTiming(
  ...
  /**
   * Both timestamps arrived and are equal. Recorded as unknown because Bazel 8.4.1 reports this for
   * every action regardless of how long it took, so an equal pair carries no information about
   * duration on any version.
   */
  ...
  /**
   * Classifies a pair of timestamps. Takes the two values rather than the message, so the rule --
   * which is about what Bazel's numbers mean, not about protobuf -- stays testable without building
   * an event.
   */
