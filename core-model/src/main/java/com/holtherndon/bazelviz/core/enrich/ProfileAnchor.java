...
/**
 * The absolute instant a profile's {@code ts: 0} corresponds to, together with how well it is
 * known.
 *
 * <h2>Why this is a type and not a long</h2>
 *
 * <p>The profile's anchor is the most dangerous single value in Phase 4. Measured across four
 * versions (P1):
 *
 * <table>
 ...
 * </table>
 *
 * <p>On the older pair the field is named for the finish and holds the start, floored to a whole
 * second. It sits before the build began and roughly two seconds before it ended, while the trace
 * itself spans 1.2–1.5 s, so it cannot be a finish. Taking it at its name puts every span about one
 * build-length too late.
 *
 * <p>Flooring also costs precision, and that is the part a plain long would lose: on 6.5.0 and
 * 7.6.1 a profile span cannot be aligned with an execution-log attempt to better than one second.
 * Drawing the two on a shared timeline as though they agreed would be a claim the data does not
 * support, which plan 11.4 forbids. {@link #uncertaintyMicros()} exists so that a caller cannot
 * forget.
 *
 * @param epochMicros the instant of {@code ts: 0}
 * @param meaning what the source field actually held
 * @param sourceKey the {@code otherData} key it was read from, kept so the inspector can show where
 *     the number came from
 * @param uncertaintyMicros the width of the window the true instant lies in; zero when exact
 */
public record ProfileAnchor(
  ...
  public enum Meaning {
    ...
    EXACT_START,
    /**
     * A start floored to the whole second, published under a key naming it a finish. Bazel 6.5.0
     * and 7.6.1.
     */
  ...
  /**
   * The Bazel 6.5.0/7.6.1 shape: {@code profile_finish_ts}, which is a start floored to the second.
   */
  public static ProfileAnchor flooredStart(long epochMicros, String sourceKey) {
    return new ProfileAnchor(
        epochMicros, Meaning.START_FLOORED_TO_SECOND, sourceKey, FLOORED_UNCERTAINTY_MICROS);
  }
  ...
  /**
   * The absolute instant of a trace timestamp, or empty when there is no anchor.
   *
   * <p>Trace timestamps are relative to {@code Initialize command} and are legitimately negative —
  ...
  /**
   * Why this anchor cannot be compared with execution-log timings at full precision, or empty when
   * it can.
   */
  public Optional<String> precisionCaveat() {
    return switch (meaning) {
      ...
      case ABSENT ->
          Optional.of(
              "this profile carries no absolute start, so its spans have" + " relative time only");
      case START_FLOORED_TO_SECOND ->
          Optional.of(
