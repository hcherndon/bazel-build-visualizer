 ...
 *
 * <p>Measured across Bazel 6.5.0, 7.6.1, 8.4.1 and 9.2.0, the same message type carries different
 * fields depending on where it appears:
 *
 * <table border="1">
 ...
 * </table>
 *
 * <p>So {@link #of} returns empty rather than throwing when there is no name: a File carrying only
 * a uri is not an artifact this project can identify, and the two positions where that happens — an
 * action's primary output and a test summary's logs — have their own handling. A reader that
 * required {@code name}, or {@code uri}, or {@code length} would drop rows.
 *
 * <p>Test action outputs deliberately do <em>not</em> come through here. Their names are {@code
 * test.log} and {@code test.xml} with no prefix, so every test in the build would produce the same
 * two paths; they are stored as logs against their attempt instead.
 *
 * <h2>Identity is the path, not the name and not the uri</h2>
 *
 * <p>{@code name} alone collides: the same name appears under two prefixes when a target is built
 * for both the target and the exec configuration. {@code uri} embeds an absolute output-base path
 * that differs by machine, and whose execroot segment changed between Bazel versions. The
 * exec-root-relative path — the prefix segments joined to the name — is what survives both.
 *
 ...
 * @param digest content digest, or a <em>tree</em> digest when {@link #directory} — the two are not
 *     comparable
 * @param sizeBytes the {@code length} field; empty when the position does not report one, which is
 *     not the same as a zero-byte file
 * @param uri display only
 * @param directory true when this came from {@code directoryOutput}
 * @param source inferred from the absence of a path prefix, which is how Bazel distinguishes a
 *     source file from a generated one — it never says so directly
 */
public record FileRef(
  ...
  /**
   * Reads the fields of a {@code File} that is expected to identify an artifact. Takes them one by
   * one rather than as a message so this rule — which is about what Bazel means, not about protobuf
   * — stays testable and stays out of the proto layer.
   *
   * @param name the {@code name} field; empty means the message cannot identify an artifact and the
   *     result is empty
   ...
   * @param directory true when the message came from {@code directoryOutput}, whose digest is a
   *     tree digest and which carries neither uri nor length
   */
