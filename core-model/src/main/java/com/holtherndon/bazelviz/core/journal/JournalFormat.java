 ...
 *
 * <p>The journal is the recovery and forward-compatibility source of truth: payload bytes are
 * appended exactly as received and are never re-serialized, so a session can always be rebuilt from
 * raw data even by a later version of this application that understands more of the schema.
 *
 * <p>This class is the single definition of the byte layout. The writer, the reader, and crash
 * recovery all encode and decode through it, so the layout cannot drift between them.
 *
 ...
 * </pre>
 *
 * <p>Every frame is independently checksummed so recovery can find the last intact frame without
 * trusting any index. The frame magic lets a reader confirm it is positioned on a boundary rather
 * than inside a payload.
 *
...
public final class JournalFormat {
  ...
  /**
   * Default ceiling on a single payload. A declared length above the configured maximum is
   * corruption, not a large event: honouring it would mean allocating whatever a damaged length
   * field happens to say (plan 21.3).
   */
  ...
  public enum SourceKind {
     ...
     *
     * <p>Added in Phase 2, after the first three ordinals were already written to journals on disk.
     * New kinds go on the end for that reason: an earlier build reading this frame reports an
     * unsupported source kind and leaves it alone, which is the documented forward-compatibility
     * behavior (plan 21.5), whereas renumbering would make it read some other kind's bytes as this
     * one.
     *
     * <p>Lifecycle requests carry no BEP payload — they are the build and invocation envelope Bazel
     * sends outside the event stream — so they are journaled for completeness and drive stream
     * state, but produce no {@code bep_events} row.
     */
    BES_LIFECYCLE;
  ...
  /**
   * Writes a complete frame — header, payload and CRC — into {@code target}. The CRC covers the
   * header and payload together, so a frame damaged in either part fails validation.
   */
  public static void writeFrame(
      ByteBuffer target, FrameHeader header, byte[] payload, int payloadOffset, int payloadLength) {
    requireLittleEndian(target);
  ...
  /**
   * Decodes a frame header from {@code source} at its current position, advancing the position past
   * the header.
   *
   * @throws IllegalArgumentException if the magic is wrong or the declared length exceeds {@code
   *     maxPayloadBytes}; callers treat both as corruption rather than retrying
   */
