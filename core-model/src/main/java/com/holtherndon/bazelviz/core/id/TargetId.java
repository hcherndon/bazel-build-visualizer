...
/**
 * A configured target: a label built in a particular configuration (plan 11.1).
 *
 * <h2>Why the configuration is part of the identity</h2>
 *
 * <p>The same label is routinely built more than once in one invocation — once for the target
 * platform and once for the execution platform that builds the tools, at minimum. Those are
 * different configured targets with different actions, different outputs and different durations,
 * and a table keyed on label alone merges them into one row whose numbers belong to neither.
 *
 * <p>Aspects are part of the identity for the same reason. Bazel emits a separate completion event
 * for an aspect applied to an already-completed target, and the two describe different work.
 *
...
public record TargetId(String label, ConfigurationId configuration, Optional<String> aspect) {
   ...
   *
   * <p>Newline-separated because a Bazel label can contain almost anything else — spaces, colons,
   * at-signs, plus signs — and a separator that can appear inside a component makes two different
   * targets collide. A label cannot contain a newline.
   */
