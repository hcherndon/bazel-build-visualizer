 ...
 *
 * <p>Plan 22.2: "default secret-name patterns should cover common token, password, credential, and
 * key names, while remaining user-editable". The name list below is those four families plus the
 * ones that turned up in measured builds — {@code --remote_header}, which carries an {@code
 * Authorization} value, and the cloud providers' fixed variable names.
 *
 * <h2>Why the value rules are few</h2>
 *
 * <p>Only three shapes are matched by value, and that restraint is deliberate. A rule broad enough
 * to catch "any long random-looking string" would redact digests, action keys and configuration
 * checksums — which is most of what makes an exported session useful — and would still miss a
 * password that happens to be a dictionary word. The three here are shapes that are secret wherever
 * they occur and are not produced by Bazel: a bearer token, a URL with credentials embedded in its
 * authority, and a PEM private-key block.
 *
 * <h2>The list is not a guarantee</h2>
 *
 * <p>Pattern matching finds what it was told to look for. An export is therefore accompanied by a
 * report of what was redacted (plan 22.2: "shows the user exactly what was redacted before anything
 * is written"), so the decision to share rests on something a person read rather than on a claim
 * that everything sensitive was caught.
 */
public final class SecretPatterns {
   ...
   *
   * <p>Each expression is anchored to a literal prefix, so it cannot backtrack over an arbitrary
   * argument, and each declares group 1 as the part that is actually secret — the token after
   * {@code Bearer}, the password inside a URL's authority — so the surrounding text survives and an
   * exported command still reads as a command.
   */
   ...
   *
   * @param additionalNameGlobs user-supplied name patterns; a blank entry is rejected rather than
   *     silently ignored, because a pattern that matches everything and a pattern that matches
   *     nothing are both configuration errors worth hearing about
   */
