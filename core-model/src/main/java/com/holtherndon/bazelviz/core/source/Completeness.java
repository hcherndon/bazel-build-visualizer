 ...
 *
 * <p>The distinction between {@link #TRUNCATED} and {@link #CORRUPT_PARTIAL} is deliberate and
 * load-bearing (plan section 21.3): a clean short tail is a normal outcome of a cancelled or
 * still-running build, while a checksum failure on a fully present record means the bytes on disk
 * are wrong. They warrant different messages and different user actions, so they must never be
 * collapsed into one "incomplete" state.
 *
 * <p>{@link #UNKNOWN} is a real answer, not a placeholder for zero — a source whose completeness
 * has not been established must say so (plan 11.4).
 */
