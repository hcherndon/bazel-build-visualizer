...
/**
 * One independent enrichment job and everything the user needs to understand it having failed.
 *
 * <p>Plan 21.4 is specific about what a failed enrichment must show — task, source, exit status,
 * error excerpt, retriability, and the metrics that are unavailable as a result — and specific that
 * a failed profile import must not invalidate the BEP. Both are properties of this record: tasks
 * carry their own outcome, and nothing about a failed one reaches back into the entity tables.
 *
 ...
 * @param unavailableMetrics what the user does not have because this failed, named in the words the
 *     UI uses for them
...
public record EnrichmentTask(
  ...
  public enum State {
     ...
     *
     * <p>Distinct from {@link #FAILED} because the rows that did land are real and are shown; the
     * coverage panel says how far it got.
     */
    PARTIAL,
    /** Failed with nothing usable imported. */
    FAILED,
    /**
     * Not run, and not a failure: the source does not exist on this Bazel version, or was not
     * requested.
     */
    SKIPPED;
  ...
  public EnrichmentTask {
    ...
    if (state == State.SUCCEEDED && errorExcerpt.isPresent()) {
      throw new IllegalArgumentException("a succeeded task cannot carry an error excerpt");
    }
    if (state == State.SUCCEEDED && !unavailableMetrics.isEmpty()) {
      throw new IllegalArgumentException("a succeeded task cannot have made metrics unavailable");
    }
    if ((state == State.FAILED || state == State.PARTIAL) && errorExcerpt.isEmpty()) {
      throw new IllegalArgumentException("a " + state + " task must say what went wrong");
    }
  }
   ...
   *
   * @param reason why, in the words the panel shows — for example that Bazel 6.5.0 has no compact
   *     execution log
   */
