 ...
 *
 * <p>Plan 8.6: never claim an exact graph match unless the configuration equivalence has been
 * verified. This is where "verified" is defined.
 */
final class ConfigurationMatchTest {
  ...
  void nothingToCompareIsUnknown() {
    ...
    assertThat(ConfigurationMatch.of(List.of(), List.of(A))).isEqualTo(ConfigurationMatch.UNKNOWN);
    assertThat(ConfigurationMatch.of(List.of(A), List.of())).isEqualTo(ConfigurationMatch.UNKNOWN);
  ...
  void exactlyOneStatePermitsTheClaim() {
    assertThat(
            Arrays.stream(ConfigurationMatch.values())
