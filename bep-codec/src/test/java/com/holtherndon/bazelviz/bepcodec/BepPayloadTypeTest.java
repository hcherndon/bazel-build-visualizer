...
final class BepPayloadTypeTest {
  ...
  void mapsPayloadsToTheirWireFieldNumbers() {
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setProgress(Progress.getDefaultInstance()).build()))
        .isEqualTo(3);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setAborted(Aborted.getDefaultInstance()).build()))
        .isEqualTo(4);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setStarted(BuildStarted.getDefaultInstance()).build()))
        .isEqualTo(5);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setCompleted(TargetComplete.getDefaultInstance()).build()))
        .isEqualTo(8);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setTestResult(TestResult.getDefaultInstance()).build()))
        .isEqualTo(10);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder().setFinished(BuildFinished.getDefaultInstance()).build()))
        .isEqualTo(14);
    assertThat(
            BepPayloadType.of(
                BuildEvent.newBuilder()
                    .setNamedSetOfFiles(NamedSetOfFiles.getDefaultInstance())
                    .build()))
        .isEqualTo(15);
  }
  ...
  void anEventWithNoPayloadIsNoneNotAGuess() {
    ...
    assertThat(BepPayloadType.of(BuildEvent.getDefaultInstance())).isEqualTo(BepPayloadType.NONE);
  ...
  @Test
  @SuppressWarnings("ProtocolBufferOrdinal") // This test proves that ordinal is not persisted.
