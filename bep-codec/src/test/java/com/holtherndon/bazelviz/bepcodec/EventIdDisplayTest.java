...
final class EventIdDisplayTest {
  ...
  private static BuildEventId id(Consumer<BuildEventId.Builder> build) {
  ...
  void rendersProgress() {
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setProgress(BuildEventId.ProgressId.newBuilder().setOpaqueCount(12)))))
  ...
  void rendersActionCompletedByLabelAndOutput() {
    ...
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setActionCompleted(BuildEventId.ActionCompletedId.getDefaultInstance()))))
  ...
  void rendersLabelIds() {
    assertThat(
            EventIdDisplay.of(
                id(
                    b ->
                        b.setUnconfiguredLabel(
                            BuildEventId.UnconfiguredLabelId.newBuilder()
                                .setLabel("//foo:missing")))))
  ...
  void rendersPatternsAndSummarisesLongLists() {
    ...
    BuildEventId manyPatterns =
        id(
            b -> {
              BuildEventId.PatternExpandedId.Builder pattern =
    ...
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setPattern(BuildEventId.PatternExpandedId.getDefaultInstance()))))
  ...
  void rendersOpaqueAndSingletonIds() {
    ...
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setUnknown(BuildEventId.UnknownBuildEventId.getDefaultInstance()))))
        .isEqualTo("UnknownBuildEvent");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setStarted(BuildEventId.BuildStartedId.getDefaultInstance()))))
        .isEqualTo("BuildStarted");
    assertThat(
            EventIdDisplay.of(
                id(
                    b ->
                        b.setUnstructuredCommandLine(
    ...
    assertThat(
            EventIdDisplay.of(
                id(
                    b ->
                        b.setStructuredCommandLine(
                            BuildEventId.StructuredCommandLineId.newBuilder()
                                .setCommandLineLabel("canonical")))))
        .isEqualTo("StructuredCommandLine canonical");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setWorkspaceStatus(BuildEventId.WorkspaceStatusId.getDefaultInstance()))))
        .isEqualTo("WorkspaceStatus");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setOptionsParsed(BuildEventId.OptionsParsedId.getDefaultInstance()))))
    ...
    assertThat(
            EventIdDisplay.of(
                id(
                    b ->
                        b.setFetch(
                            BuildEventId.FetchId.newBuilder()
                                .setUrl("https://example.test/x.tar.gz")))))
    ...
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setNamedSet(BuildEventId.NamedSetOfFilesId.newBuilder().setId("42")))))
        .isEqualTo("NamedSetOfFiles 42");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setBuildFinished(BuildEventId.BuildFinishedId.getDefaultInstance()))))
        .isEqualTo("BuildFinished");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setBuildToolLogs(BuildEventId.BuildToolLogsId.getDefaultInstance()))))
        .isEqualTo("BuildToolLogs");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setBuildMetrics(BuildEventId.BuildMetricsId.getDefaultInstance()))))
        .isEqualTo("BuildMetrics");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setWorkspace(BuildEventId.WorkspaceConfigId.getDefaultInstance()))))
        .isEqualTo("WorkspaceConfig");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setBuildMetadata(BuildEventId.BuildMetadataId.getDefaultInstance()))))
        .isEqualTo("BuildMetadata");
    assertThat(
            EventIdDisplay.of(
                id(
                    b ->
                        b.setConvenienceSymlinksIdentified(
                            BuildEventId.ConvenienceSymlinksIdentifiedId.getDefaultInstance()))))
        .isEqualTo("ConvenienceSymlinksIdentified");
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setExecRequest(BuildEventId.ExecRequestId.getDefaultInstance()))))
  ...
  void anAbsentLabelIsMarkedNotBlanked() {
    assertThat(
            EventIdDisplay.of(
                id(b -> b.setTargetCompleted(BuildEventId.TargetCompletedId.getDefaultInstance()))))
  ...
  void aRunawayPatternListIsCappedVisibly() {
    ...
    BuildEventId id =
        id(b -> b.setPattern(BuildEventId.PatternExpandedId.newBuilder().addPattern(longPattern)));

    String display = EventIdDisplay.of(id);

    assertThat(display).startsWith("PatternExpanded //aaa");
    assertThat(display).contains(" chars)");
    assertThat(display.length()).isLessThanOrEqualTo(EventIdDisplay.MAX_DISPLAY_CHARS + 24);
  }

  // ---------------------------------------------------------------- labels

  /** Every label-carrying id kind, with a distinctive label in each. */
  private static List<BuildEventId> labelCarryingIds(String label) {
    return List.of(
        id(
            b ->
                b.setTargetConfigured(
            ...
            b ->
                b.setTestProgress(
                    BuildEventId.TestProgressId.newBuilder()
                        ...
                        .setAttempt(3)
                        .setOpaqueCount(4))),
        id(b -> b.setTestSummary(BuildEventId.TestSummaryId.newBuilder().setLabel(label))),
        id(
            b ->
                b.setTargetSummary(
                    BuildEventId.TargetSummaryId.newBuilder()
  ...
  void theStructuredAccessorAnswersEmptyWhereThereIsNoLabel() {
    assertThat(
            EventIdDisplay.label(
                id(b -> b.setProgress(BuildEventId.ProgressId.newBuilder().setOpaqueCount(1)))))
        .isEmpty();
    assertThat(
            EventIdDisplay.label(
                id(b -> b.setStarted(BuildEventId.BuildStartedId.getDefaultInstance()))))
  ...
  void theDisplayParseAgreesWithTheStructuredAccessor() {
    ...
    for (String label :
        List.of("//pkg/sub:target", "@repo//pkg:name", "@@canonical~repo//pkg:name")) {
      for (BuildEventId id : labelCarryingIds(label)) {
        assertThat(EventIdDisplay.labelOfDisplay(EventIdDisplay.of(id)))
            .as("label parsed back from %s of %s", EventIdDisplay.of(id), id.getIdCase())
  ...
  void theDisplayParseDoesNotInventLabels() {
    ...
    assertThat(EventIdDisplay.labelOfDisplay("TargetCompleted " + EventIdDisplay.ABSENT_LABEL))
        .isEmpty();
    // An ActionCompleted with only a primary output has no label, and the
    // output path must not be promoted into one.
    assertThat(
            EventIdDisplay.labelOfDisplay(
                EventIdDisplay.of(
                    id(
                        b ->
                            b.setActionCompleted(
