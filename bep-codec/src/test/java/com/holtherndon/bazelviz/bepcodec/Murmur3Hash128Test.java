 ...
 *
 * <p>The golden vectors below were verified against Guava's
 * {@code Hashing.murmur3_128()} — the canonical MurmurHash3 x64 128 with seed 0
 * — over these strings and over 200 random inputs of every length from 0 to 199,
 * both halves matching. Guava is not a declared dependency of this module, so
 * the agreed values are recorded here instead of re-deriving them at test time.
 *
 * <p>A failure here is not a test to update. These hashes are written into
 * session databases, so changing them orphans every already-indexed session;
 * fixing a failure means a format-version decision.
 */
final class Murmur3Hash128Test {
    ...
    void matchesGoldenVectors() {
        ...
        for (Vector vector : vectors) {
            ...
            assertThat(hash.high())
                    .as("high half of \"%s\"", vector.input())
                    .isEqualTo(vector.high());
            assertThat(hash.low())
                    .as("low half of \"%s\"", vector.input())
                    .isEqualTo(vector.low());
        }
    }
    ...
    void leavesTheCallersBufferUntouched() {
        ByteBuffer buffer = ByteBuffer.wrap("abcdefghijklmnopqrstuv".getBytes(StandardCharsets.UTF_8))
    ...
    /**
     * The low half alone carries the {@code event_id_hash} column, so dispersion
     * of that half — not of the full 128 bits — is what the schema relies on.
     */
    @Test
