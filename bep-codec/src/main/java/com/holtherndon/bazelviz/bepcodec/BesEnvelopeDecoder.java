...
/**
 * Reads the BES request that a journal frame of kind
 * {@code BES_ENVELOPE} or {@code BES_LIFECYCLE} contains.
 *
 * <p>The journal stores the request exactly as it arrived on the wire, so this
 * is where those bytes become meaning. It stops at the envelope: the BEP event
 * inside is handed back as bytes for {@link BepEventDecoder}, which owns every
 * decision about BEP decoding including the unknown-field scan.
 *
 * <h2>Size limits</h2>
 *
 * <p>Parsing is bounded by the same maximum the journal enforces (plan 21.3,
 * "limit protobuf message size"). The limit matters more here than for a file
 * import: these bytes came off a socket, and a length that says 2 GB is a reason
 * to stop rather than an instruction to allocate.
 *
...
public final class BesEnvelopeDecoder {
     ...
     *
     * @param status {@link DecodeStatus#OK} when the envelope parsed,
     *     {@link DecodeStatus#FAILED} when it did not. Unknown fields in the
     *     <em>envelope</em> are not reported: the envelope is a transport
     *     wrapper, and a newer BES adding a field to it says nothing about
     *     whether the build event inside is fully understood, which is what
     *     {@code has_unknown_fields} is asked about
     * @param envelope the parsed envelope, absent on failure
     * @param failureDetail why it failed, present only on failure
     */
    public record Result(DecodeStatus status, Optional<BesEnvelope> envelope, Optional<String> failureDetail) {
        ...
        public Result {
            ...
            if ((status == DecodeStatus.FAILED) == envelope.isPresent()) {
                throw new IllegalArgumentException("status " + status + " disagrees with the envelope's presence");
            }
        }
    ...
    public Result decodeToolEvent(byte[] payload, int offset, int length) {
        ...
        if (length > maxMessageBytes) {
            return Result.failed("BES envelope of " + length + " bytes exceeds the " + maxMessageBytes
        ...
        try {
            ...
            PublishBuildToolEventStreamRequest request = PublishBuildToolEventStreamRequest.parseFrom(input);
    ...
    public Result decodeLifecycle(byte[] payload, int offset, int length) {
        Objects.checkFromIndexSize(offset, length, payload.length);
        if (length > maxMessageBytes) {
            return Result.failed("BES lifecycle request of " + length + " bytes exceeds the " + maxMessageBytes
                    + "-byte limit; refusing to parse it");
        }
        try {
            CodedInputStream input = CodedInputStream.newInstance(payload, offset, length);
            ...
            return Result.ok(new BesEnvelope(
    ...
    private static BesEnvelope fromOrdered(OrderedBuildEvent ordered) {
        ...
        return new BesEnvelope(
                ...
                kind.carriesBuildEvent()
                        ? Optional.of(event.getBazelEvent().getValue())
                        : Optional.empty(),
    ...
    private static BesEnvelope.Kind kindOf(BuildEvent event) {
        return switch (event.getEventCase()) {
            ...
            case INVOCATION_ATTEMPT_STARTED,
                    ...
                    BUILD_ENQUEUED,
                    BUILD_FINISHED -> BesEnvelope.Kind.LIFECYCLE;
            default -> BesEnvelope.Kind.OTHER;
        };
    }

    /**
     * The envelope's own timestamp. Absent when unset rather than reported as
     * the epoch: Bazel leaves {@code event_time} unset on some envelopes, and
     * 1970 shown in a timeline is an unavailable value rendered as a number
     * (plan 11.4).
     */
