 ...
 *
 * <p>{@link #bazelEventBytes} is the untouched {@code Any.value} carrying a
 * {@code build_event_stream.BuildEvent}. It is handed back as bytes so that the
 * same {@link BepEventDecoder} that reads a BEP file reads a BES event too —
 * one decoder, one unknown-field scan, one set of rules for what counts as a
 * failure. Parsing it here as well would create a second path that could drift
 * from the first, and "a captured session and an imported session produce
 * identical rows" is a property this project checks.
 *
 * <h2>Not every envelope carries an event</h2>
 *
 * <p>The lifecycle requests, the console-output events and the final
 * {@code component_stream_finished} marker are all real, accepted BES traffic
 * with no BEP payload inside them. They are journaled like everything else
 * (ADR-004) and they move stream state, but they produce no {@code bep_events}
 * row — a row whose event type was "none" would claim a build event arrived and
 * could not be decoded, which is a different and worse statement than "this was
 * not a build event".
 *
 ...
 * @param bazelEventBytes the inner BEP event, present only for
 *     {@link Kind#BAZEL_EVENT}
 * @param eventTimeMicros the envelope's {@code event_time}, which is Bazel's
 *     clock rather than ours and is therefore kept apart from receive time
 *     (plan 11.5)
 */
public record BesEnvelope(
    ...
    public enum Kind {
        ...
        LIFECYCLE,
        /**
         * Something this build does not model — including an arm added by a
         * newer Bazel. Reported rather than treated as an error, because the
         * bytes are already journaled and a future build can interpret them
         * (plan 21.5).
         */
        OTHER;
