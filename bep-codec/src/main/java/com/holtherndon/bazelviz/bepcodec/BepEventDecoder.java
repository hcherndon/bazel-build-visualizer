...
/**
 * Turns raw BEP payload bytes into a {@link BuildEvent}, reporting rather than
 * throwing when the bytes are unusable.
 *
 * <p>Three project rules shape this class:
 *
 * <ul>
 *   <li><b>A bad event is a diagnostic, not the end of the session</b>
 *       (plan 21.3). Every failure path returns {@link DecodeResult#failed} with
 *       an explanation the importer can write to {@code import_diagnostics};
 *       nothing escapes as an exception.
 *   <li><b>Unknown fields are preserved, not stripped</b> (plan 21.5).
 *       protobuf-java retains unrecognised fields on the parsed message, so the
 *       message returned here still round-trips them. This class only
 *       <em>detects</em> them so the import can surface an "unknown event
 *       fields" diagnostic and mark the row for later reindexing.
 *   <li><b>Message size is bounded</b> (plan 21.3). A payload larger than the
 *       configured maximum is rejected as a failure rather than allocated.
 * </ul>
 *
 * <p>The decoder never re-serializes: the raw bytes remain the source of truth
 * in the journal (ADR-004), and this class only reads them.
 *
 * <p>Instances are immutable and stateless; a single decoder may be shared by
 * any number of threads.
 */
public final class BepEventDecoder {

    /**
     * Default ceiling on one decoded message, matched to the journal's frame
     * ceiling so a payload that the journal accepted is never rejected here for
     * a different reason.
     */
    ...
    public enum UnknownFieldScan {
        /**
         * Inspect only the outermost {@code BuildEvent}. Cheap, but blind to the
         * common case: a newer Bazel adding a field inside a payload message.
         */
        TOP_LEVEL,

        /**
         * Walk the whole message tree, including repeated and map entries.
         * The default, because a new field almost always appears nested.
         */
        DEEP
    }
    ...
    /**
     * @param maxMessageBytes largest payload this decoder will parse; a larger
     *     payload is reported as a failure, never allocated
    ...
    public BepEventDecoder(int maxMessageBytes, UnknownFieldScan unknownFieldScan) {
        ...
        this.unknownFieldScan = java.util.Objects.requireNonNull(unknownFieldScan, "unknownFieldScan");
    }
    ...
    /**
     * Decodes {@code length} bytes of {@code payload} starting at {@code offset}
     * — the shape a journal reader hands over, so no copy is needed.
     */
    public DecodeResult decode(byte[] payload, int offset, int length) {
        ...
        if (offset < 0 || length < 0 || length > payload.length - offset) {
            return DecodeResult.failed("payload range [" + offset + ", " + (offset + length)
                    + ") lies outside a " + payload.length + "-byte array");
        }
    ...
    /**
     * Decodes the remaining bytes of {@code payload}. The buffer's position is
     * not advanced, so a caller mapping a journal segment can decode a frame
     * without disturbing its own cursor.
     */
    ...
    private String tooLargeDetail(int length) {
        return "payload of " + length + " bytes exceeds the " + maxMessageBytes
    ...
    /**
     * Runs a parse and classifies the outcome. Every checked and unchecked
     * failure below {@code Error} becomes a {@link DecodeStatus#FAILED} result:
     * a malformed payload can make protobuf-java raise
     * {@code InvalidProtocolBufferException}, but a truncated length-delimited
     * field can also surface as an {@code IndexOutOfBoundsException} or a
     * {@code NegativeArraySizeException} from the underlying buffer, and none of
     * those may reach the import loop.
     */
    private DecodeResult finish(Parse parse, int length) {
        ...
        boolean unknown;
        try {
            unknown = unknownFieldScan == UnknownFieldScan.DEEP
            ...
            return DecodeResult.failed("payload of " + length
                    + " bytes parsed, but the unknown-field scan failed: " + describeThrowable(e));
        }
    ...
    /**
     * True when {@code root} or anything reachable from it carries an
     * unrecognised field.
     *
     * <p>Iterative on purpose: a hostile payload can nest messages up to
     * protobuf's recursion limit, and a recursive walk would trade a bounded
     * parse failure for an unbounded {@code StackOverflowError}.
     */
