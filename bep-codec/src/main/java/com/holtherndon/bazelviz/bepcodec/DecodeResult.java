 ...
 *
 * <p>Failure is a value here, not an exception: the import pipeline records the detail in {@code
 * import_diagnostics} and keeps going, because a single unparseable event must not abort a session
 * (plan 21.3). The unavailable half of the result is an empty {@link Optional} rather than a null
 * or a placeholder, per the project rule that unknown data stays visibly unknown.
 *
 * @param status decode outcome; {@link DecodeStatus#name()} is what {@code
 *     bep_events.decode_status} stores
 * @param event the parsed message, present unless {@code status} is {@link DecodeStatus#FAILED}
 * @param failureDetail a human-readable explanation, present only when {@code status} is {@link
 *     DecodeStatus#FAILED}
 */
public record DecodeResult(
  ...
  public static DecodeResult ok(BuildEvent event) {
    return new DecodeResult(DecodeStatus.OK, Optional.of(event), Optional.empty());
  }
  ...
  public static DecodeResult withUnknownFields(BuildEvent event) {
    return new DecodeResult(DecodeStatus.UNKNOWN_FIELDS, Optional.of(event), Optional.empty());
  }
  ...
  /**
   * True when the message carried at least one field this build does not recognise. This is the
   * value for {@code bep_events.has_unknown_fields}.
   */
   ...
   * The parsed message.
   *
   * @throws IllegalStateException if this result is {@link DecodeStatus#FAILED}; callers that may
   *     see failures must branch on {@link #isFailed()} first
   */
