 ...
 *
 * <p>This is the hash behind {@link EventIdKey}, and therefore behind the {@code event_id_hash}
 * column. Two properties matter and both are pinned by tests:
 *
 * <ul>
 *   <li><b>Stability.</b> Hashes are written into session databases. A change to this algorithm
 *       silently invalidates every already-indexed session, so {@code Murmur3Hash128Test} pins
 *       concrete values; a failure there means a schema/format decision, not a test to update.
 *   <li><b>Dispersion.</b> Only the low 64 bits reach the {@code event_id_hash} column, so the low
 *       half alone must scatter well. The event-id key keeps the full serialized id alongside the
 *       hash precisely so that the residual birthday risk — around 7e-5 at fifty million distinct
 *       ids — can be resolved exactly rather than assumed away.
 * </ul>
 *
 * <p>A non-cryptographic hash is the right tool: this is an identity index over locally produced
 * data, not a defence against an adversary choosing colliding ids, and it must run once per event
 * on builds with tens of millions of them.
 */
public final class Murmur3Hash128 {
  ...
  /**
   * Hashes a {@link ByteString} without copying it when its backing storage allows a direct view.
   */
  ...
  /**
   * Hashes the remaining bytes of {@code input}. The caller's buffer is not modified — neither its
   * position nor its byte order.
   */
