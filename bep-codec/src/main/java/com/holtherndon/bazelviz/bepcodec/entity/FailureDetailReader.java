...
/**
 * Reads a {@code FailureDetail} into a {@link FailureInfo}, through descriptors rather than through
 * generated accessors.
 *
 * <p>The proto documents its own forward-compatibility contract: the set oneof field is the
 * category, and field number 1 inside it is the subcategory enum. Reading it that way means a
 * category or subcategory added by a Bazel newer than this build's vendored protos is reported by
 * name instead of vanishing — which matters, because the categories are exactly the vocabulary a
 * failures view groups by.
 */
public final class FailureDetailReader {
  ...
  /**
   * @return empty when the detail is entirely blank, which is how the wire spells "there was no
   *     failure detail" — an empty submessage would otherwise become a failure row for something
   *     that did not fail
   */
  ...
  /**
   * The subcategory enum at field number 1, read positionally because that is the contract the
   * proto documents — not by a field name this build would have to know in advance.
   */
   ...
   *
   * <p>Zero is read as absent rather than as a real code. The proto sets this field only for {@code
   * NON_ZERO_EXIT}, so a zero here is proto3's default for "not set" and never a process that
   * exited successfully — a failed spawn that exited 0 is a contradiction the wire cannot express.
   */
