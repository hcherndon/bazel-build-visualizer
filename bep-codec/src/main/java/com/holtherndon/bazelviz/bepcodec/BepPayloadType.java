...
/**
 * The value for {@code bep_events.event_type}: the wire field number of a
 * {@code BuildEvent}'s payload oneof.
 *
 * <p>The field number, not the enum ordinal, is what gets stored. Ordinals shift
 * whenever a case is inserted into the generated enum; field numbers are fixed
 * by the {@code .proto} and are the one identifier Bazel guarantees across
 * versions. Since {@code event_type} is indexed and queried for the lifetime of
 * a session database, storing anything less stable would make old sessions
 * mean different things under a newer build.
 *
 * <p>{@link #NONE} is reserved for an event with no payload set. That is a real
 * state — the {@code last_message} sentinel carries none — not a missing value,
 * which is why {@code event_type} is {@code NOT NULL} while other unavailable
...
public final class BepPayloadType {
    ...
    /**
     * The payload case for a stored {@code event_type}, or {@code null} when the
     * number belongs to a payload this build does not model — a session indexed
     * by a newer version, or reindexed from a journal a newer Bazel wrote.
     */
     ...
     *
     * <p>An unmodelled number renders as {@code payload#31} rather than as a
     * guess or a blank, so a stream from a newer Bazel is visibly a payload kind
     * this build does not name yet.
     */
