...
/**
 * Ties the catalog to a hand-written expectation and to the generator. Three independent statements
 * of the same structure must agree: the literal list below, the catalog's arithmetic, and the
 * events the generator actually builds. Two agreeing would prove nothing much; three do.
 */
final class BepFixtureCatalogTest {

  /**
   * The canonical 21-event stream: prologue, one target unit, the test unit, no padding, epilogue.
   * Written out by hand from the documented layout.
   */
  private static final List<BuildEvent.PayloadCase> EXPECTED_21 =
      List.of(
          STARTED, // 0
          PROGRESS, // 1
          OPTIONS_PARSED, // 2
