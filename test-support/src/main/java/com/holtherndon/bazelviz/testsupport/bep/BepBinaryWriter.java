...
/**
 * Writes a byte-exact {@code --build_event_binary_file}: each {@code BuildEvent} serialized with a
 * varint length prefix, concatenated, nothing else. That is precisely {@link
 * BuildEvent#writeDelimitedTo(OutputStream)}, which is the same call Bazel's binary transport
 * makes, so the fixture is the real layout rather than an approximation of it.
 *
 * <p>Events are consumed from an iterator and written one at a time; no list of events is ever
 * held, so a 200,000-event fixture costs one message of memory.
 */
