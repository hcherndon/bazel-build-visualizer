...
/**
 * The expected structure of a {@link SyntheticBepStream}, derived from the documented layout rules
 * rather than by reading the generator's output.
 *
 * <h2>Why this is a separate implementation</h2>
 *
 * A test that checks a parser against structure recovered from the same code that produced the
 * bytes proves only that the generator is self-consistent. So this class re-derives event count,
 * payload-case order, id-case order and the full announcement graph from the layout arithmetic
 * alone, with no call into {@link SyntheticBepStream}'s message building. When the two disagree,
 * one of them is wrong and the fixture's own test says so.
 *
 * <p>{@link #payloadCaseAt(int)} and {@link #idCaseAt(int)} are O(1) and allocate nothing. The
 * list-returning methods materialize one small value per event and are fixture-scale conveniences —
 * do not call {@link #payloadCases()} on a 200,000-event stream and expect it to be free.
 */
public final class BepFixtureCatalog {
   ...
   * @param childIndex index of the event that later carries the id, or {@link #CHILD_NEVER_ARRIVES}
   *     when the child is announced and never emitted
   * @param childIdCase which {@code BuildEventId} variant the announced id is
   */
  public record AnnouncedEdge(
      int parentIndex, int ordinal, int childIndex, BuildEventId.IdCase childIdCase) {
  ...
  public int progressEventIndex(int opaque) {
    ...
    if (opaque <= targetUnits) {
      return SyntheticBepStream.PROLOGUE_EVENTS
          + SyntheticBepStream.EVENTS_PER_TARGET_UNIT * (opaque - 1)
          + 1;
    }
  ...
  /**
   * Every announced parent → child pair, in the order the announcing events appear and, within an
   * event, in {@code children} order. Fixture-scale convenience.
   */
  public List<AnnouncedEdge> announcedEdges() {
    ...
    for (int opaque = 0; opaque < progressCount - 1; opaque++) {
      edges.add(
          new AnnouncedEdge(
              progressEventIndex(opaque),
              0,
              progressEventIndex(opaque + 1),
              BuildEventId.IdCase.PROGRESS));
    }
    ...
    if (announceMissingTargetSummary) {
      edges.add(
          new AnnouncedEdge(testStart, 4, CHILD_NEVER_ARRIVES, BuildEventId.IdCase.TARGET_SUMMARY));
    }
