...
/**
 * One call that lays a complete set of BEP fixture files into a directory: the good stream in all
 * three on-disk shapes, plus one file per damage mode.
 *
 ...
 * }</pre>
 *
 * Every damaged file is derived from the same {@link #binary()} bytes, so the intact prefix of each
 * is identical and a test can compare event counts against {@link BepFixtureCatalog} directly.
 */
public final class BepFixtures {
   ...
   *
   * @param unknownFieldPayloadOffset byte offset of the payload of the one event in {@link
   *     #unknownField()} that carries unknown fields
  ...
  public static Set writeAll(Path directory, SyntheticBepStream stream) throws IOException {
    ...
    BepDamage.Damage flipped =
        BepDamage.flipPayloadByte(
            binary, directory.resolve(CORRUPT_PAYLOAD_BYTE_NAME), corruptFrame, 1);

    BepDamage.Damage garbage =
        BepDamage.appendTrailingGarbage(
            binary, directory.resolve(TRAILING_GARBAGE_NAME), 64, 0xC0FFEEL);

    int unknownFieldEventIndex =
        Math.min(stream.eventCount() - 1, SyntheticBepStream.PROLOGUE_EVENTS);
    Path unknownField = directory.resolve(UNKNOWN_FIELD_NAME);
    long unknownOffset =
        UnknownFieldFixture.writeBinaryStreamWithUnknownFieldEvent(
