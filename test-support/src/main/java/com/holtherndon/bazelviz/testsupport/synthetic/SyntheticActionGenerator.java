 ...
 *
 * <p>The load-bearing property is O(1) random access: {@link #actionAt(long)}
 * is a pure function of {@code (seed, index)}, so a 50-million-row table model
 * can fetch any page without anything being materialized or stored. All
 * benchmark and spike code must go through this API; do not pre-generate
 * arrays of actions.
 */
public final class SyntheticActionGenerator {
    ...
    public static final String[] MNEMONICS = {
        "CppCompile", "CppLink", "Javac", "JavaLink", "GoCompilePkg", "GoLink",
        "TsProject", "Turbine", "KotlinCompile", "ProtoCompile", "GenProtoDescriptor",
        "ObjcCompile", "SwiftCompile", "Genrule", "FileWrite", "SymlinkTree",
        "TestRunner", "SourceSymlinkManifest", "PackageZip", "ActionCacheCheck",
        "CcStrip", "SolibSymlink", "Middleman", "StarlarkAction",
    };
    ...
    public SyntheticAction actionAt(long index) {
        ...
        return new SyntheticAction(index, start, start + duration, mnemonic, target, pkg,
                status, cacheState, runner, inputCount, inputBytes, outputCount, outputBytes);
    }
