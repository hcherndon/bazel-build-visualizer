...
/**
 * Deterministic synthetic action-dependency edges forming a DAG: every edge
 * points from a producer with a smaller index to a consumer with a larger
 * index, so cycle-free ordering is guaranteed by construction. Edges are
 * streamed, never materialized here; CSR construction owns materialization.
 */
