...
/**
 * Deterministic generator of synthetic length-delimited binary frames standing
 * in for raw BEP journal payloads in journal read/write benchmarks. Frame
 * {@code index} is a pure O(1) function of {@code (seed, index)}: the payload
 * bytes come from a SplitMix64 chain and the length from a cubic-skewed
 * distribution over [{@value #MIN_FRAME_BYTES}, {@value #MAX_FRAME_BYTES}]
 * bytes, skewed small like real BEP progress/action events.
 *
 * <p>{@link #frameAt(long, byte[])} allocates nothing beyond the caller's
 * buffer, so benchmarks can pump millions of frames without GC pressure.
 */
public final class SyntheticRawEvents {
    ...
    /**
     * Writes the payload of frame {@code index} into {@code dest} starting at
     * offset 0 and returns its length in bytes, always within
     * [{@value #MIN_FRAME_BYTES}, {@value #MAX_FRAME_BYTES}]. Size buffers to
     * {@link #MAX_FRAME_BYTES} so any frame fits.
     *
    ...
    public int frameAt(long index, byte[] dest) {
        ...
        if (dest.length < length) {
            throw new IllegalArgumentException(
                    "dest holds " + dest.length + " bytes but frame " + index + " needs " + length
    ...
    /**
     * Expected total payload bytes across all frames. Closed form, not a scan:
     * the cubic skew has E[u^3] = 1/4, so the expected frame length is
     * MIN + span/4 (~560 bytes).
     */
