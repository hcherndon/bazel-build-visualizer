...
/**
 * A deterministic, structurally realistic Build Event Protocol stream built from the real generated
 * {@code build_event_stream.proto} classes.
 *
 * <h2>Why this exists</h2>
 *
 * Every Phase 1 parser (binary framing, JSON records, normalization, parent/child linking) needs
 * input that behaves like a real Bazel invocation. Hand-rolled two-event fixtures pass tests that a
 * real stream would fail, so this generator produces the actual event shapes Bazel emits, in the
 * order Bazel emits them, with the announcement graph wired up.
 *
 * <h2>Announcement graph</h2>
 *
 * BEP is a tree: every event other than the root {@code started} event is announced as a child of
 * exactly one earlier event, and the stream is complete when every announced child has arrived.
 * This generator guarantees:
 *
 * <ul>
 *   <li>every event except index 0 is announced by a strictly earlier event;
 *   <li>each announced child id is byte-identical to the {@code id} of the event that later carries
 *       it;
 *   <li>no id is announced twice;
 *   <li>the final event carries {@code last_message = true}.
 * </ul>
 *
 * The one deliberate exception is opt-in: {@link Options#announceMissingTargetSummary()} makes the
 * test target's {@code target_configured} event announce a {@code target_summary} child that never
 * arrives, which is the case plan 17.11 and the {@code bep_announced_missing} table exist for. It
 * is off by default so that the default fixture satisfies "every announced child arrived".
 *
 * <h2>Layout</h2>
 *
 * The stream is prologue + N target units + one test unit + optional progress padding + epilogue,
 * so any requested event count {@code >= } {@link #MIN_EVENT_COUNT} is produced <em>exactly</em>:
 *
 * <pre>
 *   index                       event
 ...
 * <h2>Determinism</h2>
 *
 * {@link #eventAt(int)} is a pure function of {@code (seed, eventCount, index)}: nothing is cached
 * and nothing is materialized, so a 200,000-event stream costs one message at a time. It is O(1)
 * for every event except the {@code pattern} event at index 7, which announces one child per
 * configured target and is therefore O(target count) in both time and message size — as Bazel's own
 * {@code pattern_expanded} event is. On a large stream that single event is megabytes, which is
 * deliberate: a parser that assumes every event is small is wrong about real captures too.
 *
 * <p>Two runs with the same options produce byte-identical serialized output. Map-typed protobuf
 * fields are deliberately left unset — protobuf map iteration order is unspecified, so setting one
 * would make the serialized bytes non-reproducible and break offset assertions.
 *
 * <p>The expected structure of a stream is available separately from {@link BepFixtureCatalog},
 * which derives it from the layout rules rather than from this generator, so a test asserting
 * against the catalog is not merely asking the generator to agree with itself.
 */
...
public final class SyntheticBepStream {
  ...
  /**
   * Generation parameters. {@code eventCount} is exact — the produced stream contains precisely
   * that many events.
   */
  ...
  /**
   * Builds the event at {@code index}. Pure function of {@code (seed, eventCount, index)} — calling
   * it twice returns equal messages. Cost is independent of {@code index} except at index 7, the
   * {@code pattern} event, which announces every configured target.
   */
        ...
        CommandLineOuterClass.CommandLineSection.newBuilder()
            ...
            .setOptionList(
                CommandLineOuterClass.OptionList.newBuilder()
                    .addOption(
                        CommandLineOuterClass.Option.newBuilder()
  ...
  private BuildEvent workspaceStatusEvent() {
    return BuildEvent.newBuilder()
        ...
        .setWorkspaceStatus(
            WorkspaceStatus.newBuilder()
                .addItem(
                    WorkspaceStatus.Item.newBuilder()
                        .setKey("BUILD_TIMESTAMP")
                        .setValue(Long.toString(BASE_EPOCH_MICROS / 1_000_000L)))
                .addItem(
                    WorkspaceStatus.Item.newBuilder().setKey("BUILD_HOST").setValue("builder-07"))
                .addItem(WorkspaceStatus.Item.newBuilder().setKey("BUILD_USER").setValue("builder"))
                .addItem(
                    WorkspaceStatus.Item.newBuilder()
  ...
  private BuildEvent patternEvent() {
    ...
    b.setExpanded(
        PatternExpanded.newBuilder()
            .addTestSuiteExpansions(
                PatternExpanded.TestSuiteExpansion.newBuilder()
  ...
  private BuildEvent targetConfiguredEvent(int unit) {
    ...
    return BuildEvent.newBuilder()
        ...
        .setConfigured(
            TargetConfigured.newBuilder()
                .setTargetKind(
                    TARGET_KINDS[Math.floorMod((int) hash(0x71D, unit), TARGET_KINDS.length)])
            ...
            ActionExecuted.newBuilder()
                ...
                .setPrimaryOutput(
                    File.newBuilder()
                        ...
                        .setDigest(
                            String.format("%016x%016x", hash(0xD16, unit), hash(0xD17, unit)))
                        .setLength(1024L + Math.floorMod(hash(0x1E4, unit), 8_000_000L)))
                .addCommandLine("/usr/bin/" + mnemonic.toLowerCase(Locale.ROOT))
                .addCommandLine("-c")
                .addCommandLine(
                    PACKAGE_PATHS[Math.floorMod(unit, PACKAGE_PATHS.length)] + "/src" + unit)
                .addCommandLine("-o")
            ...
            TargetComplete.newBuilder()
                ...
                .addOutputGroup(
                    OutputGroup.newBuilder()
                        ...
                        .setIncomplete(false)
                        .addFileSets(
                            BuildEventId.NamedSetOfFilesId.newBuilder().setId(namedSetId(unit))))
                .addTag(unit % 3 == 0 ? "no-remote-cache" : "manual"))
        .build();
  }

  private File file(int unit, int fileIndex) {
            ...
            TargetComplete.newBuilder()
                ...
                .addOutputGroup(
                    OutputGroup.newBuilder()
                        .setName("default")
                        .addFileSets(
                            BuildEventId.NamedSetOfFilesId.newBuilder().setId(testNamedSetId())))
                .addTag("flaky"))
        .build();
  }
    ...
    return BuildEvent.newBuilder()
        ...
        .setTestResult(
            TestResult.newBuilder()
                ...
                .setExecutionInfo(
                    TestResult.ExecutionInfo.newBuilder()
                        .setStrategy("remote")
  ...
  private File testOutputFile(String name) {
    return File.newBuilder()
        .setName(name)
        .setUri(
            "file://"
                + WORKSPACE_DIRECTORY
                + "/bazel-out/k8-fastbuild/testlogs/src/test/java/com/example/core/core_test/"
                + name)
    ...
    return BuildEvent.newBuilder()
        ...
        .setBuildMetrics(
            BuildMetrics.newBuilder()
                .setActionSummary(
                    BuildMetrics.ActionSummary.newBuilder()
                        ...
                        .addActionData(
                            BuildMetrics.ActionSummary.ActionData.newBuilder()
                                .setMnemonic("Javac")
                ...
                .setCumulativeMetrics(
                    BuildMetrics.CumulativeMetrics.newBuilder().setNumAnalyses(3).setNumBuilds(2)))
        .build();
  }
  ...
  private BuildEvent buildToolLogsEvent() {
    return BuildEvent.newBuilder()
        ...
        .setLastMessage(true)
        .setBuildToolLogs(
            BuildToolLogs.newBuilder()
                .addLog(
                    File.newBuilder()
                        .setName("command.profile.gz")
                        .setUri(
                            "file:///home/builder/.cache/bazel/_bazel_builder/9f2c/command.profile.gz"))
                .addLog(
                    File.newBuilder()
                        .setName("elapsed time")
                        .setContents(ByteString.copyFromUtf8("15.4"))))
        .build();
  }
    ...
    return BuildEventId.newBuilder()
        .setActionCompleted(
            BuildEventId.ActionCompletedId.newBuilder()
                ...
                .setLabel(label)
                .setConfiguration(
                    BuildEventId.ConfigurationId.newBuilder().setId(CONFIGURATION_ID)))
        .build();
  }
