...
class CommandLineParserTest {
    ...
    void startupOptionsComeFirst() {
        BazelCommand command = new CommandLineParser().parse(BAZEL, CWD, List.of(
                "--output_base=/tmp/ob", "--max_idle_secs=10", "build", "--keep_going", "//..."));

        assertThat(command.startupArgs()).containsExactly("--output_base=/tmp/ob", "--max_idle_secs=10");
    ...
    void flagsAndTargetsInterleave() {
        BazelCommand command = new CommandLineParser().parse(BAZEL, CWD, List.of(
                "build", "//a", "--keep_going", "//b", "--verbose_failures", "//c"));
        ...
        assertThat(command.toArgv()).containsExactly(
    ...
    void onlyTheFirstSeparatorSeparates() {
        ...
        assertThat(command.toArgv()).containsExactly(
                "/usr/bin/bazel", "run", "//:tool", "--", "--flag", "--", "value");
    }

    @Test
    @DisplayName("injected flags land before a user's -- separator, never after it")
    void injectedFlagsPrecedeTheSeparator() {
        BazelCommand original = new CommandLineParser().parse(BAZEL, CWD, List.of(
                "run", "//:tool", "--", "--tool-arg"));

        BazelCommand instrumented = original.toBuilder()
                .addCommandArg("--bes_backend=grpc://127.0.0.1:1234")
                .build();
    ...
    void separateValuesAreBoundUsingCapabilities() {
        ...
        BazelCommand command = new CommandLineParser(Optional.of(capabilities)).parse(BAZEL, CWD, List.of(
                "build", "--build_event_json_file", "/tmp/out.json", "//..."));

        assertThat(command.commandArgs())
                .containsExactly("--build_event_json_file", "/tmp/out.json");
        assertThat(command.targets()).containsExactly("//...");
    }
    ...
    void valuelessFlagsDoNotSwallowTargets() {
        BazelCapabilities capabilities = capabilitiesWith(new FlagSpec(
                "keep_going", Set.of("build"), true, false, Optional.of(false),
                Optional.empty(), List.of()));

        BazelCommand command = new CommandLineParser(Optional.of(capabilities)).parse(BAZEL, CWD, List.of(
                "build", "--keep_going", "//foo:bar"));
    ...
    void unknownFlagsDoNotSwallowTargetShapedTokens() {
        BazelCommand command = new CommandLineParser().parse(BAZEL, CWD, List.of(
                "build", "--some_unknown_flag", "//foo:bar", "@repo//x", ":local", "..."));
    ...
    private static BazelCapabilities capabilitiesWith(FlagSpec... specs) {
        Map<String, FlagSpec> byName = new java.util.LinkedHashMap<>();
