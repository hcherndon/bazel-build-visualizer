...
/**
 * Something about the user's command that the planner will not decide on its own
 * (plan 4.3 "conflicts").
 *
 * <h2>Mandatory conflicts block the launch</h2>
 *
 * <p>Plan 4.3 ends with "do not launch until mandatory conflicts are resolved",
 * and {@link #mandatory} is that flag. The rule exists because every mandatory
 * conflict here is a case where guessing would either destroy the user's data or
 * silently produce a session that is not a recording of the command they asked
 * for. Replacing a corporate BES backend without asking would divert build
 * results away from the system their team relies on; overwriting an existing
 * {@code --build_event_binary_file} would delete a file the user pointed at.
 *
 * <p>Advisory conflicts are things the user should see but that have a safe
 * default: a later option overriding an earlier one, for example, is Bazel's
 * documented behavior and the plan asks only that it be made visible.
 *
 ...
 * @param resolutions the choices offered, in the order they should be shown; the
...
public record PlanConflict(
     ...
     * @param consequence what the user gives up by choosing it — never empty,
     *     because a choice presented without its cost is not an informed one
     */
