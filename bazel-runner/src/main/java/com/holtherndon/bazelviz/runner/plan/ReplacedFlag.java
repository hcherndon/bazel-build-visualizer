...
/**
 * A user-supplied option the plan removes or overrides, and the confirmation
 * that permitted it.
 *
 * <p>Plan 8.4: "user-supplied explicit options take precedence unless the user
 * confirms replacement". A replacement therefore cannot be constructed without
 * naming the {@link PlanConflict.Resolution} the user chose — {@link #approvedBy}
 * is not optional. That makes it impossible to build a plan that quietly drops
 * one of the user's flags: the type will not let you.
 *
