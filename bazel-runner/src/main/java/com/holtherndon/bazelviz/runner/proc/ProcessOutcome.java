 ...
 *
 * <p>{@link #exitCode} is an {@link OptionalInt} because a process this
 * application killed, or one whose wait was interrupted, has no exit code that
 * means anything — and reporting {@code 0} or {@code -1} for it would be an
 * unavailable value rendered as a number (plan 11.4). {@link #terminatedBy}
 * says which stop was used, so a session can distinguish "the user cancelled"
 * from "Bazel failed", which are different sessions with different states.
 *
 * <p>Bazel's own exit codes are recorded but not interpreted here: 1 is a build
 * failure, 2 a command-line error, 8 an interrupt, 37 an out-of-memory abort,
 * and the set grows between versions. Mapping them belongs where the version is
 * known, not in a record that only observed the number.
 *
 * @param exitCode the process exit status, absent when it was killed or the wait
 *     did not complete
 * @param terminatedBy the stop this application applied, absent when the process
 *     ended on its own
...
public record ProcessOutcome(
    ...
    public static ProcessOutcome cancelled(OptionalInt code, CancellationMode mode, Duration duration) {
