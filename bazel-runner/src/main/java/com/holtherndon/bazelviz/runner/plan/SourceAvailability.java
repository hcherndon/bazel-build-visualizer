...
/**
 * What the session will and will not contain if this plan runs (plan 7.1,
 * {@code InstrumentationPlanner} output: "source availability plan").
 *
 * <p>This exists so that the answer to "why is the dependency graph empty" is
 * available <em>before</em> the build rather than discovered afterwards in an
 * empty view. Every later phase's views gate on it: a view whose source is
 * {@link Availability#UNAVAILABLE} explains itself instead of rendering nothing,
 * which is the difference between a tool that looks broken and one that is
...
public record SourceAvailability(Map<DataSource, Entry> bySource) {
     ...
     * @param reason why, in one sentence; always present, because an empty view
     *     with no explanation is the failure this record exists to prevent
    ...
    /**
     * The verdict for {@code source}. A source nobody recorded a verdict for is
     * {@link Availability#UNKNOWN}, never {@code UNAVAILABLE}: forgetting to
     * plan for a source is not evidence that it cannot be captured.
     */
    public Entry entry(DataSource source) {
        return bySource.getOrDefault(
                source,
                new Entry(Availability.UNKNOWN, "no availability was recorded for " + source, Optional.empty()));
    }
