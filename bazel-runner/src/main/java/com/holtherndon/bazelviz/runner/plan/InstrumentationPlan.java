...
/**
 * The complete answer to "what will actually be run, and why" (plan 7.1
 * {@code InstrumentationPlanner} output, rendered by plan 4.3).
 *
 * <p>ADR-007 is the whole point of this record: the user sees the command they
 * typed and the command that will run, side by side, with a line of explanation
 * for every difference. A plan is inert — building one runs nothing — so it can
 * be shown, edited, vetoed and re-planned before any process starts.
 *
 * <h2>The launch gate</h2>
 *
 * <p>{@link #canLaunch()} is the single place plan 4.3's "do not launch until
 * mandatory conflicts are resolved" is enforced. Callers ask the plan rather
 * than re-deriving the condition, so a new conflict kind cannot be introduced
 * without every launcher honouring it.
 *
 ...
 * @param expectedOutputs files the build is expected to write, so the coordinator
 *     knows what to look for and the user knows what is being created
...
public record InstrumentationPlan(
    ...
    /**
     * Whether this plan may be launched. False while any mandatory conflict is
     * unresolved, and false when the plan recorded an error — an error means the
     * planner could not build a command that would capture anything.
     */
    ...
    /**
     * The injected arguments as plain strings, for
     * {@code manifest.injectedFlags} (plan 10.3). Only the applied ones: the
     * manifest records what ran, not what was considered.
     */
