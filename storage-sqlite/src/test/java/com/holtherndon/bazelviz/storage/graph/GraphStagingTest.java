...
/**
 * The staging tables and their one algorithm: path resolution over the fragment tree.
 *
 * <p>Covered until now only through the importer, which cannot stage a malformed chain — its input
 * is a protobuf that always declares parents eventually. These tests stage the pathological shapes
 * directly: the forward reference (finding Q4), the fragment nobody declared, and the cycle a
...
final class GraphStagingTest {
  ...
  private String stagedPath(int artifact) throws SQLException {
    try (Statement statement = connection.createStatement();
        ResultSet rows =
            statement.executeQuery("SELECT path FROM stage_path WHERE artifact = " + artifact)) {
