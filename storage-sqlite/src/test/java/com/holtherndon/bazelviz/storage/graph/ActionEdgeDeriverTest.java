 ...
 *
 * <p>{@code enrichment}'s {@code ActionEdgeAndIndexTest} covers the same
 * recipe through a real aquery file, which proves the import feeds it; this
 * covers it against rows whose shape is chosen, so each step of plan 13.1 —
 * transitive depset reach, source exclusion, deduplication, the self-edge
 * guard, the via-artifact rule — has a fixture that isolates it.
 *
 * <h2>The fixture</h2>
 *
 * <p>Three actions. A produces {@code a.out}. B declares {@code a.out} through
 * depset d1 and produces {@code b.out}. C declares depset d2, whose children
 * are d1 and whose direct artifacts are {@code b.out} and the source file
 * {@code src.c} — so C reaches {@code a.out} only transitively, and the
 * source file reaches nothing because nothing produces it.
 */
final class ActionEdgeDeriverTest {
    ...
    void selfEdgesAreExcluded() throws Exception {
        ...
        assertThat(scalar("SELECT count(*) FROM action_edges"
                + " WHERE producer_id = consumer_id")).isZero();
    }
    ...
    void multipleArtifactsDeduplicateToOneEdge() throws Exception {
        ...
        assertThat(scalar("SELECT count(*) FROM action_edges"
                + " WHERE producer_id = 1 AND consumer_id = 2")).isEqualTo(1);
        ...
        assertThat(scalar("SELECT count(*) FROM action_edges"
                ...
                + "   AND via_artifact_id IS NULL")).isEqualTo(1);
    }
    ...
    void singleArtifactIsRecorded() throws Exception {
        ...
        assertThat(scalar("SELECT via_artifact_id FROM action_edges"
                + " WHERE producer_id = 1 AND consumer_id = 2")).isEqualTo(1);
    }
    ...
    void observedEdgesFollowTheExecutionLog() throws Exception {
        ...
        assertThat(scalar("SELECT count(*) FROM action_edges"
                ...
                + "   AND producer_id = 1 AND consumer_id = 2")).isEqualTo(1);
    ...
    private java.util.List<String> edges() throws SQLException {
