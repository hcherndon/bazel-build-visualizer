 ...
 *
 * <p>The fixture reproduces the shape measurement TS2 found: Bazel's summary window is much
 * narrower than the attempts it summarises, because it excludes the failed retries. A view that
 * read the summary and called it "elapsed across attempts" would understate the test by the width
 * of the retries — 13x on the six-attempt test that was measured.
 */
final class TestQueriesTest {
  ...
  void elapsedIsComputedFromTheAttempts() throws Exception {
    try (TestQueries queries = new TestQueries(database.newReadConnection())) {
      ...
      assertThat(scalar("SELECT bazel_first_start_micros FROM tests")).isEqualTo(6_000_000L);
