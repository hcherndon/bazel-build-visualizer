 ...
 *
 * <p>Small next to the plan's five-million Tier 3 ceiling, and large enough that the two things
 * that would make this unusable — a query per action, or an aggregation that retains every duration
 * — show up as minutes rather than seconds. The figures it prints go into docs/performance.md.
 *
 * <p>Synthetic rather than a real Bazel invocation, deliberately: a build of this size costs a
 * Bazel server and several gigabytes, and nothing being measured here depends on Bazel's behaviour.
 */
  ...
  void aQuarterMillionActionsCollectInOnePass() throws Exception {
    try (SessionDatabase database = SessionDatabase.open(tempDir.resolve("scale.db"))) {
      ...
      try (MetricQueries queries = new MetricQueries(database.newReadConnection())) {
        metrics =
            queries.collect(
                MetricQueries.Request.everything(CriticalPath.DurationSource.EXECUTION_ATTEMPT));
      }
      long collectMillis = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - started);

      long ruled = System.nanoTime();
      var findings = FindingRules.run(metrics.findingInputs(FindingThresholds.defaults()));
      ...
      System.out.printf(
          ...
          ruleMillis,
          sweep.peakActive(),
          sweep.parallelismFactor().orElse(0),
          findings.size(),
          metrics.aggregate(GroupAggregate.Dimension.MNEMONIC).orElseThrow().totalGroups(),
          metrics.aggregate(GroupAggregate.Dimension.PACKAGE).orElseThrow().totalGroups(),
      ...
      assertThat(metrics.aggregate(GroupAggregate.Dimension.MNEMONIC).orElseThrow().totalGroups())
          .isEqualTo(MNEMONICS);
  ...
  private void populate(Connection writer) throws Exception {
    try (Statement statement = writer.createStatement()) {
      ...
      for (int i = 0; i < MNEMONICS; i++) {
        statement.execute(
            "INSERT INTO mnemonics (id, value) VALUES (" + (i + 1) + ", 'Mnemonic" + i + "')");
      }
      for (int i = 0; i < PACKAGES; i++) {
        statement.execute(
            "INSERT INTO labels (id, value) VALUES (" + (i + 1) + ", '//pkg/" + i + ":lib')");
      }
    }
    boolean autoCommit = writer.getAutoCommit();
    writer.setAutoCommit(false);
    try (PreparedStatement action =
            writer.prepareStatement(
                "INSERT INTO actions (id, primary_output, label_id, mnemonic_id, outcome)"
                    + " VALUES (?, ?, ?, ?, 'SUCCESS')");
        PreparedStatement attempt =
            writer.prepareStatement(
