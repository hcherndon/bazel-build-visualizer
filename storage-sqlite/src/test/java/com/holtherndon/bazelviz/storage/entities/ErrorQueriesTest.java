 ...
 *
 * <p>The property under test is unglamorous and was the whole bug: the query
 * selects each progress event's journal address, and the address has to survive
 * as far as the caller. {@code progress.stderr} is the only copy of a compiler
 * or parser diagnostic there is (finding X2), so a {@code ProgressRef} that
 * knows how many bytes exist and not where they are is a row that can report a
 * diagnostic's size and never its text.
 */
final class ErrorQueriesTest {
    ...
    void buildFixture() throws Exception {
        ...
        exec("INSERT INTO progress_output (bep_event_id, ordinal, stdout_bytes, stderr_bytes)"
    ...
    void onlyStderrEvents() throws Exception {
        try (ErrorQueries queries = new ErrorQueries(database.newReadConnection())) {
            ...
            assertThat(refs).extracting(ErrorQueries.ProgressRef::bepEventId)
                    .containsExactly(1L, 2L);
            assertThat(refs).extracting(ErrorQueries.ProgressRef::stderrBytes)
                    .containsExactly(512, 128);
        }
    }
    ...
    private void event(long id, long sequence, int segment, long offset, int length)
        ...
        exec("INSERT INTO bep_events (id, stream_id, sequence, event_type, raw_segment,"
                ...
                + id + ", 1, " + sequence + ", 3, " + segment + ", " + offset + ", " + length
                + ", 'OK', " + (1_700_000_000_000_000L + id) + ")");
    }
