...
/**
 * One row of {@code import_diagnostics}: something the import needs the user
 * to know about.
 *
 * <p>Every limit hit, every byte dropped and every record skipped becomes one
 * of these. Plan rule 12 — never silently truncate, sample, drop or override —
 * is enforced by this table existing and being shown, so a producer that
 * discards data without writing a diagnostic has a defect.
 *
 * <p>{@code segmentIndex} and {@code byteOffset} are {@link OptionalInt} and
 * {@link OptionalLong} rather than {@code int}/{@code long} because a
 * diagnostic about the session as a whole has no position, and "offset 0" is a
 * real, very different statement from "no offset" (plan 11.4).
 */
