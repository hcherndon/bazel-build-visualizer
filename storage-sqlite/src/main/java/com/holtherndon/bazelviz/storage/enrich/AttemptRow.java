 ...
 *
 * <p>Every optional is a fact about the build or the Bazel version. {@code startMicros} is absent
 * for every attempt captured on Bazel 6.5.0, which never reports one; {@code queueMicros} and its
 * neighbours are absent for anything that ran locally; {@code digestHash} is absent unless a cache
 * was in play.
 *
 * @param correlationNote why this attempt is attached to what it is attached to, or why it is
 *     attached to nothing — the exit criterion "ambiguous correlations remain visible" is this
 *     field reaching the screen
...
public record AttemptRow(
   ...
   *
   * <p>Kept as a group so the inspector can render every component that was measured and say
   * nothing about the ones that were not, rather than showing a row of zeroes for a
   * locally-executed spawn that never queued, never uploaded and never fetched.
   */
  public record Timing(
     ...
     *
     * <p>An unmeasured component is not a zero-length one. A local spawn has no queue time because
     * it never queued, and a row saying "Queue: 0ms" would be an answer to a question nobody asked.
     */
     ...
     *
     * <p>{@code total_time} is measured separately from its parts, and on a local build most parts
     * are absent, so the remainder is usually most of the total. It is shown as unaccounted rather
     * than folded into execution, which would claim a measurement that was not made.
     */
    public OptionalLong unaccountedMicros() {
      ...
      long summed =
          measuredComponents().stream().mapToLong(component -> component.micros().orElse(0)).sum();
