 ...
 *
 * <p>Codes are a small stable vocabulary rather than free text so the UI can group and explain
 * them, and so the truncation/corruption distinction the plan insists on (21.3) survives into the
 * database instead of being buried in a message string. Producers outside this module may write
 * their own codes; the column is plain TEXT and this class is a convention, not a constraint.
 */
public final class DiagnosticCodes {
   ...
   *
   * <p>Measured unique across every stream on all four supported Bazel versions, so this should
   * never appear on a clean import. When it does, the identity assumption has broken and one of the
   * two actions is not in the table — which the user has to be told, because the alternative is a
   ...
   *
   * <p>Zero occurrences in 1,829 measured references. A non-zero count means the capture is missing
   * events, so every byte total that walks through those sets is a lower bound rather than a total.
   */
