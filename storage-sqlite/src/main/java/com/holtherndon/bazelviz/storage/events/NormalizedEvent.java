...
/**
 * Everything one raw event contributes to the database: the event row, the identity it declares (if
 * any), and the children it announces (if any).
 *
 * <p>Bundling them keeps the three writes ordered correctly — the event row must exist before edges
 * can reference it — without every caller having to know that.
 *
 * @param event the {@code bep_events} row
 * @param identity the {@code bep_event_ids} row this event's own id defines, empty when the event
 *     carries no id
 * @param announcedChildHashes canonical hashes of the children this event announces, in
 *     announcement order; the list index becomes {@code ordinal}
 */
