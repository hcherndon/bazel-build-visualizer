 ...
 *
 * <p>It does not join Bazel's critical path to actions. The components name themselves with a
 * progress message and nothing else (P5), and ADR-009 wants Bazel's own answer preserved as Bazel's
 * regardless.
 *
 * <p>It does not write an anchor without saying what the anchor means. On Bazel 6.5.0 and 7.6.1 the
 * profile's absolute reference is floored to the whole second and published under a key that calls
 * it a finish, so the meaning and the uncertainty are columns beside the value rather than
...
public final class ProfileWriter implements AutoCloseable {
   ...
   *
   * <p>A marker is an instant event, so a phase's end is the next one's start (P2). There are five
   * to seven of them, so holding them costs nothing; every other command is written as it arrives.
   */
   ...
   *
   * <p>The first version of this class added every span, counter and thread to a batch and executed
   * all of them in {@link #finish()}. That is correct for the fifteen action spans a six-target
   * build produces and indefensible for a real one: a profile grows with everything the build did,
   * and plan 19.4 requires bounded memory of every import path. The driver holds each batched
   * statement's parameters until execution.
   */
  ...
  public void apply(EnrichmentCommand command) throws SQLException {
    switch (command) {
      ...
      case EnrichmentCommand.CriticalPathComponentSeen component -> writeCriticalPath(component);
      default -> {
  ...
  /**
   * Writes what had to wait for the whole file: the phases, whose ends are the next marker's start,
   * and the trace's extent.
   */
  ...
  private void writeHeader(EnrichmentCommand.ProfileHeaderSeen header) throws SQLException {
    ...
    buildIdMatches =
        sessionBuildId.isPresent() && header.buildId().isPresent()
   ...
   *
   * <p>{@code end_is_derived} is set for every one of them because every one of them is: the
   * profile states starts and never ends. The last phase has no successor and gets a null end
   * rather than the trace's maximum, which would be a boundary nothing measured.
   */
  private void writePhases() throws SQLException {
    if (phases.isEmpty()) {
      return;
    }
    phases.sort(Comparator.comparingLong(EnrichmentCommand.PhaseMarkerSeen::startMicros));
   ...
   *
   * <p>The difference is what {@code --experimental_profile_include_primary_output} would have
   * bought: without it the profile has spans and no way to say which action each belongs to (P4).
   */
  ...
  /**
   * Whether the profile's build id matched the session's; empty when one of them was absent and the
