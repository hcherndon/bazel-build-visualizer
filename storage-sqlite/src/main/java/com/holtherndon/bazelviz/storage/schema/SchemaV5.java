...
/**
 * Schema version 5: the declared action graph, the configured-target graph, and the action edges
 * derived from them.
 *
 * <h2>Why these are separate tables and not more columns on `actions`</h2>
 *
 * <p>Phase 3's {@code actions} table is what the build event stream said <em>executed</em>. {@code
 * aquery} reports what analysis <em>declared</em>. These are two populations that overlap and
 * neither contains the other: measured, {@code aquery} declares test-log actions a {@code build}
 * never runs, and the BEP reports a {@code stable-status.txt} action {@code aquery} never declares
 * (Q7 in {@code docs/aquery-and-cquery.md}).
 *
 * <p>So a declared action gets its own row, with a nullable link to the executed one. Merging them
 * would produce a table that is the build's action count in neither sense, and ADR-009 forbids
 * collapsing two sources into one unexplained number anyway.
 *
 * <p>Every non-obvious choice below cites a finding id from {@code docs/aquery-and-cquery.md}.
 */
final class SchemaV5 {
  ...
  public static final List<String> STATEMENTS =
      List.of(
          ...
          """,
          "CREATE INDEX ix_declared_outputs_artifact" + " ON declared_action_outputs (artifact_id)",
