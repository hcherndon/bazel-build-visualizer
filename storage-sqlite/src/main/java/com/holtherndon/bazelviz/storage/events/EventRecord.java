 ...
 *
 * <p>The row id is deliberately absent: SQLite assigns it, in normalization order, and a duplicate
 * delivery consumes none. Assigning ids in Java would mean burning an id on every retransmission
 * and then having to explain the holes.
 *
 * <p>{@code eventIdHash} and {@code eventMicros} are {@link OptionalLong} because both are
 * genuinely absent for some events — a progress event carries no timestamp, and not every event has
 * an id. Storing 0 for those would make them read back as "the epoch" and "id hash zero" (plan
 * 11.4).
 *
 ...
 * @param eventIdHash low 64 bits of the canonical event-id hash, empty when the event carries no id
