...
/**
 * Resolves {@code event_streams} rows, which every {@code bep_events} row has a foreign key to.
 *
 * <p>A stream is identified externally by its {@code stream_key} — a stable string the capture
 * layer derives from the BES stream identity or from the imported file — and internally by its row
 * id. Nothing outside this database ever sees the row id, per plan 11.1: row ids are not
 * cross-session identities.
 *
 * <p>{@link #open(String, Optional, Optional)} is idempotent on the stream key, so replaying a
 * journal after a crash re-resolves the same id instead of creating a second stream.
 *
 * <p>Sequence bookkeeping ({@code first_sequence}, {@code last_sequence}, {@code
 * contiguous_through}, {@code duplicate_count}, {@code gap_count}) is left to the ingestion layer
 * that actually observes the sequence numbers; this class only persists what it is told, through
 * {@link #updateProgress}. Those columns stay NULL until something has been observed — an unstarted
...
public final class StreamRegistry implements AutoCloseable {
  ...
  /**
   * Persists observed sequence bookkeeping for a stream. Absent values are written as NULL, never
   * as 0.
   */
