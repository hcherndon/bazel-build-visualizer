 ...
 *
 * <p>Diagnostics are written and committed <em>immediately</em>, not batched.
 * That is deliberate: the diagnostics that matter most describe things going
 * wrong — a corrupt frame, a full disk (plan 21.2), a truncated tail — and a
 * diagnostic that is still sitting in a batch when the process dies explains
 * nothing to the user afterwards.
 *
 * <p>Committing on a connection whose auto-commit is suspended by an active
 * {@link com.holtherndon.bazelviz.storage.BatchedInsert} is safe: that class
 * executes and commits its batch in one step, so between batches there is
 * never executed-but-uncommitted work for this commit to sweep up early.
 *
 * <p><b>Caveat for other writers on the same connection.</b> A commit is a
 * connection-wide act. If a caller has opened its own multi-statement
 * transaction on the writer connection by hand and executed part of it,
 * recording a diagnostic mid-way would commit that partial work early. Record
