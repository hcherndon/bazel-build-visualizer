 ...
 *
 * <p>{@code status} is a narrower domain than a test's overall status: {@code FLAKY} describes a
 * target's history and never an attempt.
 *
 * @param cachedLocally a cached attempt replays timestamps from before the build began — measured
 *     2.1 s earlier — so any timeline has to treat these rows deliberately rather than plot them
