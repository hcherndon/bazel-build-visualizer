 ...
 *
 * <p>A session is a capture of work somebody did, sometimes the only record of
 * a failure that has since stopped reproducing. So retention produces a
 * {@link Plan} — the exact list, with why each one is on it and how much space
 * it would free — and deletes only when that plan is handed back. A sweep that
 * ran on a timer and reported afterwards would be the wrong shape for the thing
 * being swept.
 *
 * <h2>A pinned session is never on the list</h2>
 *
 * <p>Pinning is the only mechanism a user has for saying "this one matters",
 * and a retention rule that could override it would make pinning advisory.
 * {@link Plan} carries the pinned count it skipped, so a user whose disk is
 * still full can see why.
 *
...
public record RetentionPolicy(
     ...
     * Keep everything.
     *
     * <p>The default, deliberately. Disk is cheap and a build capture the user
     * did not know they would need is the one they will ask for; an application
     * that quietly deleted them by default would be optimising the wrong thing.
     */
     ...
     * @param bytesFreed the total of what the candidates report, which is a
     *     figure from the catalog rather than a fresh measurement of the disk
     */
