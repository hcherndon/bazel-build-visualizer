...
/**
 * Reads what Phase 4 added: attempts, phases, and how much of the build each
 * source actually covers.
 *
 * <p>Coverage is computed, never stored. How many actions have attempt data is
 * a count over {@code action_attempts}; a cached copy would be one more number
 * that can disagree with its rows.
 */
public final class EnrichmentQueries implements AutoCloseable {
     ...
     *
     * <p>How a test's attempts are reached, since they attach to no action
     * (K2). Returns both of the two spawns a test produces, in log order, with
     * no attempt to decide which is the real one.
     */
    ...
    public Coverage coverage() throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(CORRELATION_COUNTS);
            ...
            while (rows.next()) {
                try {
                    byCorrelation.put(
                            AttemptCorrelation.valueOf(rows.getString(1)), rows.getLong(2));
    ...
    public List<CriticalPathComponent> bazelCriticalPath() throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(CRITICAL_PATH);
            ...
            while (rows.next()) {
                ...
                components.add(new CriticalPathComponent(
                        rows.getInt("ordinal"),
    ...
    public Optional<ProfileAnchor> anchor() throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(ANCHOR);
            ...
            return Optional.of(new ProfileAnchor(
                    micros, meaning, rows.getString("anchor_source_key"),
    ...
    private static java.util.OptionalInt optionalInt(ResultSet rows, String column)
            throws SQLException {
        int value = rows.getInt(column);
        return rows.wasNull() ? java.util.OptionalInt.empty() : java.util.OptionalInt.of(value);
    }
     ...
     *
     * <p>{@code actionsWithAttempts} being far below {@code actions} is the
     * normal state, not a defect: two thirds of a build's actions run inside
     * the Bazel server and never spawn a subprocess (K1). The panel says so in
     * those words.
     */
    public record Coverage(
         ...
         *
         * <p>Means the build ran without
         * {@code --experimental_profile_include_primary_output} (P4).
         */
     ...
     * One build phase.
     *
     * @param endMicros absent for the last phase, whose end the profile never
     *     states
     * @param endIsDerived always true in practice: the profile states starts
     *     and never ends, so an end is the next marker's start
     */
    public record Phase(
