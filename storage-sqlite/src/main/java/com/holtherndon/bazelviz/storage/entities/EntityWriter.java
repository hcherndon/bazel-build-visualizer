 ...
 *
 * <p>Every statement here that needs a parent row's id finds it with a SELECT inside the INSERT —
 * the same technique {@code EventWriter} uses for announced edges, and for the same three reasons.
 * A writer that cached ids in Java would need a map from every artifact path to its row id, which
 * at five million actions is hundreds of megabytes of strings the session already has on disk. It
 * would hand out an id for a row a conflicting insert never created. And it would have to be
 * rebuilt from scratch after a crash, from exactly the table it was meant to avoid reading.
 *
 * <p>The cost is an index probe per reference. Every one of them rides a UNIQUE constraint that the
 * table definition already carries, so none of them waits for the finalize indexes.
 *
 * <h2>Every write is idempotent</h2>
 *
 * <p>Re-delivery is normal rather than exceptional (plan 9.2): BES retransmits sequences and crash
 * recovery replays the journal, so the same event legitimately reaches this class twice. Each
 * statement therefore either does nothing on conflict or merges what the second delivery adds, and
 * the merges are all {@code coalesce} — a later sighting can fill a hole but never overwrite an
 * observation with a blank.
 *
 * <p>The one exception is {@code actions}, where a conflict is not merged at all. See {@link
 * #conflictingActions()}.
 *
 * <h2>Referenced-but-undeclared parents are recorded, not dropped</h2>
 *
 * <p>Two kinds of parent are legitimately referenced before — or without ever — being described.
 * Configuration {@code system} is referenced on every build on every measured version and never
 * published as a Configuration event. And a named set could in principle be referenced before its
 * own event, though 1,829 measured references contained zero such cases.
 *
 * <p>Both get a placeholder row on first reference: a configuration with {@code declared = 0}, a
 * depset with a NULL {@code bep_event_id}. Nothing is invented — the row records that the id was
 * referenced and nothing described it, which is exactly what happened — the foreign keys hold, and
 * {@link #undefinedDepsets(long)} can afterwards report any set that stayed undescribed as the
 * truncation evidence it is.
 *
 * <h2>Threading and transactions</h2>
 *
 * <p>Not thread-safe: it owns statements on the single writer connection. It suspends auto-commit
 * and commits every {@link #commitEvery} commands, because the Phase 0 spike measured the commit,
 * not the insert, as the expensive part. All work here is blocking I/O and must never run on the
 * Swing EDT (plan 19.1).
 *
 * <p>The caller must have written the {@code bep_events} row for a sequence before applying that
 * sequence's commands: every table's {@code bep_event_id} is resolved by a lookup on {@code
 * (stream_id, sequence)}, and that lookup is what makes any normalized row traceable back to its
 * raw bytes (ADR-004). A command applied ahead of its event lands with a NULL there, and on {@code
 * aborted_events}, where the event <em>is</em> the identity, it fails outright rather than
 * duplicating.
 */
public final class EntityWriter implements AutoCloseable {
   ...
   *
   * <p>Purely a way to skip a no-op INSERT: a miss costs one redundant statement, never a wrong
   * answer, so the cache can be as small as memory requires. Actions of one target arrive together,
   * so even a small window catches most repeats.
   */
  ...
  private static final String TARGET_JOIN =
      " FROM targets t JOIN labels l ON l.id = t.label_id" + " WHERE l.value = ? AND t.aspect = ?";
  ...
  private static final String INSERT_DEPSET =
      "INSERT INTO depsets (stream_id, bep_id, bep_event_id) VALUES (?, ?, "
          + EVENT_LOOKUP
          + ")"
          + " ON CONFLICT (stream_id, bep_id) DO UPDATE SET"
   ...
   *
   * <p>Bazel emits no {@code testResult} and no {@code testSummary} for a test whose own build
   * failed (TS6), so without this such a target reaches the tests view not at all — and a run where
   * every test failed to build showed "this session recorded no tests". Requirement 39 asks for the
   * explicit state, and {@code FAILED_TO_BUILD} is the one Bazel's own vocabulary has for it.
   *
   * <p>Guarded on the target actually being a test: {@code testSize} comes from the analysis
   * payload under either command, and {@code testTimeoutSeconds} from the completion under {@code
   * bazel test}. A non-test target that failed to build is a failed target and nothing more.
   */
  private static final String MARK_TEST_FAILED_TO_BUILD =
      "INSERT INTO tests (configured_target_id, overall_status)"
          + " SELECT ct.id, 'FAILED_TO_BUILD'"
          + CONFIGURED_TARGET_JOIN
          + CONFIGURED_TARGET_WHERE
  ...
  private static final String INSERT_TEST_LOG =
      "INSERT INTO test_logs (test_id, test_attempt_id, name, uri, summary_status)"
          ...
          + "   AND ta.run = ? AND ta.shard = ? AND ta.attempt = ?), ?, ?, ?"
          + TEST_JOIN
  ...
  public EntityWriter(Connection connection, int commitEvery, int dictionaryCache)
    ...
    if (dictionaryCache < 1) {
      throw new IllegalArgumentException("dictionaryCache must be >= 1, got " + dictionaryCache);
    }
   ...
   *
   * <p>Expected to be non-zero only after a resumed import, where the journal replays events the
   * previous run already normalized. On a clean import a non-zero count means two different actions
   * reported the same primary output — which the measurements say cannot happen, and which the user
   * therefore needs told rather than silently resolved by keeping whichever row arrived first. The
   * importer surfaces it as a diagnostic; nothing here merges the two.
   */
  ...
  /**
   * Named sets referenced by something but never defined by an event of their own.
   *
   * <p>Zero in 1,829 measured references. A non-zero count is evidence of a truncated capture
   * rather than a normal state, and the byte totals under those sets are lower bounds.
   */
   ...
   * @param sequence the event's sequence, used to find its {@code bep_events} row so the resulting
   *     entities can be traced back to raw bytes
   */
  ...
  private void dispatch(long streamId, long sequence, EntityCommand command) throws SQLException {
    switch (command) {
      ...
      case EntityCommand.TestAttemptCompleted attempt -> testAttempt(streamId, sequence, attempt);
      ...
      case EntityCommand.ProgressOutputSeen progress -> progress(streamId, sequence, progress);
   ...
   *
   * <p>Only ever sets the flag, never clears it. A session can be re-indexed, resumed, or fed a
   * fallback file after a live capture, and none of those unsees an end marker that arrived.
   */
  ...
  private void invocationStarted(long streamId, EntityCommand.InvocationStarted started)
    ...
    setLong(
        statement,
        8,
        started.serverPid() == 0 ? OptionalLong.empty() : OptionalLong.of(started.serverPid()));
   ...
   *
   * <p>The id {@code system} arrives this way on every build, and {@code none} can too. The
   * placeholder carries {@code declared = 0} and no payload columns, which is the whole of what is
   * known about it.
   */
  ...
  private void targetConfigured(long streamId, long sequence, EntityCommand.TargetConfigured target)
      throws SQLException {
  ...
  private void targetCompleted(long streamId, long sequence, EntityCommand.TargetCompleted target)
      throws SQLException {
    String aspect = target.aspect().orElse("");
    ...
    statement.setString(1, (target.success() ? TargetOutcome.BUILT : TargetOutcome.FAILED).name());
  ...
  /**
   * Makes sure a target row exists for a label something completed or aborted.
   *
   * <p>Measured ordering says {@code configured} always precedes {@code completed}, so this is
   * normally a no-op — but from Bazel 7.6.1 an analysis-failed target emits no {@code configured}
   * payload at all, and this is the only thing that gives it a row. {@code CONFIGURED} is not a
  ...
  private void action(long streamId, long sequence, EntityCommand.ActionCompleted action)
    ...
    statement.setString(4, ActionOutcome.ofSuccessFlag(action.success()).name());
   ...
   *
   * <p>Not shell-quoted: re-quoting would produce something that looks executable and is not, and
   * the command was never run through a shell (plan 22.2). Not newline-joined either, which is what
   * this used to do — an argument may itself contain a newline (every {@code /bin/bash -c} script
   * does), so joining on one destroys the argument boundaries and no reader can recover them. JSON
   * is what requirement 21 asks for and the only form here that round-trips.
   *
   * <p>Written by hand rather than through a JSON library because the shape is fixed — an array of
   * strings — and pulling a binding into the storage layer to emit six escapes would be the larger
   * cost.
   *
   * <p>Empty means the event carried no command line, which is every action that did not run a
   * spawn.
   */
  ...
  /**
   * RFC 8259 string escaping: the six named escapes, then the numeric form for the remaining
   * control characters.
   */
  ...
  private void testAttempt(long streamId, long sequence, EntityCommand.TestAttemptCompleted attempt)
      throws SQLException {
    ...
    for (EntityCommand.TestLogRef log : attempt.outputs()) {
      testLog(
          streamId,
          attempt.label(),
          attempt.configurationId(),
          OptionalInt.of(attempt.run()),
          OptionalInt.of(attempt.shard()),
          OptionalInt.of(attempt.attempt()),
          log);
    }
  }
  ...
  private void aborted(long streamId, long sequence, EntityCommand.TargetAborted aborted)
    ...
    setText(
        statement,
        6,
        aborted.description().isEmpty() ? Optional.empty() : Optional.of(aborted.description()));
    statement.setLong(7, streamId);
    statement.setLong(8, sequence);
    statement.executeUpdate();
  }
  ...
  /**
   * Whether an abort's id kind identifies a target rather than something else.
   *
   * <p>Patterns and other ids abort too; they carry no label and describe no target, so they stay
   * in {@code aborted_events} and nowhere else.
   */
  private static boolean namesATarget(String idKind) {
    return switch (idKind) {
      case "targetConfigured", "targetCompleted", "unconfiguredLabel", "configuredLabel" -> true;
      default -> false;
    };
  }
   ...
   *
   * <p>{@code INSERT_TARGET}'s conflict clause protects {@code ABORTED} from being written back to
   * {@code CONFIGURED} by a later event, which matters because aborts arrive after everything else.
   */
  private void markTargetAborted(long streamId, long sequence, EntityCommand.TargetAborted aborted)
      throws SQLException {
