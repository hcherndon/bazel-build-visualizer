...
/**
 * Session database schema version 2 (plan 10.7): configurations, targets, actions, artifacts, file
 * sets, tests and build-level metrics — what Phase 3 normalizes out of the raw events schema v1
 * stores.
 *
 * <p>Every shape here is traceable to a measurement against real Bazel 6.5.0, 7.6.1, 8.4.1 and
 * 9.2.0. {@code docs/bep-content.md} is the record, and its numbered findings ({@code A1}, {@code
 * T4}, {@code F2}, …) are cited where they decided a column, because a constraint whose reason is
 * not written down is a constraint the next person will relax.
 *
 * <h2>Absence has three meanings and they are not interchangeable</h2>
 *
 * <p>Bazel emits protobuf-JSON, which omits a field at its default. So an absent {@code success}
 * means <em>false</em>; an absent {@code startTime} on Bazel 7 means <em>this version cannot report
 * it</em>; an absent action event means <em>the flag that publishes it was not passed</em>. All
 * three look identical in the JSON and could not be more different in a database.
 *
 * <p>Columns whose absence is a proto3 default are NOT NULL — the parser supplies the default,
 * because the value is known. Columns whose absence means the observation does not exist are
 * nullable and stay NULL, never 0 (plan 11.4). Which of the two a column is, is stated where it is
 * declared. What disambiguates the second and third cases at read time is {@code build_invocation}:
 * the Bazel version and the capture flags. Without that row every NULL here is unreadable.
 *
 * <h2>Identity comes from event ids, never from payload content</h2>
 *
 * <p>An action is its {@code id.actionCompleted.primaryOutput} (A1, A2). A configuration is its
 * opaque id and nothing else — two configuration events were observed with byte-identical payloads
 * and different ids (C2). A named set is {@code (invocation, id)}, because the ids are dense
 * integers reshuffled between runs of the same build (F2).
 *
 * <h2>Depsets are stored as a DAG, never flattened</h2>
 *
 * <p>A named set is a node with two edge kinds (F1, finding 26). Sets are shared — in-degree up to
 * 4 and depth up to 10 in a small probe workspace — so materialising each set's transitive files
 * per referencing target multiplies rows by the sharing factor, which is largest on exactly the
 * builds worth measuring. It is also the only path to a target's outputs on Bazel 8 and 9, where
 * {@code importantOutput} is empty by default (T1).
 *
 * <h2>One invocation per session</h2>
 *
 * <p>These tables describe a single build. A session can hold more than one event stream — BES
 * splits lifecycle events from build-tool events, and a fallback BEP file can re-deliver a build
 * the live capture missed — but exactly one build-tool stream is normalized, chosen by the capture
 * coordinator, and {@code build_invocation} records which. {@code stream_id} still appears on
 * {@code configurations} and {@code depsets} because their ids are only meaningful inside one
 * stream, and a global unique index there would cross-link one build's outputs into another's the
 * moment a second stream is ingested (F2).
 */
public final class SchemaV2 {
   ...
   *
   * <p>Chosen for the queries Phase 3's views actually make: the actions table pages by id and
   * sorts by start time, duration and mnemonic; the failures view filters actions and targets by
   * outcome and unions aborted events by label; the targets tree walks label order; the inspector
   * looks an entity up by its natural key and walks the depset graph in both directions.
   */
  public static final List<String> INDEXES =
      List.of(
