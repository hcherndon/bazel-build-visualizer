 ...
 *
 * <p>Plan 10.6: "the catalog must not contain the full build data". The catalog exists so the start
 * screen can list forty sessions in a few milliseconds without opening forty databases, and the
 * moment it starts carrying action rows it is a second copy of the thing it indexes — one that can
 * disagree with the session, and one that has to be migrated whenever the session schema changes.
 *
 ...
 * conflict. So every field here is derived from a manifest that can be read again, and {@link
 * SessionCatalog#rescan} rebuilds the whole table from the directories. That is what makes plan
 * 24's "sessions survive application restart and relocation" true: a moved sessions root
 * invalidates every path in the catalog and none of the sessions.
 *
 * @param directory where the session lives now, which changes when a user moves their sessions root
 * @param summary a short line of metrics for the card — plan 10.6's "thumbnail/summary metrics", as
 *     words rather than an image
 * @param missing the directory was not there the last time anybody looked; the row is kept so a
 *     user can see what happened rather than finding a session silently gone
 */
public record CatalogEntry(
  ...
  public static CatalogEntry of(
      String sessionUuid, String displayName, Path directory, String state) {
