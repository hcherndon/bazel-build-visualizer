 ...
 *
 * <p>Version 2 built the entity layer out of one source, the BEP. This version adds two more, and
 * the governing rule is ADR-009: where two sources measure the same thing, both values survive
 * under names that say whose they are. Nothing here overwrites a column version 2 wrote.
 *
 * <p>Every non-obvious choice below cites a finding id from {@code docs/exec-log-and-profile.md},
 * which is measurement against real Bazel 6.5.0, 7.6.1, 8.4.1 and 9.2.0 rather than documentation.
 *
 * <p>There is no {@code action_coverage} table. How many actions have attempt data is a count over
 * {@code action_attempts}, and a stored copy would be one more number that can disagree with the
 * rows it summarises — which is exactly the defect the Phase 3 audit found in {@code
 * saw_last_message}.
 */
