 ...
 *
 * <p>Carries the raw location and the id display because the view needs both per row. They were
 * briefly excluded, which forced one point lookup per row on top of the page query — 200 extra
 * round trips per page. The location is twelve bytes and travels with the row; only the payload
 * <em>bytes</em> are fetched on demand for the selected event, which is what plan 17.11 is actually
 * about.
 *
 * <p>{@code idDisplay} is empty when the event carries no {@code BuildEventId}, which is a real
 * state rather than a missing join.
 *
