...
/**
 * The submitted text is not a single read-only statement, so it was never sent
 * to SQLite.
 *
 * <p>Deliberately not a {@code SQLException}: nothing was executed, and the
 * only thing a caller can do is show {@link #getMessage()} beside the text the
 * user typed. {@link #submitted()} carries that text back so a panel can point
 * at it.
 */
