 ...
 *
 * <p>The case that matters is a database written by a <em>newer</em> build.
 * Downgrading it silently is not an option: this build does not know which
 * tables or columns the newer schema added, so writing to it would produce a
 * database that neither version can read. The session is refused with an
 * explicit message instead (plan 21.5 — unknown schema is surfaced, never
 * guessed at).
 */
public final class SchemaVersionException extends SQLException {
    ...
    static SchemaVersionException tooNew(int foundVersion, int supportedVersion) {
        return new SchemaVersionException(
                "session database schema version " + foundVersion
                        + " was written by a newer build of Bazel Build Visualizer; this build"
                        + " understands version " + supportedVersion
    ...
    static SchemaVersionException unreadable(String detail, int supportedVersion) {
        return new SchemaVersionException(
                "session database schema version could not be determined: " + detail
