 ...
 *
 * <p>A {@code null} element is a SQL NULL and nothing else. It is deliberately not mapped to {@code
 * ""} or to {@code 0} on the way out of the database — rule 11 — so that the layer which renders it
 * can say "NULL" and mean it. An empty string column and a NULL column are different facts about a
 * build and must not arrive here looking the same.
 *
 * <p>Rows are created per page and discarded with it. A page is a couple of hundred rows, so this
 * is the bounded end of rule 9: nothing accumulates.
 */
public final class QueryRow {
  ...
  /** Reads the current row, mapping SQL NULL to {@code null} and BLOB to {@link BlobValue}. */
