 ...
 *
 * <p>Carries the two things a person needs in order to act: SQLite's own message (as {@link
 * #getMessage()}), and the exact text that was executed ({@link #statement()}) — which is not
 * always what the user typed, because counting and paging wrap a tabular query in {@code SELECT …
 * FROM (…)}. An error naming a statement the user never wrote, with no way to see it, is the kind
 * of error people learn to ignore.
 *
 * <p>{@link #stage()} says which of the three executions failed, because "columns", "count" and
 * "page 41" fail for different reasons and the fix differs.
 */
public final class QueryFailedException extends RuntimeException {
  ...
  private static String message(Stage stage, boolean stopped, Throwable cause) {
    ...
    return "SQLite refused the statement while " + stage.description() + ": " + cause.getMessage();
  }
  ...
  /**
   * True when the statement was interrupted on purpose — by the user, or by the runaway-query
   * deadline — rather than refused.
   *
   * <p>The distinction matters on screen: a cancelled query is not a broken one, and reporting it
   * as an error teaches people to distrust errors.
   */
