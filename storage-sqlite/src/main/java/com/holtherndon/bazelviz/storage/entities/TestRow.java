 ...
 *
 * <p>{@code overallStatus} comes from {@code testSummary} and from nowhere else: the target's own
 * success flag is {@code true} for a test that failed, so a view deriving pass/fail from the target
 * would show every failure green.
 *
 ...
 * @param firstStartMicros the earliest attempt's start, computed from {@code test_attempts} rather
 *     than read from the summary — the summary's first start was measured 218–747 ms later than the
 *     earliest attempt
 * @param lastStopMicros the latest attempt's end, likewise
 * @param bazelReportedDurationMicros Bazel's own figure, which excludes failed retries; shown as
 *     Bazel's and never used as a timeline bound
 */
