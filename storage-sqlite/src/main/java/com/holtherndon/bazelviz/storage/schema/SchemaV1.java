...
/**
 * Session database schema version 1 (plan section 10.7), covering what Phase 1 needs: raw BEP
 * events, their identities and announced-child edges, the streams they arrived on, the sources they
 * came from, and import diagnostics. Targets, actions, artifacts and tests arrive with Phase 3.
 *
 * <p>Two rules shape every column here.
 *
 * <p><b>Unknown is never zero</b> (plan 11.4). Any value that can be unavailable is nullable and is
 * left NULL. That is why {@code byte_size}, {@code event_micros}, {@code finalized_micros} and
 * {@code event_id_hash} are nullable — a missing timestamp must not read back as the epoch, and an
 * unknown size must not read back as an empty file.
 *
 * <p><b>Duplicate delivery is idempotent</b> (plan 9.2). BES may retransmit a sequence, and
 * recovery replays the journal from the last checkpoint, so the same event can legitimately be
 * normalized twice. {@code UNIQUE (stream_id, sequence)} makes the second write a no-op rather than
 * a duplicate row.
 *
 * <p>Indexes are deliberately separate from the tables: the Phase 0 SQLite spike showed bulk
 * loading before index creation is substantially faster, so ingestion creates tables, loads, then
 * calls {@link #INDEXES} as an explicit finalize step.
 */
public final class SchemaV1 {
  ...
  public static final List<String> INDEXES =
      List.of(
          ...
          "CREATE INDEX IF NOT EXISTS idx_bep_events_type_sequence ON bep_events(event_type,"
              + " sequence)",
          "CREATE INDEX IF NOT EXISTS idx_bep_events_id_hash ON bep_events(event_id_hash)",
          "CREATE INDEX IF NOT EXISTS idx_bep_event_edges_child ON"
              + " bep_event_edges(child_event_id_hash)");
}
