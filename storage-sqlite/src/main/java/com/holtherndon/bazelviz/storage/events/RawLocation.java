 ...
 *
 * <p>Every normalized row can be traced back to the exact bytes it came from (ADR-004), which is
 * what lets the Events view show a raw protobuf inspector and what lets a later build re-derive
 * richer data without re-running the build.
 *
 * @param segment journal segment index, matching {@code bes-%06d.journal}
 * @param offset byte offset of the frame within that segment, from the start of the frame header
