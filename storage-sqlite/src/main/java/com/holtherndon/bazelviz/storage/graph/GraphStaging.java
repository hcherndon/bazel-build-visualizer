...
/**
 * Temporary tables that hold an action graph exactly as the file carried it, before any reference
 * is resolved.
 *
 * <h2>Why staging exists at all</h2>
 *
 * <p>{@code aquery} output references entities before it declares them — actions before depsets,
 * depsets before artifacts, path fragments before their parents — on three of the four supported
 * Bazel versions (finding Q4 in {@code docs/aquery-and-cquery.md}). So resolution cannot happen
 * during the read, and everything the file says has to exist somewhere first.
 *
 * <h2>Why in SQLite rather than in Java</h2>
 *
 * <p>"Somewhere" could have been a few hash maps. At the plan's five-million action target that is
 * tens of millions of path fragments and artifacts, and holding them as objects is exactly what
 * plan 19.4 forbids and what {@code CsrGraph} exists to avoid. SQLite's temporary tables spill to
 * disk on their own, so the memory bound is the page cache rather than the graph.
 *
 * <p>They are {@code TEMP} tables: they live in a separate database attached to this connection
 * alone and vanish when it closes, so a crashed import leaves no half-staged rows in the session.
 *
 * <p>Path resolution is a recursive CTE rather than a Java loop, for the same reason: walking
 * millions of parent chains in Java allocates a string per level, and SQLite can do it without
...
public final class GraphStaging implements AutoCloseable {
  ...
  private static final List<String> DROP =
      List.of(
          "stage_path",
          "stage_action_output",
          "stage_action_input",
          "stage_action",
          "stage_depset_artifact",
          "stage_depset_child",
          "stage_depset",
          "stage_config",
          "stage_rule_class",
   ...
   *
   * <p>A recursive CTE, seeded at the fragments with no parent and walking down. {@code parent = 0}
   * is the root marker: proto3 omits a zero, so an absent {@code parent_id} reads as zero, and no
   * real fragment has id zero.
   *
   * <p>Runs after every fragment is staged, which is the whole point — 2 to 4 fragments per file
   * name their parent before it is declared (Q4).
   */
  /**
   * How deep a path may be before the chain is treated as malformed.
   *
   * <p>A path fragment chain is a tree, so it terminates. A file that made it a cycle would make
   * this CTE run until the disk filled, and an imported session is untrusted outright (plan 22.4).
   * The bound is far above any real path — the deepest in the probe was six — and turns a hostile
   * file into missing paths, which {@link #unresolvedArtifacts()} then counts.
   */
   ...
   *
   * <p>This includes both declared artifacts whose fragment chain is broken and artifact ids
   * referenced by actions or depsets but never declared. The final joins silently omit either
   * shape, so both must prevent a completeness claim.
   */
   ...
   *
   * <p>These otherwise disappear from the importer joins: an action loses an entire input set, or a
   * depset loses a transitive child. Count the references, rather than just distinct ids, because
   * each omitted link is a separate structural hole.
   */
