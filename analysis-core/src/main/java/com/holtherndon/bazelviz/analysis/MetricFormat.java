 ...
 *
 * <p>Findings carry their numbers as text, because a finding's sentence has to read as a sentence
 * and because the value shown must be the value the rule compared against its threshold. If the
 * rules formatted one way and the dashboard another, the same duration would appear as "1.2 s" on
 * one screen and "1200 ms" on the next, and a reader would reasonably wonder which was measured.
 *
 * <h2>Unknown has a spelling</h2>
 *
 * <p>{@link #UNKNOWN} is what every optional overload writes when there is no value. It is a word,
 * not an empty cell and not a dash, because an empty cell in a table of numbers reads as a zero
...
public final class MetricFormat {
  ...
  public static String ratio(OptionalDouble value) {
    return value.isPresent() ? String.format(Locale.ROOT, "%.2f", value.getAsDouble()) : UNKNOWN;
  }

  /**
   * A quantile interval, collapsed to one value when the sketch knows it exactly.
   *
   * <p>Writing "1.20 s to 1.22 s" for every percentile would be noise; the range appears only when
   * it is real, so a reader who sees one can tell that the sketch, and not the build, is where the
   * imprecision came from.
   */
  public static String bounds(Optional<QuantileSketch.Bounds> bounds) {
    return bounds
        .map(
            value ->
                value.isExact()
