 ...
 *
 * <p>Plan 13.7 names layered for dependency subgraphs, radial for neighbourhoods, linear for
 * critical paths and grid for cluster summaries, and ends "do not run force-directed layout on an
 * unbounded action graph". There is none here at all: every layout below uses bounded linear
 * passes, so the answer to "how long will this take" is the graph size rather than a convergence
 * criterion.
 *
 * <h2>Linear, not merely non-iterative</h2>
 *
 * <p>Layering by repeated edge relaxation is easier to write and is O(V·E) in the worst case; at
 * the 50,000-node limit of {@link GraphExtract} that is not a layout, it is a hang. The graph
 * traversals here build a local adjacency index once and then run in O(V+E) — Kahn's algorithm for
 * layering and queue-based breadth-first walks for hierarchy and rings.
 *
 * <h2>Deterministic</h2>
 *
 * <p>Plan 13.7 again: "deterministic stable positioning where possible". The same subgraph laid out
 * twice produces identical coordinates — no random seeds, no hash iteration order, no clock. A
 * graph that rearranged itself when nothing changed could not be compared against yesterday's
 * screenshot, and worse, would make a user think something had.
 *
 * <h2>Cancellable, because plan 24 says so</h2>
 *
 * <p>Every layout checks a flag as it goes and returns {@link Result#cancelled} rather than a
 * partial placement. A user who changes the query while fifty thousand nodes are being placed
 * should not wait for the answer to a question they have stopped asking, and should certainly not
 * be shown half of it.
 */
public final class GraphLayout {
   ...
   *
   * <p>Longest path rather than shortest, so every edge points strictly rightwards. Shortest-path
   * layering puts a node one layer after its <em>earliest</em> predecessor, which leaves the edges
   * from its later predecessors pointing backwards — and a dependency arrow pointing the wrong way
   * is a worse drawing than an untidy one.
   *
   * <p>Kahn's algorithm, so it is one pass over nodes and one over edges. A subgraph that is not a
   * DAG — which the action graph should never be, but a bad import could produce — leaves its cycle
   * unprocessed rather than looping; those nodes are placed in a final layer of their own.
   */
   ...
   *
   * <p>A dependency DAG is not a tree: a generated artifact can feed several consumers and an
   * action can need several producers. This layout does not duplicate nodes or pretend otherwise.
   * First discovery chooses one primary parent for placement; every other real dependency remains a
   * cross-link for the renderer to disclose and draw on demand.
   *
   * <p>Rooted views start from their selected node. Dependencies walk towards producers, reverse
   * dependencies walk towards consumers, and a neighbourhood walks both ways. Whole-build and
   * cluster views start from zero-indegree nodes. Any nodes left by a disconnected component or
   * cycle are seeded in stable node order, so the algorithm always terminates and places every node
   * exactly once.
   *
   * <p>Child subtrees receive contiguous leaf spans and each parent is centred over its span. The
   * implementation is iterative and O(V+E): primitive layout arrays, no recursive stack and no
   * object per edge.
   */
  public static Result hierarchy(GraphExtract.Result extract, AtomicBoolean cancelled) {
    ...
    if (extract.mode() == GraphExtract.Mode.WHOLE || extract.mode() == GraphExtract.Mode.CLUSTERS) {
      for (int node = 0; node < count; node++) {
        ...
        if (traversal.inDegree[node] == 0 && !seen[node]) {
          int added =
              discoverTree(
                  node, traversal, seen, parent, depth, discovery, discovered, queue, cancelled);
          if (added < 0) {
            return Result.cancelled(Kind.HIERARCHY);
          }
          discovered += added;
      ...
      int added =
          discoverTree(0, traversal, seen, parent, depth, discovery, discovered, queue, cancelled);
      if (added < 0) {
        return Result.cancelled(Kind.HIERARCHY);
      }
      discovered += added;
   ...
   *
   * <p>For a neighbourhood, where the question is "what is near this" and distance from the centre
   * is the answer. Ring membership is graph distance ignoring edge direction — a neighbourhood's
   * rings are about closeness, not about which way the dependency points. Position within a ring is
   * discovery order, which the traversal already made deterministic.
   *
   * <p>Nodes the walk never reaches — an extraction can contain them, since {@link
   * GraphExtract#neighbourhood} merges two traversals — land in a final outer ring rather than on
   * top of the centre.
   */
   ...
   *
   * <p>For a path — plan 13.5's "path between two actions" and "critical path". The order is the
   * path's, not the graph's, which is the whole point: a critical path drawn in node-id order would
   * be a scatter of dots.
   */
   ...
   *
   * <p>For cluster summaries, where there are no edges worth routing and the question is "how many
   * and how big". Deliberately boring: a grid is the layout that adds no meaning of its own, which
   * is the right choice when the meaning is entirely in the sizes and the labels.
   */
   ...
   *
   * <p>An extraction names nodes by their graph-wide index; a layout works in positions within the
   * extraction. Building the translation once is what keeps both traversals linear.
   */
   ...
   *
   * <p>Parallel primitive arrays rather than a list of points: fifty thousand nodes would otherwise
   * be fifty thousand objects, and the canvas reads these inside its paint loop.
   *
   * <p>A class rather than a record, which it was until the Phase 7 audit. A record with array
   * components gets an {@code equals} that compares those arrays by reference — so two identical
   * layouts compare unequal, which is exactly the claim the determinism test exists to make — and
   * mandates public accessors that hand the arrays out. Copying them defensively on every call is
   * fifty thousand doubles for a caller who wanted one, and not copying them lets a caller move a
   * node. Neither is right, so neither is offered: {@link #xAt} and {@link #yAt} are the only way
   * in.
   */
  public static final class Result {
    ...
    Result(
        Kind kind, List<Integer> nodes, double[] x, double[] y, int[] parent, boolean cancelled) {
      this.kind = kind;
    ...
    /**
     * This node's primary hierarchy parent as a layout position, or -1. Other layouts have no
     * primary-parent claim and return -1 everywhere.
     */
