...
/**
 * One row of an aggregate: everything the session can say about the actions that share a key.
 *
 * <h2>The dimensions the plan asks for, and the ones that exist</h2>
 *
 * <p>Plan 15.3 lists eleven aggregate dimensions. {@link Dimension} carries the ones this session's
 * schema can group by, and does not carry the others — a dimension that appeared in a menu and
 * returned an empty chart would be a worse answer than one that is not offered, because the empty
 * chart looks like a build with nothing in it.
 *
 * @param key the mnemonic, runner, label or status shared by these actions; null when the group is
 *     the actions whose key was never recorded
 ...
 * @param declaredNotCacheable actions whose spawns Bazel marked uncacheable — its own declaration,
 *     read verbatim, not a guess from a runner name
 * @param declaredNotRemotable actions whose spawns Bazel marked as unable to run remotely
 */
public record GroupAggregate(
  ...
  /**
   * The share of this group whose cache state was reported as a hit, over the actions that reported
   * one at all.
   *
   * <p>Empty when none did. A miss rate computed with unknowns counted as misses is the single
   * easiest way to invent a cache problem, and plan 16.1 requires "sufficient cache-state coverage"
   * before a cache finding may be raised at all.
   */
  ...
  public String describe() {
    StringBuilder text =
        new StringBuilder(displayKey())
            .append(": ")
            .append(actions)
            .append(actions == 1 ? " action" : " actions");
    duration
        .observedSum()
        .ifPresent(total -> text.append(", ").append(total / 1000).append(" ms of observed work"));
   ...
   *
   * <p>{@link #totalGroups} and {@link #totalActions} describe the whole aggregation; {@link
   * #groups} may be the heaviest few of them. Rule 12 forbids truncating silently, so the
   * difference between the two is always available and {@link #describe()} states it.
   *
   * @param totalGroups distinct keys the dimension produced, whether or not they are listed
   * @param totalActions actions the aggregation ran over, whether or not their group is listed
   */
  public record Table(
      Dimension dimension, List<GroupAggregate> groups, long totalGroups, long totalActions) {
     ...
     *
     * <p>Exposed rather than only asserted in a test, for the same reason {@code GraphClustering}
     * exposes its own: a view drawing these numbers should be able to check the arithmetic it is
     * drawing.
     */
