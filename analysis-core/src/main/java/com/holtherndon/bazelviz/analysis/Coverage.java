 ...
 *
 * <p>Plan 15.5 gives the shape verbatim — timing coverage, runner coverage, input-size coverage,
 * action-graph correlation, target-graph coverage, correlation confidence — and the reason is that
 * every other number on the screen is a statement about the covered subset. A cache-hit rate over
 * the 30% of actions whose cache state was recorded is not the build's cache-hit rate, and the only
 * thing standing between those two readings is this figure being on the same screen.
 *
 * <h2>Two ways to be uncovered</h2>
 *
 * <p>A source can be absent — no execution log was captured — or present and silent about a
 * particular action. Both leave the same hole, and {@link #reason} is what separates "nobody ran
 * this enrichment" from "this enrichment ran and Bazel does not report the field", which are
...
public record Coverage(
   ...
   *
   * <p>Zero of zero would divide by nothing and read as complete; this reads as unavailable, which
   * is what it is.
   */
  ...
  public String describe() {
    ...
    if (covered == 0) {
      return name + ": unavailable for all " + total + reason.map(why -> " — " + why).orElse("");
    }
    String line =
        name
            + ": "
            + String.format("%.1f%%", fraction().orElse(0) * 100)
            + " ("
            + covered
            + " of "
            + total
