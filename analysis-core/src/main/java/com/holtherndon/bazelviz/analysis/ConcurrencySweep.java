 ...
 *
 * <p>Plan 15.2 asks for peak observed concurrency, average observed concurrency, the parallelism
 * factor, time with zero active actions, and time with low concurrency. Plan 16.1 asks for the
 * low-parallelism windows themselves, so a finding can point at one. All of those come from one
 * exact sweep over the observed spans, which is what this is.
 *
 * <h2>Exact, not binned</h2>
 *
 * <p>The timeline's level-of-detail pyramid (Phase 6) also counts active actions, but per bin: it
 * answers "how busy was this stretch" at the resolution a screen can show. This answers "what is
 * the largest number of actions that were ever running at one instant", and a binned count cannot,
 * because a peak narrower than a bin is averaged away inside it. So this sorts the endpoints and
 * walks every transition.
 *
 * <h2>Half-open spans</h2>
 *
 * <p>A span occupies {@code [start, end)}. An action ending at exactly the instant another starts
 * does not overlap it, so a chain of back-to-back actions reports a concurrency of one rather than
 * briefly two. The walk enforces that by applying every end at an instant before every start at it.
 *
 * <p>The consequence worth knowing is that a span of zero length occupies no time and so is never
 * running. That is not a corner case: Bazel 8.4.1 publishes {@code endTime == startTime} for every
 * action it reports, a five-second sleep included (docs/bazel-ground-truth.md). A sweep over BEP
 * action timings from that version finds nothing running at any instant, and {@link
 * Result#instantaneousSpans()} is how the answer says so rather than reporting a build that ran
 * nothing.
 *
 * <h2>Unknown timings are counted, not assumed</h2>
 *
 * <p>An action with no observed start or end is added with {@link Spans#addUntimed()} and takes no
 * part in the sweep. It still travels into the result, so every figure here can be read against how
 * much of the build it covers instead of being taken for the whole of it (rule 13).
 */
public final class ConcurrencySweep {
   ...
   *
   * <p>A builder rather than two arrays because the caller streams rows out of a database and does
   * not know the count up front — and because the actions that could not be timed have to be
   * counted in the same pass. A caller that filtered those out before calling would leave nothing
   * able to say how much of the build the sweep covered.
   *
  ...
  public static final class Spans {
     ...
     *
     * <p>A span of zero length is counted and not stored: it is never running under the half-open
     * convention, so including it would only put an event pair into the walk that cancels itself.
     *
     ...
     *
     * <p>Plan 15.1's start concurrency and completion concurrency: "number of active actions at
     * start" and "number active immediately before completion", both per action. Two binary
     * searches over the sorted endpoints, so asking for every action costs {@code n log n} rather
     * than a sweep each.
     *
     * <p>Half-open as everywhere else, so an action's own start counts it and its own end does not.
     * An action asking for its completion concurrency should therefore ask about the instant before
     * it finished, which is what "immediately before completion" means.
     */
    ...
    /**
     * The stretches where fewer than {@code threshold} actions were running.
     *
     * <p>A second walk rather than a field on {@link Result}, because the threshold is not knowable
     * until the first sweep has run: plan 16.1 defines a low-parallelism window relative to "the
     * session's typical concurrency", which is one of the numbers the sweep produces.
     *
     * @param threshold report stretches where the active count stays below this
     * @param minimumMicros ignore stretches shorter than this, so the microsecond between two
     *     actions is not reported as a stall
     */
   ...
   *
   * <p>Two walks over the sorted endpoints: the first finds the peak so the level histogram can be
   * sized exactly, the second fills it. Both use the same walk, so the peak and the histogram
   * cannot disagree about a tie. Sorting two primitive arrays dominates, so the whole thing is
  ...
  /**
   * Walks the sorted endpoints, handing each constant-concurrency interval to {@code visitor} in
   * time order.
   *
   * <p>The one tricky loop in this class, written once so the peak pass and the histogram pass
   * cannot drift apart. Every event at an instant is applied before the interval that follows it is
   * emitted, so no interval is empty and no instant is visited twice. Ends are applied before
   * starts, which is the half-open convention; that is safe because a zero-length span was never
   * stored, so every end at an instant belongs to a span that started strictly earlier and has
   * therefore already been counted.
   */
  private static void walk(long[] starts, long[] ends, int size, LevelVisitor visitor) {
    ...
    while (endIndex < size) {
      long next = startIndex < size ? Math.min(starts[startIndex], ends[endIndex]) : ends[endIndex];
  ...
  public static final class Result {
    ...
    Result(
        int peakActive,
        long peakFirstSeenMicros,
        long[] timeAtLevel,
        long windowStartMicros,
        long windowEndMicros,
        OptionalLong totalSpanMicros,
        long sweptSpans,
        long untimedSpans,
        long instantaneousSpans) {
     ...
     *
     * <p>Plan 15.2's "time with zero active actions", except that this is over the observed spans
     * and not over Bazel's execution phase. A caller that knows the phase boundaries can compare
     * the two; one that does not should not be told that it does.
     */
     ...
     *
     * <p>An aggregate concurrency indicator and <em>not</em> CPU utilization. A build of remote
     * actions can report far more than the machine has cores, and a build of one long local action
     * reports one whatever it did to the CPU.
     */
     ...
     *
     * <p>Identical to {@link #parallelismFactor()} by construction. Both names exist because plan
     * 15.2 and plan 15.4 ask for them under different names, and a reader who found only one would
     ...
     *
     * <p>They occupy no time, so nothing was running during them. Bazel 8.4.1 reports every action
     * that way, which makes a sweep of its BEP timings find nothing running at any instant — true
     * of the timings and false of the build.
     */
    ...
    public String describe() {
      if (sweptSpans == 0) {
        if (instantaneousSpans > 0) {
          return "All "
              + instantaneousSpans
              + " timed actions report the same start and"
        ...
        return untimedSpans == 0
            ...
            : "None of the "
                + untimedSpans
                + " actions has an observed start and end,"
                + " so concurrency is unknown rather than zero.";
      }
      StringBuilder text = new StringBuilder();
      text.append("Peak ")
          .append(peakActive)
          .append(" actions running at once, ")
          .append(String.format("%.2f", parallelismFactor().orElse(0)))
          .append(" on average across ")
          .append(windowMicros() / 1000)
          .append(" ms, from ")
          .append(sweptSpans)
          .append(" observed spans.");
      if (untimedSpans > 0) {
        text.append(' ')
            .append(untimedSpans)
