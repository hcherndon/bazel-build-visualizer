 ...
 *
 * <p>Every rule here reports a measurement and a threshold, and stops. None of
 * them says why the measurement is what it is, because a build that ran once
 * cannot support that: the queue-dominated rule can say that queue time was
 * most of an attempt, and plan 16.1's own text for it says "do not assume the
 * cause is Bazel rather than remote infrastructure". {@link FindingLanguage}
 * enforces that at construction, so a rule that drifted into diagnosis would
 * fail rather than ship.
 *
 * <h2>Severity and confidence are different questions</h2>
 *
 * <p>Severity is how much of the build the finding is about; confidence is how
 * well the data supports it. A rule fed by a source that covered a third of the
 * actions produces a low-confidence finding however large the effect it found,
 * and a reader shown a single combined number could not tell those apart.
 *
 * <h2>Rules that are not here</h2>
 *
 * <p>Every rule in plan 16.1 is implemented. Where the plan's wording invites a
 * guess, the rule is grounded in something Bazel actually said instead: the
 * "non-cacheable or local-only concentration" rule reads Bazel's own
 * {@code cacheable} and {@code remotable} declarations from the execution log
 * rather than deciding what a runner string like {@code darwin-sandbox} implies,
 * because {@code spawn.proto} constrains that field to nothing and says it
 * varies under the dynamic strategy.
 */
public final class FindingRules {
    ...
    public static List<Finding> run(FindingInputs inputs) {
        ...
        findings.sort(Comparator
                ...
                .thenComparing(Comparator.comparingInt(
                        (Finding finding) -> finding.confidence().ordinal()).reversed())
    ...
    private static List<Finding> longCriticalChain(FindingInputs inputs) {
        ...
        if (evidence.isEmpty()) {
            ...
            // supporting record.
            evidence.add(Evidence.group(path.displayName(),
                    path.path().size() + " graph nodes, " + path.describe()));
        }
        ...
        return List.of(new Finding(
                ...
                "This chain is what the dependency graph implies, not what Bazel scheduled — the"
                        ...
                        + (path.isPartial()
                                ? " " + path.untimedNodes() + " of " + path.nodeCount()
    ...
    private static List<Finding> lowParallelismWindows(FindingInputs inputs) {
        ...
        List<ConcurrencySweep.Window> worst = inputs.lowParallelismWindows().stream()
                .sorted(Comparator.comparingLong(
        ...
        for (ConcurrencySweep.Window window : worst) {
            evidence.add(Evidence.window(window.startMicros(),
                    MetricFormat.duration(window.durationMicros()) + " from "
                            + MetricFormat.duration(
                                    window.startMicros() - concurrency.windowStartMicros())
                            + " in",
        ...
        return List.of(new Finding(
                ...
                MetricFormat.count(inputs.lowParallelismWindows().size())
                        ...
                        + MetricFormat.percent(share) + " of the elapsed time",
                share >= 0.3 ? Severity.HIGH : Severity.MEDIUM,
                concurrency.isPartial() ? Confidence.LOW : Confidence.HIGH,
                evidence,
                List.of(
                        new MetricValue("Peak concurrency",
                                MetricFormat.count(concurrency.peakActive()),
                                "computed here from observed action spans"),
                        new MetricValue("Average while busy",
                                MetricFormat.ratio(concurrency.averageActiveWhileBusy()),
                                "computed here from observed action spans"),
                        new MetricValue("Time below the threshold",
                                MetricFormat.duration(idleTotal), "computed here")),
                "below the session's typical concurrency divided by "
                        + inputs.thresholds().lowParallelismDivisor() + ", lasting at least "
                        + MetricFormat.duration(
                                inputs.thresholds().lowParallelismMinimumMicros()),
                ...
                "Concurrency here is measured from action spans, so an action nothing timed is"
                        ...
                        + (concurrency.untimedSpans() > 0
                                ? concurrency.untimedSpans() + " actions have no observed timing,"
    ...
    private static List<Finding> slowTail(FindingInputs inputs) {
        ...
        ConcurrencySweep.Result concurrency = sweep.orElseThrow();
        Optional<ConcurrencySweep.Window> trailing = inputs.lowParallelismWindows().stream()
        ...
        List<ActionMetrics> stragglers = inputs.candidates().stream()
                .filter(action -> action.endMicros().isPresent()
        ...
        if (evidence.isEmpty()) {
            evidence.add(Evidence.window(tail.startMicros(),
                    "the last " + MetricFormat.duration(tail.durationMicros()),
                    "at most " + tail.peakActive() + " actions running, and none of the"
        ...
        return List.of(new Finding(
                ...
                "The last " + MetricFormat.duration(tail.durationMicros()) + " of the build ran "
                        + tail.peakActive() + " actions or fewer",
                share >= 0.2 ? Severity.HIGH : Severity.MEDIUM,
                concurrency.isPartial() ? Confidence.LOW : Confidence.MEDIUM,
                evidence,
                List.of(
                        new MetricValue("Tail length",
                                MetricFormat.duration(tail.durationMicros()), "computed here"),
                        new MetricValue("Share of elapsed time",
                                MetricFormat.percent(share), "computed here"),
                        new MetricValue("Actions running in the tail",
                                MetricFormat.count(tail.peakActive()), "computed here")),
    ...
    private static List<Finding> manyTinyActions(FindingInputs inputs) {
        ...
        for (GroupAggregate group : inputs.table(GroupAggregate.Dimension.MNEMONIC).groups()) {
            ...
            findings.add(new Finding(
                    "many-tiny-actions",
                    MetricFormat.count(group.actions()) + " " + group.displayKey()
                    ...
                    List.of(
                            new MetricValue("Actions", MetricFormat.count(group.actions()),
                                    "counted in this session"),
                            MetricValue.from(group.duration(),
                                    "median " + MetricFormat.bounds(median)),
                            new MetricValue("Observed work",
                                    MetricFormat.duration(group.duration().observedSum()),
                                    group.duration().describe())),
                    "at least " + MetricFormat.count(inputs.thresholds().tinyActionCount())
    ...
    private static List<Finding> cacheMissConcentration(FindingInputs inputs) {
        ...
        for (GroupAggregate group : inputs.table(GroupAggregate.Dimension.MNEMONIC).groups()) {
            ...
            findings.add(new Finding(
                    ...
                    group.displayKey() + " missed the cache on "
                            + MetricFormat.percent(missRate) + " of the actions that reported",
                    ...
                    List.of(
                            new MetricValue("Cache misses",
                                    MetricFormat.count(group.cacheMisses()) + " of "
                                            + MetricFormat.count(known) + " reporting",
                                    "execution log"),
                            new MetricValue("Cache-state coverage",
    ...
    private static List<Finding> nonCacheableConcentration(FindingInputs inputs) {
        ...
        for (GroupAggregate group : inputs.table(GroupAggregate.Dimension.MNEMONIC).groups()) {
            ...
            findings.add(new Finding(
                    ...
                    "Bazel marked " + MetricFormat.count(blocked) + " of "
                            + MetricFormat.count(group.actions()) + " " + group.displayKey()
    ...
    private static List<Finding> highFanOut(FindingInputs inputs) {
        List<ActionMetrics> wide = inputs.candidates().stream()
                .filter(action -> action.directConsumers().orElse(0)
                        >= inputs.thresholds().highFanOut())
                .sorted(Comparator.comparingLong(
                        (ActionMetrics action) -> action.directConsumers().orElse(0)).reversed())
                .limit(EVIDENCE_LIMIT)
                .toList();
        if (wide.isEmpty()) {
            return List.of();
        ...
        return List.of(new Finding(
                ...
                MetricFormat.count(widest.directConsumers().orElse(0))
                        + " actions depend directly on " + widest.displayLabel(),
                Severity.MEDIUM,
                Confidence.HIGH,
                evidence,
                List.of(
                ...
                "at least " + MetricFormat.count(inputs.thresholds().highFanOut())
                        + " direct consumers",
                ...
                List.of(Link.focused(Link.View.GRAPH, "Show what depends on it",
                        widest.actionId())),
                false));
    }
    ...
    private static List<Finding> volumeFinding(
            ...
            String ruleId,
            java.util.function.Function<ActionMetrics, OptionalLong> measure,
            long threshold,
            String verb,
            String metricName,
            String why,
            String caveat) {
        List<ActionMetrics> heavy = inputs.candidates().stream()
                .filter(action -> measure.apply(action).orElse(0) >= threshold)
                .sorted(Comparator.comparingLong(
                        (ActionMetrics action) -> measure.apply(action).orElse(0)).reversed())
                .limit(EVIDENCE_LIMIT)
                .toList();
        if (heavy.isEmpty()) {
            return List.of();
        ...
        for (ActionMetrics action : heavy) {
            evidence.add(Evidence.action(action.actionId(), action.displayLabel(),
                    action.mnemonic() + ", " + verb + " "
                            + MetricFormat.bytes(measure.apply(action))));
        }
        ActionMetrics heaviest = heavy.getFirst();
        return List.of(new Finding(
                ruleId,
                heaviest.displayLabel() + " " + verb + " "
                        + MetricFormat.bytes(measure.apply(heaviest)),
                Severity.LOW,
                Confidence.MEDIUM,
                evidence,
                List.of(
                        new MetricValue(metricName,
                                MetricFormat.bytes(measure.apply(heaviest)), "execution log"),
                        new MetricValue("Its own duration",
                ...
                List.of(Link.focused(Link.View.ACTIONS, "Open this action",
                        heaviest.actionId())),
                false));
    }
    ...
    private static List<Finding> queueDominated(FindingInputs inputs) {
        List<ActionMetrics> waiting = inputs.candidates().stream()
                .filter(action -> action.queueFraction().orElse(0)
                        >= inputs.thresholds().queueDominatedFraction())
                .sorted(Comparator.comparingDouble(
                        (ActionMetrics action) -> action.queueFraction().orElse(0)).reversed())
                .limit(EVIDENCE_LIMIT)
                .toList();
        if (waiting.isEmpty()) {
            return List.of();
        ...
        for (ActionMetrics action : waiting) {
            evidence.add(Evidence.action(action.actionId(), action.displayLabel(),
                    MetricFormat.percent(action.queueFraction()) + " of "
                            + MetricFormat.duration(action.durationMicros()) + " was queue time"));
        }
        ActionMetrics worst = waiting.getFirst();
        return List.of(new Finding(
                "queue-dominated",
                MetricFormat.count(waiting.size()) + " actions spent most of their time queued,"
                        + " up to " + MetricFormat.percent(worst.queueFraction()),
                Severity.MEDIUM,
                Confidence.HIGH,
                evidence,
                List.of(
                        new MetricValue("Queue time",
                                MetricFormat.duration(worst.queueMicros()), "execution log"),
                ...
                List.of(Link.focused(Link.View.ACTIONS, "Open the worst one",
                        worst.actionId())),
                false));
    }
    ...
    private static List<Finding> transferDominated(FindingInputs inputs) {
        List<ActionMetrics> moving = inputs.candidates().stream()
                .filter(action -> action.transferFraction().orElse(0)
                        >= inputs.thresholds().transferDominatedFraction())
                .sorted(Comparator.comparingDouble(
                        (ActionMetrics action) -> action.transferFraction().orElse(0)).reversed())
                .limit(EVIDENCE_LIMIT)
                .toList();
        if (moving.isEmpty()) {
            return List.of();
        ...
        return List.of(new Finding(
                "transfer-dominated",
                MetricFormat.count(moving.size()) + " actions spent most of their time moving"
                        + " bytes rather than running",
                Severity.MEDIUM,
                Confidence.HIGH,
                evidence,
                List.of(
                        new MetricValue("Network time",
                                MetricFormat.duration(worst.networkMicros()), "execution log"),
                        new MetricValue("Upload time",
                                MetricFormat.duration(worst.uploadMicros()), "execution log"),
    ...
    private static List<Finding> repeatedAttempts(FindingInputs inputs) {
        List<ActionMetrics> repeated = inputs.candidates().stream()
        ...
        for (ActionMetrics action : repeated) {
            evidence.add(Evidence.action(action.actionId(), action.displayLabel(),
                    action.attempts() + " attempts, runner "
                            + (action.runner() == null
                                    ? "not the same for all of them" : action.runner())));
        }
        ActionMetrics most = repeated.getFirst();
        return List.of(new Finding(
                "repeated-attempts",
                MetricFormat.count(repeated.size()) + " actions ran more than once, up to "
                        + most.attempts() + " times",
                Severity.LOW,
                Confidence.HIGH,
                evidence,
    ...
    private static List<Finding> graphMismatch(FindingInputs inputs) {
        ...
        return List.of(new Finding(
                "graph-mismatch",
                "Only " + MetricFormat.percent(fraction)
