 ...
 *
 * <p>A rule that could query would be a rule whose cost depends on what it
 * decides to ask, and thirteen of them running against a Tier 3 session would
 * be thirteen unpredictable scans. Everything here comes from the single scan
 * {@code MetricQueries.collect} already performs plus a handful of bounded
 * top-N queries, so running every rule is arithmetic over data already in
 * memory — which is also what lets every rule be tested with a literal.
 *
 * @param candidates a bounded set of actions worth examining, the top few by
...
public record FindingInputs(
    ...
    public GroupAggregate.Table table(GroupAggregate.Dimension dimension) {
        return aggregates.getOrDefault(
                dimension, new GroupAggregate.Table(dimension, List.of(), 0, 0));
    }
}
