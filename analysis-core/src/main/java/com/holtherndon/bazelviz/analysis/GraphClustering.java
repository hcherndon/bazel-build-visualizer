 ...
 * offer to raise the limit". {@link GraphExtract#whole} refuses a graph that will not fit rather
 * than truncating it; this is what the user gets instead. A build of two million actions has
 * perhaps a few thousand packages and a few dozen mnemonics, so the same build that cannot be drawn
 * node by node is legible as soon as the question becomes "which package".
 *
 * <h2>It is a real aggregation, not a sample</h2>
 *
 * <p>Rule 12 forbids silently sampling or dropping. Every node lands in exactly one cluster and
 * every edge is counted — inside a cluster or between two — so the cluster counts sum to the
 * graph's node count and the edge weights sum to its edge count. The tests assert both sums,
 * because an aggregation whose parts do not add up to the whole is the kind of wrong that looks
 * right.
 *
 * <h2>Unknown is a cluster, not a blank</h2>
 *
 * <p>Plan 11.4: unknown values must be visibly unknown. A node whose label or mnemonic the import
 * never learned goes into a cluster that says so, rather than into an empty-string cluster that
 * reads as a real package named nothing.
 */
public final class GraphClustering {
   ...
   *
   * <p>Deliberately far below {@link GraphExtract#DEFAULT_NODE_LIMIT}: a cluster box carries a name
   * and a count, so it needs room that a dot does not.
   */
   ...
   *
   * <p>{@code //src/main:lib} is package {@code //src/main}. A label with no colon — which is legal
   * shorthand, {@code //src/main} meaning {@code //src/main:main} — is already its own package. An
   * external repository's {@code @rules_java//java:defs} keeps its repository prefix, because
   * {@code //java} in two repositories are two different packages and merging them would produce a
   ...
   *
   * <p>One pass over the nodes and one over the edges, so the cost is the graph's size and nothing
   * else. Cluster ordinals follow the keys' sort order, with the unknown cluster last, so the same
   * graph clusters into the same picture every time.
   *
   * @param keyByNode one key per node index, aligned with the graph; null or blank means the import
   *     never learned it, and lands in the unknown cluster
  ...
  static Result cluster(
    ...
    for (int node = 0; node < nodeCount; node++) {
      String key = keyByNode[node];
      int ordinal = key == null || key.isBlank() ? unknownOrdinal : ordinals.get(key);
    ...
    for (Map.Entry<Long, Long> entry : between.entrySet()) {
      ...
      edges.add(
          new ClusterEdge((int) (packed >>> 32), (int) (packed & 0xffffffffL), entry.getValue()));
    }
   ...
   *
   * @param key the package, label or mnemonic, or null for the cluster of nodes whose key was never
   *     learned
   * @param internalEdges dependencies that begin and end inside this cluster, which is what makes a
   *     package look self-contained or not
   */
  ...
  public record Result(
     ...
     *
     * <p>The node ids are cluster <em>ordinals</em>, not graph node indices. Anything that needs a
     * name for one must ask this result, not the session's node lookups — the two numbering schemes
     * are unrelated and a mixed-up lookup would label a box with an arbitrary action.
     */
     ...
     *
     * <p>Exposed rather than merely asserted in a test: a caller that draws these numbers should be
     * able to check the arithmetic it is drawing.
     */
