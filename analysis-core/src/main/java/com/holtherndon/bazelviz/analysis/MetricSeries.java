...
/**
 * One measured quantity over a set of things, with the two facts a number is not allowed to be
 * shown without: where it came from, and how much of the set it covers.
 *
 * <h2>Why the coverage travels with the number</h2>
 *
 * <p>Plan 24's Phase 8 exit criterion is "every displayed metric reports source and completeness",
 * and this is the type that makes that structural rather than a habit. A median build duration
 * computed over the 4 actions of 13 that produced an execution-log record is a real number about a
 * real subset, and shown alone it reads as a statement about the build. The subset is not a failure
 * — most actions run inside the Bazel server and never spawn a subprocess, so no attempt record
 * exists for them and none ever will — which is exactly why the count has to be visible: a reader
 * who cannot see it goes looking for missing data that is not missing.
 *
 * <h2>Unavailable is counted, not dropped</h2>
 *
 * <p>{@link #unavailable} is the number of members that reported nothing. They are not in the
 * distribution, so they do not drag a mean toward zero (rule 11), and they are not silently absent
 * either, so a coverage figure can be computed from the series alone rather than from the series
 * plus a number the caller has to remember to pass along.
 *
 ...
 * @param completeness how much of that source was read — a truncated execution log makes every
 *     series drawn from it partial however many observations it happened to yield
...
public record MetricSeries(
  ...
  /**
   * The total of what was observed, which is a lower bound when anything was not.
   *
   * <p>Distinct from {@link #exactSum()} on purpose: a caller that wants to draw a bar has to
   * choose knowingly between a bar that is exact and a bar that is "at least this", and a single
   * {@code sum()} would let it not notice which one it drew.
   */
   ...
   *
   * <p>Formatting of the values themselves belongs to the view, which knows about locales and
   * adaptive units; this decides only what must be said.
   */
  public String describe() {
    ...
    if (!completeness.isComplete()) {
      text.append("; the ")
          .append(sourceName())
          .append(" is ")
          .append(completeness.name().toLowerCase(Locale.ROOT).replace('_', ' '))
  ...
  public static final class Builder {
    ...
    public MetricSeries build() {
      return new MetricSeries(name, units, source, completeness, sketch.snapshot(), unavailable);
    }
  }
}
