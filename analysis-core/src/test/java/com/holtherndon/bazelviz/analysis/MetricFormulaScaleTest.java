 ...
 *
 * <p>Five million actions is what plan 20.1's Tier 3 names, and both of these
 * run over every one of them. Neither needs a database or a Bazel server, so
 * measuring them at full size costs a second rather than the gigabytes a real
 * build of that size would — which is the reason the figures in
...
final class MetricFormulaScaleTest {
    ...
    void formulasRunAtTierThree() {
        ...
        began = System.nanoTime();
        java.util.List<ConcurrencySweep.Window> windows = spans.windowsBelow(64, 1_000);
