...
/**
 * Each rule fires on the shape it is about, and does not fire on the shape it is not.
 *
 * <p>Plan 24's Phase 8 exit criterion is that formulas have deterministic unit tests, and a rule
 * has two halves that can each be wrong: the arithmetic, and the threshold it compares against.
 * Both halves get a test, because a rule that fires on everything is as useless as one that never
 * fires and looks considerably more helpful.
 */
final class FindingRulesTest {
    ...
    return new InvocationMetrics(
        new InvocationMetrics.Timing(
            wallMicros.isPresent()
                ...
                : Measured.unknown(DataSource.BEP, Completeness.UNAVAILABLE, "no finish"),
  ...
  private static GroupAggregate group(
    ...
    return new GroupAggregate(
        GroupAggregate.Dimension.MNEMONIC,
        key,
        actions,
        duration.build(),
        inputs.build(),
  ...
  void longCriticalChain() {
    ...
    CriticalPath.Result path =
        CriticalPath.compute(
            chain,
            new long[] {250_000, 250_000, 250_000, 250_000},
            CriticalPath.DurationSource.EXECUTION_ATTEMPT);
    CriticalPaths paths =
        new CriticalPaths(Measured.of(900_000L, DataSource.PROFILE), List.of(), Optional.of(path));

    FindingInputs slowBuild =
        new FindingInputs(
  ...
  void longCriticalChainNamesItself() {
    ...
    CriticalPath.Result path =
        CriticalPath.compute(
            chain,
            new long[] {500_000, CriticalPath.UNKNOWN_DURATION},
            CriticalPath.DurationSource.EXECUTION_ATTEMPT);
    CriticalPaths paths =
        new CriticalPaths(
    ...
    Finding finding =
        run(new FindingInputs(
                invocation(
                    OptionalLong.of(600_000), Optional.empty(), paths, List.of(completeGraph)),
                Map.of(),
                List.of(),
                List.of(),
                List.of(),
  ...
  void criticalChainConfidenceNeedsStructuralCompleteness() {
    ...
    CriticalPath.Result path =
        CriticalPath.compute(
            chain, new long[] {300_000, 300_000}, CriticalPath.DurationSource.BEP_ACTION);
    CriticalPaths paths =
        new CriticalPaths(
            Measured.unknown(DataSource.PROFILE, Completeness.UNAVAILABLE, "no profile"),
            List.of(),
    ...
    Finding withProof =
        run(new FindingInputs(
                invocation(
                    OptionalLong.of(700_000), Optional.empty(), paths, List.of(completeGraph)),
                Map.of(),
                List.of(),
                List.of(),
                List.of(),
  ...
  void lowParallelismWindows() {
    ...
    assertThat(low.evidence().getFirst().kind()).isEqualTo(Finding.Evidence.Kind.TIME_WINDOW);
  }
  ...
  void slowTail() {
    ...
    spans.add(1_000_000, 3_000_000);
    ConcurrencySweep.Result sweep = ConcurrencySweep.sweep(spans);
    ActionMetrics straggler =
        with(
            action(9, "//pkg:slow", "Javac"),
  ...
  void cacheMissConcentrationNeedsCoverage() {
    ...
    assertThat(finding.evidence())
        .extracting(Finding.Evidence::kind)
  ...
  void nonCacheableConcentration() {
    ...
    assertThat(finding.metrics())
        .extracting(Finding.MetricValue::source)
        .allMatch(source -> source.contains("execution log") || source.contains("this session"));
  }
  ...
  void perActionRulesFireOnTheirOwnShape() {
    ActionMetrics wide = with(action(1, "//pkg:hub", "Genrule"), base -> withConsumers(base, 400));
    ActionMetrics fat =
        with(
            action(2, "//pkg:big", "CppCompile"),
            base -> withInputs(withDuration(base, 1_000_000), 900L * 1024 * 1024));
    ActionMetrics producer =
        with(
            action(3, "//pkg:out", "Genrule"),
            base -> withOutputs(withDuration(base, 1_000_000), 700L * 1024 * 1024));
    ActionMetrics queued =
        with(
            action(4, "//pkg:wait", "Javac"),
            base -> withQueue(withDuration(base, 1_000_000), 800_000));
    ActionMetrics moving =
        with(
            action(5, "//pkg:move", "Javac"),
            base -> withTransfer(withDuration(base, 1_000_000), 700_000));
    ActionMetrics retried =
        with(action(6, "//pkg:flake", "TestRunner"), base -> withAttempts(base, 3));
    ...
    List<Finding> findings =
        run(
            new FindingInputs(
                ...
                Map.of(),
                List.of(),
                List.of(wide, fat, producer, queued, moving, retried),
                List.of(),
                THRESHOLDS));

    assertThat(ruleIds(findings))
        .contains(
            "high-fan-out",
            "high-input-volume",
            "high-output-volume",
  ...
  void quietBuildsAreQuiet() {
    ActionMetrics ordinary =
        with(action(1, "//pkg:a", "Javac"), base -> withDuration(base, 200_000));
  ...
  void graphMismatch() {
    Coverage poor = Coverage.of("Action-graph correlation", 100, 1_000, DataSource.AQUERY, "why");
    assertThat(
            ruleIds(
                run(
                    new FindingInputs(
                        invocation(
                            OptionalLong.of(10_000), Optional.empty(), noPaths(), List.of(poor)),
                        Map.of(),
                        List.of(),
                        List.of(),
                        List.of(),
    ...
    assertThat(
            ruleIds(
                run(
                    new FindingInputs(
                        invocation(
                            OptionalLong.of(10_000), Optional.empty(), noPaths(), List.of(absent)),
                        Map.of(),
                        List.of(),
                        List.of(),
                        List.of(),
  ...
  void findingsAreOrderedAndComplete() {
    ActionMetrics wide = with(action(1, "//pkg:hub", "Genrule"), base -> withConsumers(base, 400));
    ActionMetrics retried =
        with(action(2, "//pkg:flake", "TestRunner"), base -> withAttempts(base, 3));
  ...
  private static ActionMetrics copy(
    ...
    return new ActionMetrics(
        ...
        attempts,
        base.cacheState(),
        base.directDependencies(),
        consumers,
        base.slackMicros(),
        base.onDerivedCriticalPath(),
        base.startConcurrency(),
        base.completionConcurrency());
  }
}
