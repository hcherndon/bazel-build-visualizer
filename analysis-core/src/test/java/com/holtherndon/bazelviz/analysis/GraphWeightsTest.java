 ...
 *
 * <p>The diamond {@code 0 → 1 → 3, 0 → 2 → 3} (producer-to-consumer) is the
 * fixture because it has everything the counts must get right: overlapping
 * reachability (3 is reachable from 0 both ways, and must be counted once), a
 * source, a sink, and asymmetric degrees.
 *
 * <p>The other half of these tests is the honesty machinery: a budget that
 * trips must leave {@code UNKNOWN} behind it and say it was reached, because
 * plan 13.3 forbids a transitive closure and a traversal that cannot give up
 * visibly would be one in disguise.
 */
final class GraphWeightsTest {
    ...
    void immediateDegreesAreCsrDegrees() {
        ...
        GraphWeights.Result dependents =
                GraphWeights.immediateDegrees(forward, List.of(0, 1, 2, 3));
        GraphWeights.Result dependencies = GraphWeights.immediateDegrees(
                CsrBuilder.reverse(forward), List.of(0, 1, 2, 3));
    ...
    void subgraphTransitiveCountsAreExact() {
        ...
        GraphWeights.Result dependents = GraphWeights.subgraphTransitiveCounts(
                List.of(0, 1, 2, 3), diamondEdges(), true,
                GraphWeights.SUBGRAPH_TRANSITIVE_WORK_BUDGET);
        GraphWeights.Result dependencies = GraphWeights.subgraphTransitiveCounts(
                List.of(0, 1, 2, 3), diamondEdges(), false,
                GraphWeights.SUBGRAPH_TRANSITIVE_WORK_BUDGET);
    ...
    void countsAreSubgraphScoped() {
        ...
        // legend states.
        List<GraphExtract.Edge> drawn = List.of(
                new GraphExtract.Edge(0, 1), new GraphExtract.Edge(1, 3));

        GraphWeights.Result dependencies = GraphWeights.subgraphTransitiveCounts(
                List.of(0, 1, 3), drawn, false,
                GraphWeights.SUBGRAPH_TRANSITIVE_WORK_BUDGET);
    ...
    void workBudgetLeavesUnknown() {
        GraphWeights.Result counted = GraphWeights.subgraphTransitiveCounts(
                List.of(0, 1, 2, 3), diamondEdges(), true, 1);
    ...
    void globalCountWithinBudget() {
        GraphWeights.BudgetedCount counted =
                GraphWeights.globalTransitiveCount(diamond(), 0, 100);
    ...
    void globalCountHittingBudget() {
        GraphWeights.BudgetedCount counted =
                GraphWeights.globalTransitiveCount(diamond(), 0, 2);
    ...
    void visitingEverythingIsComplete() {
        ...
        GraphWeights.BudgetedCount counted =
                GraphWeights.globalTransitiveCount(diamond(), 0, 4);
    ...
    void unavailableCarriesItsReason() {
        ...
        assertThat(unavailable.values())
                .containsExactly(GraphWeights.UNKNOWN, GraphWeights.UNKNOWN,
                        GraphWeights.UNKNOWN);
