...
/**
 * Phase 0 spike: custom Java2D aggregate timeline (plan section "Phase 0", benchmark target plan
 * 20.2: p95 frame under 33 ms).
 *
 * <p>Modes:
 *
 * <ul>
 *   <li>default: interactive window (drag pans, wheel zooms); {@code --jvm_flag=-Dbbv.smoke=true}
 *       closes it automatically; {@code --smoke} and {@code BBV_SMOKE=true} work too.
 *   <li>{@code --offscreen}: headless; paints {@value #MEASURED_FRAMES} frames at randomized
 *       pan/zoom into a 1600x900 RGB image and prints frame statistics with a PASS/FAIL verdict.
 *   <li>{@code --tier3}: 5M actions instead of the TIER2 default of 1M.
 * </ul>
 */
public final class TimelineSpike {
  ...
  public static void main(String[] args) {
    ...
    System.out.printf(
        Locale.ROOT,
        "Timeline spike: %s (%,d actions), wall %.1f s%n",
        scale,
        generator.actionCount(),
        generator.buildWallMicros() / 1e6);
    ...
    System.out.printf(
        Locale.ROOT,
        "LOD build: %.3f s streaming %,d spans%n",
        buildSeconds,
        index.totalSpanCount());
    for (int l = 0; l < index.levelCount(); l++) {
      System.out.printf(
          Locale.ROOT,
          "  level %d: bin %,d us x %,d bins%n",
          l,
          index.binWidthMicros(l),
          index.binCount(l));
    }
  ...
  /**
   * Streams straight off the O(1) generator; nothing is materialized and the per-call {@link
   * SyntheticAction} records are not retained.
   */
  ...
  private static void runOffscreen(TimelineLodIndex index) {
    ...
    System.out.printf(
        Locale.ROOT,
        "%s: p95 %.2f ms vs %.0f ms target (30 FPS, plan 20.2)%n",
        pass ? "PASS" : "FAIL",
        p95,
        TARGET_P95_MILLIS);
    if (!pass) {
  ...
  private static double paintRandomFrame(
      TimelineCanvas canvas, BufferedImage image, Random random, long wallSpan, long wallStart) {
