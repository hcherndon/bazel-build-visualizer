...
/**
 * Phase 0 spike: CSR graph index build + budget-limited BFS + aggregate
 * cluster painting with Java2D (plan sections 13.2/13.6).
 *
 * <p>Modes: default opens a JFrame with drag-pan / wheel-zoom over the
 * aggregate cluster view ({@code -Dbbv.smoke=true} or {@code --smoke}
 * auto-closes); {@code --offscreen} is a headless stats run. {@code --tier2}
 * switches from TIER1 to TIER2 synthetic data. The frame benchmark (300
 * frames at random pan/zoom into a 1600x900 BufferedImage) runs in every
 * mode so numbers always print.
 *
 * <p>All graph/cluster computation happens on the main thread before any
 * Swing code runs; the EDT only ever executes {@code paintComponent}.
 */
public final class GraphSpike {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf(Locale.ROOT, "GraphSpike scale=%s nodes=%,d edges=%,d%n",
                scale, nodeCount, edges.edgeCount());

        // TIER1/TIER2 indices fit int; toIntExact guards if that ever changes.
        EdgeStream stream = visitor -> edges.forEachEdge(
                (producer, consumer) -> visitor.edge(Math.toIntExact(producer), Math.toIntExact(consumer)));

        long t0 = System.nanoTime();
        CsrGraph forward = CsrBuilder.build(nodeCount, stream);
        double buildMs = (System.nanoTime() - t0) / 1e6;
        System.out.printf(Locale.ROOT, "CSR build (two passes over synthetic stream): %.1f ms%n", buildMs);
        ...
        System.out.printf(Locale.ROOT,
                ...
                model.clusterCount, CLUSTER_SIZE, model.pairFrom.length,
                (System.nanoTime() - t0) / 1e6);
        System.out.println("  note: dense int[c][c] weight matrix is spike-only; real impl needs sparse weights");
    ...
    private static void printMemoryEstimate(CsrGraph forward, CsrGraph reverse) {
        ...
        System.out.printf(Locale.ROOT,
                ...
                oneDirection / 1e6, both / 1e6, naive / 1e6, naive / (double) oneDirection);
    }
    ...
    private static void runBfsBenchmark(CsrGraph graph, int nodeCount) {
        ...
        System.out.printf(Locale.ROOT,
                "BFS: %d random-source runs, %,d node budget: %,d nodes visited in %.1f ms "
                        + "(%.1fM nodes/sec)%n",
                BFS_RUNS, BFS_NODE_BUDGET, visited, seconds * 1e3, visited / seconds / 1e6);
    }
    ...
    /**
     * Aggregate view model: nodes grouped into ceil(n/4096) clusters by
     * index, laid out on a grid; inter-cluster edge weights precomputed into
     * a compact draw list (parallel primitive arrays, no per-edge objects).
     */
    private static final class ClusterModel {
        ...
        private ClusterModel(int clusterCount, double worldWidth, double worldHeight,
                double[] centerX, double[] centerY, double[] tileHalf, float[] tileBrightness,
                int[] pairFrom, int[] pairTo, float[] pairAlpha, float[] pairWidth) {
        ...
        static ClusterModel build(CsrGraph graph, int nodeCount) {
            ...
            return new ClusterModel(clusters, cols * cell, rows * cell,
                    cx, cy, half, brightness, pFrom, pTo, pAlpha, pWidth);
        }
    ...
    private static void paintScene(Graphics2D g2, ClusterModel m, AffineTransform view,
        ...
        for (int k = 0; k < m.pairFrom.length; k++) {
            int i = m.pairFrom[k];
            int j = m.pairTo[k];
            g2.setColor(new Color(EDGE_BASE.getRed(), EDGE_BASE.getGreen(), EDGE_BASE.getBlue(),
    ...
    private static boolean benchmarkFrames(ClusterModel model) {
        ...
        System.out.printf(Locale.ROOT,
                ...
                TIMED_FRAMES, WIDTH, HEIGHT, total / (double) TIMED_FRAMES / 1e6, p50, p95, max, checksum);
        System.out.printf(Locale.ROOT, "%s: p95 %.2f ms vs %.0f ms budget%n",
                pass ? "PASS" : "FAIL", p95, FRAME_P95_BUDGET_MS);
        return pass;
    }
