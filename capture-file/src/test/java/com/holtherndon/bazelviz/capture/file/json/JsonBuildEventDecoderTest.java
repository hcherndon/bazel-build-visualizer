...
final class JsonBuildEventDecoderTest {
    ...
    void acceptsBothProtoAndLowerCamelCaseFieldNames() {
        ...
        JsonDecodeResult camel = decoder.decode(
                utf8("{\"id\":{\"progress\":{\"opaqueCount\":3}},\"lastMessage\":true}"));
        JsonDecodeResult snake = decoder.decode(
                utf8("{\"id\":{\"progress\":{\"opaque_count\":3}},\"last_message\":true}"));
    ...
    void reportsFailureRatherThanThrowingForContentItCannotDecode() {
        List<String> undecodable = List.of(
    ...
    void anEmptyObjectIsAValidBuildEventWithNoPayload() {
        ...
        assertThat(result.status()).isEqualTo(DecodeStatus.OK);
        assertThat(result.event().getPayloadCase())
                .isEqualTo(BuildEvent.PayloadCase.PAYLOAD_NOT_SET);
    }
