...
/**
 * Progress reporting: enough to drive a bar, throttled enough not to flood the
 * caller, and never a fabricated denominator.
 *
 * <p>The last point is the one with teeth. A progress API that reports a total
 * it does not have — so the bar can reach a satisfying 100% — is the same
 * defect as a metric shown as zero when it is unknown (plan 11.4). The total is
 * an {@link OptionalLong} and stays empty when it is not known, and
 * {@link ImportProgress#fractionComplete()} refuses to invent one.
 */
class ImportProgressTest {
    ...
    void progressIsThrottled(@TempDir Path temporary) throws Exception {
        ...
        assertThat(samples).extracting(ImportProgress::recordsRead)
                .isSortedAccordingTo(Comparator.naturalOrder());
        assertThat(samples).allSatisfy(sample ->
                assertThat(sample.recordsRead()).isLessThanOrEqualTo(4_000L));
        ...
        assertThat(samples)
                .filteredOn(sample -> sample.phase() == ImportPhase.DETECTING
                        || sample.phase() == ImportPhase.PRESERVING)
                .isNotEmpty()
                .allSatisfy(sample -> {
        ...
        assertThat(samples)
                .filteredOn(sample -> sample.phase() == ImportPhase.READING
                        || sample.phase() == ImportPhase.FINALIZING)
                .isNotEmpty()
                .allSatisfy(sample -> {
        ...
        assertThat(samples).extracting(ImportProgress::phase)
    ...
    void unknownTotalIsNotInvented() {
        ImportProgress unknown = new ImportProgress(
                ImportPhase.READING, 12, 3_456, OptionalLong.empty());
        ...
        ImportProgress zeroTotal = new ImportProgress(
                ImportPhase.READING, 0, 0, OptionalLong.of(0));
