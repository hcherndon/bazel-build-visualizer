 ...
 *
 * <p>A duplicate action primary output means one of two actions is not in the
 * table. A file set referenced and never defined means every byte total reached
 * through it is a lower bound. Both look exactly like a smaller build, so a
 * session that found one and said nothing would be indistinguishable from a
 * session that had nothing to find. The writer counted both from the start and
 * nothing read the counters; these tests exist so that stays fixed.
 */
class NormalizationAnomalyTest {
    ...
    void duplicatePrimaryOutputIsDiagnosed(@TempDir Path temporary) throws Exception {
        ...
        BepBinaryWriter.write(source, List.of(
                action("bazel-out/bin/a.o", "//pkg:a"),
                action("bazel-out/bin/a.o", "//pkg:b")).iterator());
    ...
    void undefinedFileSetIsDiagnosed(@TempDir Path temporary) throws Exception {
        ...
        BepBinaryWriter.write(source, List.of(
                namedSet("1"),
                targetReferencing("//pkg:lib", "7")).iterator());
        ...
        assertThat(diagnostics)
                .anySatisfy(entry -> {
                    assertThat(entry.diagnostic().code())
                            .isEqualTo(DiagnosticCodes.UNDEFINED_FILE_SET);
    ...
    void aCleanStreamIsQuiet(@TempDir Path temporary) throws Exception {
        ...
        BepBinaryWriter.write(source, List.of(
                ...
                action("bazel-out/bin/a.o", "//pkg:a")).iterator());

        List<DiagnosticEntry> diagnostics = importAndReadDiagnostics(temporary, source);

        assertThat(diagnostics)
                .as("a warning nobody needs is a warning nobody reads")
                .noneMatch(entry -> entry.diagnostic().code()
                                .equals(DiagnosticCodes.DUPLICATE_ACTION_OUTPUT)
    ...
    private static BuildEvent action(String primaryOutput, String label) {
        return BuildEvent.newBuilder()
                .setId(BuildEventId.newBuilder().setActionCompleted(
    ...
    private static BuildEvent namedSet(String id) {
        return BuildEvent.newBuilder()
                .setId(BuildEventId.newBuilder().setNamedSet(
                        BuildEventId.NamedSetOfFilesId.newBuilder().setId(id)))
