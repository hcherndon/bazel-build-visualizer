 ...
 *
 * <p>The events are printed with protobuf's own JSON printer rather than hand-written, so the
 * fixtures are encoded exactly the way Bazel encodes BEP — including how it escapes a command line
 * full of quotes and braces, which is the case the record splitter has to survive.
 */
final class JsonFixtures {
  ...
  /**
   * A command line with the two hazards a naive splitter trips on together: literal braces and
   * escaped quotes inside one string value. Real builds produce this constantly — any {@code
   * --define} of a JSON blob or any genrule command does it.
   */
  ...
  static List<BuildEvent> sampleEvents() {
    ...
    events.add(
        BuildEvent.newBuilder()
            ...
            .setUnstructuredCommandLine(
                UnstructuredCommandLine.newBuilder()
                    .addArgs("build")
                    .addArgs("//...")
    ...
    events.add(
        BuildEvent.newBuilder()
            ...
            .setProgress(
                Progress.newBuilder()
                    .setStderr(
                        "ERROR: /w/BUILD:3:1: Executing genrule //p:g failed: "
                            + "bash -c 'printf \"{\\\"a\\\": 1}\"' } { \\")
