 ...
 *
 * <p>Every case is run at several chunk sizes, down to one byte at a time, so that a boundary
 * landing exactly on a buffer edge — mid-string, mid-escape, between a brace and its neighbour — is
 * exercised rather than hoped about.
 */
final class JsonRecordScannerTest {

  /**
   * A record whose string values contain both braces and escaped quotes, the shape a real Bazel
   * failure message or command line has. Written as a text block, so after Java escape processing
   * the actual JSON bytes are:
   *
   * <pre>
   * {"msg":"closes } opens { quote \" backslash \\ both \\\" done","n":{"k":"}"}}
   * </pre>
   *
   * A brace counter that does not track string state ends this record early, at the {@code &#125;}
   * inside the first string value.
   */
  ...
  /**
   * Drives the scanner over {@code data} handing it at most {@code chunk} bytes at a time,
   * mirroring how {@link JsonBepParser} feeds it from a fixed read buffer. Offsets are absolute
   * because the window moves over one array.
   *
