...
/**
 * Exit criterion 1 for the unhappy half: a truncated file imports its prefix
 * and says so, a corrupt file is distinguished from a truncated one, and a file
 * carrying fields this build does not know is imported rather than rejected.
 *
 * <p>The distinction between {@link Completeness#TRUNCATED} and
 * {@link Completeness#CORRUPT_PARTIAL} is the point of these tests, not a
 * detail of them (plan 21.3): a short tail is the normal shape of a cancelled
 * build, while damaged bytes mean the storage underneath should be suspected.
 * Collapsing them into one "incomplete" would tell a user the wrong thing to
 * do about their file.
 */
class BepImporterDamagedSourceTest {
    ...
    void truncatedMidPayload(@TempDir Path temporary) throws Exception {
        ...
        assertThat(ImportTestSupport.readCaptureSources(result.sessionRoot()))
                .singleElement()
                .satisfies(row ->
                        assertThat(row.completeness()).isEqualTo(Completeness.TRUNCATED.name()));
    ...
    void flippedByteIsCorruptPartial(@TempDir Path temporary) throws Exception {
        ...
        List<EventRow> failed = rows.stream()
                .filter(row -> row.decodeStatus() == DecodeStatus.FAILED)
                .toList();
        ...
        assertThat(failed.get(0).eventIdHash())
                .as("an unavailable id is NULL, never 0")
                .isEmpty();

        // The undecodable bytes are still in the journal, exactly as read, so a
        // later build can reinterpret them (plan 21.5).
        byte[] preserved = JournalPayloadReader.forSession(result.sessionRoot())
                .read(failed.get(0).rawLocation());
    ...
    /**
     * Flips one payload byte in a frame, trying successive offsets until the
     * frame genuinely stops decoding.
     *
     * <p>Not every single-byte flip breaks a protobuf — plenty land in string
     * content and parse perfectly. The test needs a frame that actually fails,
     * so it looks for one rather than assuming the first flip is enough.
     */
    private static BepDamage.Damage flipUntilDecodeBreaks(Path intact, Path damaged)
            throws Exception {
        var decoder = com.holtherndon.bazelviz.bepcodec.BepEventDecoder.withDefaults();
        ...
        for (int frameIndex = 3; frameIndex < Math.min(frames.size(), 40); frameIndex++) {
            for (int within = 0; within < Math.min(frames.get(frameIndex).payloadLength(), 24); within++) {
                BepDamage.Damage damage =
                        BepDamage.flipPayloadByte(intact, damaged, frameIndex, within);
    ...
    void corruptFraming(@TempDir Path temporary) throws Exception {
        ...
        byte[] bytes = java.nio.file.Files.readAllBytes(intact);
        int at = (int) twentieth.varintOffset();
        for (int i = 0; i < 4; i++) {
            bytes[at + i] = (byte) 0xFF;
        }
        bytes[at + 4] = (byte) 0x7F;
        java.nio.file.Files.write(damaged, bytes);
        ...
        assertThat(ImportTestSupport.readDiagnostics(result.sessionRoot()))
                ...
                .satisfies(row -> {
                    ...
                    assertThat(row.byteOffset())
                            .isEqualTo(OptionalLong.of(twentieth.varintOffset()));
                });
    }
    ...
    void unknownFieldsAreKeptNotDropped(@TempDir Path temporary) throws Exception {
        ...
        assertThat(rows).filteredOn(row -> row.decodeStatus() == DecodeStatus.UNKNOWN_FIELDS)
                .hasSize(1);
        ...
        byte[] payload = JournalPayloadReader.forSession(result.sessionRoot())
                .read(damaged.rawLocation());
        ...
        assertThat(new String(payload, java.nio.charset.StandardCharsets.ISO_8859_1))
