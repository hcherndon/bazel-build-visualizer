 ...
 *
 * <p>Importing one file into two independent sessions must produce the same number of events, the
 * same journal offsets and the same id hashes. Anything that varied between the two — a hash seeded
 * by identity, an offset that depended on timing, an ordinal drawn from a shared counter — would
 * mean a session could not be compared with another, re-derived, or checked against a fixture.
 */
class BepImporterDeterminismTest {
  ...
  void binaryImportsAreReproducible(@TempDir Path temporary) throws Exception {
    ...
    assertThat(second)
        .extracting(EventRow::eventIdHash)
  ...
  void jsonImportsAreReproducible(@TempDir Path temporary) throws Exception {
    ...
    BepJsonWriter.write(
        source, BepJsonWriter.Layout.ONE_OBJECT_PER_LINE, SyntheticBepStream.of(120));
  ...
  void journalsAreByteIdentical(@TempDir Path temporary) throws Exception {
    ...
    byte[] firstSegment =
        Files.readAllBytes(first.sessionRoot().resolve("raw").resolve("bes-000000.journal"));
    byte[] secondSegment =
        Files.readAllBytes(second.sessionRoot().resolve("raw").resolve("bes-000000.journal"));
    assertThat(secondSegment.length).isEqualTo(firstSegment.length);
    assertThat(Arrays.copyOfRange(secondSegment, 32, secondSegment.length))
        .isEqualTo(Arrays.copyOfRange(firstSegment, 32, firstSegment.length));
  }
