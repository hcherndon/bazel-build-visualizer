...
/**
 * Collects every frame a parse delivers, copying the payload out of the
 * parser's reusable buffer as a real sink must.
 */
final class RecordingSink implements BinaryBepEventSink {
    ...
    public void onEvent(BinaryBepFrame frame) {
        frames.add(new Delivered(
                frame.frameOffset(), frame.payloadOffset(), frame.payloadLength(),
                frame.copyPayload()));
    }
