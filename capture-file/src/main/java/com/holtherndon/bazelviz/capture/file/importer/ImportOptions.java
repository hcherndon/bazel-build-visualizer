...
/**
 * Tunables for {@link BepImporter}. Every default is stated here rather than
 * scattered through the pipeline, so the memory ceiling of an import can be
 * read off one page.
 *
 * <p>The buffer sizes are the whole of the importer's per-record footprint:
 * {@code readBufferBytes} for the source window, {@code journalBufferBytes} for
 * the journal staging buffer, and one payload scratch array that grows to the
 * largest single record. None of them is derived from the length of the input
 * (Phase 1 exit criterion "no full file is loaded into memory"), and the
 * observed high-water mark of all of them comes back in {@link ImportResult}
 * so a test can assert the bound instead of trusting it.
 *
 * @param preservation how the original file is preserved
 * @param checkpointEveryRecords records between durability checkpoints; each
 *     one flushes the event batch, forces the journal and rewrites the resume
 *     point, so an interrupted import loses at most this many records of work
 *     (never any data — the journal still holds it)
 ...
 * @param maxRecordBytes largest single record accepted, shared by the parsers
 *     and the journal so the two limits cannot disagree
 * @param segmentBytes journal segment rotation size
 * @param maxDiagnosticsPerCode cap on repeated per-record diagnostic rows of
 *     one code. Reaching it never drops information: the true total is always
 *     recorded as a summary diagnostic at finalization
 * @param verifyDigestOnResume re-hash the preserved source when resuming, so a
 *     resume proves it is continuing over the same bytes it started on
 * @param clock source of epoch-microsecond timestamps; injectable so tests can
 *     make {@code receive_micros} reproducible
 */
public record ImportOptions(
    ...
    /**
     * Records between checkpoints. Small enough that a crash replays a trivial
     * backlog, large enough that the fsync it implies is amortized over
     * thousands of events.
     */
    ...
    public ImportOptions {
        ...
        if (readBufferBytes < JsonBepParserOptions.MIN_READ_BUFFER_BYTES) {
            throw new IllegalArgumentException("readBufferBytes must be >= "
                    + JsonBepParserOptions.MIN_READ_BUFFER_BYTES + ", got " + readBufferBytes);
        }
        if (maxRecordBytes < 1) {
    ...
    public ImportOptions withPreservation(SourcePreservation value) {
        return new ImportOptions(value, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withCheckpointEveryRecords(long value) {
        return new ImportOptions(preservation, value, progressEveryRecords, progressIntervalMillis,
                batchSize, readBufferBytes, journalBufferBytes, maxRecordBytes, segmentBytes,
                maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withProgressEveryRecords(long value) {
        return new ImportOptions(preservation, checkpointEveryRecords, value,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withProgressIntervalMillis(long value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords, value,
                batchSize, readBufferBytes, journalBufferBytes, maxRecordBytes, segmentBytes,
                maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withBatchSize(int value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, value, readBufferBytes, journalBufferBytes, maxRecordBytes,
                segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withReadBufferBytes(int value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, value, journalBufferBytes, maxRecordBytes,
                segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withJournalBufferBytes(int value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, value, maxRecordBytes,
                segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withMaxRecordBytes(int value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes, value,
                segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }
    ...
    public ImportOptions withSegmentBytes(long value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, value, maxDiagnosticsPerCode, verifyDigestOnResume, clock);
    }

    public ImportOptions withMaxDiagnosticsPerCode(int value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, segmentBytes, value, verifyDigestOnResume, clock);
    }

    public ImportOptions withVerifyDigestOnResume(boolean value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, segmentBytes, maxDiagnosticsPerCode, value, clock);
    }

    public ImportOptions withClock(MicrosClock value) {
        return new ImportOptions(preservation, checkpointEveryRecords, progressEveryRecords,
                progressIntervalMillis, batchSize, readBufferBytes, journalBufferBytes,
                maxRecordBytes, segmentBytes, maxDiagnosticsPerCode, verifyDigestOnResume, value);
    }
