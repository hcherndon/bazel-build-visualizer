 ...
 *
 * <p>Counts are separated rather than summed because they answer different questions: {@code
 * deliveredRecordCount} is how much data the session has, {@code oversizedRecordCount} is how much
 * the configured limit cost, and {@code failedDecodeCount} is how much is present as raw bytes but
 * not yet interpretable. Collapsing them would hide exactly the losses plan 21.3 requires be
 * visible.
 *
 * <p>{@code truncatedTailOffset} is present only when {@code completeness} is {@link
 * Completeness#TRUNCATED}, and is the offset of the opening brace of the incomplete final object —
 * the point a tailing reader resumes from.
 *
 * <p>{@code peakRecordBufferBytes} and {@code readBufferBytes} together are the parser's peak heap
 * footprint. They exist so a test can assert the streaming requirement directly rather than
 * inferring it (contract §7).
 *
