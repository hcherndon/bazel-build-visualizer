 ...
 *
 * <p>This is the other half of raw-first (ADR-004). Storing
 * {@code raw_segment}/{@code raw_offset}/{@code raw_length} is only worth
 * anything if the bytes can be got back, and this is what the "inspect raw
 * protobuf" pane calls, what a future reindex would read from, and what the
 * round-trip test uses to prove that every stored offset points at the payload
 * it claims to.
 *
 * <p>Every read goes through {@link JournalReader}, so the frame's CRC is
 * verified before its payload is returned. A row whose offset points at damaged
 * or non-frame bytes therefore fails loudly here rather than yielding plausible
 * garbage to a decoder.
 *
 * <p>Opens and closes a channel per call. That is the right trade for
 * inspecting one selected event; a bulk reindex should scan segments in order
 * with {@link JournalReader} directly instead of calling this per row.
 *
...
public final class JournalPayloadReader {
     ...
     *
     * @throws IOException if the segment is missing, the offset is not a frame
     *     boundary, the frame fails its checksum, or the frame's payload length
     *     disagrees with the stored one. All four mean the row and the journal
     *     disagree, which must be reported, never papered over
     */
    public byte[] read(RawLocation location) throws IOException {
        ...
        if (frameOffset < JournalFormat.SEGMENT_HEADER_BYTES) {
            throw new IOException("raw offset " + frameOffset + " lies inside the header of segment "
                    + location.segment() + "; frames start at " + JournalFormat.SEGMENT_HEADER_BYTES);
        }
        try (JournalReader reader = JournalReader.open(segment, frameOffset, config)) {
            JournalFrame frame = reader.next();
            if (frame == null) {
                throw new IOException("no journal frame at segment " + location.segment()
                        + " offset " + frameOffset + ": "
                        + reader.result().status() + " (" + reader.result().detail() + ")");
            }
            if (frame.frameOffset() != frameOffset) {
                throw new IOException("journal frame at segment " + location.segment()
                        + " starts at " + frame.frameOffset() + ", not the recorded " + frameOffset);
            }
            byte[] payload = frame.requirePayload();
            if (payload.length != location.length()) {
                throw new IOException("journal frame at segment " + location.segment() + " offset "
                        + frameOffset + " holds " + payload.length + " payload bytes, but the event"
                        + " row records " + location.length());
            }
            return payload;
        }
    }
    ...
    public JournalFrame readFrame(RawLocation location) throws IOException {
        ...
        try (JournalReader reader = JournalReader.open(segment, location.offset(), config)) {
            ...
            if (frame == null) {
                throw new IOException("no journal frame at segment " + location.segment()
                        + " offset " + location.offset() + ": " + reader.result().detail());
            }
            return frame;
        }
    }
