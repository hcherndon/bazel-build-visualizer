 ...
 *
 * @param status the outcome; {@link DecodeStatus#hasEvent()} says whether
 *     {@code event} is present
 * @param event the decoded event, or null when the record could not be decoded
 * @param message the parser's complaint, or null when there was none. Present
 *     for {@link DecodeStatus#UNKNOWN_FIELDS} too, where it names the field
 *     this build does not know — which is the whole point of surfacing it
 */
    ...
    public JsonDecodeResult {
        ...
        if (status.hasEvent() == (event == null)) {
            throw new IllegalArgumentException(
                    "decode status " + status + " is inconsistent with "
