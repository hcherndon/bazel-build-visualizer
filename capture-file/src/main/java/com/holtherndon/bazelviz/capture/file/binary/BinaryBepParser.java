...
/**
 * Streaming parser for Bazel's {@code --build_event_binary_file} format: a varint length followed
 * by that many bytes of a serialized {@code BuildEvent}, repeated to end of file. This is
 * protobuf's {@code writeDelimitedTo} framing.
 *
 * <h2>Bounded memory</h2>
 *
 * The parser reads through one reusable buffer whose capacity is <em>never</em> derived from the
 * length of the input (a Phase 1 exit criterion). It starts at {@code bufferBytes} and grows only
 * when a single frame does not fit, up to {@code maxPayloadBytes + 5}. Peak usage is therefore a
 * function of the largest event in the stream, not of the stream — a 40 GiB BEP file whose largest
 * event is 2 MiB is parsed in a 2 MiB buffer. {@link BinaryBepParseResult#peakBufferBytes()}
 * reports the high-water mark so tests can assert this rather than assume it.
 *
 * <h2>Truncation versus corruption</h2>
 *
 * A stream that ends inside a length prefix or inside a payload is {@link
 * BinaryBepParseOutcome#TRUNCATED}: normal for a build still writing, or one that was killed. Every
 * complete frame before the cut is still delivered, and the result carries the exact offset of the
 * partial frame so a later pass resumes there.
 *
 * <p>A length prefix that does not terminate within five bytes, or that declares more than {@code
 * maxPayloadBytes}, is {@link BinaryBepParseOutcome#CORRUPT}. The declared length is rejected
 * <em>before</em> any allocation: a damaged length field claiming 3 GiB must not be honoured with a
 * 3 GiB array (plan 21.3). The parser then stops rather than hunting for the next plausible
 * boundary, because scanning binary data for guessed protobuf boundaries is exactly what plan 21.3
 * forbids.
 *
 * <h2>Resuming and growing files</h2>
 *
 * {@link #parseFile(Path, long, BinaryBepEventSink)} starts at any byte offset, which must be a
 * frame boundary — in practice the {@code resumeOffset} of an earlier result or a checkpoint.
 * Parsing a file, appending to it, and parsing again from the returned resume offset delivers
 * exactly the frames a single pass over the final file would, with no rescanning. {@link
 * BinaryBepTailReader} wraps that loop for growing files (plan 9.5).
 *
 * <p>Instances are immutable configuration and can be shared; each {@code parse} call allocates its
 * own buffer and holds no state between calls.
 */
public final class BinaryBepParser {
  ...
  /**
   * Initial read buffer. Large enough that a typical BEP file — mostly progress and
   * action-completed events of a few hundred bytes — is read with one syscall per few hundred
   * events and never grows the buffer.
   */
  ...
  /**
   * @param maxPayloadBytes largest payload a frame may declare; a larger declared length is
   *     corruption, not a big event. Shares its default with the journal's frame ceiling so a
   *     parsed event can always be journaled
  ...
  public BinaryBepParser(int maxPayloadBytes, int bufferBytes) {
    ...
    if (maxPayloadBytes > Integer.MAX_VALUE - MAX_LENGTH_PREFIX_BYTES) {
      ...
      throw new IllegalArgumentException(
          "maxPayloadBytes must be <= "
              + (Integer.MAX_VALUE - MAX_LENGTH_PREFIX_BYTES)
              + ", got "
              + maxPayloadBytes);
    }
   ...
   *
   * @param startOffset a frame boundary: 0, or a {@code resumeOffset} from an earlier result.
   *     Starting anywhere else produces garbage lengths, so callers must not invent offsets
   */
   ...
   *
   * <p>{@code startOffset} is not a seek instruction — the stream is read from wherever it
   * currently sits. It only tells the parser what absolute offset the next byte has, so reported
   * offsets stay absolute across resumes.
   */
  ...
  public BinaryBepParseResult parse(
      ReadableByteChannel channel, long startOffset, BinaryBepEventSink sink) throws IOException {
    return parse(channel, startOffset, sink, NEVER_CANCELLED);
  }

  /**
   * Parses from {@code channel} until it reports end of input, the input stops making frame-shaped
   * sense, or cancellation fires.
   *
   * <p>The channel must be blocking. A non-blocking channel can return zero bytes without being at
   * end of input, which this parser would read as "no progress" and spin on; nothing in the
   * application feeds it one.
   *
   * @param startOffset absolute offset of the channel's current position
   * @param cancelRequested polled between frames; when it returns true the parse stops with {@link
   *     BinaryBepParseOutcome#CANCELLED} and a resume offset on the frame boundary just reached, so
   *     the import can continue later instead of starting over
   */
  public BinaryBepParseResult parse(
    ...
    while (true) {
      if (cancelRequested.getAsBoolean()) {
        return result(
            BinaryBepParseOutcome.CANCELLED,
            events,
            startOffset,
            frameOffset,
            0,
      ...
      if (prefixBytes == VARINT_NEED_MORE) {
        ...
        if (tail == 0) {
          return result(
              BinaryBepParseOutcome.COMPLETE,
              events,
              startOffset,
              frameOffset,
              0,
              OptionalLong.empty(),
              window,
              "input ended on a frame boundary at offset "
                  + frameOffset
                  + " after "
                  + events
                  + " events");
        ...
        return result(
            BinaryBepParseOutcome.TRUNCATED,
            events,
            startOffset,
            frameOffset,
            tail,
            OptionalLong.empty(),
            window,
            "input ends inside a length prefix at offset "
                + frameOffset
                + "; "
                + tail
                + " byte(s) of the prefix are present");
      }
      if (prefixBytes == VARINT_MALFORMED) {
        return result(
            BinaryBepParseOutcome.CORRUPT,
            events,
            startOffset,
            frameOffset,
            window.available(),
      ...
      if (window.available() < frameBytes) {
        ...
        return result(
            ...
            "frame at offset "
                ...
                + Math.max(0, tail - prefixBytes)
                + " are present before end of input");
      }

      ByteBuffer payload =
          ByteBuffer.wrap(window.data, window.start + prefixBytes, payloadLength)
              .slice()
              .asReadOnlyBuffer();
      sink.onEvent(
          new BinaryBepFrame(frameOffset, frameOffset + prefixBytes, payloadLength, payload));
  ...
  private static BinaryBepParseResult result(
      ...
      Window window,
      String detail) {
    return new BinaryBepParseResult(
        outcome,
        events,
        startOffset,
        resumeOffset,
   ...
   *
   * @return the number of bytes it occupied, {@link #VARINT_NEED_MORE} if the buffer ends
   *     mid-prefix, or {@link #VARINT_MALFORMED} if five bytes all carry a continuation bit. Five
   *     bytes is the ceiling for a uint32; a sixth would mean the writer emitted something that is
   *     not a length
   */
  ...
  /**
   * The single read buffer: bytes {@code [start, end)} of {@code data} are filled and unconsumed.
   * Compacts in place rather than reallocating, and grows only when one frame needs more room than
   * it has.
   */
  private static final class Window {
    ...
    void ensureCapacity(int needed) {
      ...
      if (grown > hardCapacity) {
        ...
        throw new IllegalStateException(
            "refusing to grow read buffer to "
                + grown
                + " bytes, above the "
                + hardCapacity
                + "-byte ceiling");
