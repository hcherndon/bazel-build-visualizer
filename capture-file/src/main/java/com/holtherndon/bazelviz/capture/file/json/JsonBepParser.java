 ...
 *
 * <p>The file is never read into memory (Phase 1 exit criterion). Bytes flow
 * through a fixed read buffer; a record is accumulated only for as long as it
 * takes to hand it to the listener, and the accumulator itself is capped by
 * {@link JsonBepParserOptions#maxRecordBytes()}. Peak footprint is therefore
 * the read buffer plus at most one record, whatever the file's size, and both
 * numbers come back in {@link JsonBepParseResult} so a test can assert it.
 *
 * <p>Record boundaries come from {@link JsonRecordScanner}, which tracks JSON
 * structural depth. That is deliberate: the one-object-per-line layout Bazel
 * usually emits is not guaranteed, and pretty-printed output from other
 * versions has no separator at all. See that class for why brace counting
 * alone is not enough.
 *
 * <p>What the parser will not do:
 * <ul>
 *   <li>Skip anything silently. An oversized record is reported with its exact
 *       offset and true length before parsing resumes at the next boundary;
 *       nothing is dropped without a diagnostic (plan 21.3).</li>
 *   <li>Allocate whatever the file asks for. A record over the limit stops
 *       being buffered at the limit, so a corrupt or hostile file cannot make
 *       the parser allocate its way out of memory.</li>
 *   <li>Confuse truncation with corruption. A final object that simply ends
 *       early is {@link Completeness#TRUNCATED} — the normal shape of a
 *       cancelled or still-running build — reported with the offset a tailing
 *       reader resumes from, and every record before it is delivered.</li>
 *   <li>Lose what it cannot understand. Raw bytes are preserved verbatim
 *       regardless of decode outcome (ADR-004), and a record carrying fields
 *       these protos do not know is delivered and flagged, not discarded
 *       (plan 21.5).</li>
 * </ul>
 *
 * <p>Instances are stateless and reusable; each {@code parse} call runs
 * independently. Parsing must not run on the Swing EDT.
 */
public final class JsonBepParser {
    ...
    /**
     * Parses {@code source}, streaming records to {@code listener}. The stream
     * is read sequentially and is not closed by this method.
     */
    public JsonBepParseResult parse(InputStream source, JsonBepListener listener)
            throws IOException {
        Objects.requireNonNull(source, "source");
    ...
    private final class Run {
        ...
        /**
         * Reads the first three bytes separately so a UTF-8 byte order mark can
         * be recognized and reported before it is mistaken for junk between
         * records. Everything that is not a BOM is pushed back into the normal
         * read buffer, so offsets stay absolute from byte zero either way.
         */
        private void consumeByteOrderMark() throws IOException {
            ...
            if (head.length == 3
                    ...
                    && head[2] == (byte) 0xBF) {
                start = 3;
                emit(JsonParseDiagnostic.at(Severity.INFO, Code.BYTE_ORDER_MARK_SKIPPED, 0,
        ...
        private boolean startRecord() throws IOException {
            ...
            if (scanner.sawSeparatorComma() && !commaSeparatorReported) {
                ...
                emit(JsonParseDiagnostic.at(Severity.INFO, Code.COMMA_SEPARATED_RECORDS, offset,
            ...
            if (readBuffer[index] != '{') {
                ...
                emit(JsonParseDiagnostic.at(Severity.ERROR, Code.MALFORMED_TOP_LEVEL, offset,
        ...
        /**
         * Accumulates {@code readBuffer[from, to)} into the open record, or
         * declines to once the record has outgrown the configured limit. The
         * true length keeps being counted either way, because the user needs
         * the real number to choose a bigger limit.
         */
        private void append(int from, int to) {
            ...
            if (!overflowed) {
                if (grown > options.maxRecordBytes()) {
                    ...
                    accumulator = new byte[Math.min(
                            INITIAL_RECORD_BUFFER_BYTES, options.maxRecordBytes())];
                    accumulated = 0;
                } else {
        ...
        private void completeRecord() throws IOException {
            ...
            if (overflowed) {
                oversizedRecords++;
                emit(JsonParseDiagnostic.forRecord(Severity.ERROR, Code.RECORD_TOO_LARGE,
                        ...
                        ("JSON record %d at byte offset %d is %d bytes, over the configured "
                                ...
                                + "it.")
                                .formatted(ordinal, recordStartOffset, recordLength,
            ...
            if (decoder != null) {
                ...
                switch (status) {
                    case FAILED -> {
                        failedDecodes++;
                        emit(JsonParseDiagnostic.forRecord(Severity.ERROR, Code.DECODE_FAILED,
                                recordStartOffset, raw.length,
                    ...
                    case UNKNOWN_FIELDS -> {
                        unknownFieldRecords++;
                        emit(JsonParseDiagnostic.forRecord(Severity.WARNING, Code.UNKNOWN_FIELDS,
                                recordStartOffset, raw.length,
                    ...
                    default -> { /* OK and NOT_ATTEMPTED need no diagnostic. */ }
                }
            }

            listener.onRecord(new JsonBepRecord(
                    ordinal, recordStartOffset, raw.length, raw, status, event, detail));
            deliveredRecords++;
        }
        ...
        private JsonBepParseResult finish() throws IOException {
            ...
            if (scanner.isInRecord()) {
                ...
                emit(JsonParseDiagnostic.forRecord(Severity.WARNING, Code.TRUNCATED_TAIL,
                        recordStartOffset, recordLength,
                        ("the source ends inside the JSON object that begins at byte offset %d; "
                                ...
                                + "it were read.")
                                .formatted(recordStartOffset, recordLength,
                                        scanner.isInString()
                                                ? "cut inside a string literal"
                                                : "still " + scanner.depth() + " level(s) of "
                                                        + "nesting open",
                                        deliveredRecords)));
                ...
                if (bytesRead == 0) {
                    emit(JsonParseDiagnostic.at(Severity.INFO, Code.EMPTY_INPUT, 0,
