...
/**
 * What one import run did, in enough detail to write a user-facing report
 * without re-opening the session.
 *
 * <p>The counts are separate rather than summed because they answer different
 * questions: {@code recordsJournaled} is how much of the source reached the
 * journal, {@code eventsNormalized} is how much of the journal reached the
 * database, and {@code ingest.eventsInserted} is how many rows the database
 * actually gained. On a first import all three agree; on a resume they do not,
 * and the difference is exactly the work the resume repeated idempotently.
 *
 ...
 * @param recordsJournaled records read from the source and appended to the
 *     journal by this run
 * @param eventsNormalized journal frames turned into rows by this run,
 *     including any replayed from a checkpoint
 ...
 * @param damageOffset byte offset of the truncation or corruption in the
 *     source, empty when there was none. Never zero as a stand-in for absent
 * @param resumedFromCheckpoint true when this run continued an earlier one
 * @param peakBufferBytes the largest number of bytes the pipeline's buffers
 *     held at once during this run — read buffer, journal staging buffer and
 *     payload scratch together. Bounded by configuration, never by the size of
 *     the source; this is the number the bounded-memory test asserts against
