 ...
 *
 * <p>Atomic replacement matters for the same reason it does for the manifest:
 * the next process to open this session reads this file to decide where to
 * carry on reading, and a half-written resume point is worse than none — it
 * would resume from a plausible-looking wrong offset instead of failing.
 */
final class SourceCheckpointStore {
    ...
    SourceCheckpointStore(Path checkpointDirectory) {
        this.file = Objects.requireNonNull(checkpointDirectory, "checkpointDirectory")
                .resolve(FILE_NAME);
    }
