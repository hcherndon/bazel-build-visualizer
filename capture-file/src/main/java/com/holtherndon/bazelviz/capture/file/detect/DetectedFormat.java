...
/**
 * What a candidate capture input turned out to be (plan 5.2: detect the format
 * by content, not only by extension).
 *
 * <p>{@link #UNKNOWN} is a real verdict with a stated reason, not a fallback
 * bucket. Guessing between two plausible formats would send a file to a parser
 * that cannot read it and produce a confusing failure deep inside an import
