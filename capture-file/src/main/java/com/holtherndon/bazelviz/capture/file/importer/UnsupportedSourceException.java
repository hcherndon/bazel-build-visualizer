 ...
 *
 * <p>Thrown before any session directory is created, so a refused import leaves nothing behind to
 * clean up.
 *
 * <p>The detector's reason is carried through verbatim into the message. "Unsupported file" tells a
 * user nothing; "the first frame declares 4 bytes that do not parse as a BuildEvent" tells them
 * their file is truncated at the head or is not BEP at all. Guessing a format instead would move
 * the failure from the door to the middle of an import, where it is far harder to explain (plan
 * 5.2: ambiguity is reported, not guessed).
 */
