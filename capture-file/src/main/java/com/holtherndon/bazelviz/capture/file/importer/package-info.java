 ...
 * the source-side half of a resume point, and
 * {@link com.holtherndon.bazelviz.capture.file.importer.JournalPayloadReader}
 * for getting the raw bytes back out again.
 *
 * <p>How a payload becomes rows lives one package over, in
 * {@link com.holtherndon.bazelviz.capture.normalize.EventNormalizer}. It moved
 * there in Phase 2, when live BES capture became a second producer of the same
 * rows: keeping one definition is what makes a captured session and an imported
 * session agree, and leaving it in a package named {@code importer} would have
 * invited a second copy for the capture path.
 *
