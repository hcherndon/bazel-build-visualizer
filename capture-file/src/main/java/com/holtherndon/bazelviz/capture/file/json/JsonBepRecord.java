...
/**
 * One top-level JSON object read out of a BEP JSON file, delivered with the
 * exact bytes it occupied and where it occupied them.
 *
 * <p>{@code rawBytes} is the verbatim slice of the source file, from the
 * opening brace through the closing brace inclusive. It is the record's
 * primary representation, not a debugging aid: it is what gets journaled, and
 * re-serializing {@link #event()} instead would silently discard anything this
 * build's protos do not understand (ADR-004).
 *
 * <p>{@code byteOffset} and {@code byteLength} are exact, so
 * {@code file[byteOffset, byteOffset + byteLength)} is byte-for-byte
 * {@code rawBytes} and re-parsing that slice alone yields the same record.
 *
 * <p>For efficiency the array is handed over rather than copied on access.
 * Callers must not mutate it, and must copy it if they intend to retain it
 * beyond the listener callback. Consequently {@code equals} and {@code
 * hashCode} compare the array by identity; compare {@link #rawBytes()} with
 * {@code java.util.Arrays#equals} when value semantics are wanted.
 *
 * @param ordinal 0-based index of this record among all structurally delimited
 *     records in the source, including any that were too large to deliver, so
 *     ordinals stay aligned with file order
...
public record JsonBepRecord(
    ...
    public JsonBepRecord {
        ...
        if (decodeStatus.hasEvent() == (event == null)) {
            throw new IllegalArgumentException(
                    "decode status " + decodeStatus + " is inconsistent with "
                            + (event == null ? "a missing" : "a present") + " event");
        }
    }
