 ...
 *
 * <p>{@code maxRecordBytes} is the configurable per-record limit plan 9.5 and
 * 21.3 require. It defaults to {@link JournalFormat#DEFAULT_MAX_PAYLOAD_BYTES}
 * on purpose: an accepted record is journaled as a single frame, so a record
 * the parser would accept but the journal would reject is a limit mismatch
 * waiting to happen. Exceeding the limit is reported with the record's offset
 * and true length, never skipped silently and never honoured by allocating
 * whatever the file happens to contain.
 *
 * <p>{@code readBufferBytes} bounds the parser's streaming window. Together
 * with the record buffer it is the parser's entire memory footprint; the file
 * itself is never read into memory (Phase 1 exit criterion).
 *
...
public record JsonBepParserOptions(int maxRecordBytes, int readBufferBytes, boolean decodeEvents) {
    ...
    private static final JsonBepParserOptions DEFAULTS = new JsonBepParserOptions(
            DEFAULT_MAX_RECORD_BYTES, DEFAULT_READ_BUFFER_BYTES, true);
