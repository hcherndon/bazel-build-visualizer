...
/**
 * Reads a binary BEP file that is still being written (plan 9.5, file-tail fallback), one poll at a
 * time.
 *
 * <p>Each {@link #poll} consumes every frame that is now complete and stops at the first partial
 * one, remembering its offset. The next poll starts from that offset — it never rescans bytes it
 * has already delivered, which is what makes tailing a multi-gigabyte file affordable. The state
 * that survives between polls is a single {@code long}, so it can be checkpointed and resumed
 * across an application restart just as easily as across a poll.
 *
 * <p>A partial tail is not an error while the writer is alive: {@link #poll} returning {@link
 * BinaryBepParseOutcome#TRUNCATED} only means "the last frame has not landed yet". Only {@link
 * #finish}, called after the writing process has exited and the file is final, turns a
 * still-partial tail into a truncated source. That two-step is why the plan says to finalize only
 * after process exit and a final read.
 *
 * <p>Polling schedule is the caller's business: this class does no waiting, no file watching and no
 * threading, so it can be driven from a background executor without ever touching the EDT.
 *
...
public final class BinaryBepTailReader {
  ...
  /**
   * Tails {@code file} from a previously recorded frame boundary — the {@code resumeOffset} of an
   * earlier result, or a checkpoint written before a restart.
   */
  public static BinaryBepTailReader resumingAt(
      Path file, BinaryBepParser parser, long startOffset) {
    if (startOffset < 0) {
   ...
   *
   * @throws SourceReplacedException if the file shrank below the consumed offset, which means it is
   *     a different file than the one being read
  ...
  /**
   * Final read, for use once the writer has exited. Consumes anything that arrived since the last
   * poll and freezes the reader.
   *
   * <p>The returned result's completeness is now authoritative: a partial frame at this point is a
   * genuinely truncated source, not a slow writer.
   */
  ...
  /**
   * Completeness of the source as currently known. Before {@link #finish} a partial tail is {@link
   * Completeness#UNKNOWN}, not {@code TRUNCATED} — the writer may still be mid-frame, and calling
   * that truncation would report a healthy live capture as damaged.
   */
