 ...
 *
 * <p>Every limit the parser enforces and every byte it declines to interpret produces one of these
 * (plan 21.3, project rule "no silent truncation"). The shape mirrors the {@code
 * import_diagnostics} row it will become — severity, code, message, byte offset — so the import
 * pipeline can persist it without inventing fields. {@code recordLength} is carried in addition
 * because an oversized record's true size is exactly what the user needs in order to choose a
 * larger limit, and it is not recoverable afterwards.
 *
...
public record JsonParseDiagnostic(
    Severity severity, Code code, String message, long byteOffset, OptionalLong recordLength) {
  ...
  public enum Code {
    ...
    /**
     * Objects were separated by commas rather than concatenated. Tolerated, reported once, because
     * it means the producer was writing an unwrapped JSON array and a future record may be an array
     * terminator.
     */
    COMMA_SEPARATED_RECORDS,

    /**
     * A record exceeded the configured maximum size. Its offset and true length are reported and
     * its bytes are not retained; parsing continues from the next record boundary.
     */
    RECORD_TOO_LARGE,
