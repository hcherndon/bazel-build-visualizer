...
/**
 * The record of the original capture file that the session keeps, written
 * before a single byte is decoded (Phase 1 exit criterion "the original source
 * is preserved").
 *
 * <p>The digest is the proof obligation: whichever {@link SourcePreservation}
 * mode was used, {@link #sha256} is the digest of the exact byte stream the
 * importer went on to parse, so the session can afterwards demonstrate that
 * what it indexed is what it read.
 *
 * @param preservation which mode produced this record
 * @param originalPath absolute path the file was read from
 * @param storedPath the copy inside the session, present only for
 *     {@link SourcePreservation#COPY_INTO_SESSION}
 * @param sha256 lower-case hex digest of the source bytes
 * @param byteSize size of the source in bytes
 * @param lastModifiedMillis last-modified time of the original when it was
 *     read, used to notice a reference source changing underneath
 */
