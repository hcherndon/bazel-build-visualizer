 ...
 *
 * <p>Bazel writes BEP JSON in protobuf's canonical JSON encoding, so decoding
 * goes through {@link JsonFormat} rather than any hand-rolled mapping.
 *
 * <h2>Why it parses twice</h2>
 *
 * <p>{@code JsonFormat.Parser.ignoringUnknownFields()} makes unknown fields
 * disappear silently, which is precisely the outcome plan 21.5 forbids: a
 * newer Bazel adds a field, this build drops it, and nothing tells the user.
 * So the decoder first parses strictly. Only if that fails does it retry
 * leniently, and success on the retry is what proves the strict failure was an
 * unknown field rather than malformed input — no error-message sniffing
 * involved. The record is then flagged {@link DecodeStatus#UNKNOWN_FIELDS}
 * with the strict parser's complaint attached, so the unrecognized content is
 * visible rather than lost. The second parse only happens for records that
 * actually carry unknown fields, so a file from a matching Bazel pays nothing.
 *
 * <p>Either way the caller keeps the raw bytes (ADR-004); this class never
 * becomes the only representation of a record.
 *
 * <h2>Type registry</h2>
 *
 * <p>{@code ActionExecuted.strategy_details} is a {@code google.protobuf.Any},
 * and protobuf-JSON cannot decode an {@code Any} without a registry that can
 * resolve its type URL. The registry below covers {@code build_event_stream}
 * and its transitive imports plus {@code tools.protos.SpawnExec}, which the
 * proto documents as the default strategy-details type. An {@code Any} of some
 * other type still fails to decode, and correctly reports
 * {@link DecodeStatus#FAILED} with the bytes preserved, rather than
 * pretending to have understood the record.
 *
...
public final class JsonBuildEventDecoder {
    ...
    /**
     * Decodes {@code raw[offset, offset + length)}, which must be exactly one
     * JSON object.
     *
     * <p>Never throws for bad content: a record this build cannot interpret is
     * a reportable fact about the file, not an exception the import should die
     * on (plan 21.3, "catch parse errors per frame where boundaries are known").
     */
    public JsonDecodeResult decode(byte[] raw, int offset, int length) {
        ...
        try {
            ...
            return new JsonDecodeResult(
                    DecodeStatus.UNKNOWN_FIELDS, lenient.build(), strictComplaint);
        } catch (IOException | RuntimeException e) {
