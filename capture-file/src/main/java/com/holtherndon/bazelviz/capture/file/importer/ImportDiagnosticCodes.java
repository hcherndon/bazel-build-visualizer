...
/**
 * {@code import_diagnostics.code} values the file importer adds to the ones in
 * {@link com.holtherndon.bazelviz.storage.events.DiagnosticCodes}.
 *
 * <p>Spelled out as constants rather than derived from enum names: these
 * strings are stored in session databases and are read back by later builds, so
 * they are part of the on-disk contract and must not change because an enum was
 * renamed.
 */
public final class ImportDiagnosticCodes {
    ...
    /**
     * The source contains a record whose framing contradicts itself, so no
     * reliable next boundary exists and reading stopped at the offset given.
     */
    ...
    /**
     * Repeated diagnostics of one code hit the configured cap; this row carries
     * the true total so nothing is lost by the cap, only repeated.
     */
