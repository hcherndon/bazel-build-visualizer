...
/**
 * The source-side half of a resume point: where in the <em>capture file</em>
 * reading had reached, alongside the journal-side
 * {@link com.holtherndon.bazelviz.format.journal.ImportCheckpoint} that says
 * where in the journal writing had reached.
 *
 * <h2>Why this exists as a second file</h2>
 *
 * <p>The frozen checkpoint contract records a journal position and frame
 * counts; it has no field for a source offset, and it must not grow one — its
 * format is shared with live BES capture, where there is no source file at all.
 * But a resumed <em>file</em> import needs to know where to carry on reading,
 * and the answer is not derivable from the journal: a JSON record's journaled
 * bytes are the object alone, while the source also holds the whitespace,
 * newlines and separators between objects. Recomputing the source offset from
 * journaled payload lengths would therefore be wrong for JSON and merely
 * fragile for binary.
 *
 * <h2>Why a stale copy of this file is safe</h2>
 *
 * <p>This sidecar is written <em>after</em> the journal has been forced and
 * after the journal checkpoint has been replaced, so it can lag the journal but
 * can never lead it. A crash in that window leaves {@code framesWritten} lower
 * than the number of frames actually in the journal, and the importer resumes
 * by reading from {@code sourceOffset} and discarding the first
 * {@code journalFrames - framesWritten} records it re-reads instead of
 * journaling them again. The journal therefore ends up byte-identical to an
 * uninterrupted run, which is what makes offsets reproducible across a resume.
 *
 * @param formatVersion version of this sidecar's own layout
 * @param sourceOffset absolute byte offset in the source where reading stopped;
 *     always a record boundary
 * @param framesWritten journal frames that had been written when this offset
 *     was recorded
 * @param format the detected format of the source, so a resume does not have to
 *     re-run detection and cannot silently change its mind about it
...
public record SourceCheckpoint(
     ...
     *
     * <p>Strict: a missing or mistyped field fails rather than being defaulted,
     * because a repaired resume point resumes from the wrong place, and doing
     * that quietly is how an import loses records without anyone noticing.
     */
    ...
    private static long number(JsonObject object, String key) throws ImportFormatException {
        JsonValue member = object.member(key)
                .orElseThrow(() -> new ImportFormatException("source checkpoint is missing \"" + key + "\""));
    ...
    private static <E extends Enum<E>> E enumValue(JsonObject object, String key, Class<E> type)
        ...
        try {
            ...
            throw new ImportFormatException("source checkpoint field \"" + key + "\" holds '" + raw
                    + "', which this build does not know; the session may have been written by a"
                    + " newer version", unknown);
        }
    }
}
