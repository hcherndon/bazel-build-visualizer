...
public final class GraphExplorerView extends JPanel {
    ...
    private final javax.swing.JTextField search = new javax.swing.JTextField(18);
    private final javax.swing.JPopupMenu findPopup = new javax.swing.JPopupMenu();
    private final javax.swing.JToggleButton browse =
            new javax.swing.JToggleButton("Browse nodes…");
    ...
    private final javax.swing.Timer findTimer =
            new javax.swing.Timer(150, event -> runFindSearch());
    ...
    public GraphExplorerView() {
        ...
        sourceChoice.setToolTipText(PlainText.tooltip(
                "Choose which imported dependency graph the canvas represents."));
        search.setToolTipText(PlainText.tooltip(
                "Find an action or target by target label, mnemonic, rule class, or output."));
        sourceHelp.setToolTipText(PlainText.tooltip(
                "Show what the selected graph contains and how trustworthy it is."));
        sourceHelp.getAccessibleContext().setAccessibleDescription(
                "Show what the selected graph contains and how trustworthy it is.");
        ...
        sourceHelp.addActionListener(event -> {
    ...
    public CompletionStage<Void> closeSessionAsync() {
        ...
        return ViewClose.runAsync("bbv-graph-close", () -> {
            if (executor != null) {
     ...
     *
     * <p>A short debounce combines a burst of keystrokes into one bounded
     * search on the worker — never on the event thread — and the answer comes
     * back as a dropdown of up to {@link #FIND_LIMIT} matches under the field.
     * Each entry names its node
    ...
    public void showCriticalPath(List<Integer> nodes) {
        ...
        if (queries == null || layouts == null) {
            String reason = graphLoadFailure == null
    ...
    private void drawCriticalPath(List<Integer> nodes) {
        ...
        if (!selectSource(GraphKind.DECLARED_ACTIONS)) {
            String reason = "The successful declared action graph is not available, so the"
    ...
    public void installEntityActions(
    ...
    private static JLabel labelFor(String text, javax.swing.JComponent target, String name) {
