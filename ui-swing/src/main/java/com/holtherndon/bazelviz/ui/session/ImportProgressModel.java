...
/**
 * Turns the importer's raw progress samples into something a progress view can
 * render, without inventing any of it.
 *
 * <p>No Swing types, no EDT assumptions: this is the piece the tests drive.
 * The view is a thin renderer of {@link Snapshot}.
 *
 * <h2>What is deliberately unknown</h2>
 *
 * <ul>
 *   <li><b>Percentage.</b> {@link ImportProgress#totalBytes()} is empty while
 *       the importer is detecting the format and while it is hashing and
 *       copying the source, and stays empty for any source whose size could not
 *       be determined. {@link Snapshot#determinate()} is false in exactly those
 *       cases and the bar must run indeterminate. Substituting "bytes read" for
 *       "bytes total" would produce a bar that sits at 100% for the whole
 *       import — a fabricated percentage, which plan 11.4 forbids.</li>
 *   <li><b>Rate.</b> Empty until two samples are far enough apart in time to
 *       divide by. A rate computed across a sub-millisecond interval is noise,
 *       and a rate shown as 0 while the first block is being read is a lie. The
 *       last rate actually measured is carried forward between samples rather
 *       than being recomputed from a zero-length interval.</li>
 * </ul>
 *
 * <h2>Threading</h2>
 *
 * <p>{@link #update} is called on the importing thread, one sample at a time,
 * and returns an immutable {@link Snapshot} that is safe to hand to the EDT.
 * The mutable state is confined to the importing thread.
 */
public final class ImportProgressModel {
     ...
     * @param fractionComplete completed fraction, empty whenever
     *     {@code totalBytes} is
     * @param recordsPerSecond most recently measured record rate, empty before
     *     one could be measured
     * @param bytesPerSecond most recently measured byte rate, empty before one
     *     could be measured
        ...
        public Snapshot {
            ...
            if (determinate != fractionComplete.isPresent()) {
                throw new IllegalArgumentException(
                        "determinate must mean exactly 'a fraction is known'");
            }
        }
    }
