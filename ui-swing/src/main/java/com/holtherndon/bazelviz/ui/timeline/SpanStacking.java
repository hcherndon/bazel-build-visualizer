 ...
 * <h2>The rule</h2>
 *
 * <p>Within a lane, spans stack top-to-bottom by start time: each span takes
 * the highest sub-row whose previous occupant has already ended. Two spans
 * that do not overlap share a sub-row; a burst of concurrent work spreads
 * downward, so the depth of the stack <em>is</em> the concurrency, visibly.
 *
 ...
 * few hundred microseconds) and read by every paint until the next window.
 */
public final class SpanStacking {

    /**
     * Sub-rows one lane may split into before overlapping spans pile into the
     * last one. Documented in docs/limits.md; LimitsDocTest checks the value.
     */
     ...
     *
     * <p>Spans are processed in start order regardless of the window's own
     * order, so the sub-row a span lands in is a fact about the span and its
     * lane-mates, never about fetch order — the same stability rule the lane
     * key itself enforces horizontally.
     */
    ...
    /**
     * How many sub-rows {@code laneKey} actually uses — 1 for a lane with no
     * overlap, at most {@link #MAX_SUB_ROWS}, and 1 for a lane this stacking
     * never saw (a lane with no spans in the window still gets its row).
     */
    ...
    /**
     * Spans drawn overlapping in the last sub-row because their lane's
     * concurrency exceeded {@link #MAX_SUB_ROWS}. Zero means every span has
     * its own patch of pixels; non-zero is stated on the status line with
     * this exact number, never smoothed over.
     */
