 ...
 *
 * <p>Bazel repaints a progress block several times a second, each repaint a
 * carriage return and a rewritten line. Setting the document's whole text on
 * every one of those is quadratic in the log's length — measured at 15–50 ms
 * per repaint once the retention cap is reached, on the event dispatch thread —
 * and it also throws away the user's selection and scroll position each time.
 *
 * <p>So the document is appended to. {@link ConsoleModel} resolves the carriage
 * returns into committed lines and one partial line; committed lines are
 * appended once and never touched again, and only the partial line is rewritten
 * as it changes.
 *
 * <h2>Follow means follow</h2>
 *
 * <p>The caret is set to never update on its own, so appending text does not
 * drag the viewport. Scrolling to the bottom happens only when "Follow output"
 * is ticked. A user who has scrolled up to read an error is reading it, and the
 * single most annoying thing a live console can do is yank them away from it on
 * the next progress repaint.
 */
public final class ConsoleView extends JPanel {
     ...
     * Coalesces repaints.
     *
     * <p>A build can deliver dozens of chunks between two frames. At most one
     * update is queued at a time, and it renders whatever has accumulated by the
     * time it runs.
     */
