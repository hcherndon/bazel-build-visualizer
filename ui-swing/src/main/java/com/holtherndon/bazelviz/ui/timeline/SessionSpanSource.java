 ...
 *
 * <p>Plan 14.1 lists seven kinds of timeline entity. This streams the two that carry a wall-clock
 * window and a duration worth drawing: logical actions from the build event stream, and execution
 * attempts from the execution log.
 *
 * <p>Tests are not streamed as a third kind. A test's window is its attempts', which are already
 * here, and drawing both would count the same work twice in the density — the bins are counts of
 * spans, and a viewer cannot tell a doubled bin from a busy one. Bazel phases are drawn as a
 * separate band because there are five of them and they are not spans of work.
 *
 * <h2>Replayable, and the same twice</h2>
 *
 * <p>{@link SpanSource} requires the feed to be identical on every call; the index streams it once
 * but the contract is not conditional on that. Each call runs the same query against the same
 * connection, so it is — provided nothing writes in between, which for a finished session is
 * guaranteed and for a live one is the reason the index is rebuilt rather than updated in place.
 *
 * <h2>Spans without a start</h2>
 *
 * <p>Actions on Bazel 6.5.0 and 7.6.1 have no timestamps at all, and attempts on 6.5.0 have a
 * duration and no position. Those rows are skipped, and {@link #skippedForNoTime()} counts them: a
 * timeline drawn from a session where most actions have no time is nearly empty, and the count is
 * what turns that from a puzzle into a sentence.
 */
public final class SessionSpanSource implements SpanSource {
   ...
   *
   * <p>The mnemonic id doubles as the category index. It is already a small dense integer because
   * Phase 3 interns mnemonics, which is exactly what the index wants and the reason no separate
   * mapping exists.
   */
   ...
   *
   * <p>Joined to the action only for its mnemonic; an attempt attached to no action still has one
   * of its own.
   */
  ...
  /**
   * @param includeAttempts whether execution-log attempts are drawn alongside actions. Both is the
   *     truthful default — they are different measurements of overlapping work — and a viewer
   *     comparing density between two sessions, one with an execution log and one without, needs to
  ...
  public long spanCount() {
    if (spanCount < 0) {
      try {
        spanCount =
            count("SELECT count(*) FROM (" + ACTIONS + ")")
  ...
  private void streamActions(SpanConsumer consumer) throws SQLException {
    try (PreparedStatement statement = connection.prepareStatement(ACTIONS);
      ...
      while (rows.next()) {
        ...
        consumer.accept(rows.getLong(1), rows.getLong(2), rows.getInt(3), flags, BYTES_UNKNOWN);
      }
    }
  }
   ...
   *
   * <p>Empty is the honest answer for a Bazel 6.5.0 or 7.6.1 capture with no execution log: those
   * versions report no action timestamps at all (finding A4), so there is no time axis to draw and
   * the view says that rather than drawing an empty one.
   */
