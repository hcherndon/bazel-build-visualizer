 ...
 *
 * <p>Bazel's console output is not a sequence of lines. It repaints a progress
 * block using carriage returns and ANSI cursor movement, so a naive appender
 * produces a window full of half-overwritten progress and a naive line reader
 * blocks forever on a line that never ends. This model handles the two cases
 * that actually occur: a carriage return replaces the line being built, and
 * ANSI escape sequences are removed for display.
 *
 * <p>Removed <em>for display only</em>. {@code raw/stdout.log} and
 * {@code raw/stderr.log} hold the bytes exactly as they arrived (ADR-004), so
 * nothing here loses information — it is a rendering, and the original is on
 * disk.
 *
 * <h2>Bounded, and honest about it</h2>
 *
 * <p>A large build's console output is unbounded, and this is a UI model, so it
 * keeps the last {@link #maxLines} lines and counts what it dropped. Plan
 * section 3 requires every imposed display limit to be visible, which is what
 * {@link #droppedLines()} is for: the view shows it rather than silently
 * presenting a truncated log as the whole thing.
 *
 * <p>Not thread-safe. Owned by the EDT; the capture threads hand bytes to a
 * controller that hops here.
 */
public final class ConsoleModel {
     ...
     *
     * <p>Three states rather than a boolean, because the {@code [} that follows
     * {@code ESC} in a CSI sequence is itself inside the final-byte range
     * {@code @}..{@code ~}. A single "in an escape" flag ends the sequence on
     * that very character and leaves the parameters — {@code 32m} — in the
     ...
     *
     * <p>Decoded as UTF-8 per chunk. A multi-byte character split across two
     * chunks renders as a replacement character rather than being buffered:
     * holding bytes back to wait for a continuation would stall the display of
     * a line that is already complete, and the raw log has the true bytes.
     */
