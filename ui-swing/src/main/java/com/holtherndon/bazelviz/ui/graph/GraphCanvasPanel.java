 ...
 *
 * <p>The controller half of the split plan 17.7 requires: this class holds the service that reads
 * the session, and hands the canvas a finished {@link GraphModel}. The canvas can neither query nor
 * lay out, which is checked by reflection in {@code GraphPaintIsolationTest} rather than by review.
 *
 * <h2>The sentence under the drawing is not decoration</h2>
 *
 * <p>Plan 13.6 ends "never claim the omitted nodes do not exist", and a drawing cannot say that on
 * its own — a view of nine actions from a build of ninety thousand looks exactly like a build with
 * nine actions. So every rendering carries a sentence naming the totals, and it is shown whether or
 * not anything was left out.
 */
public final class GraphCanvasPanel extends JPanel {
   ...
   *
   * <p>One, not zero: a limit of zero draws nothing and would read as "the graph is empty", which
   * is a claim about the build rather than about the setting. Any positive budget is a real, if
   * strange, request.
   */
   ...
   *
   * <p>The largest planned graph is the Tier 3 fixture's five million nodes (plan 19.4), so no
   * honest request needs more — and the spinner needs some ceiling, because Integer.MAX_VALUE in a
   * spinner reads as "no limit", which is exactly the claim plan 13.6 forbids.
   */
   ...
   *
   * <p>The weight changes visual encoding only: positions belong to the layouts, so switching
   * weights restyles the current drawing without re-extracting or re-laying-out — and without
   * moving the camera.
   */
  private final JComboBox<GraphWeight> weightChoice = new JComboBox<>(GraphWeight.values());
   ...
   *
   * <p>Plan 13.6's "raise limit" existed only as the over-limit bar's doubling button, so the one
   * way to choose a budget was to be refused at the old one first. The spinner makes the limit a
   * setting: type a number, the next drawing honours it, and the over-limit machinery still says
   * exactly what did not fit.
   */
  private final JSpinner nodeLimitControl =
      new JSpinner(
          new SpinnerNumberModel(
              GraphExtract.DEFAULT_NODE_LIMIT, MIN_NODE_LIMIT, MAX_NODE_LIMIT, 1_000));

  private final JTextArea description = WrappingLabel.create(" ");
  private final JTextArea omission = WrappingLabel.create(" ");
  private final JTextArea selected = WrappingLabel.create(" ");
  private final JTextArea scopeExplanation = explanationArea();
  private final JTextArea appearanceExplanation = explanationArea();
   ...
   *
   * <p>"Offer raise limit, export, and refine filter", and never claim the omitted nodes do not
   * exist. It is hidden when there is nothing to say — a standing notice trains a user to stop
   * reading the place the real warning appears.
   */
  private final JPanel overLimit = new JPanel();

  private final JTextArea overLimitText = WrappingLabel.create(" ");
  ...
  /**
   * True when the cluster view on screen was chosen by the application, not by the user.
   *
   * <p>It changes what the bar says — "grouped instead" is a different statement from "grouped, as
   * you asked" — and it stops a second automatic switch when the grouping itself does not fit.
   */
  ...
  /**
   * Bumped whenever what is on screen changes, so a weight computation or a whole-graph count that
   * finishes late restyles nothing. Read and written on the EDT only.
   */
  ...
  public GraphCanvasPanel() {
    ...
    edgeDisplay.setRenderer(new Renderer<>(value -> ((GraphEdgeDisplay) value).displayName()));
    ...
    weightChoice.addActionListener(
        event -> {
          updateExplanations();
          weightChanged();
        });
    ...
    resetPositions.setToolTipText(
        PlainText.tooltip("Put every dragged node back where the layout placed it."));
    ...
    showClusters.addActionListener(event -> mode.setSelectedItem(GraphExtract.Mode.CLUSTERS));
   ...
   * The canvas itself.
   *
   * <p>Package-private: nothing outside {@code ui.graph} should reach past the panel to the
   * component it manages, and the audit that counts callers is what noticed it was public for no
   * reason.
   */
  GraphCanvas canvas() {
    return canvas;
   ...
   *
   * <p>Node indexes do not translate between graphs — index 7 is an action in one numbering and a
   * label in the other — so the root is dropped rather than reinterpreted, and the prompt asks for
   * a new one.
   */
  public void setShownGraph(GraphKind graph) {
    ...
    if (rooted || chosen == GraphExtract.Mode.PATH || chosen == GraphExtract.Mode.CRITICAL_PATH) {
      if (!rooted) {
  ...
  private String pickPrompt() {
    return "Pick "
        + (shownGraph == GraphKind.CONFIGURED_TARGETS ? "a target" : "an action")
   ...
   *
   * <p>Plan 13.5's "path between two actions", which the trees can state in words but only the
   * canvas can show the shape of.
   */
  public boolean showPath(List<Integer> nodes, GraphExtract.Mode pathMode) {
    ...
    try {
      ...
      GraphExtract.requirePathWithinLimits(nodes.size(), pathMode, nodeLimit, edgeLimit);
   ...
   *
   * <p>Explicit because the point of the limit is that a user chooses to pay for the bigger
   * drawing, having been told what it will cost.
   */
  public void raiseLimits(int nodes, int edges) {
    ...
    try {
      if (mode.getSelectedItem() != GraphExtract.Mode.CLUSTERS) {
        nodeLimitControl.setValue(Math.max(MIN_NODE_LIMIT, Math.min(MAX_NODE_LIMIT, nodes)));
      }
    } finally {
   ...
   *
   * <p>The missing half of exploration: before this, clicking selected a node but the canvas kept
   * drawing the old root's neighbourhood, so there was no way to walk the graph by looking at it.
   * Reached by double-click and by the context menu's "Focus here".
   */
  void focusOnPosition(int position) {
    ...
    if (model.isCluster()) {
      ...
      setText(
          description,
          "Groups cannot be focused. Switch to a node view and pick one " + noun() + ".");
      return;
    }
   ...
   *
   * <p>The double-click does the same thing faster, but nothing on screen advertises a
   * double-click; a context menu is the affordance a user can find by trying.
   */
    ...
    canvas.addMouseListener(
        new MouseAdapter() {
          ...
          private void maybeShow(MouseEvent event) {
            ...
            OptionalInt hit = canvas.positionAt(event.getX(), event.getY());
            ...
            JMenuItem focus =
                new JMenuItem(
                    "Focus here — redraw around " + canvas.model().canvasLabelAt(position));
            PlainText.disableHtml(focus);
            focus.addActionListener(action -> focusOnPosition(position));
            menu.add(focus);
            JMenuItem resetPositions = new JMenuItem("Reset positions");
  ...
  private void appendEntityActions(JPopupMenu menu, int position) {
    EntityActions actions = entityActions;
    Optional<String> label = targetLabelAt(position);
    if (actions == null || label.isEmpty()) {
      return;
    }
    JPopupMenu shared =
        actions.popupFor(
            List.of(new EntityRef.TargetLabel(label.orElseThrow())),
    ...
    while (shared.getComponentCount() > 0) {
      Component item = shared.getComponent(0);
      shared.remove(0);
      menu.add(item);
    }
  }

  private Optional<String> targetLabelAt(int position) {
    GraphModel model = canvas.model();
  ...
  private void modeChanged() {
    ...
    boolean path = chosen == GraphExtract.Mode.PATH || chosen == GraphExtract.Mode.CRITICAL_PATH;
  ...
  private void edgeDisplayChanged() {
    ...
    canvas.setEdgeDisplay(
        chosenLayout == GraphLayout.Kind.HIERARCHY ? hierarchyEdgeDisplay : GraphEdgeDisplay.ALL);
    updateExplanations();
    updateOmission();
  }
  ...
  private void updateExplanations() {
    ...
    GraphEdgeDisplay wantedEdges = hierarchy ? hierarchyEdgeDisplay : GraphEdgeDisplay.ALL;
    ...
    if (chosenMode != GraphExtract.Mode.CLUSTERS) {
      text.append("  Node size and colour encode ").append(chosenWeight.subject()).append('.');
   ...
   *
   * <p>Restyle, never re-layout: the drawing's positions are weight-independent, so the camera and
   * the selection survive and the layout cache is untouched.
   */
  ...
  private void weightFailed(GraphWeight weight, long wanted, Throwable failure) {
    if (wanted == weightGeneration) {
      setText(
          selected, "The " + weight.subject() + " could not be computed: " + failure.getMessage());
    }
  }
  ...
  void refresh() {
    ...
    GraphLayoutService.Request request =
        switch (chosen) {
          ...
          case WHOLE -> GraphLayoutService.Request.whole(shownGraph, nodeLimit, edgeLimit);
          default ->
              GraphLayoutService.Request.around(
                  shownGraph, chosen, Math.max(0, rootNode), (Integer) depth.getValue());
        };
    request =
        request
  ...
  private void preparedRendering(
    ...
    setOverLimit(
        result.refused() || result.extract().hitLimit() || aggregatedAutomatically, result);
   ...
   *
   * <p>For a rooted view the narrower question is a shallower one, so this steps the depth down.
   * For the whole build the narrower question is a neighbourhood, so it becomes one — when there is
   * an action to centre it on, and otherwise it says what it needs instead of quietly doing
   * nothing.
   */
  private void narrow() {
    GraphExtract.Mode chosen = (GraphExtract.Mode) mode.getSelectedItem();
    ...
    if (aggregatedAutomatically) {
      if (rootNode < 0) {
        setText(
            description, "Pick an action first — narrowing means drawing the graph around one.");
      } else {
        mode.setSelectedItem(GraphExtract.Mode.NEIGHBOURHOOD);
      }
      return;
    ...
    if (rooted) {
      ...
      if (depthNow > 1) {
        ...
        setText(
            description, "This is already the narrowest view: one step from the chosen action.");
      }
      return;
    }
    if (rootNode < 0) {
      setText(description, "Pick an action first — narrowing means drawing the graph around one.");
      return;
    }
    mode.setSelectedItem(GraphExtract.Mode.NEIGHBOURHOOD);
  }
   ...
   *
   * <p>The return leg of {@code GraphExplorerView.showAction}: a user who found an action in the
   * table can draw its neighbourhood, and a user who found one in the drawing can open its detail.
   * Nodes with no executed action — every test's TestRunner in a {@code build} invocation — simply
   * do not fire it, rather than firing a zero that would open the wrong row.
   */
  ...
  OptionalLong actionIdAt(int position) {
    if (canvas.model().isCluster()
        || position < 0
   ...
   *
   * <p>On a background thread: a visible graph is bounded, but the file system is not, and a slow
   * disk must not freeze the window.
   */
  public void exportVisible(Path target, GraphExport.Format format) {
    GraphModel model = canvas.model();
    if (model.size() == 0) {
   ...
   *
   * <p>Plan 13.6 lists export among the three things offered above the limit, and this is the one
   * that makes the limit acceptable: the drawing is bounded, the data is not.
   */
  public void exportComplete(Path target, GraphExport.Format format) {
    GraphLayoutService active = service;
    if (active == null) {
      return;
    }
  ...
  private void exported(GraphExport.Result result, long wanted, GraphLayoutService active) {
    if (wanted == exportGeneration && active == service) {
      setText(selected, result.describe());
    }
  }

  private void exportFailed(Throwable failure, long wanted, GraphLayoutService active) {
    if (wanted == exportGeneration && active == service) {
   ...
   *
   * <p>Two exports rather than one because they answer different questions, and a user who exported
   * "the graph" and got only what happened to be on screen would be badly surprised.
   */
  private void exportChosen() {
    ...
    chooseAndExport(
        chosen == 0 ? "Export the visible graph" : "Export the complete graph", chosen == 1);
  }

  private void chooseAndExport(String title, boolean complete) {
    JFileChooser chooser = new JFileChooser();
    chooser.setDialogTitle(title);
    chooser.setSelectedFile(new File("build-graph.dot"));
    if (chooser.showSaveDialog(this) != JFileChooser.APPROVE_OPTION) {
      return;
    }
    File chosen = chooser.getSelectedFile();
    GraphExport.Format format =
        chosen.getName().endsWith(".csv") ? GraphExport.Format.CSV : GraphExport.Format.DOT;
    if (complete) {
  ...
  Optional<LimitEstimate> estimate() {
  ...
  private String describeSelection(GraphModel model, int position) {
    ...
    if (weight == GraphWeight.DURATION) {
      return model.durationAt(position).stream()
    ...
    return model.weightAt(position).stream()
        .mapToObj(value -> text + "  —  " + weight.subject() + ": " + weight.format(value))
        .findFirst()
   ...
   *
   * <p>The on-screen count is exact for what is drawn and silent about the rest; this is the rest,
   * for one node at a time, and it says "≥N (budget reached)" when the traversal gave up — a full
   * transitive closure is forbidden, so giving up must remain possible and visible.
   */
  private void appendGlobalCount(GraphModel model, int position) {
    ...
    service.globalTransitiveCount(
        ...
        counted -> {
          ...
          if (wanted != weightGeneration || now.length != 1 || now[0] != position) {
            return;
          }
          setText(
              selected,
  ...
  private String legend() {
    ...
    if (model.isCluster()) {
      ...
      String text =
          clustering.clusters().size()
              + " groups holding "
              + clustering.clusteredNodes()
              + " "
              + noun()
              + "s and "
              + clustering.clusteredEdges()
              + " dependencies between them.";
      long unnamed =
          clustering.clusters().stream()
      ...
      return unnamed == 0 ? text : text + "  " + unnamed + " of them have no recorded name.";
    }
    ...
    String scale =
        model.slowestDuration().stream()
            .mapToObj(
                slowest -> "Colour is duration, blue to red, up to " + slowest / 1_000 + " ms.")
            .findFirst()
            .orElse("Nothing here was timed, so nothing is coloured by duration.");
    if (untimed == 0) {
      return scale;
    }
    return scale + "  " + untimed + " of " + model.size() + " were not timed and are grey.";
  }
  ...
  private String weightLegend(GraphModel model) {
    GraphWeight weight = model.weight();
    StringBuilder text = new StringBuilder();
    OptionalLong max = model.maxWeight();
    if (max.isPresent()) {
      text.append("Colour and size are ")
          .append(weight.subject())
          .append(", blue to red, up to ")
          .append(weight.format(max.getAsLong()))
          .append('.');
      ...
      text.append("Nothing here has a recorded ")
          .append(weight.subject())
  ...
  private static JPanel labeledSetting(String text, JComponent control, String name) {
  ...
  private static final class Renderer<T> extends DefaultListCellRenderer {

    private static final long serialVersionUID = 1L;

    private final Function<Object, String> naming;

    Renderer(Function<Object, String> naming) {
      this.naming = naming;
