 ...
 *
 * <p>The plan says "quadtree or equivalent spatial index". This is the equivalent, and for this
 * data it is the better one: a layout produces points on or near a lattice — layered puts them at
 * exact multiples of the layer and row gaps — so the recursive subdivision a quadtree pays for buys
 * nothing. A uniform grid is one counting pass and one filling pass, holds three int arrays and no
 * nodes, and answers a rectangle query by visiting only the cells the rectangle touches.
 *
 * <h2>Positions, not node ids</h2>
 *
 * <p>Everything here is an index into the layout's coordinate arrays. That is not a graph node
 * index and it is certainly not an action id; the layout's {@code nodes()} list is the translation.
 * Keeping the index in position space is what lets it be an array rather than a map.
 *
 * <h2>Degenerate layouts are the normal case, not the edge case</h2>
 *
 * <p>A linear layout has zero height and a one-node layout has zero of both. Both fall out of the
 * sizing below as a single row or a single cell rather than as a division by zero, because a
 * critical path is one of the views a user reaches for most.
 */
public final class GraphSpatialIndex {
  ...
  public static GraphSpatialIndex of(GraphLayout.Result layout) {
    ...
    if (count == 0) {
      return new GraphSpatialIndex(x, y, 0, 0, 1, 1, 1, 1, new int[] {0, 0}, new int[0]);
    }
    ...
    int columns =
        Math.max(1, Math.min(target, (int) Math.round(Math.sqrt(target * width / height))));
   ...
   *
   * <p>The culling call. Its cost is the cells the rectangle touches plus the points in them, not
   * the size of the graph, which is the entire reason for the index: panning around a corner of a
   * fifty-thousand-node layout should cost what the corner costs.
   */
   ...
   *
   * <p>The hit-testing call. The radius is what makes a click on empty canvas a deselection rather
   * than a selection of whatever happened to be closest — a graph with one node in it should not
   * respond to a click in the far corner by selecting that node.
   */
   ...
   *
   * <p>For the canvas's drag overlay: a dragged node's indexed position is where the layout put it,
   * not where the user moved it, so hit testing excludes dragged positions here and tests them
   * separately at their displaced coordinates. The index itself is never mutated — it is shared
   * with the layout cache — which is why this is a filter and not an update.
   */
  public OptionalInt nearest(double worldX, double worldY, double radius, IntPredicate accept) {
