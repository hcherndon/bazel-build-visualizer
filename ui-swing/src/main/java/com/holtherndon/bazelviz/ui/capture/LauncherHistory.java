 ...
 *
 * <p>Commands are values, not a log of clicks: recording the same command
 * promotes it to newest rather than adding another copy. Navigation keeps the
 * editable draft that was present before the first Up press and restores it
 * after Down passes the newest entry.
 */
public final class LauncherHistory {
    ...
    /**
     * Replaces the model from persisted newest-first values, removing corrupt
     * duplicates without changing the first occurrence's position.
     */
