 ...
 *
 * <p>Plan 17.8 asks for cache and runner colour modes. Both describe facts that most sessions do
 * not have: without an execution log nothing knows whether an action was a cache hit or where it
 * ran. A mode that painted those spans in the "miss" or "local" colour would put the most
 * interesting claim on the screen on no evidence at all.
 *
 * <p>So every mode has an explicit unknown colour, and a mode with nothing to say refuses to be
 * selected — {@link Mode#isAvailable} rather than a mode that quietly paints one flat colour and
...
public final class TimelineColours {
  ...
  /**
   * Work that finished and succeeded. Green, so that blue can mean exactly one thing on this
   * timeline: {@link #IN_FLIGHT}. When success was also blue, a live view could not tell a target
   * still building from an action that finished fine — the two most different claims a span can
   * make.
   */
  public static final Color SUCCESS = new Color(0x34, 0xA8, 0x53);

  /**
   * Work still happening: a target configured and not yet completed, in the live band. Reserved —
   * nothing that has finished may wear this colour.
   */
  ...
  public enum Mode {
     ...
     *
     * <p>A mode that is unavailable is not offered. Offering it and painting everything grey would
     * be technically honest and practically a bug report.
     */
    ...
    public Optional<String> unavailableReason(TimelineModel model) {
      ...
      return Optional.of(
          "no execution log has been imported, so nothing in this session"
              + " knows "
              + (this == CACHE ? "which actions were cache hits" : "where actions ran"));
    }
  }
   ...
   *
   * <p>A bin holds many spans, so its colour is the majority fact rather than a single span's.
   * Where the bin has no majority — mixed hits and misses, say — the unknown colour is used,
   * because a bin coloured for whichever happened to be counted first would be arbitrary.
   */
  ...
  public static Color forSpan(Mode mode, int flags) {
    ...
    return switch (mode) {
      case OUTCOME -> SUCCESS;
      case CACHE ->
          (flags & SpanSource.FLAG_CACHE_KNOWN) == 0
              ? UNKNOWN
              : (flags & SpanSource.FLAG_CACHE_HIT) != 0 ? CACHE_HIT : CACHE_MISS;
      case RUNNER ->
          (flags & SpanSource.FLAG_RUNNER_KNOWN) == 0
              ? UNKNOWN
