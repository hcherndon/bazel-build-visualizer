 ...
 *
 * <p>Ordered by how a user reads a row: what it built, what kind of work it was, how it ended, how
 * long it took, when. The output path is last and widest because it is the identity but rarely the
 * thing being scanned for.
 *
 * <p>Every extractor is a field read or a format call. They run on the EDT during paint, so nothing
 * here may query, parse or allocate at scale.
 */
public final class ActionTableColumns {
   ...
   *
   * <p>An action with several spawns says how many rather than naming one of them: two spawns that
   * ran under different runners give the action no single runner, and showing the first would
   * present one spawn's answer as the action's.
   */
  ...
  static String cached(ActionRow row) {
    ...
    if (execution.isAbsent() || execution.isAmbiguous()) {
      return EntityFormat.text(Optional.empty());
    }
    return execution
        .cacheHit()
        .map(hit -> hit ? "yes" : "no")
        .orElseGet(() -> EntityFormat.text(Optional.empty()));
  }
  ...
  /**
   * The columns with a backend ordering behind them: header name → the {@link ActionSort} the
   * storage layer pages under. Exactly these four, because these are the orderings {@code SchemaV2}
   * indexes for keyset paging — a header that sorted anything else would have to either scan the
   * table or sort only the rows already fetched, and both would lie.
   */
  private static final Map<String, ActionSort> SORTABLE =
      Map.of(
  ...
  /**
   * The header id that stands for {@code sort} — the column name when a column carries that
   * ordering, else the enum's own name, so a toolbar choice with no column of its own (start time,
   * arrival descending) still round-trips through the persisted column state.
   */
  ...
  /**
   * Why a column without a backend ordering does not sort — the header tooltip's text. Honest about
   * the reason: the table is keyset-paged over up to five million actions, so any ordering must
   * come from a database index, and no index orders these.
   */
