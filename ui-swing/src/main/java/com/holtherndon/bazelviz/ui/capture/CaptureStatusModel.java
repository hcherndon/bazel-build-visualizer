 ...
 *
 * <p>A record rather than mutable widget state so the panel can be tested
 * without a display and so a snapshot handed to the EDT cannot change under it
...
public record CaptureStatusModel(
    ...
    public CaptureStatusModel withProgress(CaptureProgress value) {
        return new CaptureStatusModel(
                phase == Phase.WAITING ? Phase.CAPTURING : phase,
                Optional.of(value),
                detail,
                cancellable);
    }
     ...
     *
     * <p>All three are shown, because the differences between them are the only
     * visible sign of pressure: received minus journaled is what a crash would
     * lose, and journaled minus indexed is why the tables lag the console.
     */
    public String counterLine() {
        return progress
                .map(value -> "%,d received · %,d journaled · %,d indexed%s".formatted(
                        value.received(),
                        value.journaled(),
                        value.normalized(),
