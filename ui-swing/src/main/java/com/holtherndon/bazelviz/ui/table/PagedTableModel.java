...
/**
 * Windowed table model over a {@link RowSource}, designed for tens of millions of logical rows
 * (plan 17.4).
 *
 * <p>{@link #getValueAt} never blocks: a cache miss returns {@link #PLACEHOLDER} and schedules a
 * page fetch on the supplied executor. When a page arrives it is put into the {@link PageCache} and
 * a coalesced {@code fireTableRowsUpdated} is dispatched on the EDT — multiple pages completing in
 * a burst share one EDT dispatch. In-flight fetches whose page has since moved far away from the
 * viewport (further than the cache capacity, in pages) are skipped as obsolete: fast scrubbing must
 * not fetch every page the user flew past.
 *
 * <p>Row identity is the {@code long} row index for now; stable-ID selection arrives with the real
 * model. The source's row count must fit in an {@code int} because {@code JTable} row geometry is
 * int-based; larger counts need the custom scroll shell of plan 17.5 and are rejected here rather
...
public final class PagedTableModel<T> extends AbstractTableModel {
  ...
  /**
   * Rendered for any cell whose page failed to load. Deliberately distinct from {@link
   * #PLACEHOLDER}: a failure the user cannot tell apart from still-loading is a silent drop.
   */
   ...
   *
   * <p>Never blocks and never schedules a fetch: it answers from the cache or says it cannot. A
   * selection handler uses it to describe the selected row without doing I/O on the EDT, and treats
   * null as "not yet" — the model fires a row update when the page lands, and the handler runs
   * again.
   *
   * <p>Deliberately does not fetch. A selection that triggered its own fetch would race the
   * viewport's, and the losing one would be discarded as obsolete after the work was already done.
   */
  ...
  /**
   * True once enough distinct pages have failed that the source itself is treated as broken and no
   * further fetches are scheduled.
   */
    ...
    try {
      ...
      if (page.pageIndex() != pageIndex) {
        throw new IllegalStateException(
            "source returned page " + page.pageIndex() + " for requested page " + pageIndex);
      }
      cache.put(page);
  ...
  private void recordFailure(long pageIndex, Throwable failure) {
    ...
    lastFailure = failure;
    log.error(
        "Page {} failed to load; its rows will render as {}",
        pageIndex,
        ERROR_PLACEHOLDER,
        failure);
  ...
  /**
   * Clears recorded failures so the affected pages are fetched again on the next repaint. This is
   * the hook behind a user-facing "retry" affordance; nothing retries on its own.
   */
