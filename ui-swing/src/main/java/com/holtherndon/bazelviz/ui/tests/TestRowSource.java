 ...
 *
 * <p>The ordering is fixed: failures first, then timeouts and build failures,
 * then flakes, then everything that passed. A user opens this view because
 * something went wrong, and making them sort to find it would be making them
 * ask a question the view already knows the answer to.
 */
public final class TestRowSource implements RowSource<TestRow> {
    ...
    public Page<TestRow> fetchPage(long pageIndex, int requestedPageSize) {
        ...
        List<TestRow> rows = anchor.isPresent()
