...
public final class GraphModel {
     ...
     *
     * <p>The weight drives colour, node radius and edge thickness — and only
     * those. Positions belong to the layout, so swapping the weight swaps
     * these arrays and nothing else; {@code GraphWeight.UNKNOWN}-style absent
     * values use {@link #UNKNOWN_DURATION}'s sentinel value, and are drawn
     * grey at base size rather than as the smallest, coldest thing.
     */
    ...
    /**
     * The same drawing restyled by another weight, without touching the
     * layout.
     *
     * <p>Positions, labels, durations, the spatial index and the edge arrays
     * are shared with this model — the weight drives visual encoding only, so
     * re-selecting a weight is a re-render and never a re-layout.
     *
     * @param valueByNode the weight per graph node index; nodes absent from
     *     the map are unknown, drawn grey at base size, never as zero
     */
    public GraphModel withWeights(
            GraphWeight newWeight,
            java.util.Map<Integer, Long> valueByNode,
            boolean truncated,
            String note) {
        List<Integer> nodes = rendered.layout().nodes();
        long[] values = new long[nodes.size()];
        java.util.Arrays.fill(values, UNKNOWN_DURATION);
        if (!isCluster()) {
            for (int i = 0; i < nodes.size(); i++) {
     ...
     *
     * <p>A heat scale over the largest weight drawn, so the scale means
     * something relative to what is on screen rather than to an absolute
     * nobody chose. A node with no known weight gets the unknown colour,
     * which is neither end of the scale.
     */
    ...
    /**
     * The weight-driven node-radius multiplier, in
     * [{@link #MIN_RADIUS_SCALE}, {@link #MAX_RADIUS_SCALE}].
     *
     * <p>Square-root scaled so a node twice the weight reads as visibly, not
     * absurdly, bigger. Unknown weights sit at 1.0 — base size — because a
     * node the computation could not reach must not be drawn as the smallest
     * thing on screen.
     */
    ...
    /**
     * An edge's thickness bucket, {@code 0} (thin) to
     * {@code EDGE_BUCKETS - 1}.
     *
     * <p>Buckets rather than continuous widths so the canvas pays for a
     * handful of stroke changes per frame instead of one per edge.
     */
    ...
    /**
     * The highest bucket any edge uses; the canvas draws one pass per bucket
     * up to this, so an unweighted drawing costs exactly one pass, as before.
     */
    ...
    private static double[] radiusScalesFor(long[] values, long max) {
        ...
        for (int i = 0; i < values.length; i++) {
            if (values[i] != UNKNOWN_DURATION) {
                scales[i] = MIN_RADIUS_SCALE
                        + (MAX_RADIUS_SCALE - MIN_RADIUS_SCALE)
                                * Math.sqrt((double) values[i] / max);
            }
        }
        return scales;
    }
    ...
    /**
     * One thickness bucket per edge, from the mean of its known endpoints'
     * weight fractions. Edges whose endpoints are both unknown stay in the
     * thinnest bucket, which draws exactly as every edge drew before weights
     * existed.
     */
