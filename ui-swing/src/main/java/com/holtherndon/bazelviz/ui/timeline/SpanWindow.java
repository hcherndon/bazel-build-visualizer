 ...
 *
 * <p>Close zoom draws spans one by one, and a window is refetched every time the user pans. A list
 * of records would allocate one object per span per pan; parallel arrays allocate a handful, and
 * the paint loop reads them without a dereference. This is the same reasoning as {@code
 * CsrGraph}'s.
 *
 * <h2>Every span knows its lane</h2>
 *
 * <p>Each span carries the lane key it belongs to under the grouping the window was fetched for —
 * the same value {@code TimelineController.lanes()} puts in {@link TimelineModel.Lane#key}. The
 * painter places a span by looking its key up in the model's lane list, so where a span is drawn is
 * a fact about the span, not about its position in the fetch. Before this, the painter used {@code
 * i % lanes}: the row a span landed in depended on how many spans happened to precede it in the
 * window, so panning or zooming — which refetches the window — shuffled every span to a new row.
 *
 * <h2>Bounded, and honest about it</h2>
 *
 * <p>A range can contain more spans than a screen has pixels. The window takes at most {@link
 * #MAX_SPANS} and records how many it left out, so the view can say "showing 5,000 of 40,000 here"
 * instead of drawing a dense stripe that looks complete. Plan rule 12: never silently truncate.
 *
 * <p>Truncation is a signal to zoom in, not a failure — and the count is what makes it one.
 */
public final class SpanWindow {
   ...
   *
   * <p>Above this the individual marks are narrower than a pixel and the aggregate bins are both
   * faster and more honest, so this is the point at which drawing spans stops being better than
   * drawing bins.
   */
  ...
  public static final SpanWindow EMPTY =
      new SpanWindow(new long[0], new long[0], new int[0], new long[0], new String[0], 0, 0, 0, 0);
  ...
  /**
   * The lane key this span belongs to under the grouping this window was fetched for — the join to
   * {@link TimelineModel.Lane#key}.
   */
   ...
   *
   * <p>Non-zero means the view is showing some of what is here, and must say so rather than letting
   * a truncated stripe read as the whole.
   */
