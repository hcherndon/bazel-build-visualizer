...
import com.holtherndon.bazelviz.ui.nav.EntityRef.ActionId;
 ...
 *
 * <p>Pure: it takes a row and returns what to show. That keeps the awkward parts — which exit code
 * to display, why a duration is missing — testable without a database or a screen, which matters
 * because they are exactly the places where the tool could quietly say something untrue.
 */
public final class ActionInspection {
  ...
  /**
   * The action, plus what the execution log says about the spawns that ran it.
   *
   * <p>Attempts are shown as their own sections rather than folded into the action's timing,
   * because they are a different measurement: the action's start and end come from the BEP, the
   * attempt's from the execution log, and ADR-009 keeps both under names saying whose they are. An
   * action whose BEP duration and spawn duration disagree is telling the reader something, and
   * averaging them would tell them nothing.
   */
  public static Inspection of(ActionRow row, List<AttemptRow> attempts) {
    Inspection.Builder builder =
        new Inspection.Builder(row.label().orElseGet(row::outputFileName))
    ...
    builder.ref(new ActionId(row.id()));
    row.label().ifPresent(label -> builder.ref(new TargetLabel(label)));
    row.bepEventId().ifPresent(eventId -> builder.ref(new EventId(eventId)));

    builder
        .section("Identity")
        .file("Primary output", row.primaryOutput(), FileLink.actionOutput(row.primaryOutput()))
    ...
    row.commandLine()
        .ifPresent(argv -> builder.section("Command").field("Arguments", describeArgv(argv)));

    builder.section("Result").field("Outcome", row.outcome().name());
    if (row.outcome() == ActionOutcome.FAILED) {
      ...
      builder
          .field(
              row.spawnExitCode().isPresent()
                  ? Inspection.Field.of(
                      "Exit code", Integer.toString(row.spawnExitCode().getAsInt()))
                  : Inspection.Field.unknown(
                      "Exit code", "Bazel did not report the process's exit code"))
          .field(
              EntityFormat.field(
                  "Bazel's status code",
                  row.bazelExitCode().isPresent()
                      ? Optional.of(Integer.toString(row.bazelExitCode().getAsInt()))
                      : Optional.empty()))
   ...
   *
   * <p>Numbered rather than named, because a spawn has no name of its own and because an action
   * with two of them has two answers to "where did it run" — which is a fact about the build, not a
   * presentation problem to smooth over.
   */
  private static void addAttempts(Inspection.Builder builder, List<AttemptRow> attempts) {
    ...
    for (int i = 0; i < attempts.size(); i++) {
      ...
      builder
          .section(attempts.size() == 1 ? "Execution" : "Execution " + (i + 1))
   ...
   *
   * <p>Stored as a JSON array because an argument can contain a newline, and shown numbered because
   * that is the only way a reader can see where one argument ends and the next begins in a {@code
   * /bin/bash -c} script. The text is never re-quoted into something that looks executable: the
   * command was not run through a shell (plan 22.2).
   *
   * <p>Secrets are masked before they reach the screen. docs/privacy.md says the UI masks sensitive
   * well-known fields by default, and this is the one place a build's own credentials are shown a
   * line at a time — a {@code --remote_header} carrying a bearer token is an ordinary argument and
   * would otherwise be rendered in full. The display policy leaves paths alone: the person at the
   * keyboard already has the session on their disk, and a path they cannot paste into a terminal is
   * worse at the job the inspector exists for.
   *
   * <p>A redactor per call rather than per session: the point here is masking rather than linking,
   * and a table of pseudonyms that outlived the panel would be a table of secrets held for no
   * reason.
   */
  private static String describeArgv(String json) {
    List<String> arguments =
        new Redactor(RedactionPolicy.forDisplay())
   ...
   *
   * <p>The row already carries the reason its duration is unknown, and the start is missing for the
   * same reason, so the two never disagree.
   */
