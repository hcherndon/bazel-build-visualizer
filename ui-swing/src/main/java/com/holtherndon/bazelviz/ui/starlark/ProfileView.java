...
/** Shared queryable profile summary, tables, directed graph, and flame view. */
public class ProfileView extends JPanel implements PageChrome {
  ...
  private final JTable functionTable = new JTable(emptyModel("Function", "Source", "Self"));
  ...
  private final JTable fileTable = new JTable(emptyModel("Source file", "Self"));
  ...
  private final JTable callersTable = new JTable(emptyModel("Caller", "Value"));
  private final JTable calleesTable = new JTable(emptyModel("Callee", "Value"));
  ...
  private Optional<StarlarkProfileReader.SampleMetric> metric = Optional.empty();
  ...
  public ProfileView() {
    ...
    callGraphNodeWeight.setToolTipText(
        "Choose which sample value controls each function box's size");
  ...
  /** Opens a reader on the view worker and owns it until this view closes. */
  public void openProfile(Supplier<StarlarkProfileReader> opened) {
    ...
    showEmpty("Reading the profile…");
    openingWorker.execute(
        () -> {
          StarlarkProfileReader candidate = null;
          try {
            candidate = opened.get();
            Optional<StarlarkProfileReader.SampleMetric> loadedMetric = candidate.sampleMetric();
            ...
            SwingUtilities.invokeLater(
                () -> {
                  if (wanted == generation) {
                    metric = loadedMetric;
                    directedCallGraph.setValueFormatter(this::formatValue);
                    flameGraph.setValueFormatter(this::formatValue);
                  }
                  installReader(opened, wanted, ready, loaded);
                });
            ...
            SwingUtilities.invokeLater(
                () -> {
                  if (wanted == generation && session == opened) {
                    showEmpty("The profile could not be opened: " + describe(failure));
                  }
                });
          }
        });
  ...
  private void installReader(
    ...
    if (loaded.isAvailable()) {
      String cpu = formatValue(loaded.sampledCpuMicros());
      String functions = EntityFormat.count(loaded.functionCount());
      syncPageMetadata(
          cpu
              + (metric.isPresent() ? " · " + metric.orElseThrow().type() : " sampled CPU")
              + " · "
              + functions
              + " functions",
          loaded.detail() + " Total: " + cpu + ". Functions: " + functions + ".");
    } else {
  ...
  public CompletionStage<Void> closeSessionAsync() {
    ...
    callersModel = null;
    calleesModel = null;
    functionTable.setModel(emptyModel("Function", "Source", "Self"));
    fileTable.setModel(emptyModel("Source file", "Self"));
    callersTable.setModel(emptyModel("Caller", "Value"));
    calleesTable.setModel(emptyModel("Callee", "Value"));
  ...
  private JPanel header() {
    ...
    JLabel title = PlainText.disableHtml(new JLabel("Profile"));
    ...
    copy.add(
        WrappingLabel.create(
            "Explore hot functions, source files, call relationships, and sampled stacks."),
        BorderLayout.CENTER);
  ...
  private JPanel callGraphTab() {
    ...
    graphPane.add(new SectionPane("Directed call graph", directedCallGraph), BorderLayout.CENTER);
  ...
  private void renderSummary(StarlarkProfileReader.Summary value) {
    ...
    if (metric.isPresent()) {
      summaryCards.add(
          summaryCard(
              "Sample type",
              metric.orElseThrow().type(),
              "Original unit: " + metric.orElseThrow().unit()));
      summaryCards.add(
          summaryCard(
              "Total value",
              formatValue(value.sampledCpuMicros()),
              "sum of the selected sample values"));
      summaryCards.add(
          summaryCard(
              "Profile records",
              EntityFormat.count(value.sampleRecordCount()),
              "one record may aggregate several samples"));
      summaryCards.add(
          summaryCard(
              "Functions",
              EntityFormat.count(value.functionCount()),
              "symbols present in the profile"));
      summaryCards.add(
          summaryCard(
              "Source files",
              EntityFormat.count(value.fileCount()),
              "paths recorded by the profile producer"));
      summaryCards.add(
          summaryCard(
              "Function-attributed value",
              coverageValue(value.functionAttribution()),
              coverageNote(value.functionAttribution())));
      summaryCards.add(
          summaryCard(
              "Context-attributed value",
              coverageValue(value.contextAttribution()),
              coverageNote(value.contextAttribution())));
      summaryDetail.setText(
          "Standalone profile; not associated with a build or workspace. The profile's default"
              + " sample type is displayed (the last type when no default is named). Values retain"
              + " their original units. Self values belong to the sampled leaf; cumulative values"
              + " include callees and must not be added across functions. Missing symbols remain"
              + " unattributed; this viewer does not fetch symbols or execute tools from imported"
              + " files. Source paths and line numbers are hints from the producer, not line-level"
              + " measurements.");
      summaryCards.revalidate();
      summaryCards.repaint();
      return;
  ...
  private String formatValue(OptionalLong value) {
    return metric
        .map(selected -> selected.format(value))
        .orElseGet(() -> EntityFormat.duration(value));
  }

  private String coverageValue(StarlarkProfileReader.AttributionCoverage coverage) {
    return coverage.attributedCpuMicros().isPresent()
        ? formatValue(coverage.attributedCpuMicros())
        : "—";
  }

  private String coverageNote(StarlarkProfileReader.AttributionCoverage coverage) {
    ...
    return formatValue(coverage.unattributedCpuMicros())
        + " unattributed; "
    ...
    try {
      ...
      callGraphStatus.setText(
          EntityFormat.count(callers.rowCount())
              ...
              + " callees for "
              + function.name()
              + ". Values are aggregate sample values on each relationship.");
    } catch (IllegalArgumentException failure) {
        ...
        new ColumnSpec<>(
            ...
            function ->
                function
                    ...
                    .orElse(EntityFormat.UNKNOWN)),
        new ColumnSpec<>("Self", function -> formatValue(function.selfCpuMicros())),
        new ColumnSpec<>("Self %", function -> percentOfTotal(function.selfCpuMicros())),
        new ColumnSpec<>("Cumulative", function -> formatValue(function.cumulativeCpuMicros())),
        new ColumnSpec<>(
  ...
  private List<ColumnSpec<StarlarkProfileReader.SourceFile>> fileColumns() {
    return List.of(
        ...
        new ColumnSpec<>("Self", file -> formatValue(file.selfCpuMicros())),
        new ColumnSpec<>("Self %", file -> percentOfTotal(file.selfCpuMicros())),
        new ColumnSpec<>("Cumulative", file -> formatValue(file.cumulativeCpuMicros())),
        ...
        new ColumnSpec<>(
            ...
            edge ->
                edge.source()
                    .map(StarlarkProfileReader.SourceLocation::path)
                    .orElse(EntityFormat.UNKNOWN)),
        new ColumnSpec<>("Value", edge -> formatValue(edge.cpuMicros())),
        new ColumnSpec<>("Value %", edge -> percentOfTotal(edge.cpuMicros())),
  ...
  private static String directedGraphStatus(
    ...
    if (graph.omittedFunctionCount() == 0) {
      ...
      status
          ...
          .append(" of ")
          .append(EntityFormat.count(graph.totalFunctionCount()))
          .append(" functions by cumulative value; ")
    ...
    if (graph.omittedVisibleEdgeCount() == 0) {
      ...
      status
          ...
          .append(" lower-value arrows are outside the current ")
    ...
    return status
        ...
        .append(nodeWeight)
        .append(
            "; box heat is cumulative value; arrow width, colour, and labels are"
                + " relationship value. An unavailable size value uses a neutral box size."
