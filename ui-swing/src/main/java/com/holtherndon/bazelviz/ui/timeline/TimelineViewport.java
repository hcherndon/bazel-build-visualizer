 ...
 *
 * <p>Plan 24's fifth Phase 6 exit criterion is that live updates must not reset the viewport or the
 * selection. The hard part is not remembering the numbers — it is knowing whether the user moved. A
 * timeline that follows the build is doing the right thing until the moment someone scrolls back to
 * look at something, and from then on following is destroying their work.
 *
 * <p>So "the user has navigated" is recorded here, as a fact, the moment it happens. {@link
 * #withWall} is the only path a live update takes, and it refuses to move a viewport that a person
 * put where it is.
 *
...
public record TimelineViewport(
   ...
   *
   * <p>Stops following, always. Someone who has moved is looking at something, and the build
   * appending to the right is not a reason to take them away from it — plan 14.4: "do not
   * auto-scroll if the user has navigated into history".
   */
  public TimelineViewport navigatedTo(TimelineTransform moved) {
    return new TimelineViewport(moved, false, selectedNode, rangeFromMicros, rangeToMicros);
  }

  /** The viewport with following turned on or off by the user's toggle. */
  public TimelineViewport following(boolean follow) {
    return new TimelineViewport(transform, follow, selectedNode, rangeFromMicros, rangeToMicros);
  }
   ...
   *
   * <p>The whole point of this class. A following viewport is refitted so the user watching a build
   * sees it grow; one the user has moved is returned untouched, selection and range included.
   *
   * <p>Note what is <em>not</em> here: no case that clears the selection, and none that resets the
   * transform. A live update has no business doing either, and the way to guarantee that is to give
   * it no way to.
   */
  ...
  public Optional<String> describeRange() {
    ...
    return Optional.of(String.format(Locale.ROOT, "%.3f s selected", width / 1_000_000.0));
  }
