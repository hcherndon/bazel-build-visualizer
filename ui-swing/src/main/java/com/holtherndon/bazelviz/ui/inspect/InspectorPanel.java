...
/**
 * The shared inspector: one pane that shows whatever entity is selected, in whichever view (plan
 * 17.11).
 *
 * <h2>Unknown looks different from blank</h2>
 *
 * <p>A field with no value renders as {@code unknown} in the theme's disabled colour, followed by
 * the reason when there is one — "not reported by this Bazel version", "no spawn ran". A field
 * whose value is genuinely an empty string renders as an empty string. Those are different facts
 * and a user chasing a missing number needs to be able to tell which one they are looking at, so
 * they never render alike (plan 11.4).
 *
 * <h2>Every entity can name its source event</h2>
 *
 * <p>When the inspection carries one, a button offers to open it in the Events view. That is the
 * whole of "event-to-domain provenance is inspectable": from any row in any table, one click to the
 * bytes Bazel actually sent. Once the shared actions are installed the offer moves into {@link
 * InspectorHeader}'s overflow menu, and the button yields rather than double it.
 */
public final class InspectorPanel extends JPanel {
  ...
  /**
   * Title, subtitle and the overflow menu — the same header the Events and Timeline inspectors
   * wear, which is why it is not built here.
   */
  ...
  public InspectorPanel() {
    ...
    sourceButton.addActionListener(
        event -> current.sourceEventId().ifPresent(id -> sourceEventHandler.accept(id)));
  ...
  /**
   * Adopts the shared cross-view actions: inspections carrying {@linkplain Inspection#refs() refs}
   * grow the header's overflow menu, and the legacy source-event button yields to that menu's own
   * "Show source event" whenever it offers one — the same action must not appear twice.
   *
   * @param omit commands never offered here — a host passes the one that would navigate to itself
   */
  ...
  public void show(Inspection inspection) {
    ...
    if (inspection.isEmpty()) {
      ...
      sourceButton.setVisible(
          inspection.sourceEventId().isPresent()
              && !header.offers(EntityActions.Command.SHOW_SOURCE_EVENT));
      inspection
          .sourceEventId()
          .ifPresent(id -> sourceButton.setToolTipText("Open event " + id + " in the Events view"));
  ...
  private Component valueLabel(Inspection.Field field) {
    if (field.isKnown()) {
      ...
      label
          ...
          .setAccessibleDescription("Recorded value for " + field.name() + ".");
      ...
      open.setToolTipText(
          PlainText.tooltip("Open " + text + " in a separate, non-modal text window."));
    ...
    label.setToolTipText(
        PlainText.tooltip(
            field.unknownNote().orElse("This value was not reported, and is not zero.")));
    label
        .getAccessibleContext()
        .setAccessibleDescription(
            field
                ...
                .orElse(
                    "Unknown " + field.name() + "; this value was not reported and is not zero."));
    return label;
  }
