 ...
 *
 * <p>Plan 14.2 lists eight groupings and five sort orders. Both are enums
 * rather than strings because each carries the SQL that produces it and the
 * sentence that describes it, and a grouping the session cannot supply must be
 * able to say so rather than silently producing one lane called "null".
 */
public final class LaneGrouping {
    ...
    public enum By {
        /**
         * Where the work ran. Needs an execution log; without one every action
         * falls in a single unknown lane, which is why {@link #needsExecutionLog}
         * exists rather than letting the view show one useless row.
         */
        RUNNER("Runner", true),
         ...
         */
        public java.util.Optional<String> unavailableReason(boolean hasExecutionLog) {
            if (!needsExecutionLog || hasExecutionLog) {
                return java.util.Optional.empty();
            }
            return java.util.Optional.of(displayName.toLowerCase(java.util.Locale.ROOT)
     ...
     * Orders {@code lanes}.
     *
     * @param criticalPathMicrosByLane how much of the critical path each lane
     *     accounts for; empty when no critical path has been computed, in which
     *     case sorting by contribution falls back to total duration rather than
     *     silently ordering by nothing
     */
    public static List<TimelineModel.Lane> sort(
            List<TimelineModel.Lane> lanes,
            SortBy sortBy,
            java.util.Map<String, Long> criticalPathMicrosByLane) {
        Comparator<TimelineModel.Lane> comparator = switch (sortBy) {
            ...
            case ACTION_COUNT ->
                    Comparator.comparingLong(TimelineModel.Lane::spanCount).reversed();
            case NAME -> Comparator.comparing(TimelineModel.Lane::name);
            case CRITICAL_PATH_CONTRIBUTION -> criticalPathMicrosByLane.isEmpty()
        ...
        return lanes.stream()
                .sorted(comparator.thenComparing(TimelineModel.Lane::name))
                .toList();
    }
     ...
     *
     * <p>Used by the view to say "sorted by total duration, because no critical
     * path has been computed" rather than showing a sort control that lies.
     */
    public static boolean contributionSortIsReal(
            SortBy sortBy, java.util.Map<String, Long> criticalPathMicrosByLane) {
