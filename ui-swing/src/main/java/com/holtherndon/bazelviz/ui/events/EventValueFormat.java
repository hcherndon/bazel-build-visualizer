...
/**
 * Renders event values for display, with one rule above all others: an
 * unavailable value renders as an em dash and never as {@code 0}, an empty
 * string, or the epoch (plan 11.4, project rule 11).
 *
 * <p>The distinction matters most where a number would look plausible. A BEP
 * event that carries no timestamp is not an event that happened at
 * 1970-01-01T00:00:00Z; an event whose bytes did not decode does not have zero
 * children. Both would be readable, sortable, believable lies.
 *
 * <p>Timestamps render in UTC. Local time would be friendlier and would also
 * make two people reading the same capture disagree about when an event
 * happened, so the column headers say UTC and the values are UTC.
 */
