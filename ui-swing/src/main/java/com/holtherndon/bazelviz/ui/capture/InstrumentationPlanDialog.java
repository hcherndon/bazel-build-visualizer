 ...
 *
 * <p>The dialog is the visible form of the promise: the command the user typed and the command that
 * will execute, side by side, with one line of explanation for every difference and the cost of
 * each addition. A conflict is presented as the choices plan 8.5 requires, each labelled with what
 * it gives up, and Launch stays disabled until one is chosen — the type system enforces the same
 * rule underneath, and this is where the user meets it.
 *
 * <p>Modal, and constructed on the EDT. Nothing here does I/O: a plan is an inert value, and every
 * button hands its decision back to the caller.
 */
public final class InstrumentationPlanDialog extends JDialog {
  ...
  public InstrumentationPlanDialog(Window owner, Preflight preflight) {
    ...
    if (changed > 0) {
      body.add(new DisclosurePanel("Capture changes · " + changed, captureChanges(plan), false));
      body.add(Box.createVerticalStrut(8));
    }
    ...
    if (!plan.conflicts().isEmpty() || !plan.warnings().isEmpty() || !plan.errors().isEmpty()) {
    ...
    body.add(
        new DisclosurePanel(
            "Bazel, workspace & captured outputs", technicalDetails(preflight), false));
  ...
  private static JComponent commandSummary(Preflight preflight) {
    ...
    content.add(
        labelled(
            "Working directory", wrapped(preflight.workspace().workingDirectory().toString())));
    content.add(Box.createVerticalStrut(6));
    String original =
        preflight.executable().entered()
            + " "
            + String.join(" ", plan.original().userVisibleArgs());
    content.add(labelled("Your command", monospace(original.stripTrailing())));
    content.add(Box.createVerticalStrut(6));
    content.add(labelled("Command to run", monospace(String.join(" ", plan.effective().toArgv()))));
    content.add(Box.createVerticalStrut(6));
    String environment =
        plan.effective().inheritance().toString().replace('_', ' ').toLowerCase(Locale.ROOT)
            + " · "
            + plan.effective().environmentOverrides().size()
            + " explicit override(s)";
  ...
  private static JComponent auxiliaryCommands(InstrumentationPlan plan) {
    ...
    for (AuxiliaryCommandPlan command : plan.auxiliaryCommands()) {
      StringBuilder text =
          new StringBuilder(command.purpose())
              .append("\nWhen: ")
              .append(command.timing())
              .append(" · cost: ")
              .append(command.estimatedCost().displayName())
              .append(" · failure: ")
              .append(command.failureIsFatal() ? "stops the capture" : "non-fatal")
              .append("\nCommand: ")
              .append(String.join(" ", command.argv()))
              .append("\nOutput: ")
              .append(command.outputPath());
      if (!command.carriedOptions().isEmpty()) {
        text.append("\nCarries forward: ").append(String.join(" ", command.carriedOptions()));
      }
      if (!command.droppedOptions().isEmpty()) {
        text.append("\nNot carried forward: ").append(String.join(" ", command.droppedOptions()));
      }
  ...
  private static JComponent technicalDetails(Preflight preflight) {
                  ...
                  labelled(
                      ...
                      wrapped(
                          remote.host()
                              ...
                              + "\nRemote BES address: "
                              + remote.remoteBesBackend()
                              + "\nDesktop listener: "
                              + remote.localBesListener()
                              + "\nPrivate staging: "
                              + remote.stagingDirectory()
    ...
    content.add(Box.createVerticalStrut(6));
    String workspace =
        preflight.workspace().workspaceRoot().map(Object::toString).orElse("not detected");
    content.add(
        labelled("Workspace root", wrapped(workspace + " · " + preflight.workspace().detection())));
    content.add(Box.createVerticalStrut(6));
    content.add(
        labelled("Capability check", wrapped(preflight.capabilities().detection().toString())));
    content.add(Box.createVerticalStrut(6));
    ...
    if (!plan.expectedOutputs().isEmpty()) {
      ...
      content.add(
          labelled(
              "Files this plan may create",
              wrapped(
                  plan.expectedOutputs().stream()
                      .map(Object::toString)
    ...
    if (!plan.sourceAvailability().bySource().isEmpty()) {
      content.add(Box.createVerticalStrut(6));
      String availability =
          plan.sourceAvailability().bySource().entrySet().stream()
  ...
  private JComponent addedFlagRow(AddedFlag flag) {
    ...
    JCheckBox enabled = new JCheckBox(flag.argv(), true);
    enabled.setAlignmentX(Component.LEFT_ALIGNMENT);
    enabled.setToolTipText("Untick to leave this flag out. " + describeLoss(flag));
  ...
  private static String describeLoss(AddedFlag flag) {
    return "Without it, "
        + flag.enables().name().toLowerCase(Locale.ROOT)
  ...
  private JComponent conflictPanel(PlanConflict conflict) {
    ...
    panel.setBorder(
        BorderFactory.createCompoundBorder(
            BorderFactory.createTitledBorder(conflict.mandatory() ? "Decision needed" : "Note"),
