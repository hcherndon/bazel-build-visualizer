 ...
 *
 * <p>Optional-typed wherever the database column is nullable, so the renderer
 * cannot accidentally turn "not known" into a number. {@code childCount} and
 * {@code eventType} are the interesting cases: both are {@code NOT NULL}
 * columns that the importer writes as {@code 0} when a record failed to
 * decode, because a row must exist for those bytes even though nothing could
 * be read out of them. Storing 0 is right — the column cannot be null —
 * but <em>displaying</em> 0 would claim the event had no payload and no
 * children, which is not what was observed. {@link #hasReadableStructure()}
 * is what the renderer consults, and
 * {@link EventTableColumns} renders an em dash for those cells.
 *
 * @param rawLocation where the verbatim bytes live; the inspector reads them
 *     on demand and the table never does
 * @param idDisplay the canonical id rendering from {@code bep_event_ids},
 *     empty when the event declared no id
 */
public record EventRow(
    ...
    /**
     * True when the record decoded, so its payload case and child count are
     * observations rather than placeholders.
     */
