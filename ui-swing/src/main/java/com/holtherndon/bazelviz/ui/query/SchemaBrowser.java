 ...
 *
 * <p>A session database carries many tables across evolving schema versions and this repository has
 * no generated schema page — the DDL in {@code storage-sqlite} and its javadoc are the only
 * description of it, and neither is reachable from the application. A query surface with no way to
 * see column names is a query surface for somebody who already has the source open.
 *
 * <p>So the tree is built from {@code sqlite_master} and {@code PRAGMA table_info} at run time,
 * which is also the only description that stays true for a session written by a different build.
 *
 * <h2>No row counts</h2>
 *
 * <p>Counting every table is a scan per table, and a count shown before it was taken would be a
 * zero standing in for "not known yet" (rule 11). The column list is the thing that was missing; a
 * count is one {@code SELECT COUNT(*)} away in the editor beside it.
 *
 * <p>Holds no reader and no connection: it is handed a finished list, so it can do no I/O and needs
...
final class SchemaBrowser extends JPanel {
  ...
  List<String> listedTablesForTest() {
    ...
    for (int i = 0; i < root.getChildCount(); i++) {
      names.add(String.valueOf(((DefaultMutableTreeNode) root.getChildAt(i)).getUserObject()));
    }
    return names;
  }
    ...
    for (int i = 0; i < root.getChildCount(); i++) {
      ...
      if (label.equals(table) || label.startsWith(table + "  ")) {
        ...
        for (int c = 0; c < node.getChildCount(); c++) {
          columns.add(
              String.valueOf(((DefaultMutableTreeNode) node.getChildAt(c)).getUserObject()));
        }
        return columns;
      }
    }
