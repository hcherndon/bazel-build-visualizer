 ...
 *
 * <p>A NULL is not zero and not an empty string. Swing's default renderer shows
 * all three as an empty cell, which would turn "this action has no recorded
 * duration" and "this action took 0 µs" into the same picture — two different
 * facts about a build, one of them false.
 *
 * <p>So a NULL is drawn as the word {@code NULL}, in italic and in the theme's
 * disabled foreground. A <em>string</em> whose value happens to be "NULL" is
 * drawn upright in the normal colour, and the legend under the grid says which
 * is which. Styling rather than a sentinel string, because any sentinel is a
 * value some column could legitimately hold.
 *
 * <p>{@code PagedTableModel}'s own placeholders pass through unchanged: "…"
 * still means the page is loading and "⚠" still means it failed, and neither
 * is confused with a value.
 *
 * <p>HTML stays off, for the reason {@link PlainText} exists: every string here
 * came out of somebody's build.
 */
final class SqlValueRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
        ...
        setToolTipText(PlainText.tooltip(isNull
    ...
    private void applyNullStyling(boolean isNull, boolean selected) {
        ...
        if (isNull && !selected) {
            java.awt.Color dim = UIManager.getColor("Label.disabledForeground");
            if (dim == null) {
