 ...
 *
 * <p>Every extractor is a field read plus trivial formatting, as {@link ColumnSpec} requires — they
 * run on the EDT during paint, for the visible rows only.
 *
 * <h2>What each column refuses to invent</h2>
 *
 * <ul>
 *   <li><b>Type</b> and <b>Children</b> are stored as {@code 0} for a record that did not decode,
 *       because their columns are {@code NOT NULL} and a row must exist for those bytes regardless.
 *       Displaying that 0 would assert "no payload, no children" about a record nobody could read,
 *       so a failed decode renders both as an em dash.
 *   <li><b>Event time</b> is empty for every BEP event that carries no timestamp of its own. It
 *       renders as an em dash, never as the epoch.
 *   <li><b>Event id</b> renders as an em dash when the event declared no id, and as the raw hash
 *       when it declared one whose identity row is missing — two different findings, shown
 *       differently.
 *   <li><b>Raw bytes</b> is always known: it is the length the journal actually stored, which is
 *       why it is the one column a failed decode still fills in.
 * </ul>
 */
public final class EventTableColumns {
  ...
  public static List<ColumnSpec<EventRow>> columns() {
    return List.of(
        ...
        new ColumnSpec<>("Event time (UTC)", row -> EventValueFormat.timestamp(row.eventMicros())),
        new ColumnSpec<>("Received (UTC)", row -> EventValueFormat.timestamp(row.receiveMicros())));
  }

  /**
   * The payload case, or an em dash when the record did not decode and its stored {@code
   * event_type} of 0 therefore means "unread", not "none".
   */
  ...
  /**
   * The canonical id display; the bare hash when the event declared an id whose identity row is
   * absent; an em dash when it declared none.
   */
  ...
  /**
   * The announced-child count, or an em dash when the record did not decode and nothing could have
   * been counted.
   */
