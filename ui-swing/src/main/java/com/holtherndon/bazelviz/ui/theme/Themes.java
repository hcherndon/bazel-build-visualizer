...
public final class Themes {
  ...
  /**
   * Installs a bundled theme and refreshes every open Swing and syntax-text window. Must run on the
   * EDT.
   */
  public static void install(AppTheme theme) {
    ...
    if (!FlatLaf.setup(theme.createLookAndFeel())) {
      throw new IllegalStateException("Could not install the " + theme.displayName() + " theme");
    }
    ...
    try {
      ...
      log.warn("some Swing components did not refresh for theme {}", theme.id(), refreshFailure);
    }
    try {
      SyntaxTextTheme.refreshOpenWindows();
    } catch (RuntimeException refreshFailure) {
      log.warn(
          "some syntax text components did not refresh for theme {}", theme.id(), refreshFailure);
    }
  }
