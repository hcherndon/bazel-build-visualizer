 ...
 *
 * <p>Everything here is a thin veneer over {@link QueryView.PanelHost} — the
 * shell owns the {@link QueryLibrary} and the I/O thread, this panel owns only
 * widgets, which is what keeps it out of the EDT-discipline rule's sights: it
 * cannot block because it holds nothing that can.
 *
 * <p>Double-clicking a saved query loads it into the selected tab's editor.
 * Double-clicking a saved view inserts a {@code SELECT * FROM <view>} —
 * the view itself already exists on every tab's connection, replayed there by
 * the shell. "Save view from editor" is deliberate about its input: the editor
 * must hold a {@code CREATE TEMP VIEW} definition, because that is the only
 * honest source for both the name and the body.
 */
public final class QueryLibraryPanel extends JPanel {
    ...
    QueryLibraryPanel(QueryView.PanelHost host) {
        ...
        add(section("Saved queries", queryList,
        ...
        add(section("Saved views", viewList,
    ...
    private String prompt(String message, String title, String initial) {
        Object answer = JOptionPane.showInputDialog(this, message, title,
