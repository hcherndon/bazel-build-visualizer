...
/**
 * Aggregate timeline view (plan 14.1-14.3 spike). Paints density columns for the visible window
 * straight from an immutable {@link TimelineLodIndex}: column color intensity follows per-bin
 * overlap density, with a red tint whose strength follows the failure fraction. Mouse drag pans;
 * the wheel zooms around the cursor.
 *
 * <p>Paint reads only prebuilt primitive arrays and a precomputed color palette — no data loading,
 * no locking, no per-bin allocation. The transform is confined to the painting thread (EDT in a
 * window, the caller's thread in offscreen harnesses).
 */
public final class TimelineCanvas extends JComponent {
  ...
  private int paintColumns(
      Graphics2D g2, int level, long visibleFrom, long visibleTo, int width, int plotHeight) {
  ...
  private void paintAxis(
      Graphics2D g2, int width, int height, int plotHeight, long visibleFrom, long visibleTo) {
  ...
  /**
   * Failure fractions on real builds are tiny (well under 1%), so a linear mapping would never
   * leave step 0; any failure gets at least step 1 and the rest of the ramp follows sqrt(fraction).
   */
  ...
  static String formatMicros(long micros, long stepMicros) {
    if (stepMicros >= 1_000_000) {
      ...
      return abs >= 60 ? String.format("%s%dm%02ds", sign, abs / 60, abs % 60) : sign + abs + "s";
    }
  ...
  private static Color[][] buildPalette() {
    ...
    for (int f = 0; f < FAIL_STEPS; f++) {
      ...
      for (int i = 0; i < INTENSITY_STEPS; i++) {
        ...
        palette[f][i] =
            new Color((int) lerp(r, 235, fail), (int) lerp(gr, 70, fail), (int) lerp(b, 70, fail));
      }
    }
    return palette;
  }
