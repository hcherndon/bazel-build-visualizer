...
/**
 * Immutable world (microseconds) to screen (pixels) mapping for the timeline: {@code x =
 * (worldMicros - offsetMicros) * pixelsPerMicro}. Offsets are kept as doubles so pan/zoom
 * compositions do not accumulate rounding drift; with wall spans below ~2^40 microseconds the
 * 53-bit mantissa keeps sub-pixel precision at every reachable zoom.
 *
...
public record TimelineTransform(double offsetMicros, double pixelsPerMicro) {
  ...
  public TimelineTransform {
    if (!Double.isFinite(offsetMicros) || !Double.isFinite(pixelsPerMicro) || pixelsPerMicro <= 0) {
  ...
  /**
   * Pans so content follows a pointer dragged by {@code dxPixels}: the world time previously at x
   * is afterwards at x + dxPixels.
   */
  ...
  /**
   * Multiplies the zoom by {@code factor} while keeping the world time under {@code anchorX} at
   * exactly {@code anchorX}.
   */
