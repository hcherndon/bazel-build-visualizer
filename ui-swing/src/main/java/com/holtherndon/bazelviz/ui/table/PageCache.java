 ...
 *
 * <p>Concurrency contract: one background writer ({@link #put}) plus the EDT reader ({@link
 * #get}/{@link #peek}). Everything is guarded by the instance monitor; the critical sections are
 * single map operations, so the EDT never waits behind anything slower than a hash lookup.
 */
public final class PageCache<T> {
  ...
  /**
   * Presence probe that touches neither the hit/miss counters nor LRU recency ({@code containsKey}
   * does not record an access even in access-order mode). For instrumentation and tests.
   */
  ...
  /**
   * The cached page or {@code null}, without counting a hit or a miss and without touching LRU
   * recency.
   *
   * <p>For readers that are not the viewport: a selection handler asking "what is at this row"
   * should not make the cache believe the row was displayed, or the eviction order would follow the
   * selection instead of the scroll position.
   *
   * <p>Found by iteration rather than by {@code get}, because the backing map is in access order
   * and every {@code get} — {@code getOrDefault} included — reorders it. The scan is over at most
   * {@code capacity} entries, and a capacity that made that expensive would be a capacity too large
   * to hold.
   */
