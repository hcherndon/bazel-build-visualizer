...
public final class LauncherPanel extends JPanel {
    ...
    public LauncherPanel(
        ...
        command.setToolTipText(PlainText.tooltip(
                "The Bazel command without 'bazel', for example test //...."
                        + " Up/Down recalls up to " + LauncherHistory.MAX_ENTRIES
                        + " unique commands; Ctrl+Space completes a"
                        + " common Bazel subcommand."));
        executionHost.setToolTipText(PlainText.tooltip(
                "Run directly on this computer or through an explicit OpenSSH connection."));
        sshDestination.setToolTipText(PlainText.tooltip(
                "An SSH config host alias or user@host. Authentication uses your SSH config"
                        + " and agent; the app does not store a password."));
        sshPort.setToolTipText(PlainText.tooltip(
                "Optional SSH port. Leave blank to use your SSH config or port 22."));
        savedSshConnections.setToolTipText(PlainText.tooltip(
        ...
        selectedWorkspace.setToolTipText(PlainText.tooltip(
                "The selected repository and machine. Use Workspaces to change it."));
        ...
        executionHost.addActionListener(event -> {
            switchHostDraft();
            updateHostControls();
    ...
    private void buildForm() {
        ...
        JLabel destinationLabel = label(
                "SSH destination", sshDestination, "launcher.sshDestinationLabel");
        JLabel portLabel = label("Port", sshPort, "launcher.sshPortLabel");
        JLabel savedLabel = label(
                "Connection", savedSshConnections, "launcher.savedSshConnectionsLabel");
        ...
        JLabel selectedLabel = label(
                "Workspace", selectedWorkspace, "launcher.selectedWorkspaceLabel");
    ...
    private static GridBagConstraints sshConstraints(
    ...
    private void updateHostControls() {
        ...
        forgetSshConnection.setEnabled(remote
                && savedSshConnections.getSelectedIndex() > 0);
        workspace.setToolTipText(PlainText.tooltip(remote
                ? "Absolute working directory on the SSH host. Relative targets resolve here."
                : "Where the build runs. Relative targets resolve against this directory."));
        bazelExecutable.setToolTipText(PlainText.tooltip(remote
    ...
    private void selectSshProfile() {
        ...
        if (!(selected instanceof String displayName)
                || NEW_SSH_CONNECTION.equals(displayName)) {
    ...
    private void forgetSelectedSshConnection() {
        ...
        sshProfiles = sshProfiles.stream()
                .filter(profile -> !profile.displayName().equals(displayName))
                .toList();
        refreshSavedSshConnections("");
    ...
    private void updatePresetSummary() {
        ...
        String summary = switch (selected) {
    ...
    private void installPersistenceListeners() {
        ...
        sshDestination.getDocument().addDocumentListener(
                dirtyListener(Setting.SSH_DESTINATION, false));
    ...
    private void preserveInactiveHostDraft(
            ...
            LauncherStateStore.State merged,
            Set<Setting> edited) {
    ...
    private void finishClosedLoad(SaveQueue queue, LauncherStateStore.State loaded) {
        ...
        queue.initialize(loaded);
        if (closed != null) {
            completePersistenceClose(queue.closeWhenSettled(
                    mergeLoaded(loaded, closed.snapshot(), closed.edited())));
        } else {
    ...
    private void adopt(LauncherStateStore.State loaded) {
        ...
        try {
            ...
            String selectedProfile = loaded.sshProfiles().stream()
                    .filter(profile -> profile.destination().equals(loaded.sshDestination())
     ...
     * remain on the persistence executor, including when the window closes before its initial
     * load finishes.
     */
    ...
    /**
     * Uses a workspace selected by the application-level workspace manager.
     * The launcher then shows one read-only summary instead of another host,
     * connection, directory, and executable form.
     */
    public void useManagedWorkspace(
            ...
            String destination,
            String port) {
        ManagedWorkspace selected = new ManagedWorkspace(
    ...
    private void applyManagedWorkspace(ManagedWorkspace selected) {
        ...
        try {
            ...
            String machine = selected.host() == ExecutionHost.LOCAL
                    ? "This computer"
                    : sshDestination.getText().strip()
                            + (sshPort.getText().isBlank()
                                    ? "" : ":" + sshPort.getText().strip());
            selectedWorkspace.setText(
                    selected.name() + " · " + machine + " · " + workspace.getText());
     ...
     * still loading, the explicit blank value wins the merge instead of briefly restoring the
     * prior command and overwriting this choice.
     */
    ...
    public java.util.OptionalInt sshPort() {
    ...
    public void rememberSshConnection() {
        ...
        SshConnectionProfile current = new SshConnectionProfile(
                sshDestination(), sshPort.getText(), workspace(), bazelExecutable());
        java.util.ArrayList<SshConnectionProfile> merged = new java.util.ArrayList<>();
        merged.add(current);
        for (SshConnectionProfile profile : sshProfiles) {
    ...
    public void setRunEnabled(boolean enabled) {
        ...
        forgetSshConnection.setEnabled(enabled && isRemote()
                && savedSshConnections.getSelectedIndex() > 0);
    ...
    private static LauncherStateStore.State mergeLoaded(
            ...
            LauncherStateStore.State current,
            Set<Setting> edited) {
        ExecutionHost mergedHost = edited.contains(Setting.EXECUTION_HOST)
                ? current.executionHost()
                : loaded.executionHost();
        ...
        return new LauncherStateStore.State(
                ...
                edited.contains(Setting.SSH_DESTINATION)
                        ? current.sshDestination() : loaded.sshDestination(),
    ...
    private static List<SshConnectionProfile> mergeProfiles(
            List<SshConnectionProfile> preferred,
            List<SshConnectionProfile> fallback) {
        java.util.LinkedHashMap<String, SshConnectionProfile> merged = new java.util.LinkedHashMap<>();
    ...
    private static final class SaveQueue {
        ...
        private synchronized CompletionStage<Void> closeWhenSettled(
        ...
        private synchronized void complete(LauncherStateStore.State completed, boolean success) {
            ...
            if (closeRequested && !success && !superseded) {
                closeCompletion.completeExceptionally(new IllegalStateException(
        ...
        private void completeCloseIfSettled() {
            if (closeRequested
                    && initialized
                    && inFlight == null
                    && Objects.equals(desired, persisted)) {
