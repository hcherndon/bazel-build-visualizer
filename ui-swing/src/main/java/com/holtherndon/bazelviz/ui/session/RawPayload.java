...
/**
 * The verbatim bytes of one journaled record, together with what kind of
 * record they are.
 *
 * <p>The {@link SourceKind} travels with the bytes because the inspector
 * cannot otherwise know how to read them: a binary BEP payload is a
 * {@code BuildEvent}, a JSON import record is protobuf-JSON text, and a BES
 * envelope is neither. Guessing by sniffing the first byte is exactly the kind
 * of assumption the format detector exists to avoid, and the journal frame
 * already records the answer (contract §1, source kind field).
 *
