...
/**
 * The one header every inspector wears: a title line, a subtitle line, and an overflow button that
 * opens the entity's cross-view actions.
 *
 * <h2>Why the actions are a menu and not a strip</h2>
 *
 * <p>Every inspector before this put {@link EntityActions#buttonStripFor a row of buttons} at the
 * header's right edge. A row of buttons has a preferred width that grows with the number of offers
 * and never shrinks, and every one of these headers lives in a pane the user can drag narrow.
 * {@code BorderLayout} hands its {@code EAST} child that full preferred width whatever the
 * container's own width is, so at a narrow split the strip walked left off its own panel: the
 * toolbar was, in the user's words, "entirely hidden unless the pane is pulled all the way to the
 * side".
 *
 * <p>One button of fixed width cannot do that. The offers move into the menu it opens — a menu is a
 * window, so it is bounded by the screen rather than by the pane — and the edge of the header stops
 * competing with the title for space. What shrinks at a narrow width is the <em>text</em>: the two
 * labels ellipsize and keep the whole string in a tooltip, so nothing is lost, only folded.
 *
 * <h2>Honest absence</h2>
 *
 * <p>The button exists only when {@link EntityActions} would actually offer something for the refs
 * in hand: no facility installed, no refs, or no wired command that applies means no button at all,
 * rather than a button that opens an empty rectangle. This mirrors {@link EntityActions#popupFor}'s
 * own contract, which is explicit that an empty menu must never be shown.
 *
 * <h2>Threading</h2>
 *
 * <p>EDT only, like every Swing component. Everything here is widget construction over values
 * already in hand; the navigation handler is where work begins, and it is the host's job to keep
 * that quick.
 */
public final class InspectorHeader extends JPanel {
  ...
  /**
   * The overflow button's face. A single ellipsis character, so the button is as narrow as a button
   * gets — its whole job is to not grow.
   */
  ...
  /**
   * The width the text block is guaranteed even at the narrowest: enough for an ellipsis and a
   * character or two of what was elided. It exists so the header's minimum size is a small number a
   * split pane can honour rather than the natural width of whatever label the build happened to
   * produce.
   */
  ...
  /**
   * Adds a control of the host's own to the fixed edge, right of the overflow button — the
   * Timeline's Close, the shared panel's legacy source-event button. It shares the edge's
   * guarantee: the text shrinks around it, it does not shrink.
   */
  ...
  /**
   * Adopts the shared cross-view actions. Until this runs the header shows no overflow button at
   * all, which is how a host that has not adopted the facility renders exactly as it did.
   *
   * @param omit commands never offered here — a host passes the one that would navigate to what is
   *     already on screen
   */
  ...
  /**
   * Whether the overflow menu currently offers {@code command}. Hosts ask so a bespoke button of
   * their own can yield rather than double an offer the menu already carries.
   */
   ...
   *
   * <p>The default would be the natural width of the title, which is the width of whatever label
   * the build produced — and an ancestor honouring it is an ancestor that refuses to narrow. The
   * edge is what must survive, so the floor is the edge plus room for an ellipsis.
   */
  @Override
  public Dimension getMinimumSize() {
    ...
    return new Dimension(
        insets.left + insets.right + edge.getPreferredSize().width + EDGE_GAP + TEXT_FLOOR,
  ...
  private JPopupMenu menu() {
    return entityActions == null ? new JPopupMenu() : entityActions.popupFor(refs, omit);
  }
