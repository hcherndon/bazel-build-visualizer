...
/**
 * Level-of-detail pyramid of per-bin aggregates over action spans (plan sections 14.1-14.3, Phase 0
 * spike scope).
 *
 * <p>Level 0 uses ~1 ms bins; each coarser level multiplies the bin width by {@value #LEVEL_GROWTH}
 * until one level covers the wall span with at most {@value #MAX_TOP_LEVEL_BINS} bins. Per bin the
 * index stores, in primitive arrays only: the number of spans starting in the bin, the summed
 * overlap duration of all spans intersecting the bin, the count of intersecting spans (active-count
 * estimate), and the count of intersecting failed spans. Top-category tracking is out of scope for
 * the spike.
 *
 * <p>The build streams the {@link SpanSource} exactly once. Because synthetic durations are
 * heavy-tailed (individual spans can cover 100k+ finest-level bins), the build never walks a span's
 * bins directly: range quantities use difference arrays (+1 at first bin, -1 after last) resolved
 * by one prefix-sum pass per level, so cost is O(spans x levels + totalBins), not O(spans x
 * binsPerSpan).
 *
 * <p>Instances are immutable after {@link #build} returns and safe to read from any thread, which
 * is what lets {@code TimelineCanvas} paint on the EDT without locking.
 */
public final class TimelineLodIndex {
   ...
   *
   * <p>Phase 0 counted 32: {@code starts} (int) + {@code overlap} (long) + the three difference
  ...
  /**
   * Hard cap on level-0 bins, derived from {@link #MAX_PYRAMID_BYTES} rather than picked as a round
   * number: bins are only a proxy for what actually matters, which is bytes. Beyond this the
   * level-0 bin width grows by {@link #LEVEL_GROWTH} instead, trading time resolution for a bounded
   * footprint. Tier 3 (a ~2,344 s wall) needs ~2.3M bins at 1 ms, so the benchmark tiers keep full
   * 1 ms resolution.
   */
  ...
  private TimelineLodIndex(
      long wallStartMicros,
      long wallEndMicros,
      long totalSpanCount,
      long[] binWidthMicros,
      int[] binCounts,
  ...
  /**
   * Streams {@code source} once and builds the pyramid over {@code [wallStartMicros,
   * wallEndMicros)}. Spans reaching outside the wall are clipped to it; spans entirely outside are
   * ignored. A clipped span's "start" is attributed to the bin containing its clipped start.
   */
  public static TimelineLodIndex build(
      SpanSource source, long wallStartMicros, long wallEndMicros) {
    ...
    long[] fed = new long[1];
    source.forEachSpan(
        (s, e, categoryIndex, flags, spanBytes) -> {
  ...
  /**
   * Finest level whose bins are at least {@value #TARGET_BIN_PIXELS} px wide at this scale; the
   * coarsest level when even it would be narrower (deep zoom-out).
   */
   ...
   *
   * <p>Zero means nobody said, not that nothing was cached. A session with no execution log has
   * zero here for every bin, and a timeline colouring by cache result must render that as unknown
   ...
   *
   * <p>Derived rather than stored, so it cannot disagree with the two numbers it comes from.
   */
   ...
   *
   * <p>A lower bound wherever a span reported none: spans with no byte count contribute nothing
   * rather than a guess, so this is "at least this many" and never "this many" (plan rule 13).
   */
   ...
   *
   * <p>This is a weaker claim than plan 14.3's "top mnemonics or category summary" and it is the
   * strongest one the build can make honestly. A Boyer-Moore vote costs two words per bin and
   * survives with a candidate that is a true majority only if one exists; proving which case
   * happened needs a second pass over the bin's members, which the single streaming pass does not
   * have. What the vote <em>can</em> establish for free is the special case where its counter never
   * dropped — every span was the same category — and that is what this reports.
   *
   * <p>It is not a consolation prize. Real builds spend long stretches doing one kind of work, so a
   * bin that is all {@code Javac} is common and saying so is worth more than a ranking that might
   * be wrong.
   */
