 ...
 *
 * <p>Plan 17.11 requires that full protobuf text be rendered for the selected event only. This
 * model is where that is enforced: the table's page fetches never touch payload bytes, and this
 * model reads exactly one payload per distinct selection.
 *
 * <p>"Per distinct selection" is doing real work. {@code JTable} fires selection events liberally —
 * while a drag is adjusting, when focus moves, when the model announces that a page arrived — and a
 * payload can be megabytes off disk. Re-selecting the row that is already shown is therefore a
 * no-op, and a selection that is superseded before its fetch starts is dropped instead of being
 * read and discarded.
 *
 * <h2>Threading</h2>
 *
 * <p>{@link #select} is called on the EDT and returns immediately. The read runs on {@code
 * fetchExecutor} — single-threaded, owning the {@link SessionReader}, separate from the table's
 * page executor so a slow journal read cannot stall scrolling. Results reach listeners through
...
public final class EventInspectorModel {
  ...
  public EventInspectorModel(
      SessionReader reader, Executor fetchExecutor, Executor uiDispatcher, SessionInfo session) {
    this(
        reader,
        fetchExecutor,
        uiDispatcher,
  ...
  /**
   * Raw payload reads performed since this model was created. The inspector's central promise is
  ...
  /**
   * Selects {@code eventId}, reading its detail and payload off the EDT. Selecting the event that
  ...
  private EventInspection load(long eventId) {
    try {
      ...
      if (detail.isEmpty()) {
        return EventInspection.failed(
            eventId, "There is no event with row id " + eventId + " in this session.");
      }
      EventRow row = EventRow.of(detail.get());
      payloadFetches.incrementAndGet();
      RawPayload payload = reader.rawPayload(row.rawLocation());
      RawPayloadRenderer.Rendered rendered = RawPayloadRenderer.render(payload, row.decodeStatus());
