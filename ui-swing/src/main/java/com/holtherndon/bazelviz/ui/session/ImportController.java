...
/**
 * Drives one {@link BepImporter} run from the UI: off the EDT, cancellable,
 * and reporting through {@link Listener} on whichever dispatcher the caller
 * supplies.
 *
 * <h2>Threads</h2>
 *
 * <ul>
 *   <li>{@link #start} and {@link #cancel} are called on the EDT and return
 *       immediately. Neither touches the filesystem.</li>
 *   <li>The import itself — detection, hashing, copying, parsing, SQL — runs on
 *       {@code worker}.</li>
 *   <li>Every {@link Listener} callback is handed to {@code uiDispatcher}. In
 *       the application that is {@code SwingUtilities::invokeLater}; in the
 *       tests it is a direct executor, which is what makes this class testable
 *       headlessly.</li>
 * </ul>
 *
 * <h2>Cancellation</h2>
 *
 * <p>{@link #cancel} sets a flag the importer polls between records. It stops
 * on a record boundary, writes a final checkpoint and leaves the session
 * resumable — so cancelling is not the same as discarding, and the resulting
 * session can be reopened or resumed. The button therefore really cancels
 * rather than merely detaching the UI from a run that keeps going.
 */
public final class ImportController {
     ...
     *
     * @return false when a run is already in flight; the caller must not start
     *     a second one, because two imports writing two sessions would fight
     *     over the same progress view
     */
    public boolean start(Path source, Listener listener) {
        Objects.requireNonNull(source, "source");
        Objects.requireNonNull(listener, "listener");
        return begin(source, false, listener, () ->
                importer.importFile(
                        source,
                        SessionId.random(),
    ...
    /**
     * Resumes the interrupted import in {@code sessionRoot} — the UI face of
     * Phase 1 exit criterion 2. The source is not re-read from the beginning;
     * see {@link BepImporter#resume}.
     */
    public boolean resume(Path sessionRoot, Listener listener) {
        Objects.requireNonNull(sessionRoot, "sessionRoot");
        Objects.requireNonNull(listener, "listener");
        return begin(sessionRoot, true, listener, () ->
                importer.resume(
                        sessionRoot,
                        sample -> publish(listener, sample),
                        cancelRequested::get));
    ...
    /**
     * Folds a sample into the model on the importing thread — where the elapsed
     * intervals the rate is computed from are the import's own, not the EDT's
     * queueing delay — and hands the immutable snapshot to the UI.
     */
