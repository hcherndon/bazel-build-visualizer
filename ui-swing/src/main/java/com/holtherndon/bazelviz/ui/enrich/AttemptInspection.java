 ...
 *
 * <p>Pure, for the same reason {@code ActionInspection} is: the awkward
 * decisions here — what to say when there is a duration and no start, how to
 * word an ambiguous correlation, which timing components to show — are exactly
 * the places the tool could quietly say something untrue, and they are
...
public final class AttemptInspection {
    ...
    public static Inspection of(AttemptRow row) {
        Inspection.Builder builder = new Inspection.Builder(title(row))
                .subtitle(subtitle(row));

        builder.section("Execution")
                .field(EntityFormat.field("Runner", row.runner()))
                .field("Cache hit", row.cacheHit() ? "yes" : "no")
                .field(row.exitCode().isPresent()
                        ? Inspection.Field.of("Exit code",
                                Integer.toString(row.exitCode().getAsInt()))
                        : Inspection.Field.unknown("Exit code",
                                "this spawn's record carries no exit code"))
     ...
     *
     * <p>Only the components that were measured. A spawn that ran locally never
     * queued, never uploaded and never fetched, so rows reading "Queue: 0ms"
     * would be answers to questions nobody asked and would suggest measurements
     * that were not taken.
     *
     * <p>The remainder between the total and the parts is shown as unaccounted
     * rather than folded into execution — {@code total_time} is measured
     * separately from its components and the difference is real.
     */
    private static void timingSection(Inspection.Builder builder, AttemptRow row) {
        builder.section("Timing")
                .field(EntityFormat.durationField("Total", row.elapsed()));

        // A duration with no position is the whole story on Bazel 6.5.0, which
        // never reports a spawn's start (S2).
        builder.field(row.startMicros().isPresent()
                ? Inspection.Field.of("Start",
                        EntityFormat.count(row.startMicros()) + " µs")
                : Inspection.Field.unknown("Start", row.startUnknownReason().orElse(
                        "this spawn's record carries no start time")));

        List<AttemptRow.Timing.Component> measured = row.timing().measuredComponents();
        if (measured.isEmpty()) {
            builder.field(Inspection.Field.unknown("Breakdown",
                    "this Bazel version reports no per-phase timing for a spawn"));
            return;
        }
        ...
        row.timing().unaccountedMicros().ifPresent(remainder ->
                builder.field("Unaccounted", remainder + " µs"));
    }

    /**
     * What this attempt is attached to, and why.
     *
     * <p>Plan 24 makes "ambiguous correlations remain visible" an exit
     * criterion, and this is where it becomes visible. The note is always
     * shown when there is one, including for the healthy cases, because "this
     * is a test execution and tests attach to tests" is information the reader
    ...
    private static void correlationSection(Inspection.Builder builder, AttemptRow row) {
        builder.section("Correlation")
                .field("Attached to", describe(row.correlation()));
    ...
    private static void outputsSection(Inspection.Builder builder, AttemptRow row) {
        builder.section("Outputs")
                .field("Produced", Long.toString(row.producedOutputs()));
    ...
    private static java.util.Optional<String> optionalText(OptionalLong value) {
        return value.isPresent()
                ? java.util.Optional.of(Long.toString(value.getAsLong()))
