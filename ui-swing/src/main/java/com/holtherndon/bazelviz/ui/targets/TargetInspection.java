 ...
 *
 * <p>"What did analysis say about this label" and "what happened when it was built in this
 * configuration" are different, and a target can have an answer to the first and none to the second
 * — which is the entire content of a build interrupted during analysis. The inspector shows both
 * under their own names rather than merging them into one status that would be wrong for one of
 * them.
 *
 * <h2>Tags keep their source</h2>
 *
 * <p>From Bazel 7.6.1 the completion event appends synthetic tags — the test size, the timeout,
 * {@code noflaky} — that the user never wrote in a BUILD file. They are listed separately from the
 * ones analysis reported, so a reader can tell which are theirs.
 */
public final class TargetInspection {
  ...
  public static Inspection of(
    ...
    Inspection.Builder builder =
        new Inspection.Builder(target.label())
            .subtitle(
                target
                    .outcome()
                    .map(Enum::name)
    ...
    target.bepEventId().ifPresent(eventId -> builder.ref(new EntityRef.EventId(eventId)));
    ...
    builder
        ...
        .field(
            target.targetKind().isPresent()
                ...
                : Inspection.Field.unknown("Kind", "no analysis payload arrived for this target"))
    ...
    builder
        ...
        .field(
            target.outcome().isPresent()
                ? Inspection.Field.of("Outcome", target.outcome().orElseThrow().name())
                : Inspection.Field.unknown(
                    "Outcome", "no completion event arrived; the build stopped first"));

    if (!tags.isEmpty()) {
      builder.section("Tags");
      for (TargetQueries.Tag tag : tags) {
        builder.field(tag.tag(), "from " + tag.fromEvent().toLowerCase(Locale.ROOT) + " event");
      }
    }
    ...
    if (!groups.isEmpty()) {
      ...
      for (TargetQueries.OutputGroup group : groups) {
        ...
        String detail = group.rootDepsetId().map(id -> "file set " + id).orElse("no file set");
        builder.field(group.name(), group.incomplete() ? detail + " — incomplete" : detail);
      }
    }
