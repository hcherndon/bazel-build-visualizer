...
/**
 * The read service the ad hoc query card is written against — the same
 * arrangement {@link SessionReader} makes for the Events view and
 * {@link EntityReader} for the Phase 3 tables, and for the same reason (plan
 * rule 19: UI components depend on service interfaces, not on SQLite
 * implementation classes).
 *
 * <p>No {@code Connection}, no {@code Statement} and no {@code SQLException}
 * crosses this line. What does cross is the same kind of value record the other
 * readers already hand back.
 *
 * <h2>Threading</h2>
 *
 * <p>Every method except {@link #cancel()} blocks on SQL, so <b>none of them
 * may be called on the Swing EDT</b>. One reader is bound to one thread.
 *
 * <p>{@link #cancel()} is the exception on purpose: it is
 * {@code sqlite3_interrupt}, it returns immediately, and it is what the Cancel
 * button calls from the event thread. Without a cancel that is safe to call
 * from the EDT there is no way to abandon a query except by waiting for it.
 */
public interface QueryReader extends AutoCloseable {

    /**
     * Every table and view in the session database, with its columns, read
     * from the file rather than from a generated list.
     *
     ...
     * @throws com.holtherndon.bazelviz.storage.query.SqlNotAllowedException if
     *     the text is not one read-only statement — nothing was executed
     * @throws com.holtherndon.bazelviz.storage.query.QueryFailedException if
     *     SQLite refused it or it was stopped
     */
     ...
     * the EDT. Safe to call when nothing is running.
     */
    void cancel();
