 ...
 *
 * <p>Everything unknown renders as one em dash, everywhere, so a user scanning
 * a column can tell at a glance which cells are values and which are absences.
 * The <em>reason</em> for an absence goes in the tooltip and in the inspector,
 * not in the cell: a table column wide enough for "not reported by this Bazel
 * version" is a table column too wide for the number it usually holds.
 *
 * <p>Durations are formatted with a unit that keeps three significant figures,
 * because an actions table sorted by duration spans microseconds to minutes and
 * a single unit makes one end of it unreadable.
 */
public final class EntityFormat {
     ...
     *
     * <p>"unknown" rather than "no", because the two are asked about exactly
     * the fields where the difference matters — whether the build succeeded,
     * whether all actions were published.
     */
    ...
    public static Inspection.Field field(String name, Optional<String> value) {
        return value.filter(text -> !text.isEmpty())
