...
/**
 * The import progress view: what phase the import is in, how much it has read,
 * how fast, and a cancel button that really cancels.
 *
 * <p>All rendering, no logic — {@link ImportProgressModel} decides what is
 * known and this panel shows exactly that. The bar is determinate only when
 * {@link ImportProgressModel.Snapshot#determinate()} says a real percentage
 * exists; during detection and while the source is being hashed and copied
 * there is no total to divide by, so the bar runs indeterminate rather than
...
public final class ImportProgressPanel extends JPanel {
    ...
    public void update(ImportProgressModel.Snapshot snapshot) {
        ...
        bytesValue.setText(snapshot.totalBytes().isPresent()
                ? EventValueFormat.bytes(snapshot.bytesRead()) + " of "
