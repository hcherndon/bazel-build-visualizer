 ...
 *
 * <p>The whole point of {@code SessionReader} being an interface (plan rule 19) is here: every
 * model in {@code ui.events} can be driven headlessly, with exact control over what the store
 * contains, without a SQLite file or a journal on disk. It also counts calls and records which
 * thread made them, which is how the tests assert that the inspector fetches once and that it never
 * fetches on the EDT.
 *
 * <p>Deliberately keyset-only, exactly like the real one: there is no way to ask it for "row N", so
 * a test that passes here cannot be relying on an offset-shaped shortcut the real store does not
 * offer.
 */
final class FakeSessionReader implements SessionReader {
  ...
  private EventSummary summary(long id, long sequence, DecodeStatus status) {
    long hash = 0x1000 + id;
    byte[] payloadBytes = ("payload-" + id).getBytes(StandardCharsets.UTF_8);
  ...
  /**
   * Drops every event past {@code lastKeptId}, simulating a store that changed underneath an
   * already-open view.
   */
  ...
  /**
   * Renumbers every event by {@code delta}, keeping the row count and the ordering. Nothing real
   * does this; it is the cheapest way to make a store that passed the density probe stop agreeing
   * with it, which is the situation {@code reportPredictionMismatch} exists for.
   */
  ...
  public Optional<EventDetail> event(long id) {
    ...
    for (EventSummary summary : summaries) {
      if (summary.id() == id) {
        return Optional.of(
            new EventDetail(summary, locations.get(id), Optional.ofNullable(identities.get(id))));
      }
    }
