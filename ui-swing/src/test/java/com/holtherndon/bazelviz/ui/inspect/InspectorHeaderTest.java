 ...
 *
 * <p>The thing being pinned down is a layout promise: at any width the pane can be dragged to, the
 * actions stay reachable and the text gives way. Before this widget the opposite happened — the
 * button strip kept its full width and walked off the panel, so the actions were "entirely hidden
 * unless the pane is pulled all the way to the side". A layout promise needs measuring rather than
 * describing, so these tests lay the header out at a narrow width and read the resulting bounds.
 *
 * <p>Headless: nothing here realizes a peer, so the layout is driven by {@code doLayout} directly —
 * {@code validate} is a no-op without one.
 */
class InspectorHeaderTest {
  ...
  void narrowWidthShrinksTheTextNotTheButton() throws Exception {
    ...
    onEdt(
        () -> {
          header.installEntityActions(
              new EntityActions(EnumSet.of(EntityActions.Command.OPEN_TARGET), new Recorder()),
              Set.of());
          header.show(
              LONG_TITLE,
              Optional.of("Javac · SUCCEEDED"),
    ...
    Point buttonAt =
        onEdt(() -> SwingUtilities.convertPoint(header.overflowForTest(), 0, 0, header));
    assertThat(buttonAt.x)
        .as("and stays inside the header rather than walking off its left edge")
        .isGreaterThanOrEqualTo(0);
    assertThat(buttonAt.x + buttonWidth).as("and off its right edge").isLessThanOrEqualTo(NARROW);
    ...
    onEdt(
        () -> {
          header.installEntityActions(
              ...
              Set.of());
          header.show(
              LONG_TITLE, Optional.of(LONG_TITLE), List.of(new EntityRef.TargetLabel(LONG_TITLE)));
          return null;
        });
  ...
  void theButtonAppearsOnlyWithOffers() throws Exception {
    ...
    onEdt(
        () -> {
          header.show("//pkg:a", Optional.empty(), List.of(new EntityRef.TargetLabel("//pkg:a")));
          return null;
        });
    ...
    onEdt(
        () -> {
          ...
          header.show("Select an event to inspect its raw bytes.", Optional.empty(), List.of());
          return null;
        });
    assertThat(onEdt(() -> header.overflowForTest().isVisible()))
    ...
    onEdt(
        () -> {
          header.show("event 12", Optional.empty(), List.of(new EntityRef.EventId(12)));
          return null;
        });
    assertThat(onEdt(() -> header.overflowForTest().isVisible()))
        .as("a ref no wired command acts on means no button")
        .isFalse();
    assertThat(onEdt(() -> header.overflowMenuForTest().getComponentCount())).isZero();
  ...
  void trailingControlsRideTheEdge() throws Exception {
    InspectorHeader header = onEdt(InspectorHeader::new);
    JButton close = onEdt(() -> new JButton("Close"));
    onEdt(
        () -> {
  ...
  /**
   * Lays a container out at a size, top down. {@code validate()} is a no-op on a component with no
   * peer, which every component in a headless test is, so the layout managers are run by hand
   * instead.
   */
