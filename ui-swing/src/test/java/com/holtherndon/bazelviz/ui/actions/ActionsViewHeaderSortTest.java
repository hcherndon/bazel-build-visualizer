...
/**
 * The Actions tab's one sort state, worn by two faces: the toolbar combo and
 * the table headers. A header click must move the combo and reload through
 * the backend ordering; a combo change must move the header indicator; and
 * the cycle's third click must land on the default — arrival ascending —
 * because that is the ordering the view opens with.
 *
 * <p>No client-side {@code RowSorter} is ever involved: every transition
 * below is observed as a new {@link ActionRowSource} built with the chosen
 * {@link ActionSort}, which is the honest way to sort five million rows.
 */
final class ActionsViewHeaderSortTest {
    ...
    @Test
    @Timeout(180)
    @DisplayName("header clicks drive the backend sort and the toolbar follows; the combo drives the header back")
    void headerAndComboAreOneState(@TempDir Path temporary) throws Exception {
        ...
        await(() -> onEdt(() -> view.rowSourceForTest().sort() == ActionSort.ARRIVAL
                && !view.rowSourceForTest().descending()));
        ...
        await(() -> onEdt(() -> view.rowSourceForTest().sort() == ActionSort.MNEMONIC
                && view.rowSourceForTest().descending()));
        assertThat(onEdt(() -> view.headerInteractionsForTest().currentSort()))
    ...
    private static javax.swing.JTable actionsTable(ActionsView view) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (java.awt.Component child : current.getComponents()) {
                if (child instanceof javax.swing.JTable table
                        && table.getModel() == view.tableModelForTest()) {
                    return table;
                }
