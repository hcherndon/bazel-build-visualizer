 ...
 *
 * <p>Plan 17.7: "never query SQLite from {@code paintComponent}". The same check the timeline gets,
 * for the same reason — reviewing a paint method works once, and a test that inspects what the
 * painting classes can reach works every time someone adds a field.
 *
 * <p>Deliberately about reachability rather than behaviour. A paint method that happens not to
 * query today but holds a {@code Connection} is one refactor away from doing so, and the design's
 * promise is that the refactor should be impossible rather than merely discouraged.
 */
final class GraphPaintIsolationTest {
  ...
  void thePaintedTypesCannotQuery() {
    for (Class<?> painted :
        List.of(
            GraphModel.class, GraphSpatialIndex.class, GraphTransform.class, GraphColours.class)) {
