...
/**
 * {@code TargetsView.revealLabel}: random access into the lazily loaded tree
 * by label — the landing point of the shared "Open target" action.
 *
 * <p>The interesting part is the asynchrony: the package's children are not
 * loaded until the reveal expands it, so selection has to wait for the load
 * it triggered, and a label the session never declared has to say so instead
 * of selecting nothing silently.
 */
