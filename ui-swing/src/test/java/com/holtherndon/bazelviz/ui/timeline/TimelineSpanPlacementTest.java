 ...
 *
 * <p>{@code paintSpans} used to place span {@code i} at row {@code i % lanes}: the model's lane
 * list and the painted spans were two unconnected data sets, so any refetch — every pan, every zoom
 * — changed each span's {@code i} and shuffled the whole plot vertically. Now every span carries
 * its lane key and the painter joins it to the model's lanes, so these tests refetch the window in
 * different orders and subsets and require every span to stay put.
 */
final class TimelineSpanPlacementTest {
  ...
  void staleOverlappingWindowDoesNotFlashToDensity() {
    ...
    canvas.getMouseWheelListeners()[0].mouseWheelMoved(
        new MouseWheelEvent(
            canvas,
  ...
  void panAndZoomKeepRows() {
    ...
    JComponent canvas = view.canvasForTest();
    canvas.getMouseListeners()[0].mousePressed(
        new MouseEvent(
            canvas,
            ...
            false,
            MouseEvent.BUTTON1));
    canvas.getMouseMotionListeners()[0].mouseDragged(
        new MouseEvent(
            canvas,
  ...
  void tinySpanIsReadableAndHittable() {
    ...
    assertThat(paint.height)
        ...
        .isGreaterThan(1);
    assertThat(view.statusText())
        .contains("1 short span uses a " + TimelineView.SHORT_SPAN_MARKER_WIDTH + " px needle");
