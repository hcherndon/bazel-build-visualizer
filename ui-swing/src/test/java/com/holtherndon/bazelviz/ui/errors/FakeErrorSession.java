...
/**
 * A session the Errors card can open, holding console events and nothing else of substance.
 *
 * <p>Two readers, exactly as the real source hands out: an {@link EntityReader} for the rows and a
 * {@link SessionReader} for the journal bytes behind them. Both count the thread they were called
 * on, which is how the suite asserts that no console read ever happens on the EDT.
 *
 * <p>A console event can be registered <em>with</em> its payload or without one. Without is not a
 * broken fixture: a redacted session keeps its database and drops its {@code raw/} directory, so
 * its rows name frames that are no longer there, and the view has to say so rather than fail.
 */
final class FakeErrorSession implements SessionSource {
    ...
    public RawPayload rawPayload(RawLocation location) {
      ...
      if (payload == null) {
        throw new SessionDataException(
            "no journal frame at segment " + location.segment() + " offset " + location.offset());
      }
      return payload;
    }
  ...
  private final class Rows implements EntityReader {
    ...
    public CountedPage<TargetRow, TargetQueries.TargetAnchor> targetsInPackagePage(
        String packagePath,
        String labelText,
        Optional<TargetQueries.TargetAnchor> after,
        int limit) {
      return emptyPage();
    }
