...
/**
 * A page that fails to load must be visibly different from a page that is
 * still loading, and must not be retried forever. Rendering a failure as the
 * loading placeholder is a silent drop.
 */
final class PagedTableModelFailureTest {
    ...
    private static PagedTableModel<String> model(RowSource<String> source, Executor executor) {
        return new PagedTableModel<>(source,
                List.of(new ColumnSpec<>("value", v -> v)), executor, PAGE_SIZE, CACHE_CAPACITY);
    }
