...
/**
 * The Actions tab's adoption of the shared navigation actions: its rows and inspection carry the
 * three identities the facility acts on, and the "show actions for this target" command lands here
 * as a visible, clearable label filter rather than an invisible narrowing.
 */
class ActionsViewEntityActionsTest {
   ...
   *
   * <p>This is how the label-chip fix is actually verified: a real running window relays out
   * asynchronously once {@code revalidate()} is called, but that pump requires a genuine {@code
   * Window} ancestor, and headless forbids creating one at all ({@code new JFrame()} throws {@code
   * HeadlessException} even unshown). {@code JComponent.setVisible(true)} already revalidates the
   * component <em>itself</em> as a side effect -- that is not the bug fixed here, and is not enough
   * on its own: it is the chip's <em>toolbar</em>, laid out while the chip was invisible, that has
   * to be told to redo its {@code FlowLayout} for the chip to get real bounds. Recording identities
   * (rather than actually laying anything out) is observable without a display and distinguishes
  ...
  private static final class RecordingRepaintManager extends RepaintManager {
    final Set<Component> requested = Collections.newSetFromMap(new IdentityHashMap<>());
  ...
  void inspectionCarriesTheRefs(@TempDir Path temporary) throws Exception {
    ...
    try (EntityReader reader = opened.openEntityReader()) {
      List<ActionRow> rows =
          reader.firstActionPage(ActionFilter.NONE, ActionSort.ARRIVAL, false, 50);
      assertThat(rows).isNotEmpty();
      ActionRow labelled =
          rows.stream().filter(row -> row.label().isPresent()).findFirst().orElseThrow();

      List<EntityRef> refs = ActionInspection.of(labelled).refs();
      assertThat(refs).contains(new EntityRef.ActionId(labelled.id()));
      assertThat(refs).contains(new EntityRef.TargetLabel(labelled.label().orElseThrow()));
      assertThat(refs).contains(new EntityRef.EventId(labelled.bepEventId().orElseThrow()));
    } finally {
      opened.close();
    }
  }
  ...
  void labelFilterNarrowsVisiblyAndClears(@TempDir Path temporary) throws Exception {
    ...
    try (EntityReader reader = opened.openEntityReader()) {
      label =
          reader.firstActionPage(ActionFilter.NONE, ActionSort.ARRIVAL, false, 50).stream()
    ...
    try {
      ...
      await(() -> onEdt(() -> view.rowSourceForTest().filter().labelContains().isPresent()));
      ...
      assertThat(onEdt(() -> probe.requested.contains(toolbar)))
          .as(
              "filterToLabel must ask the chip's own toolbar to re-lay-out -- relying"
      ...
      assertThat(onEdt(() -> view.rowSourceForTest().filter().labelContains())).hasValue(label);
      ...
      Rectangle chipBounds = onEdt(() -> view.labelChipComponentForTest().getBounds());
      ...
      await(() -> onEdt(() -> view.rowSourceForTest().filter().labelContains().isEmpty()));
      ...
      assertThat(onEdt(() -> probe.requested.contains(toolbar)))
          .as("clearLabelFilter must also ask the toolbar to re-lay-out when the chip" + " hides")
          .isTrue();
  ...
  @Timeout(180)
  @DisplayName(
      "a loaded row's refs feed the shared menu, and the toolbar buttons dispatch through it")
  void rowsAndButtonsGoThroughTheFacility(@TempDir Path temporary) throws Exception {
    ...
    onEdt(
        () -> {
          var menu = actions.popupFor(refs, Set.of(EntityActions.Command.REVEAL_ACTION));
