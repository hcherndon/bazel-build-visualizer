 ...
 *
 * <p>A dependency tree built from a query that analysed a different configuration looks exactly
 * like a correct one. These sentences are the only thing between the user and a confident wrong
 * answer, which is why they are tested rather than merely written.
 */
final class GraphSourceSummaryTest {
  ...
  private static GraphQueries.GraphSource source(
      String state, ConfigurationMatch match, String detail, OptionalLong unresolvedArtifacts) {
  ...
  void mismatchIsLoudAndNotHidden() {
    GraphQueries.GraphSource mismatched =
        source(
            "SUCCEEDED",
            ConfigurationMatch.MISMATCHED,
  ...
  void fallbackScopeIsLoud() {
    GraphQueries.GraphSource fallback =
        new GraphQueries.GraphSource(
            "DECLARED_ACTIONS",
            Optional.of("bazel aquery --query_file=aquery.query"),
            "SUCCEEDED",
  ...
  void unresolvedPathsAreWarnedAbout() {
    GraphQueries.GraphSource incomplete =
        source("SUCCEEDED", ConfigurationMatch.EXACT, null, OptionalLong.of(2));
  ...
  void missingCompletenessMetadataIsUnknown() {
    GraphQueries.GraphSource old =
        source("SUCCEEDED", ConfigurationMatch.EXACT, null, OptionalLong.empty());
  ...
  void coverageIsStated() {
    String text = GraphSourceSummary.describe(source("SUCCEEDED", ConfigurationMatch.EXACT, null));
  ...
  void failedIsNeverPreferred() {
    assertThat(
            GraphSourceSummary.preferred(
                List.of(source("FAILED", ConfigurationMatch.UNKNOWN, null))))
        .isEmpty();
