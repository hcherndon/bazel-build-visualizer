...
/**
 * The Targets card's header toolbar: the shared navigation vocabulary offered
 * for the selected target, and honest about itself when it cannot be used.
 *
 * <p>The toolbar exists instead of a row menu because it can say <em>why</em> a
 * jump is unavailable, so most of what is worth testing here is the disabled
 * states: nothing selected, no source event on the row, and a command with no
 * read path behind it. A button that is merely dark is not the point; a button
 * that is dark and names its reason is.
 */
class TargetsViewWiringTest {
    ...
    void nothingSelectedDisablesWithReasons() throws Exception {
        ...
        for (EntityActions.Command command : onEdt(() -> view.toolbarCommandsForTest())) {
            ...
            assertThat(onEdt(() -> button.isEnabled()))
                    .as("%s with no selection", command)
                    .isFalse();
            assertThat(onEdt(() -> button.getToolTipText()))
                    .as("%s must say why it is off", command)
                    .isNotBlank();
        }
        assertThat(onEdt(() -> view.toolbarButtonForTest(
                        EntityActions.Command.OPEN_IN_TREE).getToolTipText()))
                .contains("Select a target");

        // A button nobody can press dispatches nothing.
        SwingUtilities.invokeAndWait(() -> view.toolbarButtonForTest(
                EntityActions.Command.OPEN_IN_TREE).doClick());
    ...
    void unwiredCommandsExplainThemselves() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            view.installEntityActions(
                    new EntityActions(AS_WIRED_IN_THE_WINDOW, new Recorder()));
            held.set(view);
        });
        TargetsView view = held.get();
        ...
        String tooltip = onEdt(() -> view.toolbarButtonForTest(
                EntityActions.Command.SHOW_EVENTS_FOR_LABEL).getToolTipText());
        assertThat(tooltip).contains("events-by-label read path");
        assertThat(onEdt(() -> view.toolbarButtonForTest(
                        EntityActions.Command.SHOW_EVENTS_FOR_LABEL).isEnabled()))
                .isFalse();
    }
    ...
    void withoutTheFacilityNothingIsOffered() throws Exception {
        ...
        JButton button = onEdt(() -> view.toolbarButtonForTest(
                EntityActions.Command.SHOW_ACTIONS_FOR_LABEL));
    ...
    void selectionArmsTheToolbarAndDispatches(@TempDir Path temporary) throws Exception {
        ...
        for (EntityActions.Command command : List.of(
                ...
                EntityActions.Command.SHOW_ACTIONS_FOR_LABEL)) {
            SwingUtilities.invokeAndWait(
                    () -> view.toolbarButtonForTest(command).doClick());
        }
        SwingUtilities.invokeAndWait(() -> view.toolbarButtonForTest(
                EntityActions.Command.SHOW_SOURCE_EVENT).doClick());

        assertThat(recorder.commands).containsExactly(
                EntityActions.Command.OPEN_IN_TREE,
                EntityActions.Command.OPEN_IN_GRAPH,
                EntityActions.Command.SHOW_ACTIONS_FOR_LABEL,
                EntityActions.Command.SHOW_SOURCE_EVENT);
        assertThat(recorder.refs).containsExactly(
        ...
        assertThat(onEdt(() -> view.toolbarButtonForTest(
                        EntityActions.Command.OPEN_IN_TREE).isEnabled()))
                .isFalse();
        opened.close();
    }
