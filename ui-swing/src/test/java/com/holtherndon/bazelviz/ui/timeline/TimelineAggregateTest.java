 ...
 *
 * <p>Plan 14.3 lists what each bin stores. The interesting half is the distinction between a zero
 * and an unknown: a session with no execution log knows nothing about caching, and a bin that
 * reported that as "no cache hits" would colour a whole build as cache misses.
 */
final class TimelineAggregateTest {
  ...
  void unknownBytesAreNotZero() {
    TimelineLodIndex index =
        indexOf(new Span(0, 100, 1, 0, 4_096), new Span(0, 100, 1, 0, SpanSource.BYTES_UNKNOWN));
  ...
  void uniformBinsNameTheirCategory() {
    TimelineLodIndex index =
        indexOf(new Span(0, 100, 7, 0, 0), new Span(0, 100, 7, 0, 0), new Span(0, 100, 7, 0, 0));
    ...
    TimelineLodIndex index =
        TimelineLodIndex.build(
            sourceOf(
                List.of(
                    new Span(
                        0, 100, 4, SpanSource.FLAG_CACHE_KNOWN | SpanSource.FLAG_CACHE_HIT, 10),
                    new Span(
                        5_000,
                        5_100,
                        4,
  ...
  void failuresAreCounted() {
    TimelineLodIndex index =
        indexOf(new Span(0, 100, 1, SpanSource.FLAG_FAILED, 0), new Span(0, 100, 1, 0, 0));
  ...
  void cacheCountsAreStartAttributed() {
    ...
    TimelineLodIndex index =
        indexOf(
            new Span(0, 1_000_000, 1, SpanSource.FLAG_CACHE_KNOWN | SpanSource.FLAG_CACHE_HIT, 0));
