 ...
 *
 * <p>Written against a real session database and the real panel, because five of the six are
 * statements about what a user sees rather than about what a function returns. The sixth — that
 * panning and selection stay responsive — is measured in {@code GraphCanvasScaleTest} at the plan's
 * own limits, and is only restated here.
 *
 * <p>The fixture is a 60-action chain across three packages. Small enough to assert exactly, big
 * enough that a limit of two is a real refusal rather than a contrivance.
 */
final class Phase7ExitCriteriaTest {
  ...
  void aggregationLosesNothing() throws Exception {
    ...
    assertThat(panel.canvas().model().clustering().clusteredEdges()).isEqualTo(ACTIONS - 1);
