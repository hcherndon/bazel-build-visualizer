 ...
 *
 * <p>The tests are about sentences rather than layout, because the failure mode here is a true
 * number that reads as a defect: "4 of 13 actions have attempt data" is correct and alarming, and
 * the sentence explaining why is the whole value of the panel.
 */
final class CoverageViewTest {
  ...
  void partialCoverageIsExplained() throws Exception {
    List<String> text =
        render(
            new EnrichmentQueries.Coverage(
                13, 4, 4, 0, 0, Map.of(AttemptCorrelation.MATCHED_BY_OUTPUT, 4L)));

    assertThat(text).contains("13", "4");
    assertThat(joined(text)).contains("never start a subprocess").contains("not missing data");
  }
    ...
    List<String> text =
        render(
            new EnrichmentQueries.Coverage(
                ...
                0,
                Map.of(
                    AttemptCorrelation.MATCHED_BY_OUTPUT,
                    2L,
  ...
  void flooredAnchorIsSurfaced() throws Exception {
    ...
    onEdt(
        () -> {
          view.render(
              new CoverageView.Snapshot(
                  new EnrichmentQueries.Coverage(13, 4, 4, 900, 900, Map.of()),
                  List.of(),
                  List.of(
                      new EnrichmentQueries.Phase(
                          0, "Launch Blaze", -19000, OptionalLong.of(0), true)),
                  Optional.of(ProfileAnchor.flooredStart(1787434075000000L, "profile_finish_ts")),
                  List.of(),
                  List.of()));
          return null;
        });
        ...
        () -> {
          view.render(
              new CoverageView.Snapshot(
                  ...
                  Optional.empty(),
                  List.of(
                      new EnrichmentQueries.CriticalPathComponent(
                          0, "action 'Executing genrule //pkg:gen_a'", OptionalLong.of(36_440))),
                  List.of()));
          return null;
        });
        ...
        () -> {
          ...
          assertThat(view.emptyStateForTest().getComponent(0))
              .isInstanceOfSatisfying(
                  JLabel.class, message -> assertThat(message.isEnabled()).isFalse());
  ...
  void cardsUseReadableResponsiveRows() throws Exception {
    List<String> criticalDescriptions = new ArrayList<>();
    List<EnrichmentQueries.CriticalPathComponent> criticalComponents = new ArrayList<>();
    for (int index = 0; index < 6; index++) {
      String description =
          "action 'Compiling a deliberately long generated source for "
              + "//pkg/subpackage:target_"
              + index
              + " from bazel-out/darwin-fastbuild/bin/"
              + "pkg/subpackage/generated/descriptor_"
              + index
              + ".java under the selected Java toolchain [for tool]'";
      criticalDescriptions.add(description);
      criticalComponents.add(
          new EnrichmentQueries.CriticalPathComponent(index, description, OptionalLong.of(8_000)));
    }
    CoverageView view = onEdt(CoverageView::new);
    onEdt(
        () -> {
          ...
          for (String text : criticalDescriptions) {
            JTextArea description = findTextArea(cards, text);
            assertThat(description).isNotNull();
            assertThat(description.getWidth()).isGreaterThan((int) (critical.getWidth() * 0.65));
          ...
          JTextArea duration = findTextArea(cards, "8.0 ms");
          assertThat(duration).isNotNull();
          assertThat(duration.getWidth()).isLessThan((int) (critical.getWidth() * 0.34));
          assertThat(critical.getHeight())
              .isGreaterThanOrEqualTo(critical.getPreferredSize().height);
  ...
  void longValuesWrapWithoutHorizontalOverflow() throws Exception {
    String error =
        "Expected a profile object but received a deliberately long malformed"
    ...
    onEdt(
        () -> {
          ...
          JTextArea errorArea = findTextArea(view.cardsForTest(), error);
  ...
  private static JTextArea findTextArea(Container root, String text) {
    for (Component child : root.getComponents()) {
      ...
      if (child instanceof Container nested) {
        JTextArea found = findTextArea(nested, text);
        if (found != null) {
          return found;
        }
      }
