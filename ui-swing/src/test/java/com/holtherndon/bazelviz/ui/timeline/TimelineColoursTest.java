 ...
 *
 * <p>The cache and runner modes describe facts most sessions do not have. A mode painting those
 * spans as misses or as local would put the most interesting claim on the screen on no evidence,
...
final class TimelineColoursTest {
  ...
  private static TimelineModel modelOf(TimelineLodIndex index, long cacheKnown, long runnersKnown) {
    return new TimelineModel(index, List.of(), List.of(), Map.of(), 0, cacheKnown, runnersKnown);
  }
  ...
  void tiesAreNotResolvedByLuck() {
    TimelineLodIndex tied =
        indexOf(
            SpanSource.FLAG_CACHE_KNOWN | SpanSource.FLAG_CACHE_HIT, SpanSource.FLAG_CACHE_KNOWN);
  ...
  void failureAlwaysShows() {
    TimelineLodIndex failed =
        indexOf(SpanSource.FLAG_FAILED | SpanSource.FLAG_CACHE_KNOWN | SpanSource.FLAG_CACHE_HIT);
  ...
  void outcomeColoursAreDistinct() {
    ...
    assertThat(TimelineColours.forSpan(TimelineColours.Mode.OUTCOME, SpanSource.FLAG_FAILED))
        .isEqualTo(TimelineColours.FAILED);
    ...
    assertThat(TimelineColours.forBin(indexOf(0), TimelineColours.Mode.OUTCOME, 0, 0))
        .isEqualTo(TimelineColours.SUCCESS);
  }
  ...
  void knownSpansAreColoured() {
    assertThat(TimelineColours.forSpan(TimelineColours.Mode.CACHE, SpanSource.FLAG_CACHE_KNOWN))
        .isEqualTo(TimelineColours.CACHE_MISS);
    assertThat(
            TimelineColours.forSpan(
                TimelineColours.Mode.RUNNER, SpanSource.FLAG_RUNNER_KNOWN | SpanSource.FLAG_REMOTE))
  ...
  void coverageIsStated() {
    TimelineModel partial =
        new TimelineModel(indexOf(0), List.of(), List.of(), Map.of(), 11_960, 0, 0);

    // A timeline showing 40 of a build's 12,000 actions is not wrong; one
    // that does not say so is.
    assertThat(partial.coverageNote())
        .hasValueSatisfying(note -> assertThat(note).contains("11960").contains("no timestamps"));
