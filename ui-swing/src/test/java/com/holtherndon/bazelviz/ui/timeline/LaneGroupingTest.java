 ...
 *
 * <p>Plan 14.2. The interesting cases are the ones where the session cannot
 * supply what the control offers: a grouping that needs an execution log, and a
 * sort by critical-path contribution with no critical path.
 */
final class LaneGroupingTest {
    ...
    private static final List<TimelineModel.Lane> LANES = List.of(
    ...
    void byContribution() {
        ...
        assertThat(LaneGrouping.sort(
                        LANES, LaneGrouping.SortBy.CRITICAL_PATH_CONTRIBUTION, contribution))
                .extracting(TimelineModel.Lane::name)
                .containsExactly("CppCompile", "Javac", "Genrule");
        assertThat(LaneGrouping.contributionSortIsReal(
                LaneGrouping.SortBy.CRITICAL_PATH_CONTRIBUTION, contribution)).isTrue();
    }

    @Test
    @DisplayName("with no critical path it falls back, and admits it did")
    void contributionFallsBackHonestly() {
        List<TimelineModel.Lane> sorted = LaneGrouping.sort(
                LANES, LaneGrouping.SortBy.CRITICAL_PATH_CONTRIBUTION, Map.of());

        // Ordering by a map of zeroes would leave the rows in whatever order
        // they arrived and look like an answer.
        assertThat(sorted).extracting(TimelineModel.Lane::name)
                .containsExactly("Genrule", "Javac", "CppCompile");
        assertThat(LaneGrouping.contributionSortIsReal(
                LaneGrouping.SortBy.CRITICAL_PATH_CONTRIBUTION, Map.of())).isFalse();
    }

    @Test
    @DisplayName("ties break by name, so two runs order the rows the same way")
    void tiesAreStable() {
        List<TimelineModel.Lane> tied = List.of(
                lane("beta", 1, 100, 0, 1),
                lane("alpha", 1, 100, 0, 1),
                lane("gamma", 1, 100, 0, 1));
