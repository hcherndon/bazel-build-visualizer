...
class RawPayloadRendererTest {
    ...
    void besEnvelopeShowsTheInnerEvent() {
        BuildEvent inner = BuildEvent.newBuilder()
                .setId(BuildEventId.newBuilder().setStarted(BuildEventId.BuildStartedId.getDefaultInstance()))
        ...
        assertThat(rendered.text()).contains("BAZEL_EVENT").contains("sequence 7").contains("build-1234");
    ...
    @Test
    @DisplayName("a stream-control envelope says it holds no event, and does not call that a limitation")
    void streamControlEnvelopeIsNotReportedAsAShortfall() {
        PublishBuildToolEventStreamRequest request = PublishBuildToolEventStreamRequest.newBuilder()
                .setOrderedBuildEvent(OrderedBuildEvent.newBuilder()
                        .setStreamId(StreamId.newBuilder().setBuildId("build-1234").setInvocationId("inv-1"))
