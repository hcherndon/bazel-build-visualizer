...
/**
 * Unknown-is-not-zero, at the only place the user can see it (plan 11.4, project rule 11).
 *
 * <p>A record whose bytes did not decode has {@code event_type = 0} and {@code child_count = 0} in
 * the database, because those columns are {@code NOT NULL} and a row must exist for the bytes
 * regardless. Rendering those zeroes would tell the user the event had no payload and no children —
 * a specific, false claim about a record nobody could read. These tests pin every such cell to the
 * em dash.
 */
class EventTableColumnsTest {
  ...
  void identityRowMissingIsItsOwnFinding() {
    ...
    assertThat(EventTableColumns.identity(row)).isEqualTo("0x0000000000001234 (no identity row)");
  }
  ...
  void columnsAreTheOnesThePlanNames() {
    assertThat(EventTableColumns.columns())
