 ...
 *
 * <p>A spatial index is only worth having if it agrees exactly with the loop it replaces. Several
 * tests below compute the answer both ways and compare, which catches the off-by-one at a cell
 * boundary that a hand-picked expectation would sail past.
 */
final class GraphSpatialIndexTest {
  ...
  private static GraphLayout.Result laidOut(int nodes) {
    ...
    return GraphLayout.layered(GraphExtract.whole(chain, nodes + 1, nodes + 1), RUNNING);
  }
  ...
  void cullingMatchesBruteForce() {
    ...
    double[][] rectangles = {
      {0, 0, 100, 100},
      {-50, -50, 5_000, 5_000},
      {137.5, 61.2, 402.9, 313.7},
      {1_000_000, 1_000_000, 1_000_001, 1_000_001},
      {0, 0, 0, 0},
    };
    for (double[] rectangle : rectangles) {
      int[] fromIndex = index.within(rectangle[0], rectangle[1], rectangle[2], rectangle[3]);
      List<Integer> expected =
          bruteForce(layout, rectangle[0], rectangle[1], rectangle[2], rectangle[3]);
      assertThat(Arrays.stream(fromIndex).boxed().sorted().toList())
          .as("rectangle %s", Arrays.toString(rectangle))
  ...
  void hitTesting() {
    ...
    assertThat(index.nearest(layout.xAt(0) + 3, layout.yAt(0) + 3, 40)).hasValue(0);
  ...
  void filteredHitTesting() {
    ...
    int nearest = index.nearest(layout.xAt(0) + 3, layout.yAt(0) + 3, 1_000).orElseThrow();
    ...
    OptionalInt second =
        index.nearest(layout.xAt(0) + 3, layout.yAt(0) + 3, 1_000, position -> position != nearest);
    assertThat(second).isPresent();
    assertThat(second.getAsInt()).isNotEqualTo(nearest);
    // And a filter that accepts everything is the unfiltered answer.
    assertThat(index.nearest(layout.xAt(0) + 3, layout.yAt(0) + 3, 1_000, position -> true))
        .hasValue(nearest);
  }
  ...
  void tiesAreBrokenTheSameWayEveryTime() {
    ...
    OptionalInt first = index.nearest(110, 0, 500);
  ...
  void emptyLayout() {
    GraphSpatialIndex index =
        GraphSpatialIndex.of(GraphLayout.Result.empty(GraphLayout.Kind.LAYERED));
  ...
  void everyNodeIsIndexed() {
    for (int count : new int[] {1, 2, 7, 64, 999}) {
      ...
      assertThat(index.within(box[0], box[1], box[2], box[3])).as("%d nodes", count).hasSize(count);
    }
  }
}
