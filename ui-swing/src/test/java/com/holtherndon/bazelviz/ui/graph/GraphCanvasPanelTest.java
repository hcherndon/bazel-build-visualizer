...
import com.holtherndon.bazelviz.analysis.GraphClustering.By;
 ...
 *
 * <p>A view of nine actions from a build of ninety thousand looks exactly like a build with nine
 * actions. Plan 13.6's "never claim the omitted nodes do not exist" therefore lives in the sentence
 * under the canvas, and these tests are about that sentence.
 */
final class GraphCanvasPanelTest {
  ...
  private void awaitDrawn() throws Exception {
    awaitCondition(
        () -> !panel.descriptionText().startsWith("Drawing") && panel.canvas().model().size() > 0,
        "a drawing");
  }
  ...
  private void awaitCondition(BooleanSupplier done, String what) throws Exception {
    ...
    throw new AssertionError(
        "timed out waiting for " + what + "; the panel says: " + panel.descriptionText());
  }
  ...
  void drawingControlsAreExplicitAndExplained() {
    ...
    assertThat(List.of(GraphLayout.Kind.values()))
        .contains(
  ...
  void scopeHelpUsesTheCurrentNodeKind() {
    ...
    panel.setShownGraph(GraphKind.CONFIGURED_TARGETS);
  ...
  void displayLabelsReachTheCanvas() throws Exception {
    ...
    for (int i = 0; i < model.size(); i++) {
      ...
      assertThat(model.canvasLabelAt(i)).contains("Javac — t").contains("target //a:target");
    }
  ...
  void totalsAreShown() throws Exception {
    ...
    assertThat(panel.descriptionText()).contains("Neighbourhood").contains("from a graph of 6");
  }
  ...
  void untimedIsAdmitted() throws Exception {
    ...
    assertThat(panel.canvas().model().untimedCount()).isEqualTo(panel.canvas().model().size());
  ...
  void detachClears() throws Exception {
    panel.showNode(2);
    awaitDrawn();
    SwingUtilities.invokeAndWait(() -> panel.setModeForTesting(Mode.WHOLE));

    panel.detach();

    assertThat(panel.canvas().model().size()).isZero();
    assertThat(panel.modeForTesting()).isEqualTo(Mode.NEIGHBOURHOOD);
  ...
  void pathsAreDrawn() throws Exception {
    panel.showPath(List.of(0, 2, 5), Mode.PATH);
    awaitDrawn();
  ...
  void pathsRespectTheCurrentDrawingBudgets() throws Exception {
    ...
    assertThat(panel.showPath(List.of(0, 2, 5), Mode.CRITICAL_PATH)).isFalse();
    ...
    assertThat(panel.showPath(List.of(0, 2, 5), Mode.CRITICAL_PATH)).isTrue();
    awaitDrawn();
  ...
  void nodeNavigationLeavesPathMode() throws Exception {
    ...
    awaitCondition(
        () ->
            panel.modeForTesting() == Mode.NEIGHBOURHOOD
  ...
  void clusterBoxesHaveNoActionId() throws Exception {
    panel.setModeForTesting(Mode.CLUSTERS);
    awaitDrawn();
  ...
  void exportingTheCompleteGraph() throws Exception {
    ...
    awaitCondition(() -> panel.legendText().startsWith("Wrote"), "the export to finish");
  ...
  void clusterScopeUsesAGroupBudget() throws Exception {
    ...
    SwingUtilities.invokeAndWait(() -> panel.setModeForTesting(Mode.CLUSTERS));
    assertThat(namedLabel(panel, "graph.nodeBudgetLabel").getText()).isEqualTo("Group budget:");
    assertThat((Integer) panel.nodeLimitControlForTesting().getValue())
        .isEqualTo(GraphClustering.DEFAULT_CLUSTER_LIMIT);
    ...
    SwingUtilities.invokeAndWait(() -> panel.setModeForTesting(Mode.WHOLE));
    assertThat(namedLabel(panel, "graph.nodeBudgetLabel").getText()).isEqualTo("Node budget:");
    assertThat((Integer) panel.nodeLimitControlForTesting().getValue())
  ...
  void automaticGroupingRefusalUsesClusterRecovery() throws Exception {
    SwingUtilities.invokeAndWait(
        () -> {
          panel.setModeForTesting(Mode.CLUSTERS);
          panel.setGroupByForTesting(By.TARGET);
          panel.nodeLimitControlForTesting().setValue(1);
          panel.setModeForTesting(Mode.WHOLE);
          panel.raiseLimits(2, 2);
        });
    awaitCondition(
        () ->
            panel.modeForTesting() == Mode.CLUSTERS
  ...
  void focusRecentresTheDrawing() throws Exception {
    ...
    awaitCondition(() -> panel.canvas().model().size() == 5, "the drawing to recentre on node 2");
  ...
  void weightRestylesWithoutRelayout() throws Exception {
    ...
    SwingUtilities.invokeAndWait(() -> panel.setWeightForTesting(GraphWeight.IMMEDIATE_DEPS));
    awaitCondition(
    ...
    assertThat(panel.canvas().model().weight()).isEqualTo(GraphWeight.IMMEDIATE_DEPS);
  ...
  void transitiveWeightsCountTheChain() throws Exception {
    ...
    SwingUtilities.invokeAndWait(() -> panel.setWeightForTesting(GraphWeight.TRANSITIVE_DEPS));
    awaitCondition(
        () -> panel.legendText().contains("Colour and size are transitive dependencies on screen"),
    ...
    awaitCondition(
        () -> panel.legendText().contains("whole graph: 2"), "the budgeted whole-graph count");
  ...
  void absentSizesAreUnavailable() throws Exception {
    ...
    SwingUtilities.invokeAndWait(() -> panel.setWeightForTesting(GraphWeight.OUTPUT_SIZE));
    awaitCondition(
  ...
  void raisingTheLimitRedraws() throws Exception {
    ...
    panel.raiseLimits(GraphExtract.DEFAULT_NODE_LIMIT, GraphExtract.DEFAULT_EDGE_LIMIT);
    awaitDrawn();
