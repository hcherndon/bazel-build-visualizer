...
/**
 * What the Errors inspector says about a console row, in each of the four
 * states its journal read can be in.
 *
 * <p>The bug this suite pins: an OUTPUT row's diagnostic is
 * {@code progress.stderr} and the row carries no message column, so the
 * inspector used to render the em-dash "unknown" over text that existed the
 * whole time. Every case here is about the difference between "there is none",
 * "it is elsewhere", and "it is coming".
 */
final class ErrorInspectionTest {
    ...
    void stderrIsShown() {
        Inspection inspection = ErrorInspection.of(
                outputRow(),
    ...
    void stdoutIsSeparate() {
        Inspection both = ErrorInspection.of(
                outputRow(), ErrorInspection.Console.text("on stderr\n", "on stdout\n"));
    ...
    void messageFieldPointsAtTheJournal() {
        ...
        assertThat(message.unknownNote()).hasValueSatisfying(note ->
                assertThat(note).contains("in the journal"));
    }
    ...
    void readingIsItsOwnState() {
        Inspection inspection =
                ErrorInspection.of(outputRow(), ErrorInspection.Console.reading());

        Inspection.Field text = field(inspection, "Console output", "Text");
        assertThat(text.isKnown()).isFalse();
        assertThat(text.unknownNote()).hasValueSatisfying(note ->
                assertThat(note).contains("reading it back from the journal"));
    }
    ...
    void decodedSilenceIsDistinct() {
        Inspection inspection =
                ErrorInspection.of(outputRow(), ErrorInspection.Console.text("", ""));

        assertThat(field(inspection, "Console output", "Text").unknownNote())
                .hasValueSatisfying(note ->
                        assertThat(note).contains("decoded and carried no console text"));
    }
    ...
    void lineLimitIsDisclosed() {
        ...
        Inspection inspection = ErrorInspection.of(
                outputRow(), ErrorInspection.Console.text(stderr.toString(), ""));
    ...
    void ordinaryRowsAreUnchanged() {
        ...
        assertThat(field(inspection, "Failure", "Message").value())
                .contains("undeclared inclusion(s)");
    ...
    private static Inspection.Section section(Inspection inspection, String heading) {
        return inspection.sections().stream()
                ...
                .orElseThrow(() -> new AssertionError(
                        "no section " + heading + " in " + headings(inspection)));
    }
    ...
    private static List<String> values(Inspection.Section section) {
        return section.fields().stream()
                .map(field -> field.value().orElseThrow(() -> new AssertionError(
