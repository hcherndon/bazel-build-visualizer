...
/**
 * The Query tab's slice of the shared column machinery: result headers never sort — the order
 * belongs to the user's SQL — and they say so.
 */
final class QueryTabColumnStateTest {
  ...
  private static final QueryTab.Host NO_HOST =
      new QueryTab.Host() {
        @Override
  ...
  void headersPointAtOrderBy() throws Exception {
    ...
    SwingUtilities.invokeAndWait(
        () -> {
          assertThat(tab.headerInteractionsForTest().isSortableForTest("anything")).isFalse();
