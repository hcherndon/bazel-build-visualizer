...
/**
 * The shared header machinery, against a plain table: the 3-state sort cycle, the visibility menu's
 * hidden-is-not-dropped contract, width and order surviving a {@code setModel} swap, and the
 * persisted state coming back in a fresh "application run".
 */
final class TableHeaderInteractionsTest {
  ...
  void threeStateCycle() throws Exception {
    onEdt(
        () -> {
          ...
          TableHeaderInteractions interactions = TableHeaderInteractions.install(table, adapter);
          ...
          assertThat(adapter.applied).containsExactly("A asc", "A desc", "<default> asc", "A asc");
          return null;
        });
  }
  ...
  void switchingColumnsRestartsTheCycle() throws Exception {
    onEdt(
        () -> {
          ...
          assertThat(interactions.currentSort()).contains(new ColumnState.Sort("B", false));
          return null;
        });
  }
  ...
  void viewDrivenSortDoesNotLoop() throws Exception {
    onEdt(
        () -> {
          ...
          assertThat(adapter.applied).as("the view already applied this sort itself").isEmpty();
          assertThat(interactions.currentSort()).contains(new ColumnState.Sort("A", true));
  ...
  @Test
  @DisplayName(
      "the header menu offers a sort submenu where sorting is real, and says why where it is not")
  void headerMenuIsHonest() throws Exception {
    onEdt(
        () -> {
          JTable sortableTable = new JTable(model());
          TableHeaderInteractions sortable =
              TableHeaderInteractions.install(sortableTable, new RecordingAdapter("A"));
          ...
          TableHeaderInteractions fixed =
              TableHeaderInteractions.install(
                  fixedTable,
                  TableHeaderInteractions.Adapter.unsortable("This order is fixed by design."));
  ...
  void tooltipsAreHonest() throws Exception {
    onEdt(
        () -> {
          JTable table = new JTable(model());
          TableHeaderInteractions interactions =
              TableHeaderInteractions.install(table, new RecordingAdapter("A"));

          assertThat(interactions.tooltipForTest(0))
              .contains("Click to sort by A")
              .contains("restores the default order");
          assertThat(interactions.tooltipForTest(1)).contains("No ordering stands behind B.");
    ...
    onEdt(
        () -> {
          ...
          assertThat(restoredTable.get().getColumnModel().getColumn(0).getPreferredWidth())
              .isEqualTo(287);
          assertThat(restored.get().currentSort()).contains(new ColumnState.Sort("A", false));
  ...
  void unchangedStateIsNotRewritten(@TempDir Path settings) throws Exception {
    ...
    onEdt(
        () -> {
          ...
          TableHeaderInteractions interactions =
              TableHeaderInteractions.install(table, new RecordingAdapter());
          interactions.attachPersistence(
              store,
              task -> {
  ...
  void closeFlushesAndInvalidatesLateLoad(@TempDir Path settings) throws Exception {
    ...
    store.save(new ColumnState(List.of("A", "B", "C"), Map.of(), Set.of("C"), Optional.empty()));
  ...
  private static void collect(MenuElement element, List<String> texts) {
    ...
    } else if (element instanceof JMenuItem item) {
