 ...
 *
 * <p>Plan 24's fourth Phase 6 exit criterion is that no SQLite access occurs during painting.
 * Reviewing a paint method for it works once; a test that inspects what the painting classes can
 * reach works every time someone adds a field.
 *
 * <p>The check is deliberately about reachability rather than about behaviour. A paint method that
 * happens not to query today but holds a {@code Connection} is one refactor away from doing so, and
 * the point of the design is that the refactor should be impossible rather than merely discouraged.
 */
final class TimelinePaintIsolationTest {
  ...
  void theModelCannotQuery() {
    for (Class<?> painted :
        List.of(
            TimelineModel.class,
            TimelineLodIndex.class,
            SpanWindow.class,
            TimelineViewport.class,
            TimelineTransform.class)) {
      assertThat(databaseFieldsOf(painted))
          .as("%s is painted from and must carry no database handle", painted.getSimpleName())
          .isEmpty();
    }
  }
    ...
    for (Class<?> current = type;
      ...
      for (Field field : current.getDeclaredFields()) {
        ...
        if (DATABASE_TYPES.stream().anyMatch(name::startsWith)) {
          offenders.add(current.getSimpleName() + "." + field.getName() + " : " + name);
        }
      }
    }
    return offenders;
  ...
  private static boolean mentionsDatabase(Class<?> type) {
    for (Method method : type.getDeclaredMethods()) {
      for (Class<?> parameter : method.getParameterTypes()) {
        if (DATABASE_TYPES.stream().anyMatch(prefix -> parameter.getName().startsWith(prefix))) {
          return true;
        }
      }
    }
