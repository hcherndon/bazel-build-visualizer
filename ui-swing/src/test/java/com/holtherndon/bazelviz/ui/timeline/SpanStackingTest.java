 ...
 *
 * <p>The rules under test: stack top-to-bottom by start time, reuse a sub-row once its occupant has
 * ended, never exceed {@link SpanStacking#MAX_SUB_ROWS}, and count — exactly — every span that had
 * to pile into the last sub-row, because a pile that is not counted is a truncation that is not
 * stated.
 */
final class SpanStackingTest {
  ...
  void overlapStacksDownward() {
    SpanWindow window = windowOf(new long[][] {{0, 100}, {10, 90}, {20, 80}});
  ...
  void subRowsAreReused() {
    SpanWindow window = windowOf(new long[][] {{0, 50}, {10, 40}, {60, 100}, {70, 90}});
    SpanStacking stacking = SpanStacking.of(window);
