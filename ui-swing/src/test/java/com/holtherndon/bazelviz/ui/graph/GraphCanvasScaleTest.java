 ...
 *
 * <p>Fifty thousand nodes and two hundred thousand edges is the default ceiling for a detailed
 * drawing, so it is the size at which "panning and selection remain responsive" has to be true. The
 * graph is synthetic — plan rule 18's requirement to build a real Bazel fixture is about Bazel's
 * behaviour, and nothing here depends on Bazel's behaviour.
 *
 * <p>The bounds are deliberately loose. A test that asserted a tight frame time would fail on a
 * loaded machine for reasons that say nothing about the design; these fail only if something is
 * wrong by an order of magnitude, which is the kind of wrong that a fresh pair of eyes would
 * otherwise not notice until a user did.
 */
final class GraphCanvasScaleTest {
  ...
  private static GraphModel bigModel() {
    ...
    assertThat(extract.nodes()).as("the fixture must fit inside the plan's limits").hasSize(NODES);
  ...
  void paintingZoomedIn() {
    ...
    assertThat(canvas.hiddenDetail()).as("nothing is omitted once the view is zoomed in").isEmpty();

    long slowest = slowestFrameOf(canvas, 1_600, 1_000);

    assertThat(slowest).as("slowest of five near-zoom frames was %dms", slowest).isLessThan(200);
  }
  ...
  void deepestValidHierarchyStillFits() {
    ...
    GraphExtract.Result extract =
        GraphExtract.whole(chain, GraphExtract.DEFAULT_NODE_LIMIT, GraphExtract.DEFAULT_EDGE_LIMIT);
    GraphLayout.Result layout = GraphLayout.hierarchy(extract, RUNNING);

    GraphTransform fitted = GraphTransform.fit(layout.bounds().orElseThrow(), 600, 400, 24);
