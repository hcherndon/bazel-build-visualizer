 ...
 *
 * <p>Plan 24's fifth Phase 6 exit criterion, and plan 14.4's "do not
 * auto-scroll if the user has navigated into history". Every one of these
 * tests is that sentence in a different shape.
 */
final class TimelineViewportTest {
    ...
    void navigatingStopsFollowing() {
        TimelineViewport moved =
                fitted().navigatedTo(fitted().transform().pannedByPixels(-120));
    ...
    void newDataDoesNotStealTheView() {
        TimelineViewport moved =
                fitted().navigatedTo(fitted().transform().zoomedAround(500, 8.0));
    ...
    void selectionSurvivesLiveUpdates() {
        ...
        assertThat(selected.navigatedTo(selected.transform().pannedByPixels(-10))
                .withWall(0, 5_000_000, WIDTH).selectedNode()).hasValue(42);
    }
    ...
    void followingCanBeResumed() {
        TimelineViewport moved =
                fitted().navigatedTo(fitted().transform().pannedByPixels(-500));
        TimelineViewport resumed = moved.following(true);

        assertThat(resumed.withWall(0, 4_000_000, WIDTH).transform())
                .isNotEqualTo(moved.transform());
    }
    ...
    void selectionAndViewAreIndependent() {
        ...
        assertThat(start.selecting(OptionalLong.of(9)).transform())
                .isEqualTo(start.transform());
    }
