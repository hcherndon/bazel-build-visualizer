...
/**
 * The Errors card's client-side sort: the shared 3-state header cycle over a {@code
 * TableRowSorter}, whose third state is the deliberate kind-priority load order — unsorted, exactly
 * as the pages arrived.
 *
 * <p>Client-side is legitimate here and only here among the entity tables: every loaded row is
 * already in memory, so sorting them misrepresents nothing. The fixture is a console row plus
 * aborts, the shape the fake session supports, loaded in that order.
 */
final class ErrorsViewSortTest {
  ...
  void cycleSortsAndRestoresLoadOrder() throws Exception {
    ...
    assertThat(labelsOf(loadOrder)).containsExactly("//zeta:last", "//alpha:first", "//mid:way");
    ...
    assertThat(onEdt(() -> view.sorterForTest().getSortKeys()))
        .singleElement()
        .satisfies(key -> assertThat(key.getSortOrder()).isEqualTo(SortOrder.ASCENDING));
  ...
  void selectionFollowsTheViewOrder() throws Exception {
    ...
    await(() -> onEdt(() -> view.inspectionForTest().title().contains("//alpha:first")));
  }

  // ------------------------------------------------------------------ plumbing

  /**
   * Opens the view over a session holding one console row (which loads first — the fake's first
   * page is the console output) plus these aborts, then presses "Load more" so the aborts are on
   * screen too.
   */
  private void openWithRows(String... labels) throws Exception {
    FakeErrorSession session = new FakeErrorSession().withConsoleEvent(11, 4, "boom\n", "");
  ...
  private JTable viewTable() {
    ...
    while (!queue.isEmpty()) {
      ...
      for (Component child : current.getComponents()) {
        if (child instanceof JTable table && table.getRowSorter() == view.sorterForTest()) {
          return table;
        }
