 ...
 *
 * <p>What matters here is that the <em>existing</em> paging shell is what
 * carries the result: {@code PagedTableModel} is constructed over this source
 * with no change to it, and the columns it is given were built from the query's
 * own result metadata rather than declared.
 */
final class QueryRowSourceTest {
    ...
    void columnsAreDynamic() {
        QueryRowSource source = new QueryRowSource(
                new FakeReader(0), outline(List.of("mnemonic", "actions", "total_ms"), 0));

        assertThat(source.columns()).extracting(ColumnSpec::name)
    ...
    void nullSurvivesTheExtractor() {
        QueryRowSource source = new QueryRowSource(
                new FakeReader(0), outline(List.of("a", "b"), 0));
        ...
        assertThat(source.columns().get(1).extractor().apply(row))
                .as("mapping it to \"\" here would make NULL and empty indistinguishable"
    ...
    void rowCountIsTheVisibleCount() {
        QueryRowSource capped = new QueryRowSource(new FakeReader(0),
                new QueryOutline("SELECT 1", ReadOnlySql.Shape.TABULAR, List.of("x"),
                        OptionalLong.of(5_000_000L), 1_000L, 1_000L, 1L, List.of()));
    ...
    void theExistingPagedModelDrivesIt() throws Exception {
        ...
        PagedTableModel<QueryRow> model = new PagedTableModel<>(
                source, source.columns(), queued::add, QueryRowSource.PAGE_SIZE, 8);
    ...
    private static QueryOutline outline(List<String> columns, long rows) {
        return new QueryOutline("SELECT " + String.join(", ", columns) + " FROM t",
                ReadOnlySql.Shape.TABULAR, columns, OptionalLong.of(rows), rows,
                1_000_000L, 1L, List.of());
    }
