 ...
 *
 * <p>Swing turns any string beginning with {@code <html>} into a live document, and that document's
 * {@code <img src="http://…">} is fetched when painted. The strings this application displays all
 * came from a build — a label, a compiler message, a tag someone wrote in a BUILD file, or an
 * imported session that is untrusted outright — so without this the application makes an outbound
...
class PlainTextTest {
  ...
  private static final String HOSTILE = "<html><img src=\"http://example.invalid/pixel.png\">gone";
    ...
    JTable table =
        onEdt(
            () -> {
              JTable created =
                  new JTable(
                      new DefaultTableModel(new Object[][] {{HOSTILE}}, new Object[] {"Target"}));
  ...
  void theInspectorShowsTextEverywhere() throws Exception {
    ...
    onEdt(
        () -> {
          panel.show(
              new Inspection.Builder(HOSTILE)
                  .subtitle(HOSTILE)
                  .section("Failure")
