 ...
 *
 * <p>Pure: no database and no screen, because the decisions worth testing here
 * are about wording and omission rather than about layout.
 */
final class AttemptInspectionTest {
    ...
    void aDurationWithNoStartExplainsItself() {
        AttemptRow row = attempt()
                .startMicros(OptionalLong.empty())
                .startUnknownReason(Optional.of(
                ...
                .satisfies(field -> {
                    ...
                    assertThat(field.unknownNote())
                            .hasValueSatisfying(reason ->
                                    assertThat(reason).contains("does not report"));
                });
    }
    ...
    void unmeasuredComponentsAreNotZeroes() {
        AttemptRow row = attempt()
                .timing(new AttemptRow.Timing(
                        OptionalLong.of(25_800), OptionalLong.of(24_000),
    ...
    void theRemainderIsNamed() {
        AttemptRow row = attempt()
                .timing(new AttemptRow.Timing(
                        OptionalLong.of(100_000), OptionalLong.of(60_000),
                        OptionalLong.empty(), OptionalLong.empty(), OptionalLong.empty(),
                        OptionalLong.empty(), OptionalLong.empty(), OptionalLong.empty(),
                        OptionalLong.empty(), OptionalLong.empty()))
                .build();
    ...
    private static final class Builder {
        ...
        private AttemptRow.Timing timing = new AttemptRow.Timing(
                OptionalLong.of(25_800), OptionalLong.of(25_000), OptionalLong.empty(),
                OptionalLong.empty(), OptionalLong.empty(), OptionalLong.empty(),
                OptionalLong.empty(), OptionalLong.empty(), OptionalLong.empty(),
                OptionalLong.empty());
        ...
        AttemptRow build() {
            return new AttemptRow(
                    1, OptionalLong.of(7), correlation, correlationNote,
                    Optional.of("//pkg:gen_a"), Optional.of("Genrule"),
                    Optional.of("darwin-sandbox"), false, OptionalInt.of(0),
                    Optional.empty(), startMicros, startUnknownReason, timing,
                    Optional.empty(), OptionalLong.empty(), OptionalLong.empty(),
                    OptionalLong.empty(), 1, unproducedOutputs);
        }
    }
    ...
    private static Inspection.Field fieldNamed(Inspection inspection, String name) {
        return inspection.sections().stream()
                ...
                .orElseThrow(() -> new AssertionError("no field named " + name
                        + "; have " + fieldNames(inspection)));
    }

    private static java.util.List<String> fieldNames(Inspection inspection) {
        return inspection.sections().stream()
                .flatMap(section -> section.fields().stream())
