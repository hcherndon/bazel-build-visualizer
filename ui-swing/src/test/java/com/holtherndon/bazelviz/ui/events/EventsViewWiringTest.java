...
/**
 * The Swing wiring, end to end and without a display: open a real imported
 * session in the view, let the first page arrive, and check that selecting a
 * row reached the inspector with that row's bytes.
 *
 * <p>This is the chain the unit tests each cover one link of — row source, page
 * cache, selection resolution, inspector fetch — and that nothing else exercises
 * together. It runs headless because none of these components realizes a peer;
 * the assertion at the top makes that a stated precondition rather than a
 * coincidence.
 */
class EventsViewWiringTest {
    ...
    static void requireHeadless() {
        assertThat(GraphicsEnvironment.isHeadless())
                .as("these tests must not depend on a display; the build sets"
    ...
    void openingASessionWiresTableAndInspector(@TempDir Path temporary) throws Exception {
        ...
        await(() -> onEdt(() ->
                model.getValueAt(0, EventTableColumns.ID_COLUMN) instanceof Long));
        ...
        assertThat(inspector.payloadFetchCount())
                .as("one selection, one payload read")
                .isEqualTo(1);
