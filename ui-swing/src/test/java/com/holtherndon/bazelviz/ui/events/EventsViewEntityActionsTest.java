...
/**
 * The Events tab's adoption of the shared navigation actions: the target label is parsed at render
 * time — structurally from the decoded payload where one is decoded, from the stored id display
 * where only that exists — and becomes actionable in the rows and the inspector, honestly absent
...
class EventsViewEntityActionsTest {
  ...
  private static EventRow row(long id, Optional<String> idDisplay) {
    return new EventRow(
        ...
        DecodeStatus.OK,
        false,
        OptionalLong.of(0x99),
        idDisplay,
        0,
        false,
        new RawLocation(0, 64, 12),
        OptionalLong.of(1_700_000_000_000_000L),
        1_700_000_000_000_100L);
  }

  @Test
  @DisplayName(
      "a row's label is parsed from its stored id display, and only when it can be vouched for")
  ...
  void inspectionPrefersStructuredLabel() {
    ...
    EventInspection structured =
        EventInspection.loaded(
            displayRow,
            payload,
            new RawPayloadRenderer.Rendered(
                "text", Optional.empty(), List.of(), Optional.of("//from-proto:x")),
            "hex");
    assertThat(structured.targetLabel()).hasValue("//from-proto:x");

    EventInspection fallback =
        EventInspection.loaded(
            displayRow,
            payload,
  ...
  @Test
  @DisplayName(
      "the inspector's header offers label actions for a labelled event" + " and nothing otherwise")
  void inspectorOverflowAppearsOnlyWithALabel() throws Exception {
    Recorder recorder = new Recorder();
    EntityActions actions =
        new EntityActions(
            EnumSet.of(
                EntityActions.Command.OPEN_TARGET, EntityActions.Command.SHOW_ACTIONS_FOR_LABEL),
            recorder);
        ...
        () -> {
          ...
          panel.show(
              EventInspection.loaded(
                  labelled,
                  payload,
                  new RawPayloadRenderer.Rendered("t", Optional.empty(), List.of()),
                  "h"));
  ...
  @Timeout(180)
  @DisplayName(
      "over a real session, a labelled row offers the label actions and dispatches the label")
  ...
  private static int onEdt(IntSupplier read) {
