...
/**
 * The exit criterion "the action table supports paging, filtering, and sorting", exercised through
 * the view rather than through the query layer.
 *
 * <p>The queries are tested on their own. What this covers is the part that could be right
 * underneath and wrong on screen: that changing the sort rebuilds the table rather than leaving the
 * old rows in place, that a filter changes both the rows and the number beside them, and that the
 * table's row count agrees with what the status line says.
 */
class ActionsViewWiringTest {
  ...
  void tablePagesSortsAndFilters(@TempDir Path temporary) throws Exception {
    ...
    assertThat(onEdt(view::captureNoteForTest)).contains("build_event_publish_all_actions");
    ...
    SwingUtilities.invokeAndWait(() -> view.applyForTest(firstMnemonic, ActionSort.ARRIVAL, false));
    ...
    assertThat(onEdt(view::statusForTest)).contains("of").contains("match the filter");
  ...
  void revealActionSelectsAnUnloadedFilteredOutRow(@TempDir Path temporary) throws Exception {
    ...
    try (EntityReader reader = opened.openEntityReader()) {
      targets = reader.firstActionPage(ActionFilter.NONE, ActionSort.ARRIVAL, false, 2);
    }
    ...
    try {
      ...
      await(() -> onEdt(() -> view.selectedActionIdForTest().orElse(-1L) == target.id()));
      ...
      await(() -> onEdt(() -> view.selectedActionIdForTest().orElse(-1L) == second.id()));
      assertThat(onEdt(view::statusForTest)).contains("action").doesNotContain("Locating");
      ...
      SwingUtilities.invokeAndWait(
          () -> {
            ...
            view.filterToRange(OptionalLong.of(0), OptionalLong.of(Long.MAX_VALUE));
      ...
      await(
          () ->
              onEdt(
                  () ->
                      view.rowSourceForTest().filter().hasRange()
                          && view.rowSourceForTest().filter().labelContains().isPresent()));
      ...
      await(
          () ->
              onEdt(
                  () ->
                      view.rowSourceForTest().filter().isEmpty()
      ...
      for (ActionSort sort : ActionSort.values()) {
        for (boolean descending : List.of(false, true)) {
          ActionRowSource rows =
              ActionRowSource.open(reader, ActionFilter.NONE, sort, descending, pageSize);
