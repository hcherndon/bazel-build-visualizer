...
/**
 * The Errors card's console rows, end to end: a row on screen, a selection, a
 * journal read on a background thread, and the stderr Bazel actually wrote.
 *
 * <p>The fixture is finding X2's build — a syntax error, no failed action, no
 * failed target, one abort, and the entire diagnostic living in
 * {@code progress.stderr}. That is the case the card exists for and the case
 * that used to render an em dash.
 */
final class ErrorsViewTest {
    ...
    void selectionShowsTheStderr() throws Exception {
        ...
        awaitCondition(() -> headings().contains("Console output (stderr)"),
                "the stderr section to appear");
        ...
        assertThat(session.edtCalls())
                .as("reads that happened on the event dispatch thread")
                .isZero();
    }
    ...
    void messageColumnPointsAtTheJournal() throws Exception {
        open(new FakeErrorSession()
                .withAbortedTarget("//failsyntax:oops")
                .withConsoleEvent(11, 4, "boom\n", ""));

        assertThat(onEdt(() -> view.messageCellForTest(0)))
                .isEqualTo(ErrorsView.MESSAGE_IN_JOURNAL);
    }
    ...
    void redactedSessionSaysSo() throws Exception {
        ...
        awaitCondition(() -> unknownNote("Console output", "Text")
        ...
        assertThat(onEdt(() -> view.inspectionForTest())
                .sections().getFirst().fields())
    ...
    private java.util.Optional<String> unknownNote(String heading, String name) {
        return onEdt(() -> view.inspectionForTest()).sections().stream()
                .filter(section -> section.heading().equals(heading))
