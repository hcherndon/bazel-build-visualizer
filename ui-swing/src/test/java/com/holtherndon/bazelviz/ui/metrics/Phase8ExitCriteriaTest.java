...
/**
 * Plan 24's five Phase 8 exit criteria, one test each, against a real session
 * database with a real action graph.
 *
 * <p>The fixture is a chain of eight actions across two packages, each with an
 * execution-log attempt, plus an aquery graph and a CSR index over it — the
 * smallest session in which both critical paths exist at once, which is what
 * the second criterion is about.
 */
final class Phase8ExitCriteriaTest {
    ...
    void buildSession() throws Exception {
        ...
        exec("INSERT INTO build_invocation (singleton, stream_id, build_tool_version,"
        ...
        exec("INSERT INTO build_metrics (singleton, critical_path_micros)"
                + " VALUES (1, 3_800_000)");
        ...
        for (int i = 0; i < ACTIONS; i++) {
            ...
            exec("INSERT INTO action_attempts (id, task_id, log_entry_index, action_id,"
                    ...
                    + " queue_micros, cacheable, remotable) VALUES (" + id + ", 1, " + i + ", "
                    + id + ", 'MATCHED_BY_OUTPUT', 'remote', 0, " + start + ", 400000, "
                    + (1024 * id) + ", " + (i == 3 ? 380_000 : 1_000) + ", 1, 1)");
            exec("INSERT INTO declared_actions (id, source_id, graph_id, action_id, label_id,"
                    + " mnemonic_id, node_index) VALUES (" + id + ", 1, " + i + ", " + id + ", "
                    + id + ", " + (i % 2 + 1) + ", " + i + ")");
        }
        ...
        new GraphIndexBuilder(writer, tempDir.resolve("indexes"))
        ...
        exec("INSERT INTO attempt_outputs (id, attempt_id, artifact_id, kind, produced)"
                + " VALUES (1, 1, 1, 'FILE', 1)");
    }
    ...
    void everyMetricReportsSourceAndCompleteness() throws Exception {
        ...
        for (GroupAggregate.Table table : metrics.aggregates().values()) {
            for (GroupAggregate group : table.groups()) {
                for (MetricSeries series
                        : List.of(group.duration(), group.inputBytes())) {
                    assertThat(series.source()).as("%s source", series.name()).isNotNull();
                    assertThat(series.completeness()).as("%s completeness", series.name())
                            .isNotNull();
        ...
        for (Finding finding : findings(metrics)) {
            for (Finding.MetricValue metric : finding.metrics()) {
                assertThat(metric.source())
                        .as("%s: %s", finding.ruleId(), metric.name())
                        .isNotBlank();
            }
        }
        ...
        view.show(new MetricsService.Result(
                metrics, findings(metrics), FindingThresholds.defaults()));
        assertThat(view.summaryTextForTest())
                .contains("Duration source")
    ...
    void criticalPathsRemainDistinct() throws Exception {
        ...
        assertThat(paths.derived().orElseThrow().outcome())
                .isEqualTo(CriticalPath.Outcome.COMPUTED);
        ...
        for (Finding finding : findings) {
            ...
            for (Finding.Evidence evidence : finding.evidence()) {
                ...
                assertThat(rowExists(evidence.id().orElseThrow()))
                        .as("%s points at action %d", finding.ruleId(),
                                evidence.id().orElseThrow())
                        .isTrue();
            }
        }
    }
    ...
    void theChainLinkAsksForTheChain() throws Exception {
        SessionMetrics metrics = collect();
        Finding chain = findings(metrics).stream()
        ...
        assertThat(chain.links()).extracting(Finding.Link::kind)
    ...
    private boolean rowExists(long actionId) throws Exception {
        try (Statement statement = writer.createStatement();
                ResultSet rows = statement.executeQuery(
                        "SELECT COUNT(*) FROM actions WHERE id = " + actionId)) {
            rows.next();
    ...
    void findingsAvoidUnsupportedCausalLanguage() throws Exception {
        List<String> banned = List.of(
                "will improve", "will reduce", "will speed", "definitely", "guaranteed",
                "is caused by", "was caused by", "the cause is", "this proves", "root cause");

        for (Finding finding : findings(collect())) {
            String text = String.join(" ",
                    finding.title(), finding.whyItMayMatter(), finding.caveats(),
                    finding.suggestedInvestigation()).toLowerCase(Locale.ROOT);
            for (String phrase : banned) {
                assertThat(text)
                        .as("%s uses \"%s\"", finding.ruleId(), phrase)
    ...
    void formulasAreDeterministic() throws Exception {
        ...
        assertThat(second.concurrency().peakActive())
                .isEqualTo(first.concurrency().peakActive());
        assertThat(second.concurrency().idleMicros())
                .isEqualTo(first.concurrency().idleMicros());
        ...
        assertThat(second.invocation().criticalPaths().derived().orElseThrow().makespanMicros())
                .isEqualTo(first.invocation().criticalPaths().derived().orElseThrow()
                        .makespanMicros());
    }
