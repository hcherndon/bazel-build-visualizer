 ...
 *
 * <p>The two fixtures are the two anchor cases: Bazel 6.5.0 publishes
 * {@code profile_finish_ts} holding a floored start, and Bazel 8.4.1 publishes
 * {@code profile_start_ts} holding an exact one (P1). Everything else about
 * reading a profile is the same; that difference is a one-build-length error
 * waiting for anyone who reads the key by its name.
 */
final class ProfileParserTest {
    ...
    void phasesCanStartNegative() throws Exception {
        ...
        assertThat(phases).extracting(EnrichmentCommand.PhaseMarkerSeen::ordinal)
    ...
    void actionSpansAreAttributable() throws Exception {
        ...
        assertThat(spans).allSatisfy(span ->
                assertThat(span.category()).isEqualTo("action processing"));
        // `out` is the same key BEP uses for action identity, and it is present
        // even on 6.5.0 (P4).
        assertThat(spans).anySatisfy(span ->
                assertThat(span.primaryOutput()).isPresent());
        assertThat(spans("bazel650-finish-ts.json")).anySatisfy(span ->
                assertThat(span.primaryOutput()).isPresent());
    }
    ...
    void emptyTargetIsAbsent() throws Exception {
        assertThat(spans("bazel841-start-ts.json"))
                .allSatisfy(span -> assertThat(span.targetLabel())
                        .satisfiesAnyOf(
    ...
    void criticalPathIsKept() throws Exception {
        ...
        assertThat(path).extracting(EnrichmentCommand.CriticalPathComponentSeen::ordinal)
                .isSorted();
    ...
    void countersAndThreadsAreKept() throws Exception {
        ...
        assertThat(of("bazel841-start-ts.json", EnrichmentCommand.ThreadNamed.class))
                .isNotEmpty();
    }
    ...
    void missingAnchorIsAbsent() throws Exception {
        List<EnrichmentCommand> commands = new ArrayList<>();
        new ProfileParser(commands::add).parse(new StringReader(
    ...
    void headerOrderDoesNotMatter() throws Exception {
        ...
        assertThat(commands).singleElement()
                .isInstanceOfSatisfying(EnrichmentCommand.ProfileHeaderSeen.class, header ->
                        assertThat(header.anchor().meaning())
                                .isEqualTo(ProfileAnchor.Meaning.EXACT_START));
    }
