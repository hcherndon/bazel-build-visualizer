...
/**
 * Receives the entities of an action graph in the order the file happens to
 * carry them.
 *
 * <p>Deliberately offers no way to resolve a reference. The file references
 * entities before declaring them on three of the four supported Bazel versions
 * (finding Q4), so anything resolved during the read would be resolved against
 * an incomplete picture — quietly, since the result is a graph slightly smaller
 * than the truth. Resolution happens after the last entity, and the type says
 * so by having no method for it.
 */
