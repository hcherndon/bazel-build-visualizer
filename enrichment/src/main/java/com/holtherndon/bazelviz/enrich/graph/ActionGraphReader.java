...
/**
 * Reads an {@code aquery --output=proto} file one top-level sub-message at a time.
 *
 * <h2>Why not {@code ActionGraphContainer.parseFrom}</h2>
 *
 * <p>The file is a single {@code ActionGraphContainer} message — there is no record framing and no
 * message boundary between entities (finding Q2 in {@code docs/aquery-and-cquery.md}). Parsing it
 * whole would hold the entire graph, and protobuf-java refuses any message above 2 GB regardless of
 * the heap, so on a large build {@code parseFrom} does not merely use a lot of memory: it fails.
 *
 * <p>So the container is never materialised. This reads the top-level tags with a {@code
 * CodedInputStream} and hands each sub-message to the visitor as it arrives, holding one at a time.
 *
 * <h2>Order guarantees, of which there are none</h2>
 *
 * <p>The top-level fields are interleaved — 57 to 71 runs of consecutive same-kind entries for a
 * six-target workspace (Q3) — and entities are referenced before they are declared on three of the
 * four supported versions (Q4). A visitor therefore may not resolve any reference while reading.
 * That is why {@link ActionGraphVisitor} has no resolution methods and why the importer stages
 * everything first.
 */
public final class ActionGraphReader {

  /**
   * Protobuf's default 64 MB read limit would refuse a real graph. The limit exists to stop a
   * hostile message allocating without bound, which this does not do: the stream is consumed
   * sub-message by sub-message and only one is ever held.
   */
