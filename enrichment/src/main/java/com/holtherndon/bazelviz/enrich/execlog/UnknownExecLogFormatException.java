 ...
 *
 * <p>Carries the bytes it saw, because "not an execution log" is a claim the
 * user is entitled to check — the usual causes are pointing at the profile by
 * mistake, or at a log Bazel never finished writing.
 */
public final class UnknownExecLogFormatException extends IOException {
    ...
    private static String describe(Path file, byte[] head, int length) {
        if (length == 0) {
            return file + " is empty, so it is either a build in which every action was"
                    + " cached or a log that was never written; which one cannot be told"
                    + " from the file alone";
        }
        return file + " does not start like any execution log this build reads."
                + " Its first " + length + " bytes are "
