...
public final class ExecLogSource implements AutoCloseable {
    ...
    public static ExecLogSource open(Path file) throws IOException {
        ...
        InputStream body = format == ExecLogFormat.COMPACT
                ? new ZstdInputStream(pushback)
                : pushback;
     ...
     *
     * <p>An empty file matches nothing: on Bazel 6.5.0 a fully cached rebuild
     * writes a zero-byte binary log (S3), and so does a run that never got
     * started. The caller has to decide which, and cannot be helped from here.
     */
    ...
    /**
     * The decoded byte stream: decompressed for a compact log, the file itself
     * otherwise.
     */
