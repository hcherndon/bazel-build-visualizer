...
/**
 * Reads the two fields Bazel 6.5.0 wrote and the current {@code spawn.proto}
 * marks {@code reserved}.
 *
 ...
 * </table>
 *
 * <p>Parsing a 6.5.0 log with the current descriptor succeeds and produces a
 * spawn with no timing and no progress message: both land in the unknown-field
 * set and are dropped. Nothing errors. The result is a table of attempts whose
 * durations are all unknown — indistinguishable from a build whose durations
 * Bazel declined to report, which is exactly the confusion plan rule 11 exists
 * to prevent.
 *
 * <p>So the bytes are recovered from {@link SpawnExec#getUnknownFields()}
 * rather than by vendoring a second copy of the proto. Two fields is less
 * surface than a whole descriptor that would then need keeping in step.
 */
final class LegacySpawnFields {
     ...
     *
     * <p>Used to decide whether the log as a whole is a legacy one, which the
     * importer records so that the UI can say why starts are missing.
     */
     ...
     *
     * <p>The field holds a {@code Duration} submessage, so its bytes are parsed
     * as one. A zero-length duration reads as unknown rather than as an
     * instantaneous spawn — a spawn that took no measurable time did not take
     * zero time.
     */
    static OptionalLong walltimeMicros(SpawnExec spawn) {
        ...
        try {
            com.google.protobuf.Duration duration =
                    com.google.protobuf.Duration.parseFrom(values.getFirst());
     ...
     *
     * <p>Bazel 6.5.0 never emits {@code start_time}, on any flag setting (S2).
     * Passing {@code --experimental_execution_log_spawn_metrics} adds
     * {@code total_time} and {@code execution_wall_time} in their modern
     * positions and still no start. So the attempt has a length and no
     * position, and this is the sentence that says so.
     */
    static String noStartReason() {
        return "this Bazel version does not report when a spawn started, only how"
                + " long it took";
    }
}
