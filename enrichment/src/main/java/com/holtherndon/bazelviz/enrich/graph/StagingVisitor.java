 ...
 *
 * <p>Resolves nothing, on purpose: the file names entities before declaring
 * them on three of four supported versions (finding Q4), so a resolution here
 * would be made against an incomplete picture and would quietly produce a
 * graph slightly smaller than the truth.
 *
 * <p>Everything is batched. The one thing held in memory is the list of
 * configuration checksums, which is bounded by the number of configurations —
 * two in the probe, and a few hundred in the worst realistic case — rather than
...
final class StagingVisitor implements ActionGraphVisitor {
     ...
     * an unbounded batch is an unbounded allocation. Same reasoning and same
     * size as the profile writer's.
     */
    ...
    StagingVisitor(GraphStaging staging) throws SQLException {
        fragment = track(staging.insert(
                "INSERT OR REPLACE INTO stage_fragment (id, label, parent) VALUES (?, ?, ?)"));
        artifact = track(staging.insert(
                "INSERT OR REPLACE INTO stage_artifact (id, fragment, is_tree) VALUES (?, ?, ?)"));
        target = track(staging.insert(
                "INSERT OR REPLACE INTO stage_target (id, label, rule_class_id) VALUES (?, ?, ?)"));
        ruleClass = track(staging.insert(
                "INSERT OR REPLACE INTO stage_rule_class (id, name) VALUES (?, ?)"));
        config = track(staging.insert(
        ...
        actionInput = track(staging.insert(
                "INSERT OR IGNORE INTO stage_action_input (ordinal, depset) VALUES (?, ?)"));
        actionOutput = track(staging.insert(
    ...
    public void action(Action value) {
        ...
        run(() -> {
            ...
            if (value.getIsExecutable()) {
                ...
                action.setNull(9, java.sql.Types.INTEGER);
            }
            action.addBatch();
