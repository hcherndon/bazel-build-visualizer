 ...
 *
 * <p>Rule 9 says never retain all events as Java objects, and plan section 25
 * says the application must not construct an object-per-edge whole graph. A
 * heap measurement proves the current build does not — `docs/performance.md`
 * has those figures, 176 MB of CSR against 704 MB of object-per-edge at Tier 2
 * — but it proves it for the inputs that were measured, and it fails long after
 * somebody has written the field that broke it.
 *
 * <p>This checks the thing that cannot be undone accidentally: the classes that
 * hold the per-event and per-edge data hold it in primitive arrays. A
 * {@code List<Edge>} added to any of them fails here, in the module's own test
 * run, with the field named.
 *
 * <h2>What is deliberately not on this list</h2>
 *
 * <p>Classes that legitimately hold collections of bounded size — a page of two
 * hundred rows, forty aggregate groups, a dozen catalog entries. The rule is
 * about data that scales with the build, not about every collection.
 */
final class BoundedMemoryTest {
     ...
     *
     * <p>These hold one entry per edge, per span or per observation, so a
     * collection of any kind in one of them is a per-item object header.
     */
    private static final List<String> MUST_BE_FLAT = List.of(
     ...
     *
     * <p>A different rule, because an LRU keyed by interned text is exactly the
     * right design here and a blanket "no maps" would forbid it. What they may
     * not do is retain the <em>domain objects</em> — an event, an action, a
     * span — which is what rule 9 is actually about.
     *
     * <p>Written after this test's first version flagged
     * {@code EntityWriter.knownLabels} and the code turned out to be right: it
     * is a {@code LinkedHashMap} with {@code removeEldestEntry}, bounded by the
     * dictionary-cache size. The rule was too coarse, not the field.
     */
    private static final List<String> MAY_CACHE_BUT_NOT_RETAIN = List.of(
