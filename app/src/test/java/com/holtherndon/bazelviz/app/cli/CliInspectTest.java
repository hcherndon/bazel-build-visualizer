 ...
 *
 * <p>The event-detail test does not take the command's word for the raw location: it reads the
 * journal itself at the segment, offset and length the command printed and checks the bytes hash to
 * what the command reported. That is the Phase 1 "offsets are reproducible" criterion expressed as
 * a round trip — a row whose recorded location pointed anywhere else would fail here.
 */
class CliInspectTest {
  ...
  void eventsAreListedInArrivalOrderWithTheirPayloadTypeNamed() {
    CliHarness.Result result =
        cli.run("inspect", sessionDirectory.toString(), "--events", "10", "--json");
  ...
  void keysetPagingWalksTheWholeSessionWithoutRepeatingOrSkippingARow() {
    ...
    for (int page = 0; page < 100 && seen.size() < EVENT_COUNT; page++) {
      List<String> arguments =
          new ArrayList<>(
              List.of("inspect", sessionDirectory.toString(), "--events", "25", "--json"));
      if (after >= 0) {
        arguments.add("--after");
  ...
  void showsOneEventWithARawLocationThatReallyPointsAtItsBytes() throws IOException {
    long eventId =
        number(
            objects(
                    cli.run("inspect", sessionDirectory.toString(), "--events", "1", "--json")
                        .json(),
                    "events")
                .get(0),
            "id");
    ...
    byte[] payload =
        JournalPayloadReader.forSession(sessionDirectory)
  ...
  void theTextDetailNamesTheJournalSegmentAndOffset() {
    ...
    CliHarness.Result result =
        cli.run("inspect", sessionDirectory.toString(), "--event", Long.toString(eventId));
  ...
  void listsTheDiagnosticsOfADamagedImport() throws IOException {
    ...
    CliHarness.Result result =
        cli.run("inspect", damagedSession.toString(), "--diagnostics", "--json");
  ...
  void anEventIdThatIsNotThereFailsWithACountRatherThanAnEmptyPage() {
    CliHarness.Result result = cli.run("inspect", sessionDirectory.toString(), "--event", "999999");

    assertThat(result.code()).isEqualTo(ExitCode.FAILED.code());
    assertThat(result.err()).contains("no event with id 999999").contains("it holds 120 events");
  }
