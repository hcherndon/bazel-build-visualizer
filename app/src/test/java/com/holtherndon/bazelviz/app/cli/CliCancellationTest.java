 ...
 *
 * <p>The shutdown hook is triggered through an injected registry rather than by
 * sending a signal: a test that sent a real SIGINT would kill the test JVM, and
 * one that raced a background thread against a fast import would be flaky. The
 * registry fires the hook the moment the command installs it, so the
 * cancellation flag is up before the importer reads its first record and the
 * run always takes the cancelled path.
 *
 * <p>The claim being tested is the Phase 1 exit criterion "restart resumes
 * interrupted indexing", stated in the terms the CLI exposes: exit code 4, then
 * a {@code --resume} that finishes the file and ends with exactly the event
 * count an uninterrupted import of the same file produces.
 */
class CliCancellationTest {
    ...
    void cancellingLeavesAResumableSessionThatResumesToTheSameCounts() throws IOException {
        ...
        CliHarness.Result cancelled = cli.run(
        ...
        CliHarness.Result resumed = resumeCli.run(
                "import", source.toString(),
                "--sessions-root", sessionsRoot.toString(),
                "--resume",
                "--json");
        ...
        JsonObject uninterrupted = reference.run(
                "import", source.toString(),
                "--sessions-root", workspace.resolve("reference").toString(),
                "--json").json();
