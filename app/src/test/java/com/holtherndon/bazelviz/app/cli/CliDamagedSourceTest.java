 ...
 *
 * <p>The exit code is the contract these tests pin down. A build script that
 * imports a BEP file cut short by a killed Bazel must be able to tell "there is
 * a session here, it is missing its tail" from "there is nothing here", because
 * the right response to the first is to analyse what arrived and the right
 * response to the second is to stop. Code 1 says the first; code 3 says the
 * second.
 */
class CliDamagedSourceTest {
    ...
    void aFileThatIsNotBepFailsOutrightAndLeavesNoSession() throws IOException {
        ...
        java.nio.file.Files.writeString(notBep, "this is not a build event protocol file\n");
        ...
        assertThat(sessionsRoot())
                .as("a refused import creates nothing to clean up")
                .doesNotExist();
    }
}
