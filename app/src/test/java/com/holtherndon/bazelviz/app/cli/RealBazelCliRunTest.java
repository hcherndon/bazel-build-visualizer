...
/**
 * The real-Bazel half of {@code bbv run}'s tests, split from {@link CliRunTest}
 * during the Bazel migration (resolving the plan's open question 5): these
 * three methods find a host Bazel through {@link BazelBinary} and launch it,
 * so their test target runs un-sandboxed with an inherited environment, while
 * the plain argument-parsing tests keep the ordinary hermetic treatment. On a
 * machine with no Bazel they skip with {@link BazelBinary#whyUnavailable}'s
 * message — skipped, never silently weakened.
 */
