 ...
 * a plain {@code if} on {@code args.length} placed before any other statement.
 * This test covers the other half — that nothing the commands themselves reach
 * needs a graphics environment, however deep in the import pipeline it lives.
 */
class CliHeadlessTest {
    ...
    void aFullImportRunsWithNoGraphicsEnvironment() throws IOException {
        ...
        CliHarness.Result inspected = cli.run(
                "inspect", CliHarness.sessionDirectory(imported.json()).toString(),
                "--events", "5", "--json");
