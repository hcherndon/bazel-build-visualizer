...
/**
 * Draws import progress on stderr, leaving stdout free for the summary so that {@code bbv import
 * --json … > summary.json} works while the operator still watches the progress.
 *
 * <p>Throttled twice over. The importer already limits how often it samples; this adds a wall-clock
 * floor of its own because a terminal is slower than a sampler and a redirected stderr would
 * otherwise collect thousands of lines. A phase change is always drawn, throttle or not:
 * "preserving" taking thirty seconds on a large file must not look like a hang.
 *
 * <p>The percentage appears only when the importer supplies a total size. When {@code totalBytes}
 * is empty — a growing file, an unstattable source — the line shows what is known and no percentage
 * at all. Inventing a denominator so that a bar can reach 100% is exactly the "unavailable rendered
 * as a number" the project rules forbid.
 */
final class ProgressReporter implements ImportProgressListener {
  ...
  /**
   * The rate over the gap between two samples rather than over the whole run, because during a long
   * import the recent throughput is the number that tells the operator whether anything is still
   * moving. Empty until there is a real interval to divide by.
   */
  ...
  private String render(ImportProgress progress, String rate) {
    ...
    line.append(
        String.format(Locale.ROOT, "%12s events", Formatting.count(progress.recordsRead())));
