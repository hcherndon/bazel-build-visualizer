    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1 && java.nio.file.Files.exists(java.nio.file.Path.of(args[0]))) {
            ...
            if (requestedLogLevel != null) {
                java.util.Optional<LogVerbosity> accepted =
                        LogVerbosity.fromId(requestedLogLevel);
                if (accepted.isPresent()) {
                    System.setProperty(
                            ApplicationLogging.LEVEL_PROPERTY, accepted.orElseThrow().id());
                } else {
                    System.setProperty(
                            ApplicationLogging.LEVEL_PROPERTY, LogVerbosity.WARN.id());
                    System.err.println("bbv: invalid bbv.log.level; using warn"
        ...
        LoggingSettingsStore.LoadResult loggingSettings =
                loggingStore.loadWithDiagnostics();
        ...
        if (themeOverride != null
            ...
            log.warn("Unknown bbv.theme '{}'; using saved theme {}",
                    themeOverride, savedTheme.displayName());
        }
        ...
        WorkspaceStore.LoadResult workspaceSettings = loadWorkspaces(
                workspaceStore,
                settingsDirectory,
        ...
        workspaceSettings.diagnostics().forEach(diagnostic ->
                log.warn("Workspace startup: {}", diagnostic));
        historyMigration.diagnostics().forEach(diagnostic ->
                log.warn("Workspace launcher history: {}", diagnostic));
        ...
        if (workspaceSettings.source() != WorkspaceStore.Source.UNUSABLE) {
            try {
                int removed = WorkspaceUiSettings.deleteOrphans(
                        settingsDirectory,
                        workspaceSettings.workspaces().stream()
                                .map(com.holtherndon.bazelviz.ui.workspace.WorkspaceProfile::id)
                                .toList());
                if (removed > 0) {
                    log.info(
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if (!applicationLogging.available()) {
                applicationLogging.startupWarning().ifPresent(warning ->
            ...
            if (smoke) {
                Timer timer = new Timer(2000, e -> {
                    ...
                    application.quit().whenComplete((ignored, failure) -> {
                        if (failure != null) {
