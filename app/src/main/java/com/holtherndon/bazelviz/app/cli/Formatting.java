 ...
 *
 * <p>One rule runs through all of it: an unavailable value prints as
 * {@value #UNKNOWN}, never as {@code 0} and never as an empty column (plan
 * 11.4). A zero here would be indistinguishable from a real measurement of
 * zero, and the whole point of the {@code OptionalLong}s that reach this class
 * is that the difference is known and must survive to the screen.
 *
 * <p>Everything is {@link Locale#ROOT} so that output is identical on every
 * machine and can be asserted on in a test.
 */
final class Formatting {
    ...
    static String byteSize(OptionalLong value) {
        return value.isPresent() ? count(value.getAsLong()) + " (" + bytes(value.getAsLong()) + ")" : UNKNOWN;
    }
    ...
    /**
     * A rate in records per second, or {@link #UNKNOWN} when too little time has
     * passed to measure one. An invented rate is worse than no rate.
     */
