...
/**
 * Everything a subcommand needs from the outside world: where to write, what
 * version to record in a manifest, where sessions live by default, and how to
 * register a shutdown hook.
 *
 * <p>All four are injected rather than reached for statically so the commands
 * can be driven in-process by a test with captured streams, a temporary
 * sessions root and a fake hook registry. A command that called
 * {@code System.out} directly could only be tested by shelling out, and a test
 * that shells out cannot see an exception or share a coverage report.
 *
 * <p>The default sessions root is a {@link Supplier} because resolving it
 * creates directories under the user's home. A run that passes
 * {@code --sessions-root} must not leave an application-support tree behind as
 * a side effect of parsing its own arguments.
 */
final class CliContext {
    ...
    /**
     * The context a real invocation uses: the JVM's streams and shutdown hooks,
     * the application's managed-sessions directory, and carriage-return progress
