...
/**
 * Desktop-environment integration (plan 17.2, and plan 24's Phase 9 "macOS app menu and open-file
 * handlers").
 *
 * <h2>Every call is guarded</h2>
 *
 * <p>{@code Desktop} is optional, and each action is optional separately. Headless CI, Linux and
 * older Windows all reach this code and all of it has to be a silent no-op there — an application
 * that failed to start because it could not install a macOS menu handler would be worse than one
 * without the menu.
 *
 * <h2>Open-file is the one that changes what the application is</h2>
 *
 * <p>With a file association declared at packaging time, macOS launches the app for a
 * double-clicked {@code .bviz} and delivers the path through {@code APP_OPEN_FILE} rather than
 * through {@code main}. Without a handler the app opens and shows an empty window, which looks
 * exactly like the association not working. The handler routes to the application controller, which
 * chooses the active workspace window or a workspace-less analysis shell. This is the same
 * classification route used by the Open menu and a command-line argument.
 *
 * <h2>Preferences uses the same window route as the Swing menu</h2>
 *
 * <p>Where the desktop owns an application Preferences item, it opens the same modeless settings
 * window as {@code Settings > Preferences}. Unsupported platforms keep the ordinary Swing menu.
 */
  ...
  static void install(ApplicationController application) {
    ...
    if (desktop.isSupported(Desktop.Action.APP_PREFERENCES)) {
      desktop.setPreferencesHandler(
          event -> SwingUtilities.invokeLater(application::showPreferences));
