...
/**
 * The headless entry point. {@link com.holtherndon.bazelviz.app.Main} hands control here whenever
 * it is given any argument at all, before it has touched the application directories, the look and
 * feel or the EDT — so a machine with no display can run every subcommand and a run with arguments
 * never opens a window.
 *
 * <p>Nothing in this package imports a Swing or AWT type, and that is enforced by the tests rather
 * than by convention: {@code CliHeadlessTest} runs an import with {@code java.awt.headless=true}
 * and fails if any of it needs a graphics environment.
 *
 * <p>{@link #run} returns the exit code instead of calling {@code System.exit} so that tests can
 * drive it in-process with captured streams. Only {@code Main} turns the return value into a
 * process exit.
 */
public final class CliMain {
  ...
  private static CliUsageException unknownCommand(String command) {
    ...
    return new CliUsageException(message + "; the commands are 'run', 'import' and 'inspect'");
  }
